...
{
	g_pFileSystemModule = Sys_LoadModule( "filesystem_stdio" DLL_EXT_STRING );
...
void LoadMaterialSystem( void )
{
	...
	g_MaterialsDLL = Sys_LoadModule( "materialsystem" DLL_EXT_STRING );

	g_MaterialsFactory = Sys_GetFactory( g_MaterialsDLL );
	if ( !g_MaterialsFactory )
	{
		DisplayError( "LoadMaterialSystem:  Failed to load materialsystem%s\n", DLL_EXT_STRING );
		return;
	}
...
int FindCommand( const char *pCommand )
{
	...
	{
		if ( !_stricmp( pCommand, g_ppArgV[i] ) )
			return i;
	}
	return -1;
}
