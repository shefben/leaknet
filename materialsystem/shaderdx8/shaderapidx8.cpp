...
public:
	...
	void CreateTexture( unsigned int textureNum, int width, 
		int height, ImageFormat dstImageFormat, int numMipLevels, int numCopies, int creationFlags, const char *pDebugName );
	void CreateDepthTexture( unsigned int textureNum, ImageFormat renderTargetFormat, int width, int height, const char *pDebugName );
...
private:
	...
	struct HardwareCaps_t
	{
		...
		bool m_SupportsPixelShaders_2_b;
		bool m_SupportsShaderModel_3_0;
	...
	m_pDebugFont( NULL ), m_pBackBufferSurface(0), m_pZBufferSurface(0)
{
	...
	m_Mode.m_Format = IMAGE_FORMAT_UNKNOWN;
...
static inline int ReadHexValue( KeyValues *pVal, const char *pName )
{
	...
	char *pTemp;
	int nVal = strtol( pString, &pTemp, 10 ); // VXP: Previously was 16, but this is stupid, since m_flValue already contains converted value
...
void CShaderAPIDX8::ComputeDXSupportLevel()
{
	...
	if ( m_Caps.m_SupportsShaderModel_3_0 )
	{
		m_Caps.m_DXSupportLevel = 95;
		return;
	}
...
bool CShaderAPIDX8::DetermineHardwareCaps( )
{
	...
	m_Caps.m_SupportsVertexShaders = D3DSHADER_VERSION_MAJOR( caps.VertexShaderVersion ) >= 1;
	m_Caps.m_SupportsPixelShaders = D3DSHADER_VERSION_MAJOR( caps.PixelShaderVersion ) >= 1;

#ifdef DX8_COMPATABILITY_MODE
	m_Caps.m_SupportsPixelShaders_1_4 = false;
	m_Caps.m_SupportsPixelShaders_2_0 = false;
	m_Caps.m_SupportsPixelShaders_2_b = false;
	m_Caps.m_SupportsVertexShaders_2_0 = false;
	m_Caps.m_SupportsShaderModel_3_0 = false;
	...
	m_Caps.m_SupportsPixelShaders_2_b = ( ( caps.PixelShaderVersion & 0xffff ) >= 0x0200 && ( caps.PS20Caps.NumInstructionSlots >= 512 ) );
	m_Caps.m_SupportsVertexShaders_2_0 = ( caps.VertexShaderVersion & 0xffff ) >= 0x0200;
	m_Caps.m_SupportsShaderModel_3_0 = ( caps.VertexShaderVersion & 0xffff ) >= 0x0300;
...
//		(caps.TextureFilterCaps & D3DPTFILTERCAPS_MAGFANISOTROPIC);
	...
	m_Caps.m_SupportsNonPow2Textures = ( !( caps.TextureCaps & D3DPTEXTURECAPS_POW2 ) || 
		( caps.TextureCaps & D3DPTEXTURECAPS_NONPOW2CONDITIONAL ) ) ? true : false;
...
void CShaderAPIDX8::SpewDriverInfo() const
{
	...
	Warning("Shader API DX%d Driver Info:\n\nDriver : %s Version : %d\n", 
		GetDXSupportLevel(), ident.Driver, ident.DriverVersion );
...
bool CShaderAPIDX8::SupportsPixelShaders_2_b() const
{
	if ( ( ShaderUtil()->GetConfig().dxSupportLevel != 0 ) &&
		( GetDXSupportLevel() < 90 ) )
		return false;
...
bool CShaderAPIDX8::SupportsShaderModel_3_0() const
{
	if ( ( ShaderUtil()->GetConfig().dxSupportLevel != 0 ) &&
...
void CShaderAPIDX8::CreateAmbientCubeTexture()
{
	if (m_Caps.m_SupportsCubeMaps)
	{
		CreateTexture( TEXTURE_AMBIENT_CUBE, AMBIENT_CUBE_TEXTURE_SIZE,
			...
			TEXTURE_CREATE_CUBEMAP | TEXTURE_CREATE_MANAGED, "TEXTURE_AMBIENT_CUBE" );
	}
}
...
void CShaderAPIDX8::ReleaseResources()
{
	...
	if (TextureCount() > 0)
	{
		for (int i = 0; i < GetTextureCount(); ++i)
		{
			...
			if ( pTexture->m_NumCopies == 1 )
			{
				if ( pTexture->GetTexture() )
					Warning( "Didn't correctly clean up texture %d (%s)\n", pTexture->m_BindId, pTexture->m_DebugName.String() );
			}
			else
			{
				for (int k = pTexture->m_NumCopies; --k >= 0; )
				{
					if ( pTexture->GetTexture( k ) != 0 )
					{
						Warning( "Didn't correctly clean up texture %d (%s)\n", pTexture->m_BindId, pTexture->m_DebugName.String() );
						break;
					}
...
void CShaderAPIDX8::SetTextureState( int stage, int textureIdx, bool force )
{
	...
	Texture_t *pTexture = &GetTexture( textureIdx );

	RECORD_COMMAND( DX8_SET_TEXTURE, 3 );
	RECORD_INT( stage );
	RECORD_INT( pTexture->m_BindId );
	RECORD_INT( pTexture->m_CurrentCopy );
	...
	g_Stats.IncrementCountedStat( MATERIAL_SYSTEM_STATS_NUM_TEXELS, 
		pTexture->m_SizeTexels );
	g_Stats.IncrementCountedStat( MATERIAL_SYSTEM_STATS_NUM_BYTES, 
		pTexture->m_SizeBytes );

	if ( pTexture->m_LastBoundFrame != m_CurrentFrame )
	{
		if ( pTexture->m_LastBoundFrame != m_CurrentFrame - 1 )
		{
		...
		pTexture->m_LastBoundFrame = m_CurrentFrame;
	}

	if (tsState.m_UTexWrap != pTexture->m_UTexWrap)
	{
		tsState.m_UTexWrap = pTexture->m_UTexWrap;
		SetSamplerState( stage, D3DSAMP_ADDRESSU, tsState.m_UTexWrap );
		g_Stats.IncrementCountedStat( MATERIAL_SYSTEM_STATS_DYNAMIC_STATE, 1 );
	}
	if (tsState.m_VTexWrap != pTexture->m_VTexWrap)
	{
		tsState.m_VTexWrap = pTexture->m_VTexWrap;
	...
	D3DTEXTUREFILTERTYPE minFilter = noFilter ? D3DTEXF_NONE : pTexture->m_MinFilter;
	...
	D3DTEXTUREFILTERTYPE magFilter = noFilter ? D3DTEXF_NONE : pTexture->m_MagFilter;
	...
	D3DTEXTUREFILTERTYPE mipFilter = noMipFilter ? D3DTEXF_NONE : pTexture->m_MipFilter;
...
bool CShaderAPIDX8::FindTexture( int id, int& idx )
{
	...
	{
		mid = (start + end) >> 1;
		Texture_t *pTexture = &GetTexture( mid );

		if ( pTexture->m_BindId == id )
		{
			idx = mid;
			return true;
		}
...
//			Assert( 0 ); // fixme
			pTexture->m_SizeBytes = 0;
			pTexture->m_SizeTexels += 6 * desc.Width * desc.Height;
		}
	}
	else
	{
...
//			Assert( 0 ); // fixme
			pTexture->m_SizeBytes = 0;
			pTexture->m_SizeTexels += desc.Width * desc.Height;
		}
	}
}
...
void CShaderAPIDX8::CreateDepthTexture( unsigned int textureNum, ImageFormat renderTargetFormat, int width, int height, const char *pDebugName )
{
...
void CShaderAPIDX8::CreateTexture( unsigned int textureNum, int width, int height, 
	ImageFormat dstImageFormat, int numMipLevels, int numCopies, int creationFlags, const char *pDebugName )
{
	...
	else
	{
		...
		pTexture->m_MipFilter = ( pTexture->m_NumLevels != 1) ?
	...
	if (( pTexture->m_BindId < m_FirstLightmapId) ||
		( pTexture->m_BindId > m_LastLightmapId))
	{
		g_Stats.IncrementCountedStat( MATERIAL_SYSTEM_STATS_TEXTURE_BYTES_ALLOCATED, 
			pTexture->m_SizeBytes * pTexture->m_NumCopies, true );
	}
	else
	{
...
void CShaderAPIDX8::DeleteD3DTexture( int idx )
{
	...
	int numCopies = pTexture->m_NumCopies;
	bool bIsDepthStencilSurfaceReleased = false;
	bool bIsSuccessfulRelease = false;

	if( pTexture->m_Flags & Texture_t::IS_DEPTH_STENCIL  )
	{
		// garymcthack - need to make sure that playback knows how to deal with these.
		RECORD_COMMAND( DX8_DESTROY_DEPTH_TEXTURE, 1 );
		RECORD_INT( pTexture->m_BindId );

		bIsDepthStencilSurfaceReleased = pTexture->GetDepthStencilSurface()->Release() == 0;
		Assert( bIsDepthStencilSurfaceReleased );
		pTexture->GetDepthStencilSurface() = 0;
		numDeallocated = 1;
	}
	else if ( pTexture->m_NumCopies == 1)
	{
		if ( pTexture->GetTexture())
		{
			...
			bIsSuccessfulRelease = DestroyD3DTexture( pTexture->GetTexture() );
			Assert( bIsSuccessfulRelease );
	...
	{
		if ( pTexture->GetTextureArray())
		{
			RECORD_COMMAND( DX8_DESTROY_TEXTURE, 1 );
			RECORD_INT( pTexture->m_BindId );

			// Multiple copy texture
			for (int j = 0; j < pTexture->m_NumCopies; ++j)
			{
				if ( pTexture->GetTexture( j ))
				{
					bIsSuccessfulRelease = DestroyD3DTexture( pTexture->GetTexture( j ) );
					Assert( bIsSuccessfulRelease );
					pTexture->GetTexture( j ) = 0;
					++numDeallocated;
				}
			}

			delete[] pTexture->GetTextureArray();
			pTexture->GetTextureArray() = 0;
		}
	}
	...
	{
		g_Stats.IncrementCountedStat( MATERIAL_SYSTEM_STATS_TEXTURE_BYTES_ALLOCATED, 
			-pTexture->m_SizeBytes * numDeallocated, true );
	}
	else
	{
		g_Stats.IncrementCountedStat( MATERIAL_SYSTEM_STATS_LIGHTMAP_BYTES_ALLOCATED, 
...
bool CShaderAPIDX8::IsTexture( unsigned int textureNum )
{
	...
	if( pTexture->m_Flags & Texture_t::IS_DEPTH_STENCIL )
	{
		return pTexture->GetDepthStencilSurface() != 0;
	}
	else if( ( pTexture->m_NumCopies == 1 && pTexture->GetTexture() != 0) ||
		( pTexture->m_NumCopies > 1 && pTexture->GetTexture( 0 ) != 0) )
	{
		return true;
	}
	else
...
IDirect3DSurface* CShaderAPIDX8::GetDepthTextureSurface( unsigned int tex )
{
	...
	Texture_t *pTexture = &GetTexture( renderTargetID );
...
void CShaderAPIDX8::TexWrap( ShaderTexCoordComponent_t coord, ShaderTexWrapMode_t wrapMode )
{
	...
	Texture_t *pTexture = &GetTexture( modifyTextureIndex );
	switch( coord )
	{
	case SHADER_TEXCOORD_S:
		pTexture->m_UTexWrap = address;
		break;
	case SHADER_TEXCOORD_T:
		pTexture->m_VTexWrap = address;
		break;
	default:
...
void CShaderAPIDX8::TexMinFilter( ShaderTexFilterMode_t texFilterMode )
{
	...
	case SHADER_TEXFILTERMODE_NEAREST:
		pTexture->m_MinFilter = D3DTEXF_POINT;
		pTexture->m_MipFilter = D3DTEXF_NONE;
		break;
	...
	case SHADER_TEXFILTERMODE_NEAREST_MIPMAP_NEAREST:
		pTexture->m_MinFilter = D3DTEXF_POINT;
		pTexture->m_MipFilter =
			pTexture->m_NumLevels != 1 ? D3DTEXF_POINT : D3DTEXF_NONE;
		break;
	...
	case SHADER_TEXFILTERMODE_NEAREST_MIPMAP_LINEAR:
		pTexture->m_MinFilter = D3DTEXF_POINT;
		pTexture->m_MipFilter =
			pTexture->m_NumLevels != 1 ? D3DTEXF_LINEAR : D3DTEXF_NONE;
		break;
	...
	case SHADER_TEXFILTERMODE_ANISOTROPIC:
		pTexture->m_MinFilter = D3DTEXF_ANISOTROPIC;
		pTexture->m_MipFilter =
			pTexture->m_NumLevels != 1 ? D3DTEXF_LINEAR : D3DTEXF_NONE;
		break;
	default:
...
void CShaderAPIDX8::TexMagFilter( ShaderTexFilterMode_t texFilterMode )
{
	...
	{
	case SHADER_TEXFILTERMODE_NEAREST:
		pTexture->m_MagFilter = D3DTEXF_POINT;
		break;
	case SHADER_TEXFILTERMODE_LINEAR:
		pTexture->m_MagFilter = D3DTEXF_LINEAR;
		break;
	...
	case SHADER_TEXFILTERMODE_LINEAR_MIPMAP_LINEAR:
		...
		break;
	case SHADER_TEXFILTERMODE_ANISOTROPIC:
		pTexture->m_MagFilter = D3DTEXF_ANISOTROPIC;
		break;
	default:
