...
void CMeshMgr::CleanUp()
{
	...
	hr = D3DDevice()->SetStreamSource( 0, 0, 0, 0 );
	Assert( hr == D3D_OK );
...
void CMeshMgr::DestroyVertexBuffers()
{
	for (int i = m_DynamicVertexBuffers.Count(); --i >= 0; )
	{
		if (m_DynamicVertexBuffers[i].m_pBuffer)
		{
			...
			m_DynamicVertexBuffers[i].m_pBuffer = 0; // VXP: Try to immediately get rid of the pointer
		}
	}
	m_DynamicVertexBuffers.RemoveAll();
	m_DynamicMesh.Reset();

	Assert( m_DynamicVertexBuffers.Count() == 0 &&
			m_DynamicMesh.GetVertexFormat() == 0 &&
			m_DynamicMesh.GetVertexBuffer() == 0 &&
			m_DynamicMesh.GetIndexBuffer() == 0 );
