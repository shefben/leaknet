...
{
	int nUsage = isRenderTarget ? D3DUSAGE_RENDERTARGET : 0;

	HRESULT hr = D3D_OK;

	// IHV depth texture formats require a slightly different check...
	if ( isRenderTarget && ( ( format == D3DFMT_D16 ) || ( format == D3DFMT_D24S8 ) ) )
	{
								 ...
								 ImageFormat displayFormat )
{
	...
	//	Assert( g_D3DColorFormat[fmt] != D3DFMT_UNKNOWN ); // VXP: Not critical
		g_D3DRenderTargetFormat[fmt] = GetNearestD3DColorFormat( (ImageFormat)fmt, use16BitTextures, true );
	//	Assert( g_D3DRenderTargetFormat[fmt] != D3DFMT_UNKNOWN ); // VXP: Not critical
		++fmt;
	}
...
{
	return	( g_D3DColorFormat[IMAGE_FORMAT_DXT1] != D3DFMT_UNKNOWN ) &&
			( g_D3DColorFormat[IMAGE_FORMAT_DXT3] != D3DFMT_UNKNOWN ) &&
			( g_D3DColorFormat[IMAGE_FORMAT_DXT5] != D3DFMT_UNKNOWN );
}
...
D3DFORMAT FindNearestSupportedDepthFormat(D3DFORMAT depthFormat, D3DFORMAT renderTargetFormat)
{
	...
	if ( (int)renderTargetFormat == D3DFMT_UNKNOWN )
	{
