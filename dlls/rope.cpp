...
BEGIN_DATADESC( CRopeKeyframe )
	...
	DEFINE_FIELD( CRopeKeyframe,	m_RopeFlags,		FIELD_INTEGER ),

	DEFINE_KEYFIELD( CRopeKeyframe,	m_iNextLinkName,	FIELD_STRING,	"NextKey" ),
	DEFINE_KEYFIELD( CRopeKeyframe,	m_Slack,			FIELD_INTEGER,	"Slack" ),
	DEFINE_KEYFIELD( CRopeKeyframe,	m_Width,			FIELD_FLOAT,	"Width" ),
	DEFINE_KEYFIELD( CRopeKeyframe, m_TextureScale,		FIELD_FLOAT,	"TextureScale" ),
	DEFINE_FIELD( CRopeKeyframe,	m_nSegments,		FIELD_INTEGER ),

	DEFINE_FIELD( CRopeKeyframe,	m_iRopeMaterialModel,FIELD_INTEGER ),
	DEFINE_KEYFIELD( CRopeKeyframe, m_Subdiv,			FIELD_INTEGER,	"Subdiv" ),
	DEFINE_FIELD( CRopeKeyframe,	m_RopeLength,		FIELD_INTEGER ),
	DEFINE_FIELD( CRopeKeyframe,	m_fLockedPoints,	FIELD_INTEGER ),
	DEFINE_FIELD( CRopeKeyframe,	m_bCreatedFromMapFile, FIELD_BOOLEAN ),
	DEFINE_FIELD( CRopeKeyframe,	m_flRemoveTime,		FIELD_TIME ),
	DEFINE_KEYFIELD( CRopeKeyframe, m_flScrollSpeed,	FIELD_FLOAT,	"ScrollSpeed" ),

	DEFINE_FIELD( CRopeKeyframe,	m_bStartPointValid, FIELD_BOOLEAN ),
	DEFINE_FIELD( CRopeKeyframe,	m_bEndPointValid,	FIELD_BOOLEAN ),

	DEFINE_FIELD( CRopeKeyframe,	m_hStartPoint,		FIELD_EHANDLE ),
	DEFINE_FIELD( CRopeKeyframe,	m_hEndPoint,		FIELD_EHANDLE ),
	DEFINE_FIELD( CRopeKeyframe,	m_iStartAttachment,	FIELD_SHORT ),
	DEFINE_FIELD( CRopeKeyframe,	m_iEndAttachment,	FIELD_SHORT ),
	
	// Inputs
	DEFINE_INPUTFUNC( CRopeKeyframe, FIELD_FLOAT,	"SetScrollSpeed",	InputSetScrollSpeed ),
	DEFINE_INPUTFUNC( CRopeKeyframe, FIELD_VECTOR,	"SetForce",			InputSetForce ),
	DEFINE_INPUTFUNC( CRopeKeyframe, FIELD_VOID,	"Break",			InputBreak ),
	
	DEFINE_FUNCTION( CRopeKeyframe, Think )
...
void CRopeKeyframe::Activate()
{
	...
	if( pEnt && pEnt->pev )
	{
		// Relink both ents so their absorigins are valid.
		Relink();
		pEnt->Relink();
...
{
	m_flRemoveTime = gpGlobals->curtime + 1.0;
}


void CRopeKeyframe::SetTransmit( CCheckTransmitInfo *pInfo )
{
	...
	{	
		...
		if ( m_hStartPoint.Get() )
			m_hStartPoint->SetTransmit( pInfo );

		if ( m_hEndPoint.Get() )
			m_hEndPoint->SetTransmit( pInfo );
	}
}
...
{
	// Send force to the client
	CEntityMessageFilter filter( this, "CRopeKeyFrame" );
	filter.MakeReliable();
...
// Purpose:
//-----------------------------------------------------------------------------
void CRopeKeyframe::Think()
{
	// Am I supposed to remove myself?
	if ( m_flRemoveTime && m_flRemoveTime < gpGlobals->curtime )
	{
		UTIL_Remove( this );
		return;
	}
	...
	if ( (m_RopeFlags & ROPE_RESIZE) && ents[0] && ents[0]->pev && ents[1] && ents[1]->pev )
	{
...
{
	string_t stringHandle = AllocPooledString( pName );
	m_iRopeMaterialModel = PrecacheModel( STRING( stringHandle ) );
}
