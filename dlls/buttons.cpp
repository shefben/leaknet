...
//
// Purpose: Implements buttons (and some other entities that belong elsewhere).
//
// $NoKeywords: $
//=============================================================================
...
BEGIN_DATADESC( CBaseButton )
	...
	DEFINE_KEYFIELD( CBaseButton, m_vecMoveDir, FIELD_VECTOR, "movedir" ),
	...
	DEFINE_FIELD( CBaseButton, m_bLocked, FIELD_BOOLEAN ),
	DEFINE_FIELD( CBaseButton, m_sNoise, FIELD_SOUNDNAME ),
	
	DEFINE_KEYFIELD( CBaseButton, m_sounds, FIELD_INTEGER, "sounds" ),
	
//	DEFINE_FIELD( CBaseButton, m_ls, FIELD_SOUNDNAME ),   // This is restored in Precache()
//  DEFINE_FIELD( CBaseButton, m_nState, FIELD_INTEGER ),

	// Function Pointers
	DEFINE_FUNCTION( CBaseButton, ButtonTouch ),
	DEFINE_FUNCTION( CBaseButton, ButtonSpark ),
	DEFINE_FUNCTION( CBaseButton, TriggerAndWait ),
	DEFINE_FUNCTION( CBaseButton, ButtonReturn ),
	DEFINE_FUNCTION( CBaseButton, ButtonBackHome ),
	DEFINE_FUNCTION( CBaseButton, ButtonUse ),

	// Inputs
	DEFINE_INPUTFUNC( CBaseButton, FIELD_VOID, "Lock", InputLock ),
	DEFINE_INPUTFUNC( CBaseButton, FIELD_VOID, "Unlock", InputUnlock ),
	DEFINE_INPUTFUNC( CBaseButton, FIELD_VOID, "Press", InputPress ),

	// Outputs
	DEFINE_OUTPUT( CBaseButton, m_OnDamaged, "OnDamaged" ),
	DEFINE_OUTPUT( CBaseButton, m_OnPressed, "OnPressed" ),
	DEFINE_OUTPUT( CBaseButton, m_OnIn, "OnIn" ),
	DEFINE_OUTPUT( CBaseButton, m_OnOut, "OnOut" ),
...
void CBaseButton::Precache( void )
{
	...
	if (m_bLockedSound)
	{
		...
		enginesound->PrecacheSound(pszSound);
		m_ls.sLockedSound = AllocPooledString(pszSound);
	}
	...
	if (m_bUnlockedSound)
	{
		...
		enginesound->PrecacheSound(pszSound);
		m_ls.sUnlockedSound = AllocPooledString(pszSound);
	}
...
void CBaseButton::InputPress( inputdata_t &inputdata )
{
	...
	if ( m_toggle_state == TS_AT_TOP)
	{
		...
		EmitSound( filter, entindex(), CHAN_VOICE, (char*)STRING(m_sNoise), 1, ATTN_NORM );
...
int CBaseButton::OnTakeDamage( const CTakeDamageInfo &info )
{
	...
	{
		CPASAttenuationFilter filter( this );
		EmitSound( filter, entindex(), CHAN_VOICE, STRING(m_sNoise), 1, ATTN_NORM );
...
{ 
	...
	enginesound->PrecacheSound(pszSound);
	m_sNoise = AllocPooledString(pszSound);
...
BEGIN_DATADESC( CMomentaryRotButton )
	...
	DEFINE_FIELD( CMomentaryRotButton, m_IdealYaw, FIELD_FLOAT ),
	DEFINE_FIELD( CMomentaryRotButton, m_flTimeDelta, FIELD_FLOAT ),
	DEFINE_FIELD( CMomentaryRotButton, m_sNoise, FIELD_SOUNDNAME ),

	DEFINE_KEYFIELD( CMomentaryRotButton, m_direction, FIELD_INTEGER, "StartDirection" ),
	DEFINE_KEYFIELD( CMomentaryRotButton, m_returnSpeed, FIELD_FLOAT, "returnspeed" ),
	DEFINE_KEYFIELD( CMomentaryRotButton, m_sounds, FIELD_INTEGER, "sounds" ),
	DEFINE_KEYFIELD( CMomentaryRotButton, m_flStartPosition, FIELD_FLOAT, "StartPosition"),

	// Function Pointers
	DEFINE_FUNCTION( CMomentaryRotButton, UseMoveDone ),
	DEFINE_FUNCTION( CMomentaryRotButton, ReturnMoveDone ),
	DEFINE_FUNCTION( CMomentaryRotButton, SetPositionMoveDone ),
	DEFINE_FUNCTION( CMomentaryRotButton, UpdateThink ),

	// Inputs
	DEFINE_INPUTFUNC( CMomentaryRotButton, FIELD_FLOAT, "SetPosition", InputSetPosition ),

	// Outputs
	DEFINE_OUTPUT( CMomentaryRotButton, m_Position, "Position" ),
	DEFINE_OUTPUT( CMomentaryRotButton, m_OnPressed, "OnPressed" ),
	DEFINE_OUTPUT( CMomentaryRotButton, m_OnUnpressed, "OnUnpressed" ),
	DEFINE_OUTPUT( CMomentaryRotButton, m_OnFullyClosed, "OnFullyClosed" ),
	DEFINE_OUTPUT( CMomentaryRotButton, m_OnFullyOpen, "OnFullyOpen" ),
