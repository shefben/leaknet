...
#include "../common/keyframe/keyframe.h" // BUG: this needs to move if keyframe is a standard thing
...
BEGIN_DATADESC( CPathKeyFrame )
	...
	DEFINE_FIELD( CPathKeyFrame, m_Origin, FIELD_VECTOR ),
	DEFINE_FIELD( CPathKeyFrame, m_Angles, FIELD_VECTOR ),
	DEFINE_FIELD( CPathKeyFrame, m_qAngle, FIELD_QUATERNION ),

	DEFINE_KEYFIELD( CPathKeyFrame, m_iNextKey, FIELD_STRING, "NextKey" ),
	DEFINE_FIELD( CPathKeyFrame, m_flNextTime, FIELD_FLOAT ),	// derived from speed
	DEFINE_KEYFIELD( CPathKeyFrame, m_flSpeed, FIELD_FLOAT, "MoveSpeed" ),
	DEFINE_FIELD( CPathKeyFrame, m_pNextKey, FIELD_CLASSPTR ),
	DEFINE_FIELD( CPathKeyFrame, m_pPrevKey, FIELD_CLASSPTR ),
...
public:
	...
	// move behaviors have an edict so that they can be handled by the engine's physics system
	int  ObjectCaps( void ) { return BaseClass::ObjectCaps() & ~FCAP_NO_EDICT_AT_SPAWN; }
...
//	DEFINE_KEYFIELD( CBaseMoveBehavior, m_iTimeModifier, FIELD_INTEGER, "TimeModifier" ),
	DEFINE_KEYFIELD( CBaseMoveBehavior, m_iPositionInterpolator, FIELD_INTEGER, "PositionInterpolator" ),
	DEFINE_KEYFIELD( CBaseMoveBehavior, m_iRotationInterpolator, FIELD_INTEGER, "RotationInterpolator" ),

	DEFINE_FIELD( CBaseMoveBehavior, m_pCurrentKeyFrame, FIELD_CLASSPTR ),
	DEFINE_FIELD( CBaseMoveBehavior, m_pTargetKeyFrame, FIELD_CLASSPTR ),
	DEFINE_FIELD( CBaseMoveBehavior, m_pPreKeyFrame, FIELD_CLASSPTR ),
	DEFINE_FIELD( CBaseMoveBehavior, m_pPostKeyFrame, FIELD_CLASSPTR ),
	
	DEFINE_FIELD( CBaseMoveBehavior, m_flAnimStartTime, FIELD_FLOAT ),
	DEFINE_FIELD( CBaseMoveBehavior, m_flAnimEndTime, FIELD_FLOAT ),
	DEFINE_FIELD( CBaseMoveBehavior, m_flAverageSpeedAcrossFrame, FIELD_FLOAT ),
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
int CBaseMoveBehavior::IsMoving( void )
{
	if ( m_iDirection != 0 )
		return true;
