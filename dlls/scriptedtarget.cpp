...
BEGIN_DATADESC( CScriptedTarget )
	...
	DEFINE_FIELD( CScriptedTarget,		m_vLastPosition,	FIELD_POSITION_VECTOR ),

	DEFINE_KEYFIELD( CScriptedTarget,	m_iDisabled,		FIELD_INTEGER,	"StartDisabled" ),
	DEFINE_KEYFIELD( CScriptedTarget,	m_iszEntity,		FIELD_STRING,	"m_iszEntity" ),
	DEFINE_KEYFIELD( CScriptedTarget,	m_flRadius,			FIELD_FLOAT,	"m_flRadius" ),

	DEFINE_KEYFIELD( CScriptedTarget,	m_nMoveSpeed,		FIELD_INTEGER,	"MoveSpeed" ),
	DEFINE_KEYFIELD( CScriptedTarget,	m_flPauseDuration,	FIELD_FLOAT,	"PauseDuration" ),
	DEFINE_FIELD( CScriptedTarget,		m_flPauseDoneTime,	FIELD_TIME ),
	DEFINE_KEYFIELD( CScriptedTarget,	m_flEffectDuration,	FIELD_FLOAT,	"EffectDuration" ),

	// Function Pointers
	DEFINE_THINKFUNC( CScriptedTarget, ScriptThink ),

	// Inputs
	DEFINE_INPUTFUNC( CScriptedTarget,	FIELD_VOID, "Enable", InputEnable ),
	DEFINE_INPUTFUNC( CScriptedTarget,	FIELD_VOID, "Disable", InputDisable ),

	// Outputs
	DEFINE_OUTPUT(CScriptedTarget, m_AtTarget,			"AtTarget" ),
	DEFINE_OUTPUT(CScriptedTarget, m_LeaveTarget,		"LeaveTarget" ),
...
void CScriptedTarget::TurnOff( void )
{
	...
	if (GetTarget())
	{
		...
		pNPC->HandleInteraction( g_interactionScriptedTarget, NULL, NULL );
	}
}
...
CBaseEntity* CScriptedTarget::FindEntity( void )
{
	...
	if (pEntity && pEntity->GetFlags() & FL_NPC)
	{
		...
		if (pNPC->HandleInteraction( g_interactionScriptedTarget, NULL, this ))
		{
			return pEntity;
		}
	}
