...
BEGIN_DATADESC( CAI_ScriptedSequence )
	...
	DEFINE_KEYFIELD( CAI_ScriptedSequence, m_iszPreIdle, FIELD_STRING, "m_iszIdle" ),
	DEFINE_KEYFIELD( CAI_ScriptedSequence, m_iszPlay, FIELD_STRING, "m_iszPlay" ),
	DEFINE_KEYFIELD( CAI_ScriptedSequence, m_iszPostIdle, FIELD_STRING, "m_iszPostIdle" ),
	DEFINE_KEYFIELD( CAI_ScriptedSequence, m_iszEntity, FIELD_STRING, "m_iszEntity" ),
	DEFINE_KEYFIELD( CAI_ScriptedSequence, m_iszNextScript, FIELD_STRING, "m_iszNextScript" ),
	DEFINE_KEYFIELD( CAI_ScriptedSequence, m_fMoveTo, FIELD_INTEGER, "m_fMoveTo" ),
	DEFINE_KEYFIELD( CAI_ScriptedSequence, m_flRepeat, FIELD_FLOAT, "m_flRepeat" ),
	DEFINE_KEYFIELD( CAI_ScriptedSequence, m_flRadius, FIELD_FLOAT, "m_flRadius" ),
	DEFINE_KEYFIELD( CAI_ScriptedSequence, m_iFinishSchedule, FIELD_INTEGER, "m_iFinishSchedule" ),
	DEFINE_KEYFIELD( CAI_ScriptedSequence, m_iszCustomMove, FIELD_STRING, "m_iszCustomMove" ),

	DEFINE_FIELD( CAI_ScriptedSequence, m_iDelay, FIELD_INTEGER ),
	DEFINE_FIELD( CAI_ScriptedSequence, m_startTime, FIELD_TIME ),

	DEFINE_FIELD( CAI_ScriptedSequence,	m_saved_movetype, FIELD_INTEGER ),
	DEFINE_FIELD( CAI_ScriptedSequence,	m_saved_movecollide, FIELD_INTEGER ),
	DEFINE_FIELD( CAI_ScriptedSequence,	m_saved_solidflags, FIELD_INTEGER ),
	DEFINE_FIELD( CAI_ScriptedSequence, m_saved_effects, FIELD_INTEGER ),
	DEFINE_FIELD( CAI_ScriptedSequence, m_interruptable, FIELD_BOOLEAN ),
	DEFINE_FIELD( CAI_ScriptedSequence, m_sequenceStarted, FIELD_BOOLEAN ),
	DEFINE_FIELD( CAI_ScriptedSequence, m_hTargetEnt, FIELD_EHANDLE ),
	DEFINE_FIELD( CAI_ScriptedSequence, m_hNextCine, FIELD_EHANDLE ),
	DEFINE_FIELD( CAI_ScriptedSequence, m_hLastFoundEntity, FIELD_EHANDLE ),

	// Function Pointers
	DEFINE_THINKFUNC( CAI_ScriptedSequence, ScriptThink ),

	// Inputs
	DEFINE_INPUTFUNC( CAI_ScriptedSequence, FIELD_VOID, "MoveToPosition", InputMoveToPosition ),
	DEFINE_INPUTFUNC( CAI_ScriptedSequence, FIELD_VOID, "BeginSequence", InputBeginSequence ),
	DEFINE_INPUTFUNC( CAI_ScriptedSequence, FIELD_VOID, "CancelSequence", InputCancelSequence ),

	// Outputs
	DEFINE_OUTPUT(CAI_ScriptedSequence, m_OnBeginSequence, "OnBeginSequence"),
	DEFINE_OUTPUT(CAI_ScriptedSequence, m_OnEndSequence, "OnEndSequence"),
	DEFINE_OUTPUT(CAI_ScriptedSequence, m_OnScriptEvent[0], "OnScriptEvent01"),
	DEFINE_OUTPUT(CAI_ScriptedSequence, m_OnScriptEvent[1], "OnScriptEvent02"),
	DEFINE_OUTPUT(CAI_ScriptedSequence, m_OnScriptEvent[2], "OnScriptEvent03"),
	DEFINE_OUTPUT(CAI_ScriptedSequence, m_OnScriptEvent[3], "OnScriptEvent04"),
	DEFINE_OUTPUT(CAI_ScriptedSequence, m_OnScriptEvent[4], "OnScriptEvent05"),
	DEFINE_OUTPUT(CAI_ScriptedSequence, m_OnScriptEvent[5], "OnScriptEvent06"),
	DEFINE_OUTPUT(CAI_ScriptedSequence, m_OnScriptEvent[6], "OnScriptEvent07"),
	DEFINE_OUTPUT(CAI_ScriptedSequence, m_OnScriptEvent[7], "OnScriptEvent08"),
...
void CAI_ScriptedSequence::Spawn( void )
{
	...
	if ( !GetEntityName() || ( m_spawnflags & SF_SCRIPT_START_ON_SPAWN ) )
	{
		...
		{
			m_startTime = gpGlobals->curtime + 1E6;
		}
	}
...
void CAI_ScriptedSequence::InputMoveToPosition( inputdata_t &inputdata )
{
	...
	if ( pTarget != NULL ) 
	{
		...
		{
			// See if we can enqueue ourselves.
		...
		// Presumably they are moving to position or waiting for the BeginSequence input.
	}
	else
	{
		// No, grab the NPC but make them wait until BeginSequence is fired.
		StartThink();
...
void CAI_ScriptedSequence::InputBeginSequence( inputdata_t &inputdata )
{
	...
	if ( pTarget ) 
	{
		// Are they already playing this script?
		if ( pTarget->m_scriptState == CAI_BaseNPC::SCRIPT_PLAYING || pTarget->m_scriptState == CAI_BaseNPC::SCRIPT_POST_IDLE )
		{
			// See if we can enqueue ourselves.
			if ( pTarget->CanPlaySequence( FCanOverrideState(), SS_INTERRUPT_BY_NAME ) )
		...
		// If not, start the action sequence.
		m_startTime = gpGlobals->curtime + 0.05;
	}
	else
	{
...
{
	return ( m_iDelay <= 0 ) && ( gpGlobals->curtime >= m_startTime );
}
...
CAI_BaseNPC *CAI_ScriptedSequence::FindScriptEntity( )
{
	...
	while ( pEntity != NULL )
	{
		...
		if ( pNPC )
		{
			...
			//
			CanPlaySequence_t eCanPlay = pNPC->CanPlaySequence( FCanOverrideState(), SS_INTERRUPT_BY_CLASS );
...
void CAI_ScriptedSequence::StartScript( void )
{
	...
	if ( pTarget )
	{
		...
		m_saved_movecollide = pTarget->GetMoveCollide();
		m_saved_solidflags = pTarget->GetSolidFlags();
		m_saved_effects = pTarget->m_fEffects;
		pTarget->m_fEffects |= m_fEffects;
		...
		case CINE_MOVETO_TELEPORT: 
			...
			pTarget->m_fEffects |= EF_NOINTERP;
		...
		pTarget->m_IdealNPCState = NPC_STATE_SCRIPT;
	}
}
...
bool CAI_ScriptedSequence::StartSequence( CAI_BaseNPC *pTarget, string_t iszSeq, bool completeOnEmpty )
{
	...
	pTarget->m_flCycle = 0;
	pTarget->ResetSequence( nSequence );
...
{
	if ( pNPC->m_IdealNPCState != NPC_STATE_DEAD )
	{
...
bool CAI_ScriptedSequence::CanEnqueueAfter( void )
{
	...
	{
		Msg( "%s is specified as the 'Next Script' and cannot be kicked out of the queue\n",  m_hNextCine->GetDebugName() );
		return false;
	}
	...
	Msg( "%s is a priority script and cannot be kicked out of the queue\n",  m_hNextCine->GetDebugName() );
...
//
// Input  : state - 1 means this script is not ready, 0 means it is ready.
//-----------------------------------------------------------------------------
void CAI_ScriptedSequence::DelayStart( int state )
{
	...
	while ( pentCine )
	{
		if ( FClassnameIs( pentCine, "scripted_sequence" ) )
		{
			...
			else
			{
				...
				{
					// They're all ready, start in 0.05 seconds.
					pTarget->m_startTime = gpGlobals->curtime + 0.05;					pTarget->m_iDelay = 0;
				}
			}
		}
...
BEGIN_DATADESC( CAI_ScriptedSchedule )
	...
	DEFINE_FIELD( CAI_ScriptedSchedule, m_hLastFoundEntity, FIELD_EHANDLE ),
	DEFINE_KEYFIELD( CAI_ScriptedSchedule, m_flRadius, FIELD_FLOAT, "m_flRadius" ),
	
	DEFINE_KEYFIELD( CAI_ScriptedSchedule, m_iszEntity, FIELD_STRING, "m_iszEntity" ),
	DEFINE_KEYFIELD( CAI_ScriptedSchedule, m_nSchedule, FIELD_INTEGER, "schedule" ),
	DEFINE_KEYFIELD( CAI_ScriptedSchedule, m_nForceState, FIELD_INTEGER, "forcestate" ),
	DEFINE_KEYFIELD( CAI_ScriptedSchedule, m_sGoalEnt, FIELD_STRING, "goalent" ),
	DEFINE_KEYFIELD( CAI_ScriptedSchedule, m_bGrabAll, FIELD_BOOLEAN, "graball" ),
	DEFINE_KEYFIELD( CAI_ScriptedSchedule, m_Interruptability, FIELD_INTEGER, "interruptability"),

	DEFINE_THINKFUNC( CAI_ScriptedSchedule, ScriptThink ),

	DEFINE_INPUTFUNC( CAI_ScriptedSchedule, FIELD_VOID, "StartSchedule", InputStartSchedule ),
...
void CAI_ScriptedSchedule::StartSchedule( CAI_BaseNPC *pTarget )
{
	...
	if ( !pGoalEnt ) 
	{
		...
		CAI_Hint *pHint = CAI_Hint::FindHint( pTarget->GetAbsOrigin(), &hintCriteria );
		if ( !pHint )
		{
...
BEGIN_DATADESC( CAI_ScriptedSentence )
	...
	DEFINE_KEYFIELD( CAI_ScriptedSentence, m_iszSentence, FIELD_STRING, "sentence" ),
	DEFINE_KEYFIELD( CAI_ScriptedSentence, m_iszEntity, FIELD_STRING, "entity" ),
	DEFINE_KEYFIELD( CAI_ScriptedSentence, m_flRadius, FIELD_FLOAT, "radius" ),
	DEFINE_KEYFIELD( CAI_ScriptedSentence, m_flDelay, FIELD_FLOAT, "delay" ),
	DEFINE_KEYFIELD( CAI_ScriptedSentence, m_flRepeat, FIELD_FLOAT, "refire" ),
	DEFINE_KEYFIELD( CAI_ScriptedSentence, m_iszListener, FIELD_STRING, "listener" ),

	DEFINE_KEYFIELD( CAI_ScriptedSentence, m_TempAttenuation, FIELD_INTEGER, "attenuation" ),

	DEFINE_FIELD( CAI_ScriptedSentence, m_iSoundLevel, FIELD_INTEGER ),
	DEFINE_FIELD( CAI_ScriptedSentence, m_flVolume, FIELD_FLOAT ),
	DEFINE_FIELD( CAI_ScriptedSentence, m_active, FIELD_BOOLEAN ),
	DEFINE_FIELD( CAI_ScriptedSentence, m_pActivator, FIELD_EHANDLE ),

	// Function Pointers
	DEFINE_FUNCTION( CAI_ScriptedSentence, FindThink ),
	DEFINE_FUNCTION( CAI_ScriptedSentence, DelayThink ),

	// Inputs
	DEFINE_INPUTFUNC(CAI_ScriptedSentence, FIELD_VOID, "BeginSentence", InputBeginSentence),

	// Outputs
	DEFINE_OUTPUT(CAI_ScriptedSentence,  m_OnBeginSentence, "OnBeginSentence"),
	DEFINE_OUTPUT(CAI_ScriptedSentence,  m_OnEndSentence, "OnEndSentence"),
