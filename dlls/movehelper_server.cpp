...
{
	CSingleUserRecipientFilter filter( m_pHostPlayer );
	// FIXME, these sounds should not go to the host entity ( SND_NOTHOST )
	if ( gpGlobals->maxClients == 1 )
	{
		// Always send sounds down in SP
		CBaseEntity::EmitSound( filter, m_pHostPlayer->entindex(), soundname, &origin );
	}
	else
	{
		filter.UsePredictionRules();
		CBaseEntity::EmitSound( filter, m_pHostPlayer->entindex(), 
			soundname, &origin );
	}
}
						...
						float volume, soundlevel_t soundlevel, int fFlags, int pitch )
{
	...
	{
		// Always send sounds down in SP
		CBaseEntity::EmitSound( filter, m_pHostPlayer->entindex(), channel, sample, 
			volume, soundlevel, fFlags, pitch, &origin );
	}
	else
	{
		filter.UsePredictionRules();

		CBaseEntity::EmitSound( filter, m_pHostPlayer->entindex(), channel, sample, volume, 
			soundlevel, fFlags, pitch, &origin );
	}
}
...
bool CMoveHelperServer::PlayerFallingDamage( void )
{
	...
	if ( m_pHostPlayer->m_iHealth <= 0 )
	{
		if ( g_pGameRules->FlPlayerFallDeathDoesScreenFade( m_pHostPlayer ) )
		{
			...
			// VXP: The 9999 fadeHold value is causing a few discrepancies in code.
			// Firstly it's converting to 65536 server-side, then to 32768 when it comes to client.
			// Secondly, it is already flagged with FFADE_STAYOUT, so fadeHold would not even be considered valid anyways.
			// Either way, I think it's safe to set the fadeHold to 0 and call it a day
