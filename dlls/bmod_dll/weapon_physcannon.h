...
public:
	...
	// BMOD: New functionality
	void	FreezeObject( CBaseEntity *pObject );
	void	UnfreezeObject( CBaseEntity *pObject );
	void	UnfreezeAllPlayerObjects( void );
	bool	IsObjectFrozen( CBaseEntity *pObject );
	void	HandleDistanceAdjustment( void );
...
protected:
	...
	// BMOD: New member variables
	CUtlVector<EHANDLE>	m_frozenObjects;		// List of frozen objects by this player
	float				m_flHoldDistance;		// Current hold distance
	float				m_flLastRKeyTime;		// Time of last R key press
	QAngle				m_angRotationOffset;	// Rotation offset for held object
	bool				m_bRotationMode;		// Whether we're in rotation mode (E held)
	Vector				m_vecLastMousePos;		// Last mouse position for rotation

	// Network variables for beam and held object
	CNetworkVector( m_targetPosition );
	CNetworkVector( m_worldPosition );
	CNetworkVar( int, m_active );
	CNetworkVar( int, m_glueTouching );
	CNetworkVar( int, m_viewModelIndex );
	CNetworkHandle( CBaseEntity, m_heldObject );

	// Enhanced physics gun state (matching GMod client analysis)
