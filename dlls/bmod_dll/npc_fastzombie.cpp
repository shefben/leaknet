...
#define FASTZOMBIE_EXCITE_DIST 256.0
...
#define FASTZOMBIE_GURGLE_SOUND "npc/fast_zombie/gurgle_loop1.wav"
...
// animation events
//=========================================================
int FASTZOMBIE_AE_LEAP         = 50;
int FASTZOMBIE_AE_GALLOP_LEFT  = 51;
int FASTZOMBIE_AE_GALLOP_RIGHT = 52;

int FASTZOMBIE_AE_CLIMB_LEFT   = 60;
int FASTZOMBIE_AE_CLIMB_RIGHT  = 61;
...
public:
	...
	bool FindNearestPhysicsObject( int iMaxMass ) { return false; }
...
BEGIN_DATADESC( CFastZombie )
	...
	DEFINE_FIELD( CFastZombie, m_flDistFactor, FIELD_FLOAT ),
	DEFINE_FIELD( CFastZombie, m_iClimbCount, FIELD_CHARACTER ),
	DEFINE_FIELD( CFastZombie, m_fIsNavJumping, FIELD_BOOLEAN ),
	DEFINE_FIELD( CFastZombie, m_fIsAttackJumping, FIELD_BOOLEAN ),
	DEFINE_FIELD( CFastZombie, m_fHitApex, FIELD_BOOLEAN ),
	DEFINE_FIELD( CFastZombie, m_flJumpDist, FIELD_FLOAT ),
	DEFINE_FIELD( CFastZombie, m_flSineFrequency, FIELD_FLOAT ),
	DEFINE_FIELD( CFastZombie, m_fHasScreamed, FIELD_BOOLEAN ),
	DEFINE_FIELD( CFastZombie, m_flTimeUpdateSine, FIELD_TIME ),
	DEFINE_FIELD( CFastZombie, m_fSineTrendUp, FIELD_BOOLEAN ),
	DEFINE_FIELD( CFastZombie, m_flNextMeleeAttack, FIELD_TIME ),
	DEFINE_FIELD( CFastZombie, m_fJustJumped, FIELD_BOOLEAN ),
	DEFINE_FIELD( CFastZombie, m_flJumpStartAltitude, FIELD_FLOAT ),

	// Function Pointers
	DEFINE_ENTITYFUNC( CFastZombie, LeapAttackTouch ),
	DEFINE_ENTITYFUNC( CFastZombie, ClimbTouch ),
	DEFINE_SOUNDPATCH( CFastZombie, m_pLayer2 ),
...
const char *CFastZombie::pMoanSounds[] =
{
	...
	"npc/fast_zombie/breathe_loop2.wav",
	"npc/fast_zombie/breathe_loop3.wav",
};
...
void CFastZombie::SetIdleSoundState( void )
{
	...
	ENVELOPE_CONTROLLER.SoundChangeVolume( m_pMoanSound, 0.5, 1.0 );
...
void CFastZombie::SetAngrySoundState( void )
{
	...
	ENVELOPE_CONTROLLER.SoundChangeVolume( m_pMoanSound, 0.7, 0.5 );
...
void CFastZombie::LeapAttackSound( void )
{
	...
	ENVELOPE_CONTROLLER.SoundChangePitch( m_pMoanSound, FASTZOMBIE_MAX_PITCH + 10, 0.1 );
}
...
{
	// Stomp the sine wave instructions.
	ENVELOPE_CONTROLLER.CommandClear( m_pMoanSound );
	...
	// Delay the next sine update.
	m_flTimeUpdateSine += m_flSineFrequency;
}
...
{
	if ( pEvent->event == FASTZOMBIE_AE_CLIMB_LEFT || pEvent->event == FASTZOMBIE_AE_CLIMB_RIGHT )
	{
	...
	if ( pEvent->event == FASTZOMBIE_AE_LEAP )
	{
		LeapAttack();
		return;
	}

	if ( pEvent->event == FASTZOMBIE_AE_GALLOP_LEFT )
	{
		EmitSound( "NPC_FastZombie.GallopLeft" );
		return;
	}

	if ( pEvent->event == FASTZOMBIE_AE_GALLOP_RIGHT )
	{
	...
	if ( pEvent->event == ZOMBIE_AE_ATTACK_RIGHT )
	{
		...
		ClawAttack( 50, 3, qa, right );
		return;
	}

	if ( pEvent->event == ZOMBIE_AE_ATTACK_LEFT )
	{
		Vector right;
...
void CFastZombie::LeapAttackTouch( CBaseEntity *pOther )
{
	...
	Vector vecPunch = forward * 500;
	
	ClawAttack( 50, 5, qaPunch, vecPunch );
...
void CFastZombie::OnChangeActivity( Activity NewActivity )
{
	if ( NewActivity == ACT_FASTZOMBIE_FRENZY )
	{
		...
		// Keep the main sound scheme code from stomping on us.
		m_flTimeUpdateSine = gpGlobals->curtime + 2.0;
	...
	else if ( GetActivity() == ACT_GLIDE )
	{
		...
		ENVELOPE_CONTROLLER.SoundChangeVolume( m_pMoanSound, 1.0, 0.3 );
	}
...
void CFastZombie::BeginNavJump( void )
{
	...
	ENVELOPE_CONTROLLER.SoundChangePitch( m_pMoanSound, 40, 3.0 );
	ENVELOPE_CONTROLLER.SoundChangeVolume( m_pMoanSound, 0.4, 0.3 );
}
