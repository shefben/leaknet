...
//Animation events
int ANTLION_AE_FOOTSTEP_SOFT  = 12;
int ANTLION_AE_FOOTSTEP       = 13;
int ANTLION_AE_FOOTSTEP_HEAVY = 14;
int ANTLION_AE_MELEE_HIT1     = 15;
int ANTLION_AE_MELEE_HIT2     = 16;
int ANTLION_AE_START_JUMP     = 18;
int ANTLION_AE_BURROW_IN      = 19;
int ANTLION_AE_BURROW_OUT     = 20;
int ANTLION_AE_VANISH         = 21;
int ANTLION_AE_OPEN_WINGS     = 22;
int ANTLION_AE_CLOSE_WINGS    = 23;
int ANTLION_AE_MELEE1_SOUND   = 24;
int ANTLION_AE_MELEE2_SOUND   = 25;
...
void CNPC_Antlion::Precache( void )
{
	...
	{
		PrecacheModel( m_AntlionGibsSmall[i] );
	}

	for ( int i = 0; i < NUM_ANTLION_GIBS_MEDIUM; i++ )
	{
		PrecacheModel( m_AntlionGibsMedium[i] );
	}

	for ( int i = 0; i < NUM_ANTLION_GIBS_LARGE; i++ )
	{
		PrecacheModel( m_AntlionGibsLarge[i] );
	}
...
{
	if ( pEvent->event == ANTLION_AE_OPEN_WINGS )
	{
		SetWings( true );
		return;
	}

	if ( pEvent->event == ANTLION_AE_CLOSE_WINGS )
	{
		SetWings( false );
		return;
	}

	if ( pEvent->event == ANTLION_AE_VANISH )
	{
		AddSolidFlags( FSOLID_NOT_SOLID );
		m_takedamage	= DAMAGE_NO;
		m_fEffects		|= EF_NODRAW;
	...
	if ( pEvent->event == ANTLION_AE_BURROW_IN )
	{
		//Burrowing sound
	...
	if ( pEvent->event == ANTLION_AE_BURROW_OUT )
	{
	...
	if ( pEvent->event == ANTLION_AE_FOOTSTEP_SOFT )
	{
		EmitSound( "NPC_Antlion.FootstepSoft" );
		return;
	}

	if ( pEvent->event == ANTLION_AE_FOOTSTEP )
	{
		EmitSound( "NPC_Antlion.Footstep" );
		return;
	}

	if ( pEvent->event == ANTLION_AE_FOOTSTEP_HEAVY )
	{
		EmitSound( "NPC_Antlion.FootstepHeavy" );
		return;
	}

	if ( pEvent->event == ANTLION_AE_MELEE_HIT1 )
	{
	...
	if ( pEvent->event == ANTLION_AE_MELEE_HIT2 )
	{
		QAngle qa( 20.0f, 0.0f, -12.0f );
	...
	if ( pEvent->event == ANTLION_AE_MELEE1_SOUND )
	{
		EmitSound( "NPC_Antlion.MeleeAttackSingle" );
		return;
	}

	if ( pEvent->event == ANTLION_AE_MELEE2_SOUND )
	{
		EmitSound( "NPC_Antlion.MeleeAttackDouble" );
		return;
	}

	if ( pEvent->event == ANTLION_AE_START_JUMP )
	{
		StartJump();
		return;
	}
...
bool CNPC_Antlion::CheckAlertRadius( void )
{
	...
	for ( CEntitySphereQuery sphere( GetAbsOrigin(), m_flAlertRadius ); (pEntity = sphere.GetCurrentEntity()) != NULL; sphere.NextEntity() )
	{
		//Must be mad at it
