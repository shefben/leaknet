...
public:
	...
	Class_T Classify() { return CLASS_MILITARY; }
...
protected:
	...
	EHANDLE	m_hDefaultTarget;		// Entity we track when we have nothing better to track.
	EHANDLE	m_hScriptedTarget;		// Entity we are tracking because we were told to by the level design.
...
BEGIN_DATADESC(CNPC_CombineCamera)
	...
	DEFINE_FIELD(CNPC_CombineCamera, m_iAmmoType, FIELD_INTEGER),
	DEFINE_KEYFIELD(CNPC_CombineCamera, m_iMinHealthDmg, FIELD_INTEGER, "minhealthdmg"),
	DEFINE_KEYFIELD(CNPC_CombineCamera, m_nInnerRadius, FIELD_INTEGER, "innerradius"),
	DEFINE_KEYFIELD(CNPC_CombineCamera, m_nOuterRadius, FIELD_INTEGER, "outerradius"),
	DEFINE_FIELD(CNPC_CombineCamera, m_bActive, FIELD_BOOLEAN),
	DEFINE_FIELD(CNPC_CombineCamera, m_bAngry, FIELD_BOOLEAN),
	DEFINE_FIELD(CNPC_CombineCamera, m_bBlinkState, FIELD_BOOLEAN),
	DEFINE_FIELD(CNPC_CombineCamera, m_bEnabled, FIELD_BOOLEAN),
	DEFINE_KEYFIELD(CNPC_CombineCamera, m_sDefaultTarget, FIELD_STRING, "defaulttarget"),
	DEFINE_FIELD(CNPC_CombineCamera, m_hDefaultTarget, FIELD_EHANDLE),
	DEFINE_FIELD(CNPC_CombineCamera, m_hScriptedTarget, FIELD_EHANDLE),
	DEFINE_FIELD(CNPC_CombineCamera, m_hEnemyTarget, FIELD_EHANDLE),
	DEFINE_FIELD(CNPC_CombineCamera, m_flLastSight, FIELD_TIME),
	DEFINE_FIELD(CNPC_CombineCamera, m_flPingTime, FIELD_TIME),
	DEFINE_FIELD(CNPC_CombineCamera, m_flClickTime, FIELD_TIME),
	DEFINE_FIELD(CNPC_CombineCamera, m_nClickCount, FIELD_INTEGER ),
	DEFINE_FIELD(CNPC_CombineCamera, m_flMoveSoundTime, FIELD_TIME),
	DEFINE_FIELD(CNPC_CombineCamera, m_flTurnOffEyeFlashTime, FIELD_TIME),
	DEFINE_FIELD(CNPC_CombineCamera, m_vecGoalAngles, FIELD_VECTOR),
	DEFINE_FIELD(CNPC_CombineCamera, m_pEyeGlow, FIELD_CLASSPTR),
	DEFINE_FIELD(CNPC_CombineCamera, m_pEyeFlash, FIELD_CLASSPTR),

	DEFINE_THINKFUNC(CNPC_CombineCamera, Deploy),
	DEFINE_THINKFUNC(CNPC_CombineCamera, IdleThink),
	DEFINE_THINKFUNC(CNPC_CombineCamera, ActiveThink),
	DEFINE_THINKFUNC(CNPC_CombineCamera, SearchThink),
	DEFINE_THINKFUNC(CNPC_CombineCamera, AutoSearchThink),
	DEFINE_THINKFUNC(CNPC_CombineCamera, DeathThink),

	// Inputs
	DEFINE_INPUTFUNC(CNPC_CombineCamera, FIELD_VOID, "Toggle", InputToggle),
	DEFINE_INPUTFUNC(CNPC_CombineCamera, FIELD_VOID, "Enable", InputEnable),
	DEFINE_INPUTFUNC(CNPC_CombineCamera, FIELD_VOID, "Disable", InputDisable),
	DEFINE_INPUTFUNC(CNPC_CombineCamera, FIELD_VOID, "SetAngry", InputSetAngry),
	DEFINE_INPUTFUNC(CNPC_CombineCamera, FIELD_VOID, "SetIdle", InputSetIdle),
	DEFINE_INPUTFUNC(CNPC_CombineCamera, FIELD_STRING, "SetTargetEntity", InputSetTargetEntity),
...
void CNPC_CombineCamera::Spawn()
{
	...
	m_iAmmoType = GetAmmoDef()->Index("SmallRound");
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_CombineCamera::Activate()
{
	BaseClass::Activate();

	// See if there is a default target to track.
	if (m_sDefaultTarget != NULL_STRING)
	{
		m_hDefaultTarget = gEntList.FindEntityByName(NULL, STRING(m_sDefaultTarget), NULL);
		if (m_hDefaultTarget)
		{
			m_bActive = true;
			Deploy();
...
//-----------------------------------------------------------------------------
// Purpose: Returns the current target to track. Any target set by the level
//			designer via SetTargetEntity takes precedence over our target enemy.
//-----------------------------------------------------------------------------
CBaseEntity *CNPC_CombineCamera::GetTarget()
{
	if (m_hScriptedTarget != NULL)
	{
		return m_hScriptedTarget;
	}
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_CombineCamera::ClearTarget()
{
	m_hScriptedTarget = NULL;
	m_hEnemyTarget = NULL;
}
...
void CNPC_CombineCamera::Deploy()
{
	...
	m_flLastSight = gpGlobals->curtime + COMBINE_CAMERA_MAX_WAIT;	
}
...
{
	if ((m_hEnemyTarget) || (m_hScriptedTarget))
	{
		return 180.0f;
	}
...
CBaseEntity *CNPC_CombineCamera::MaintainEnemy()
{
	...
	if (pEnemy)
	{
		...
		if (flDist < m_nOuterRadius)
		{
			if (FInViewCone(pEnemy))
			{
				...
				if (FVisible(pEnemy))
				{
					// There's a visible enemy within our zone of awareness.
					bGoIdle = false;

					// Track our enemy anywhere within our outer zone of awareness, but only acquire new
					// enemies within the inner zone.
					if ((pEnemy == m_hEnemyTarget) || ((flDist < m_nInnerRadius) && FInViewCone(pEnemy)))
					{
						// We have found a new enemy to track, or our current enemy
						// should still be our enemy.
						pFoundEnemy = pEnemy;
	...
	if (bGoIdle)
	{
		// We are completely unaware of any enemies.
		SetEyeState(CAMERA_EYE_IDLE);
	}
...
void CNPC_CombineCamera::ActiveThink()
{
	...
	CBaseEntity *pTarget = m_hScriptedTarget;
	if (!pTarget)
	{
		// No active target, look for suspicious characters.
		pTarget = MaintainEnemy();
		if (!pTarget)
		{
		...
		{
			if ((pTarget != m_hDefaultTarget) && (pTarget != m_hEnemyTarget))
			{
			...
			m_flLastSight = gpGlobals->curtime;
		}
	}
...
//-----------------------------------------------------------------------------
// Purpose: Think while tracking the default target.
//-----------------------------------------------------------------------------
void CNPC_CombineCamera::IdleThink()
{
	// Allow descended classes a chance to do something before the think function
	if (PreThink(CAMERA_ACTIVE))
		return;

	// Look for suspicious characters to track instead of the default target.
	if (!MaintainEnemy())
	{
		// Nobody suspicious. Just look at our default thing.
		TrackTarget(m_hDefaultTarget);
		MaintainEye();
...
void CNPC_CombineCamera::SearchThink()
{
	...
	// If our target has died, pick a new target
	if ((GetTarget() != NULL) && (GetTarget()->IsAlive() == false))
	{
		ClearTarget();
	}
...
//-----------------------------------------------------------------------------
void CNPC_CombineCamera::AutoSearchThink()
{
	// Allow descended classes a chance to do something before the think function
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_CombineCamera::InputSetTargetEntity(inputdata_t &inputdata)
{
	CBaseEntity *pPrevTarget = GetTarget();

	m_hScriptedTarget = gEntList.FindEntityByName(NULL, inputdata.value.String(), inputdata.pActivator);

	if (!m_hScriptedTarget)
	{
		if (!m_hEnemyTarget)
		{
			// Nothing to track now -- go idle.
			EmitSound("NPC_CombineCamera.BecomeIdle");
			SetEyeState(CAMERA_EYE_IDLE);
			SetAngry(false);
			SetThink(&CNPC_CombineCamera::IdleThink);
...
int CNPC_CombineCamera::DrawDebugTextOverlays(void) 
{
	...
	if (m_debugOverlays & OVERLAY_TEXT_BIT) 
	{
		...
		Q_snprintf( tempstr, sizeof( tempstr ),"Def Target: %s", m_hDefaultTarget ? m_hDefaultTarget->GetDebugName() : "<none>");
		NDebugOverlay::EntityText(entindex(),text_offset,tempstr,0);
		text_offset++;

		Q_snprintf( tempstr, sizeof( tempstr ),"Target    : %s", m_hScriptedTarget ? m_hScriptedTarget->GetDebugName() : "<none>");
		NDebugOverlay::EntityText(entindex(),text_offset,tempstr,0);
		text_offset++;
