...
BEGIN_DATADESC( CNPC_Houndeye )
	...
	DEFINE_FIELD( CNPC_Houndeye, m_flNextSecondaryAttack,	FIELD_TIME ),
	DEFINE_FIELD( CNPC_Houndeye, m_bLoopClockwise,			FIELD_BOOLEAN ),
	DEFINE_FIELD( CNPC_Houndeye, m_pEnergyWave,				FIELD_CLASSPTR ),
	DEFINE_FIELD( CNPC_Houndeye, m_flEndEnergyWaveTime,		FIELD_TIME ),	
		...
		case HOUND_AE_LEAP_HIT:
			{
				...
				if ( iSound != 0 )
				{
				//<<TEMP>>	UTIL_EmitSoundDyn( edict(), CHAN_VOICE, pAttackSounds[iSound], GetSoundVolue(), ATTN_IDLE, 0, GetVoicePitch() );
				}
