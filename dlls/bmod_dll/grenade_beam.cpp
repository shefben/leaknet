...
BEGIN_DATADESC( CGrenadeBeamChaser )
	...
	DEFINE_FIELD( CGrenadeBeamChaser, m_pTarget, FIELD_CLASSPTR ),

	// Function pointers
	DEFINE_FUNCTION( CGrenadeBeamChaser, ChaserThink ),
...
void CGrenadeBeamChaser::Spawn( void )
{
	if (!m_pTarget)
	{
		Warning("%s spawned without m_pTarget!\n", GetClassname(), GetLocalOrigin());
		UTIL_Remove(this);
	}
...
BEGIN_DATADESC( CGrenadeBeam )
	...
	DEFINE_FIELD( CGrenadeBeam, m_vLaunchPos,		FIELD_POSITION_VECTOR ),
	DEFINE_FIELD( CGrenadeBeam, m_flBeamWidth,		FIELD_FLOAT ),
	DEFINE_FIELD( CGrenadeBeam, m_flBeamSpeed,		FIELD_FLOAT ),
	DEFINE_FIELD( CGrenadeBeam, m_flBeamLag,		FIELD_FLOAT ),
	DEFINE_FIELD( CGrenadeBeam, m_flLaunchTime,		FIELD_TIME ),
	DEFINE_FIELD( CGrenadeBeam, m_flLastTouchTime,	FIELD_TIME ),
	DEFINE_FIELD( CGrenadeBeam, m_hBeamChaser,		FIELD_EHANDLE ),
	DEFINE_FIELD( CGrenadeBeam, m_nNumHits,			FIELD_INTEGER ),

	DEFINE_ARRAY( CGrenadeBeam, m_pHitLocation,		FIELD_VECTOR,	GRENADEBEAM_MAXHITS ),
	DEFINE_ARRAY( CGrenadeBeam, m_pBeam,			FIELD_CLASSPTR, GRENADEBEAM_MAXBEAMS ),

	// Function pointers
	DEFINE_FUNCTION( CGrenadeBeam, GrenadeBeamTouch ),
	DEFINE_FUNCTION( CGrenadeBeam, KillBeam ),
...
void CGrenadeBeam::KillBeam(void)
{
	...
	for (int i=0;i<GRENADEBEAM_MAXBEAMS;i++)
	{
		if (m_pBeam[i])
		{
			UTIL_RemoveImmediate(m_pBeam[i]);
		}
	}
}
//------------------------------------------------------------------------------
...
// Output  :
//------------------------------------------------------------------------------
bool CGrenadeBeam::ShouldTransmit( const edict_t *recipient, const void *pvs, int clientArea )
{
