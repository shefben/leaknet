...
BEGIN_DATADESC( CWeaponShotgun )
	...
	DEFINE_FIELD( CWeaponShotgun, m_bNeedPump, FIELD_BOOLEAN ),
	DEFINE_FIELD( CWeaponShotgun, m_bDelayedFire1, FIELD_BOOLEAN ),
	DEFINE_FIELD( CWeaponShotgun, m_bDelayedFire2, FIELD_BOOLEAN ),
...
void CWeaponShotgun::PrimaryAttack( void )
{
	...
	pPlayer->FireBullets( 7, vecSrc, vecAiming, GetBulletSpread(), MAX_TRACE_LENGTH, m_iPrimaryAmmoType, 0, -1, -1, 0, NULL, true );
...
void CWeaponShotgun::ItemPostFrame( void )
{
	...
	else 
	{
		...
		if ( !HasAnyAmmo() && m_flNextPrimaryAttack < gpGlobals->curtime ) 
		{
			...
			if ( !(GetFlags() & ITEM_FLAG_NOAUTOSWITCHEMPTY) && pOwner->SwitchToNextBestWeapon( this ) )
			{
