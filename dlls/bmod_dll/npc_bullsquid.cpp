...
//	DEFINE_FIELD( CNPC_Bullsquid, m_nSquidSpitSprite,	FIELD_INTEGER ),
	DEFINE_FIELD( CNPC_Bullsquid, m_flHungryTime,		FIELD_TIME ),
	DEFINE_FIELD( CNPC_Bullsquid, m_nextSquidSoundTime,	FIELD_TIME ),
...
void CNPC_Bullsquid::GrowlSound( void )
{
	if (gpGlobals->curtime >= m_nextSquidSoundTime)
	{
		...
		m_nextSquidSoundTime	= gpGlobals->curtime + random->RandomFloat(1.5,3.0);
	}
}
