...
#define	MD_TURN_SOUND	"npc/entity/turret/turret_loop1.wav"
...
BEGIN_DATADESC( CNPC_MissileDefense )
	...
	DEFINE_FIELD( CNPC_MissileDefense, m_iAmmoLoaded,		FIELD_INTEGER ),
	DEFINE_FIELD( CNPC_MissileDefense, m_flReloadedTime,	FIELD_TIME ),
	DEFINE_FIELD( CNPC_MissileDefense, m_vGunAng,			FIELD_VECTOR ),
...
void CNPC_MissileDefense::FireCannons( void )
{
	...
	if( flDamage > 0 )
	{
		...
		CalculateBulletDamageForce( &info, GetAmmoDef()->Index("MediumRound"), vecDir, GetEnemy()->GetAbsOrigin() );
...
void CNPC_MissileDefense::EnemyShootPosition(CBaseEntity* pEnemy, Vector *vPosition)
{
	...
	*vPosition = pEnemy->GetAbsOrigin();
	...
	Vector	vPredVel	= pEnemy->GetAbsVelocity() * 0.5;
