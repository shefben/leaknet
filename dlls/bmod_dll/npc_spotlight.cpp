...
BEGIN_DATADESC( CNPC_Spotlight )
	DEFINE_FIELD( CNPC_Spotlight, m_vInspectPos, FIELD_POSITION_VECTOR ),
	DEFINE_FIELD( CNPC_Spotlight, m_flYaw, FIELD_FLOAT ),
	DEFINE_FIELD( CNPC_Spotlight, m_flYawCenter, FIELD_FLOAT ),
	DEFINE_FIELD( CNPC_Spotlight, m_flYawSpeed, FIELD_FLOAT ),
	DEFINE_FIELD( CNPC_Spotlight, m_flYawDir, FIELD_FLOAT ),
	DEFINE_FIELD( CNPC_Spotlight, m_flPitch, FIELD_FLOAT ),
	DEFINE_FIELD( CNPC_Spotlight, m_flPitchCenter, FIELD_FLOAT ),
	DEFINE_FIELD( CNPC_Spotlight, m_flPitchSpeed, FIELD_FLOAT ),
	DEFINE_FIELD( CNPC_Spotlight, m_flPitchDir, FIELD_FLOAT ),
	DEFINE_FIELD( CNPC_Spotlight, m_flSpotlightCurLength, FIELD_FLOAT ),

	DEFINE_FIELD( CNPC_Spotlight, m_fSpotlightFlags,		FIELD_INTEGER ),
	DEFINE_FIELD( CNPC_Spotlight, m_flInspectEndTime,		FIELD_TIME ),
	DEFINE_FIELD( CNPC_Spotlight, m_flNextEntitySearchTime,	FIELD_TIME ),
	DEFINE_FIELD( CNPC_Spotlight, m_flNextHintSearchTime,	FIELD_TIME ),
	DEFINE_FIELD( CNPC_Spotlight, m_bHadEnemy,				FIELD_BOOLEAN ),
	DEFINE_FIELD( CNPC_Spotlight, m_vSpotlightTargetPos,	FIELD_POSITION_VECTOR ),
	DEFINE_FIELD( CNPC_Spotlight, m_vSpotlightCurrentPos,	FIELD_POSITION_VECTOR ),
	DEFINE_FIELD( CNPC_Spotlight, m_pSpotlight,				FIELD_CLASSPTR ),
	DEFINE_FIELD( CNPC_Spotlight, m_pSpotlightTarget,		FIELD_CLASSPTR ),
	DEFINE_FIELD( CNPC_Spotlight, m_vSpotlightDir,			FIELD_VECTOR ),
	DEFINE_FIELD( CNPC_Spotlight, m_nHaloSprite,			FIELD_INTEGER ),
	DEFINE_FIELD( CNPC_Spotlight, m_pScriptedTarget,		FIELD_CLASSPTR ),

	DEFINE_KEYFIELD( CNPC_Spotlight, m_flYawRange,			FIELD_FLOAT, "YawRange"),
	DEFINE_KEYFIELD( CNPC_Spotlight, m_flPitchMin,			FIELD_FLOAT, "PitchMin"),
	DEFINE_KEYFIELD( CNPC_Spotlight, m_flPitchMax,			FIELD_FLOAT, "PitchMax"),
	DEFINE_KEYFIELD( CNPC_Spotlight, m_flIdleSpeed,			FIELD_FLOAT, "IdleSpeed"),
	DEFINE_KEYFIELD( CNPC_Spotlight, m_flAlertSpeed,		FIELD_FLOAT, "AlertSpeed"),
	DEFINE_KEYFIELD( CNPC_Spotlight, m_flSpotlightMaxLength,FIELD_FLOAT, "SpotlightLength"),
	DEFINE_KEYFIELD( CNPC_Spotlight, m_flSpotlightGoalWidth,FIELD_FLOAT, "SpotlightWidth"),

	// DEBUG						m_pScriptedTarget

	// Inputs
	DEFINE_INPUTFUNC( CNPC_Spotlight,	FIELD_VOID,		"LightOn",		InputLightOn ),
	DEFINE_INPUTFUNC( CNPC_Spotlight,	FIELD_VOID,		"LightOff",		InputLightOff ),
	DEFINE_INPUTFUNC( CNPC_Spotlight,	FIELD_VOID,		"TrackOn",		InputTrackOn ),
	DEFINE_INPUTFUNC( CNPC_Spotlight,	FIELD_VOID,		"TrackOff",		InputTrackOff ),

	// Outputs
	DEFINE_OUTPUT(CNPC_Spotlight, m_pOutputAlert,			"OnAlert"				),
	DEFINE_OUTPUT(CNPC_Spotlight, m_pOutputDetect,			"DetectedEnemy"			),
	DEFINE_OUTPUT(CNPC_Spotlight, m_pOutputLost,			"LostEnemy"				),
	DEFINE_OUTPUT(CNPC_Spotlight, m_pOutputSquadDetect,		"SquadDetectedEnemy"	),
	DEFINE_OUTPUT(CNPC_Spotlight, m_pOutputSquadLost,		"SquadLostEnemy"		),
	DEFINE_OUTPUT(CNPC_Spotlight, m_pOutputPosition,		"LightPosition"			),
...
// Output  :
//-------------------------------------------------------------------------------------
bool CNPC_Spotlight::ShouldTransmit( const edict_t *recipient, const void *pvs, int clientArea )
{
...
void CNPC_Spotlight::ClearInspectTarget(void)
{
	...
	if (m_pHintNode)
	{
		m_pHintNode->Unlock(SPOTLIGHT_HINT_INSPECT_LENGTH);
	}
	...
	if (m_bHadEnemy)
	{
		...
		{
			m_pOutputLost.Set(NULL,this,this);
		}
	}
	...
	if (m_fSpotlightFlags & BITS_SPOTLIGHT_TRACK_ON)
	{
		...
		if (GetEnemy() == NULL)
		{
			...
			if (gpGlobals->curtime		>	m_flNextHintSearchTime	&&
				!HaveInspectTarget()							)
			{
				m_pHintNode = CAI_Hint::FindHint(this, HINT_NONE, 0, SPOTLIGHT_HINT_SEARCH_DIST);

				if (m_pHintNode)
				{
					m_flNextHintSearchTime	= gpGlobals->curtime + SPOTLIGHT_HINT_INSPECT_LENGTH;
					SetInspectTargetToHint(m_pHintNode,SPOTLIGHT_HINT_INSPECT_LENGTH);
				}
			}
		}
...
void CNPC_Spotlight::Spawn(void)
{
	...
	{
		Msg("ERROR: Invalid spotlight width <= 0, setting to 10\n");
