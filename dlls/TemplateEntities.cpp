...
BEGIN_SIMPLE_DATADESC( TemplateEntityData_t )
	//DEFINE_FIELD( TemplateEntityData_t,	pszName,			FIELD_STRING	),		// Saved custom, see below
	//DEFINE_FIELD( TemplateEntityData_t,	pszMapData,			FIELD_STRING	),		// Saved custom, see below
	DEFINE_FIELD( TemplateEntityData_t,	iszMapData,				FIELD_STRING	),
	DEFINE_FIELD( TemplateEntityData_t,	iMapDataLength,			FIELD_INTEGER	),
	DEFINE_FIELD( TemplateEntityData_t,	bNeedsEntityIOFixup,	FIELD_BOOLEAN	),

	//DEFINE_FIELD( TemplateEntityData_t,	pszFixedMapData,	FIELD_STRING	),		// Not saved at all
END_DATADESC()
			...
			do 
			{
				...
				for ( int iTName = 0; iTName < iCount; iTName++ )
				{
					...
					if ( !_stricmp( pName, sValue ) )
					{
						...
						if ( s )
						{
							Q_strncat( valueclipped, ENTITYIO_FIXUP_STRING, sizeof(valueclipped) );
							Q_strncat( valueclipped, s, sizeof(valueclipped) );
							mapData->SetValue( keyName, valueclipped );
						}
						else
						{
							Q_strncat( value, ENTITYIO_FIXUP_STRING, sizeof(value) );
							mapData->SetValue( keyName, value );
						}
