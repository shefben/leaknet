...
void CBaseProp::Spawn( void )
{
	...
	if ( !OverridePropdata() )
	{
		...
		else if ( iResult == PARSE_FAILED_NO_DATA )
		{
			...
			// VXP: NOTE: If this entity is a physics_prop - it won't be removed. Let's say I'll leave this for compatibility
		...
		else if ( iResult == PARSE_SUCCEEDED )
		{
			...
			if (	!FClassnameIs( this, "prop_physics" )
				&&	!FClassnameIs( this, "physics_prop" ) // VXP: Let's not remove this entity, if it's a physics_prop one
				&&	!FClassnameIs( this, "prop_physics_override" )
				&&	!FClassnameIs( this, "prop_dynamic_override" ) )
			{
...
public:
	...
	virtual void ParseKeyValue( void *pData, const char *pKey, const char *pValue )
	{
		...
		else if ( !Q_stricmp( pKey, "offset" ) )
		{
			UTIL_StringToVector( pModel->offset.Base(), pValue );
		}
		else if ( !Q_stricmp( pKey, "health" ) )
		{
			pModel->health = atof(pValue);
		}
		else if ( !Q_stricmp( pKey, "fadetime" ) )
		{
...
void PropBreakableCreateAll( int modelindex, IPhysicsObject *pPhysics, const breakablepropparams_t &params )
{
	...
	for ( int i = 0; i < list.Count(); i++ )
	{
		...
		if ( nActualSkin > StudioHdr_GetNumSkinFamilies(pStudioHdr) )
			nActualSkin = 0;
