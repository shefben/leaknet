...
int CAI_BaseNPC::DrawDebugTextOverlays(void)
{
	...
	if (m_debugOverlays & OVERLAY_NPC_SQUAD_BIT)
	{
		...
		if (m_pSquad)
		{
			Q_strncat(tempstr,m_pSquad->GetName(),sizeof(tempstr));

			if( m_pSquad->GetLeader() == this )
			{
				Q_strncat(tempstr," (LEADER)",sizeof(tempstr));
			}
		...
		if (GetEnemy())
		{
			...
			{
				Q_strncat(tempstr,STRING(GetEnemy()->GetEntityName()),sizeof(tempstr));
				Q_strncat(tempstr,"\n",sizeof(tempstr));
			}
			else
			{
				Q_strncat(tempstr,STRING(GetEnemy()->m_iClassname),sizeof(tempstr));
				Q_strncat(tempstr,"\n",sizeof(tempstr));
			}
		}
		else
