...
CON_COMMAND(ai_debug_node_connect, "Debug the attempted connection between two nodes")
{
	...
	Msg( "ai_debug_node_connect: debugging enbabled for %d <--> %d\n", g_DebugConnectNode1, g_DebugConnectNode2 );
}
...
BEGIN_DATADESC( CAI_NetworkManager )
	...
	DEFINE_FIELD( CAI_NetworkManager,	m_bNeedGraphRebuild, FIELD_BOOLEAN ),
	//									m_pEditOps
	//									m_pNetwork
	DEFINE_FIELD( CAI_NetworkManager,	m_fInitalized, FIELD_BOOLEAN ),

	// Function pointers
	DEFINE_FUNCTION( CAI_NetworkManager, DelayedInit ),
	DEFINE_FUNCTION( CAI_NetworkManager, RebuildThink ),
...
void CAI_NetworkManager::LoadNetworkGraph( void )
{
	...
	CUtlBuffer buf( 0, fileSize, true );
	...
	for (int link = 0; link < totalNumLinks; link++)
	{
		...
		buf.Scanf("%d", &new_link->m_iSrcID);

		buf.Scanf("%d", &new_link->m_iDestID);

		for (int hull =0;hull<NUM_HULLS;hull++)
		{
			buf.Scanf("%d", &new_link->m_iAcceptedMoveTypes[hull]);
		}
		// Now add link to source and destination nodes
		m_pNetwork->GetNode(new_link->m_iSrcID)->AddLink(new_link);
		m_pNetwork->GetNode(new_link->m_iDestID)->AddLink(new_link);
	}
...
void CAI_NetworkManager::InitializeAINetworks()
{
	...
	pNetwork->SetNextThink( gpGlobals->curtime + 0.1f );
}
...
bool CAI_NetworkManager::IsAIFileCurrent ( const char *szMapName )
{
	...
	Q_strncpy ( szBspFilename, "maps/" ,sizeof(szBspFilename));
	strcat ( szBspFilename, szMapName );
	strcat ( szBspFilename, ".bsp" );

	Q_strncpy ( szGraphFilename, "maps/graphs/" ,sizeof(szGraphFilename));
	strcat ( szGraphFilename, szMapName );
	strcat ( szGraphFilename, ".ain" );
...
void CAI_NetworkManager::DelayedInit( void )
{
	if ( !g_ai_norebuildgraph.GetInt() )
	{
		...
		else if ( !CAI_NetworkManager::NetworksLoaded() || engine->IsInEditMode())
		{
			...
			Msg( "Node Graph out of Date. Rebuilding...\n" );
	...
	if ( g_AI_Manager.NumAIs() != 0 && g_pBigAINet->NumNodes() == 0 )
		Msg( "WARNING: Level contains NPCs but has no path nodes\n" );
}
...
int CAI_NetworkEditTools::GetNodeIdFromWCId( int nWCId )
{
	...
	{
		Msg("ERROR: Trying to get WC ID with no table!\n");
		return -1;
	}

	if (!m_pNetwork->NumNodes())
	{
		Msg("ERROR: Trying to get WC ID with no network!\n");
		return -1;
	}
...
void CAI_NetworkEditTools::RecalcUsableNodesForHull(void) 
{
	...
	{
		if (m_pTestHull->GetNavigator()->CanFitAtNode(node))
		{
...
void CAI_NetworkEditTools::DrawEditInfoOverlay(void)
{
	...
	if (engine->IsInEditMode())
	{
		...
		strncat(outTxt,outTxt2,sizeof(outTxt));
...
void CAI_NetworkEditTools::DrawAINetworkOverlay(void)
{
	...
	{
		CAI_Hint::DrawHintOverlays(flDrawDuration);
	}
			...
			if (pAINode[node]->GetType() != NODE_DELETED)
			{
				...
				if (m_debugNetOverlays & bits_debugOverlayNodes) 
				{
					...
					if (m_debugNetOverlays & bits_debugOverlayNodesLev2)
					{
						...
						if (engine->IsInEditMode())
						{
							// Display both the engine node ID and the WC node ID
							Q_snprintf(id,sizeof(id),"%i (%i)",node,m_pNodeIndexTable[pAINode[node]->GetId()]);
						}
						else
						{
							Q_snprintf(id,sizeof(id),"%i",node);
						}
						Vector loc = nodePos;
						loc.x+=6;
						loc.y+=6;
						loc.z+=6;
						NDebugOverlay::Text( loc, id , true, flDrawDuration);
					}
				}
			}
		}
...
void CAI_NetworkBuilder::Build( CAI_Network *pNetwork )
{
	...
	VPROF_BUDGET( "AINet", "AINet" )
	BeginBuild();

	CFastTimer masterTimer;
	CFastTimer timer;
	
	Msg( "Building AI node graph...\n");
	masterTimer.Start();
	
	// ---------------------------
	// Initialize node positions
	// ---------------------------
	Msg( "Initializing node positions...\n" );
	timer.Start();
	int i;
	for ( i = 0; i < nNodes; i++)
	{
	...
	nNodes = pNetwork->NumNodes(); // InitNodePosition can create nodes
	timer.End();
	Msg( "...done initializing node positions. %f seconds\n", timer.GetDuration().GetSeconds() );

	// ---------------------------
	// Initialize node neighbors
	// ---------------------------
	Msg( "Initializing node neighbors...\n" );
	timer.Start();
	m_DidSetNeighborsTable.Resize( nNodes );
	m_DidSetNeighborsTable.ClearAllBits();
	m_NeighborsTable.SetSize( nNodes );
	...
	timer.End();
	Msg( "...done initializing node neighbors. %f seconds\n", timer.GetDuration().GetSeconds() );

	// ---------------------------
	// Force node neighbors for dynamic links
	// ---------------------------
	Msg( "Forcing dynamic link neighbors...\n" );
	timer.Start();
	ForceDynamicLinkNeighbors();
	timer.End();
	Msg( "...done forcing dynamic link neighbors. %f seconds\n", timer.GetDuration().GetSeconds() );

	// ---------------------------
	// Initialize accepted hulls
	// ---------------------------
	Msg( "Determining links...\n" );
	timer.Start();
	for (i = 0; i < nNodes; i++)
	{	
	...
	timer.End();
	Msg( "...done determining links. %f seconds\n", timer.GetDuration().GetSeconds() );

	// ------------------------------
	// Initialize disconnected nodes
	// ------------------------------
	Msg( "Determining zones...\n" );
	timer.Start();
	InitZones( pNetwork);
	timer.End();
	masterTimer.End();
	Msg( "...done determining zones. %f seconds\n", timer.GetDuration().GetSeconds() );
	Msg( "...done building AI node graph, %f seconds\n", masterTimer.GetDuration().GetSeconds() );
...
void CAI_NetworkBuilder::InitClimbNodePosition(CAI_Network *pNetwork, CAI_Node *pNode) 
{
	...
	for (int i = 0; i < 3; i++)
	{
		...
		// Msg( "to %f %f %f : %d %f", origin.x, origin.y, origin.z, trace.startsolid, trace.fraction );

		if (!trace.startsolid && trace.fraction == 1.0)
		{
			float floorZ = GetFloorZ(origin); // FIXME: don't use this

			if (fabs(pNode->m_vOrigin.z - floorZ) < 36)
			{
				CAI_Node *new_node		= pNetwork->AddNode( pNode->m_vOrigin, pNode->m_flYaw );
				...
				else
				{
					for (int hull = 0; hull < NUM_HULLS; hull++)
					{
						if (fabs(pNode->m_flVOffset[hull] - new_node->m_flVOffset[hull]) > 0.1)
						{
							Msg("ERROR: Climb Node %i has different exit heights\n",pNode->m_iID);
						}
					}
				}
	...
	if (numExits == 0)
	{
		Msg("ERROR: Climb Node %i has no way off\n",pNode->m_iID);
	}
...
void CAI_NetworkBuilder::InitGroundNodePosition(CAI_Network *pNetwork, CAI_Node *pNode)
{
	...
	{
		trace_t tr;
		Vector origin = pNode->GetPosition(hull);
		Vector mins, maxs;
		...
		pNode->m_flVOffset[hull] = tr.endpos.z - pNode->m_vOrigin.z + 0.1;
	}
}
...
void CAI_NetworkBuilder::InitNodePosition(CAI_Network *pNetwork, CAI_Node *pNode) 
{
	...
	else if (pNode->m_eNodeType == NODE_GROUND)
	{
		...
		if (pNode->m_flVOffset[HULL_SMALL_CENTERED] < -100)
		{
			...
			Msg("ERROR: Node %.0f %.0f %.0f too low. (WC ID# %i) Fell through floor!\n",
				pNode->m_vOrigin.x, pNode->m_vOrigin.y, pNode->m_vOrigin.z, 
	...
	else if ( UTIL_PointContents(m_vOrigin) & MASK_WATER )
	{
	...
	for (int testnode = 0; testnode < pNetwork->NumNodes(); testnode++ )
  	{
		...
		if (testNode->GetOrigin() == pNode->m_vOrigin && testNode->GetType() != NODE_CLIMB)
		{
		...
		float flDistToCheckNode = ( testNode->GetOrigin() - pNode->m_vOrigin ).LengthSqr(); 
...
void CAI_NetworkBuilder::InitNeighbors(CAI_Network *pNetwork, CAI_Node *pNode)
{
	...
	{
	AI_PROFILE_SCOPE( CAI_Node_InitNeighbors );
	...
	for (int checknode = 0; checknode < pNetwork->NumNodes(); checknode++ )
	{
		...
		for (int testnode = 0; testnode < pNetwork->NumNodes(); testnode++ )
		{
			...
			// ----------------------------------------------------------
			if ((pNetwork->GetNode(checknode)->GetType() == NODE_AIR && pNetwork->GetNode(testnode)->GetType() != NODE_AIR)||
				(pNetwork->GetNode(checknode)->GetType() != NODE_AIR && pNetwork->GetNode(testnode)->GetType() == NODE_AIR))
			{
				continue;
			}

			// ----------------------------------------------------------
			// If climb node, don't consider redundancy
			// ----------------------------------------------------------
			if (pNetwork->GetNode(checknode)->GetType() == NODE_CLIMB) 
			{
				continue;
			}
			...
			Vector	 vec2DirToCheckNode = pCheckNode->GetOrigin() - pNode->m_vOrigin; 
			float	 flDistToCheckNode  = VectorNormalize( vec2DirToCheckNode );

			Vector	 vec2DirToTestNode = ( pTestNode->GetOrigin() - pNode->m_vOrigin ); 
			float	 flDistToTestNode  = VectorNormalize( vec2DirToTestNode );

			if ( DotProduct ( vec2DirToCheckNode, vec2DirToTestNode ) >= 0.92388 ) // 45 degrees
			{
...
int CAI_NetworkBuilder::ComputeConnection( CAI_Node *pSrcNode, CAI_Node *pDestNode, Hull_t hull )
{
	...
	if ( !m_pTestHull->GetNavigator()->CanFitAtNode(srcId,MASK_NPCWORLDSTATIC) )
	{
		DebugConnectMsg( srcId, destId, "      Cannot fit at node %d\n", srcId );
		return 0;
	}
	
	if ( !m_pTestHull->GetNavigator()->CanFitAtNode(destId,MASK_NPCWORLDSTATIC) )
	{
		DebugConnectMsg( srcId, destId, "      Cannot fit at node %d\n", destId );
		return 0;
	}
	...
	if (pSrcNode->m_eNodeType == NODE_AIR || pDestNode->GetType() == NODE_AIR) 
	{
		...
		if (pSrcNode->m_eNodeType == NODE_AIR && pDestNode->GetType() == NODE_AIR)
		{
			AI_TraceHull( pSrcNode->m_vOrigin, pDestNode->GetOrigin(), NAI_Hull::Mins(hull),NAI_Hull::Maxs(hull), MASK_NPCWORLDSTATIC, m_pTestHull, COLLISION_GROUP_NONE, &tr );
			if (!tr.startsolid && tr.fraction == 1.0)
			{
	...
	else if ((pSrcNode->m_eNodeType == NODE_CLIMB) && (pDestNode->GetType() == NODE_CLIMB))
	{
		...
		if (pSrcNode->m_vOrigin == pDestNode->GetOrigin())
		{
	...
	else if ((pSrcNode->m_eNodeType == NODE_GROUND) || (pDestNode->GetType() == NODE_GROUND))
	{
		...
		{ // profile scope
		AI_PROFILE_SCOPE( CAI_Node_InitLinks_Ground );
	...
	for (int i = 0; i < pNetwork->NumNodes(); i++ )
  	{
		...
		if ( m_NeighborsTable[pNode->m_iID].GetBit(pDestNode->m_iID) ) 
		{
			...
			if (!bAllFailed) 
			{
				CAI_Link *newLink = new CAI_Link;
				newLink->m_iSrcID			= pNode->m_iID;
				newLink->m_iDestID			= pDestNode->m_iID;

				for (int hull=0;hull<NUM_HULLS;hull++)
				{
					newLink->m_iAcceptedMoveTypes[hull]	= acceptedMotions[hull];
				}
				((CAI_Node *)pNode)->AddLink(newLink);
				pDestNode->AddLink(newLink);
