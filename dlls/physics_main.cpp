...
private:
	...
	// Relink everything that is in the push hierarchy
	void	RelinkAllPushers();
...
// Relink everything that is in the push hierarchy
//-----------------------------------------------------------------------------
void CPhysicsPushedEntities::RelinkAllPushers()
{
	for ( int i = m_rgPusher.Count(); --i >=0; )
	{
		engine->RelinkEntity( m_rgPusher[i].m_pEntity->edict(), false );
	}
}
...
void CPhysicsPushedEntities::UnlinkPusherList( int *pPusherHandles )
{
	for ( int i = m_rgPusher.Count(); --i >= 0; )
	{
		pPusherHandles[i] = engine->FastUnlink( m_rgPusher[i].m_pEntity->edict() );
	}
}
...
void CPhysicsPushedEntities::RelinkPusherList( int *pPusherHandles )
{
	for ( int i = m_rgPusher.Count(); --i >= 0; )
	{
		engine->FastRelink( m_rgPusher[i].m_pEntity->edict(), pPusherHandles[i] );
	}
}
...
void CPhysicsPushedEntities::ComputeRotationalPushDirection( CBaseEntity *pBlocker, const RotatingPushMove_t &rotPushMove, Vector *pMove, CBaseEntity *pRoot )
{
	...
	Vector start = pBlocker->GetAbsOrigin();
	if ( pRoot->GetSolid() == SOLID_VPHYSICS )
	{
		...
		start.x = (pMove->x < 0) ? pBlocker->GetAbsMaxs().x : pBlocker->GetAbsMins().x;
		start.y = (pMove->y < 0) ? pBlocker->GetAbsMaxs().y : pBlocker->GetAbsMins().y;
		start.z = (pMove->z < 0) ? pBlocker->GetAbsMaxs().z : pBlocker->GetAbsMins().z;
	}
...
bool CPhysicsPushedEntities::SpeculativelyCheckPush( PhysicsPushedInfo_t &info, const Vector &vecAbsPush, bool bRotationalPush )
{
	...
	PhysicsCheckSweep( pBlocker, pBlocker->GetAbsOrigin(), vecAbsPush, &info.m_Trace );
	...
	if ( info.m_Trace.fraction )
	{
		...
		engine->RelinkEntity( pBlocker->edict(), false );
	}
	...
	// FIXME: Shouldn't we simply be testing the blocker against the pusher list here??
	// FIXME: If the trace fraction == 0 can we early out also?
	info.m_bBlocked = pBlocker->PhysicsTestEntityPosition();
	if ( !info.m_bBlocked )
		return true;

	// We're not blocked if the blocker is point-sized or non-solid
	if ( pBlocker->IsPointSized() )
		return true;

	if ( !pBlocker->IsSolid() || pBlocker->IsSolidFlagSet( FSOLID_VOLUME_CONTENTS ) )
		return true;
...
void CPhysicsPushedEntities::FinishPushers()
{
	...
	for ( int i = m_rgPusher.Count(); --i >= 0; )
	{
		...
		engine->RelinkEntity( info.m_pEntity->edict(), true, &info.m_vecStartAbsOrigin );

		info.m_pEntity->UpdatePhysicsShadowToCurrentPosition( 0 );
	}
}
...
void CPhysicsPushedEntities::FinishRotPushedEntity( CBaseEntity *pPushedEntity, const RotatingPushMove_t &rotPushMove )
{
	...
	if ( pPushedEntity->IsPlayer() )
	{
		...
		player->pl.anglechangetotal += rotPushMove.amove[1];
		player->pl.anglechangefinal = rotPushMove.amove[1];
	}
	else
	{
		QAngle angles = pPushedEntity->GetAbsAngles();
		VectorAdd( angles, rotPushMove.amove, angles );
...
void CPhysicsPushedEntities::FinishPush( bool bIsRotPush, const RotatingPushMove_t *pRotPushMove )
{
	...
	for ( int i = m_rgMoved.Count(); --i >= 0; )
	{
		...
		if ( info.m_pEntity->GetMoveType() != MOVETYPE_WALK )
		{
			info.m_pEntity->RemoveFlag( FL_ONGROUND );
		}
...
{
	// Relink all of the pusher entities to get them back into place
	RelinkAllPushers();
	...
	for ( int i = m_rgMoved.Count(); --i >= 0; )
	{
		...
		// When moving back, be sure not to touch triggers
		engine->RelinkEntity( m_rgMoved[ i ].m_pEntity->edict(), false );
	}
}
...
public:
	CPushBlockerEnum( CPhysicsPushedEntities *pPushedEntities ) : m_pPushedEntities(pPushedEntities)
	{
		...
		m_pRootHighestParent = GetHighestParent( m_pPushedEntities->m_rgPusher[0].m_pEntity );
		++s_nEnumCount;
	}
...
private:
	...
	{
		if ( pCheck->GetFlags() & FL_ONGROUND )
		{
	...
	CBaseEntity *GetPushableEntity( IHandleEntity *pHandleEntity )
	{
		...
		CBaseEntity *pCheckHighestParent = GetHighestParent(pCheck);
		...
		{
			// FIXME: This does a test against the KD tree...
			// It would be a ton faster to directly test against all the pushers
			// in the pusher hierarchy, since we know they are the only ones who changed
			// It also would avoid re-entrant KD tree iteration

			// see if the ent's bbox is inside the pusher's final position
			if ( !pCheck->PhysicsTestEntityPosition() )
				return NULL;
		}
...
void CPhysicsPushedEntities::GenerateBlockingEntityList( )
{
	...
	for ( int i = m_rgPusher.Count(); --i >= 0;  )
	{
		...
		partition->EnumerateElementsInBox( PARTITION_ENGINE_NON_STATIC_EDICTS, 
			pPusher->GetAbsMins(), pPusher->GetAbsMaxs(), false, &blockerEnum );
	}
}
...
void CPhysicsPushedEntities::RotateRootEntity( CBaseEntity *pRoot, float movetime, RotatingPushMove_t &rotation )
{
	...
	AngleMatrix( pRoot->GetAbsAngles(), rotation.origin, rotation.startLocalToWorld );
	...
	AngleMatrix( pRoot->GetAbsAngles(), rotation.origin, rotation.endLocalToWorld );

	// Relink all children without touching triggers) so everything's in the right place...
	RelinkAllPushers();
}
...
//   4 = dead stop
//-----------------------------------------------------------------------------
int CBaseEntity::PhysicsTryMove( float time, trace_t *steptrace )
{
...
// Output : trace_t
//-----------------------------------------------------------------------------
trace_t CBaseEntity::PhysicsPushEntity( const Vector& push )
{
	...
	trace_t		trace;
	::PhysicsCheckSweep( this, prevOrigin, push, &trace );
	...
	engine->RelinkEntity( pev, true, &prevOrigin );
...
void CBaseEntity::PhysicsNoclip( void )
{
	...
	Vector origin;
	VectorMA( GetAbsOrigin(), gpGlobals->frametime, GetAbsVelocity(), origin );
	SetAbsOrigin( origin );

	engine->RelinkEntity( pev, false );
}
...
void CBaseEntity::PhysicsStep()
{
	...
	float thinktime = thinktick * TICK_RATE;
