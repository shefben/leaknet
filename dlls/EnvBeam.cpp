...
BEGIN_DATADESC( CEnvBeam )
	...
	DEFINE_FIELD( CEnvBeam, m_active, FIELD_INTEGER ),
	DEFINE_FIELD( CEnvBeam, m_spriteTexture, FIELD_INTEGER ),

	DEFINE_KEYFIELD( CEnvBeam, m_iszStartEntity, FIELD_STRING, "LightningStart" ),
	DEFINE_KEYFIELD( CEnvBeam, m_iszEndEntity, FIELD_STRING, "LightningEnd" ),
	DEFINE_KEYFIELD( CEnvBeam, m_life, FIELD_FLOAT, "life" ),
	DEFINE_KEYFIELD( CEnvBeam, m_boltWidth, FIELD_FLOAT, "BoltWidth" ),
	DEFINE_KEYFIELD( CEnvBeam, m_noiseAmplitude, FIELD_FLOAT, "NoiseAmplitude" ),
	DEFINE_KEYFIELD( CEnvBeam, m_speed, FIELD_INTEGER, "TextureScroll" ),
	DEFINE_KEYFIELD( CEnvBeam, m_restrike, FIELD_FLOAT, "StrikeTime" ),
	DEFINE_KEYFIELD( CEnvBeam, m_iszSpriteName, FIELD_STRING, "texture" ),
	DEFINE_KEYFIELD( CEnvBeam, m_frameStart, FIELD_INTEGER, "framestart" ),
	DEFINE_KEYFIELD( CEnvBeam, m_radius, FIELD_FLOAT, "Radius" ),
	DEFINE_KEYFIELD( CEnvBeam, m_TouchType, FIELD_INTEGER, "TouchType" ),

	// Function Pointers
	DEFINE_FUNCTION( CEnvBeam, StrikeThink ),
	DEFINE_FUNCTION( CEnvBeam, UpdateThink ),

	// Input functions
	DEFINE_INPUTFUNC( CEnvBeam, FIELD_VOID, "TurnOn", InputTurnOn ),
	DEFINE_INPUTFUNC( CEnvBeam, FIELD_VOID, "TurnOff", InputTurnOff ),
	DEFINE_INPUTFUNC( CEnvBeam, FIELD_VOID, "Toggle", InputToggle ),
	DEFINE_INPUTFUNC( CEnvBeam, FIELD_VOID, "StrikeOnce", InputStrikeOnce ),

	DEFINE_OUTPUT( CEnvBeam, m_OnTouchedByEntity, "OnTouchedByEntity" ),
...
void CEnvBeam::UpdateThink( void )
{
	...
	if( m_TouchType != touch_none )
	{
		...
		if( tr.fraction != 1.0 )
		{
			...
			if( m_TouchType == touch_player_only || m_TouchType == touch_player_or_npc )
			{
			...
			if( m_TouchType == touch_npc_only || m_TouchType == touch_player_or_npc )
			{
				if( (CAI_BaseNPC *)tr.m_pEnt )
				{
					m_OnTouchedByEntity.FireOutput( tr.m_pEnt, this, 0 );
					return;
				}
...
void CEnvBeam::BeamUpdateVars( void )
{
	...
	SetScrollRate( min(m_speed, MAX_BEAM_SCROLLSPEED) );
