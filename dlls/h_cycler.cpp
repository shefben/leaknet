...
BEGIN_DATADESC( CCycler )
	...
	// Inputs
	DEFINE_INPUTFUNC( CCycler, FIELD_INTEGER, "SetSequence", InputSetSequence ),
...
void CCycler::InputSetSequence( inputdata_t &inputdata )
{
	if (m_animate)
	{
		ResetSequence( inputdata.value.Int() );
...
BEGIN_DATADESC( CWeaponCycler )
	...
	DEFINE_FIELD( CWeaponCycler, m_iszModel,	FIELD_STRING ),
	DEFINE_FIELD( CWeaponCycler, m_iModel,		FIELD_INTEGER ),
...
BEGIN_DATADESC( CWreckage )
	...
	DEFINE_FIELD( CWreckage, m_flDieTime, FIELD_TIME ),
...
void CWreckage::Think( void )
{
	...
	VecSrc.x = random->RandomFloat( GetAbsMins().x, GetAbsMaxs().x );
	VecSrc.y = random->RandomFloat( GetAbsMins().y, GetAbsMaxs().y );
	VecSrc.z = random->RandomFloat( GetAbsMins().z, GetAbsMaxs().z );
...
BEGIN_DATADESC( CBlendingCycler )
	...
	DEFINE_FIELD( CBlendingCycler, m_iLowerBound,	FIELD_INTEGER ),
	DEFINE_FIELD( CBlendingCycler, m_iUpperBound,	FIELD_INTEGER ),
	DEFINE_FIELD( CBlendingCycler, m_iCurrent,		FIELD_INTEGER ),
	DEFINE_FIELD( CBlendingCycler, m_iBlendspeed,	FIELD_INTEGER ),
	DEFINE_FIELD( CBlendingCycler, m_iszSequence,	FIELD_STRING ),
