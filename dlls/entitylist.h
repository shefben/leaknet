...
public:
	...
	CBaseEntity *FindEntityByName( CBaseEntity *pStartEntity, const char *szName, CBaseEntity *pActivator );
	CBaseEntity *FindEntityByName( CBaseEntity *pStartEntity, string_t iszName, CBaseEntity *pActivator )
	{
		return FindEntityByName( pStartEntity, STRING(iszName), pActivator );
	}
...
{
	IServerNetworkable *pNet = (IServerNetworkable*)LookupEntity( hEnt );
	if ( pNet )
...
inline CBaseNetworkable* CGlobalEntityList::GetBaseNetworkable( CBaseHandle hEnt ) const
{
	IServerNetworkable *pNet = (IServerNetworkable*)LookupEntity( hEnt );
	if ( pNet )
		return pNet->GetBaseNetworkable();
	else
		return NULL;
}
...
{
	return (IServerNetworkable*)LookupEntity( hEnt );
}
