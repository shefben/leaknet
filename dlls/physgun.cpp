...
BEGIN_DATADESC( CGravityPellet )
	...
	DEFINE_PHYSPTR( CGravityPellet, m_pConstraint ),
	DEFINE_FIELD( CGravityPellet,	m_isInert, FIELD_BOOLEAN ),
	// physics system will fire this input if the constraint breaks due to physics
	DEFINE_INPUTFUNC( CGravityPellet, FIELD_VOID, "ConstraintBroken", InputOnBreak ),
...
BEGIN_SIMPLE_DATADESC( CGravControllerPoint )
	...
	DEFINE_FIELD( CGravControllerPoint,	m_localPosition,		FIELD_VECTOR ),
	DEFINE_FIELD( CGravControllerPoint,	m_targetPosition,		FIELD_POSITION_VECTOR ),
	DEFINE_FIELD( CGravControllerPoint,	m_worldPosition,		FIELD_POSITION_VECTOR ),
	DEFINE_FIELD( CGravControllerPoint,	m_localAlignNormal,		FIELD_VECTOR ),
	DEFINE_FIELD( CGravControllerPoint,	m_localAlignPosition,	FIELD_VECTOR ),
	DEFINE_FIELD( CGravControllerPoint,	m_targetAlignNormal,	FIELD_VECTOR ),
	DEFINE_FIELD( CGravControllerPoint,	m_targetAlignPosition,	FIELD_POSITION_VECTOR ),
	DEFINE_FIELD( CGravControllerPoint,	m_align,				FIELD_BOOLEAN ),
	DEFINE_FIELD( CGravControllerPoint,	m_saveDamping,			FIELD_FLOAT ),
	DEFINE_FIELD( CGravControllerPoint,	m_maxVel,				FIELD_FLOAT ),
	DEFINE_FIELD( CGravControllerPoint,	m_maxAcceleration,		FIELD_FLOAT ),
	DEFINE_FIELD( CGravControllerPoint,	m_maxAngularAcceleration,	FIELD_VECTOR ),
...
BEGIN_SIMPLE_DATADESC( pelletlist_t )
	...
	DEFINE_FIELD( pelletlist_t, localNormal,				FIELD_VECTOR ),
	DEFINE_FIELD( pelletlist_t, pellet,						FIELD_EHANDLE ),
	DEFINE_FIELD( pelletlist_t, parent,						FIELD_EHANDLE ),
...
BEGIN_DATADESC( CWeaponGravityGun )
	...
	DEFINE_FIELD( CWeaponGravityGun, m_soundState,			FIELD_INTEGER ),
	DEFINE_FIELD( CWeaponGravityGun, m_viewModelIndex,		FIELD_INTEGER ),
	DEFINE_FIELD( CWeaponGravityGun, m_originalObjectPosition,	FIELD_POSITION_VECTOR ),
	DEFINE_EMBEDDED( CWeaponGravityGun, m_gravCallback ),
	// Physptrs can't be saved in embedded classes..
	DEFINE_PHYSPTR( CWeaponGravityGun, m_gravCallback.m_controller ),
	DEFINE_EMBEDDED_AUTO_ARRAY( CWeaponGravityGun, m_activePellets ),
	DEFINE_FIELD( CWeaponGravityGun, m_pelletCount,			FIELD_INTEGER ),
	DEFINE_FIELD( CWeaponGravityGun, m_objectPelletCount,	FIELD_INTEGER ),
	DEFINE_FIELD( CWeaponGravityGun, m_pelletHeld,			FIELD_INTEGER ),
...
static const char *pReloadSound = "weapons/flaregunreload.wav";
//=========================================================
//=========================================================
...
{
	BaseClass::Precache();
	enginesound->PrecacheSound(pReloadSound);
...
void CWeaponGravityGun::SoundStop( void )
{
	...
	{
	case SS_SCANNING:
		StopSound( GetOwner()->entindex(), CHAN_STATIC, pShootSounds[SI_SCANNING] );
		break;
	case SS_LOCKEDON:
		StopSound( GetOwner()->entindex(), CHAN_STATIC, pShootSounds[SI_SCANNING] );
		StopSound( GetOwner()->entindex(), CHAN_STATIC, pShootSounds[SI_LOCKEDON] );
		StopSound( GetOwner()->entindex(), CHAN_STATIC, pShootSounds[SI_LIGHTOBJECT] );
		StopSound( GetOwner()->entindex(), CHAN_STATIC, pShootSounds[SI_HEAVYOBJECT] );
		break;
	}
}
...
void CWeaponGravityGun::SoundStart( void )
{
	...
	{
	case SS_SCANNING:
		EmitSound( filter, GetOwner()->entindex(), CHAN_STATIC, pShootSounds[SI_SCANNING], VOL_NORM, ATTN_NORM );
		break;
	case SS_LOCKEDON:
		// BUGBUG - If you start a sound with a pitch of 100, the pitch shift doesn't work!
		EmitSound( filter, GetOwner()->entindex(), CHAN_STATIC, pShootSounds[SI_LOCKEDON], 0.5, ATTN_NORM, 0, 90 );
		EmitSound( filter, GetOwner()->entindex(), CHAN_STATIC, pShootSounds[SI_SCANNING], 0.5, ATTN_NORM );

		EmitSound( filter, GetOwner()->entindex(), CHAN_STATIC, pShootSounds[SI_LIGHTOBJECT], 0.1, ATTN_NORM );
		EmitSound( filter, GetOwner()->entindex(), CHAN_STATIC, pShootSounds[SI_HEAVYOBJECT], 0.1, ATTN_NORM );
		break;
	}
...
void CWeaponGravityGun::SoundUpdate( void )
{
	...
	case SS_SCANNING:
		// filter.MakeReliable
		// EmitSound( filter, GetOwner()->entindex(), CHAN_STATIC, pShootSounds[SI_SCANNING], 1.0, ATTN_NORM, SND_CHANGE_PITCH | SND_CHANGE_VOL, 100 );
		break;
	case SS_LOCKEDON:
		{
			CPASAttenuationFilter filter( GetOwner() );
			...
			EmitSound( filter, GetOwner()->entindex(), CHAN_STATIC, pShootSounds[SI_LOCKEDON], 
				0.5, ATTN_NORM, SND_CHANGE_VOL | SND_CHANGE_PITCH, pitch );
...
void CC_CollisionTest( void )
{
	...
	static Vector targets[NUM_COLLISION_TESTS];
