...
void CAI_Squad::RemoveFromSquad( CAI_BaseNPC *pNPC )
{
	if ( !pNPC )
		return;
	...
	if (myIndex == -1)
	{
		Msg("ERROR: Attempting to remove non-existing squad membmer!\n");
		return;
	}
	...
	for (member = 0; member < m_nNumMembers; member++)
	{
		CAI_BaseNPC* pSquadMem = m_hSquadMember[member];
		if (pSquadMem)
		{
...
{
	if (m_nNumMembers+1 >= MAX_SQUAD_MEMBERS)
	{
		Msg("Error!! Squad %s is too big!!! Replacing last member\n",this->m_Name);
		m_nNumMembers--;
	}
	m_hSquadMember[m_nNumMembers] = pNPC;
	pNPC->SetSquad( this );
	m_nNumMembers++;
}
...
CAI_BaseNPC *CAI_Squad::SquadMemberInRange( const Vector &vecLocation, float flDist )
{
	...
	{
		if (m_hSquadMember[i] != NULL && (vecLocation - m_hSquadMember[i]->GetLocalOrigin() ).Length2D() <= flDist)
...
CAI_BaseNPC *CAI_Squad::NearestSquadMember( CAI_BaseNPC *pMember )
{
	...
	Vector			fStartLoc = pMember->GetLocalOrigin();
	for (int i = 0; i < m_nNumMembers; i++)
	{
		if (m_hSquadMember[i] != NULL)
		{
			float fDist = (fStartLoc - m_hSquadMember[i]->GetLocalOrigin()).Length();
			if (m_hSquadMember[i]	!=	pMember	&&
				fDist				<	fBestDist	)
			{
				fBestDist	= fDist;
				fNearestEnt	= m_hSquadMember[i];
			}
		}
	}
	return fNearestEnt;
...
bool CAI_Squad::IsLeader( CAI_BaseNPC *pLeader )
{
	if ( !pLeader )
		return false;

	if ( m_hSquadMember[0] == pLeader )
		return true;
...
void CAI_Squad::SquadNewEnemy( CBaseEntity *pEnemy )
{
	if ( !pEnemy )
	{
		Msg( "ERROR: SquadNewEnemy() - pEnemy is NULL!\n" );
		return;
	}

	for (int i = 0; i < m_nNumMembers; i++)
	{
		CAI_BaseNPC *pMember = m_hSquadMember[i];
		if (pMember)
		{
...
int	CAI_Squad::BroadcastInteraction( int interactionType, void *data, CBaseCombatCharacter *sender )
{
	...
	for ( int i = 0; i < m_nNumMembers; i++ )
	{
		CAI_BaseNPC *pMember = m_hSquadMember[i]->MyNPCPointer();
		
		//Validate and don't send again to the sender
		if ( ( pMember != NULL) && ( pMember != sender ) )
		{
			//Send it
			pMember->HandleInteraction( interactionType, data, sender );
		}
	}
	...
	for ( int i = slotIDStart; i <= slotIDEnd; i++ )
	{
		...
		if (!IsSlotOccupied(pEnemy, i))
		{
			...
			if (*pSlot != SQUAD_SLOT_NONE)
			{
				...
				{
					Msg( "ERROR! Vacating an empty slot!\n");
				}
...
void CAI_Squad::UpdateEnemyMemory( CAI_BaseNPC *pUpdater, CBaseEntity *pEnemy, const Vector &position )
{
	...
	for ( int i = 0; i < m_nNumMembers; i++ )
	{
		...
		{
			m_hSquadMember[i]->UpdateEnemyMemory( pEnemy, position, false );
		}
	}
}
...
AISquadEnemyInfo_t *CAI_Squad::FindEnemyInfo( CBaseEntity *pEnemy )
{
	...
	if ( gpGlobals->curtime > m_flEnemyInfoCleanupTime )
	{
		if ( m_EnemyInfos.Count() )
		{
			...
			{
				CBaseEntity *pMemberEnemy = m_hSquadMember[i]->GetEnemy();
