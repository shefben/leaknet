...
//-----------------------------------------------------------------------------
void UTIL_Tracer( const Vector &vecStart, const Vector &vecEnd, int iEntIndex, int iAttachment, float flVelocity, bool bWhiz, const char *pCustomTracerName )
{
	CEffectData data;
	...
	if ( iAttachment != 0 )
	{
		...
		if ( pAnimating && pAnimating->GetModelPtr() )
		{
			...
			if ( pAnimating->GetAttachment( iAttachment, vOrigin, vAngles ) )
			{
				...
				return; // VXP: Don't need to proceed after this
			}
		}
	}
