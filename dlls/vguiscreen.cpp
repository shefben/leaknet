...
BEGIN_DATADESC( CVGuiScreen )
	...
	DEFINE_CUSTOM_FIELD( CVGuiScreen, m_nPanelName, &g_VguiScreenStringOps ),
	DEFINE_FIELD( CVGuiScreen, m_nAttachmentIndex, FIELD_INTEGER ),
	DEFINE_FIELD( CVGuiScreen, m_nOverlayMaterial, FIELD_INTEGER ),
	DEFINE_FIELD( CVGuiScreen, m_fScreenFlags, FIELD_INTEGER ),
	DEFINE_KEYFIELD( CVGuiScreen, m_flWidth, FIELD_FLOAT, "width" ),
	DEFINE_KEYFIELD( CVGuiScreen, m_flHeight, FIELD_FLOAT, "height" ),

	DEFINE_INPUTFUNC( CVGuiScreen, FIELD_VOID, "SetActive", InputSetActive ),
	DEFINE_INPUTFUNC( CVGuiScreen, FIELD_VOID, "SetInactive", InputSetInactive ),
...
bool CVGuiScreen::KeyValue( const char *szKeyName, const char *szValue ) 
{
	...
	if ( FStrEq( szKeyName, "overlaymaterial" ))
	{
		SetOverlayMaterial( szValue );
		return true;
	}
...
{
	m_nOverlayMaterial = GetMaterialIndex( pMaterial );
	NetworkStateChanged();
}
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CVGuiScreen::ShouldTransmit( const edict_t *recipient, const void *pvs, int clientArea )
{
	if ( GetMoveParent() )
	{
...
{
	m_nPanelName = networkstringtable->AddString( g_StringTableVguiScreen, pPanelName );
	NetworkStateChanged();
}

const char *CVGuiScreen::GetPanelName() const
{
	return networkstringtable->GetString( g_StringTableVguiScreen, m_nPanelName );
}
...
{
	networkstringtable->AddString( g_StringTableVguiScreen, pScreenType );
}
