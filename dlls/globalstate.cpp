...
typedescription_t CGlobalState::m_SaveData[] = 
{
	DEFINE_UTLVECTOR( CGlobalState, m_list, FIELD_EMBEDDED ),
	// DEFINE_FIELD( CGlobalState, m_nameList, CUtlSymbolTable ),
	// DEFINE_FIELD( CGlobalState, m_disableStateUpdates, FIELD_BOOLEAN ),
};

BEGIN_SIMPLE_DATADESC( globalentity_t )
	DEFINE_CUSTOM_FIELD( globalentity_t, name, &g_GlobalSymbolDataOps ),
	DEFINE_CUSTOM_FIELD( globalentity_t, levelName, &g_GlobalSymbolDataOps ),
...
{
	if ( !save.WriteFields( "GLOBAL", this, NULL, m_SaveData, ARRAYSIZE(m_SaveData) ) )
		return 0;
...
{
	Reset();
	if ( !restore.ReadFields( "GLOBAL", this, NULL, m_SaveData, ARRAYSIZE(m_SaveData) ) )
		return 0;
