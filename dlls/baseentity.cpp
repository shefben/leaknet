...
void CC_Ent_Create(void)
{
	...
	if (engine->Cmd_Argc() < 2)
	{
		ClientPrint(pPlayer, HUD_PRINTCONSOLE, "Usage:\n   ent_create <entity name> <param 1 name> <param name 2> ...\n");
	}
	...
	if (entity)
	{
		...
		for (int i = 2; i + 1 < engine->Cmd_Argc(); i += 2)
		{
			const char* pKeyName = engine->Cmd_Argv(i);
			const char* pValue = engine->Cmd_Argv(i + 1);
...
static ConCommand ent_create("ent_create", CC_Ent_Create, "Creates an entity of the given type where the player is looking.  Additional parameters can be passed in in the form: ent_create <entity name> <param 1 name> <param 1> <param 2 name> <param 2>...<param N name> <param N>", FCVAR_CHEAT);
