...
#define WIN_MSG_BREAK_ONE_PANE		0
#define WIN_MSG_BREAK_ALL_PANES		1
...
BEGIN_DATADESC( CWindowPane )
	...
	// Function Pointers
	DEFINE_FUNCTION( CWindowPane, Die ),
	DEFINE_FUNCTION( CWindowPane, PaneTouch ),
...
BEGIN_DATADESC( CBreakableSurface )
	...
	DEFINE_KEYFIELD( CBreakableSurface, m_nSurfaceType,		FIELD_INTEGER,	"surfacetype"),
	DEFINE_KEYFIELD( CBreakableSurface, m_nFragility,		FIELD_INTEGER,	"fragility"),
	DEFINE_KEYFIELD( CBreakableSurface, m_vLLVertex,		FIELD_VECTOR,  "lowerleft" ),
	DEFINE_KEYFIELD( CBreakableSurface, m_vULVertex,		FIELD_VECTOR,  "upperleft" ),
	DEFINE_KEYFIELD( CBreakableSurface, m_vLRVertex,		FIELD_VECTOR,  "lowerright" ),
	DEFINE_KEYFIELD( CBreakableSurface, m_vURVertex,		FIELD_VECTOR,  "upperright" ),
	DEFINE_KEYFIELD( CBreakableSurface, m_nQuadError,		FIELD_INTEGER, "error" ),

	DEFINE_FIELD( CBreakableSurface, m_nNumWide,			FIELD_INTEGER),	
	DEFINE_FIELD( CBreakableSurface, m_nNumHigh,			FIELD_INTEGER),	
	DEFINE_FIELD( CBreakableSurface, m_flPanelWidth,		FIELD_FLOAT),	
	DEFINE_FIELD( CBreakableSurface, m_flPanelHeight,	FIELD_FLOAT),	
	DEFINE_FIELD( CBreakableSurface, m_vNormal,			FIELD_VECTOR),	
	DEFINE_FIELD( CBreakableSurface, m_vCorner,			FIELD_POSITION_VECTOR),	
	DEFINE_FIELD( CBreakableSurface, m_bIsBroken,		FIELD_BOOLEAN),	
	DEFINE_FIELD( CBreakableSurface, m_nNumBrokenPanes,	FIELD_INTEGER),	
	...
	DEFINE_AUTO_ARRAY2D( CBreakableSurface, m_flSupport,	FIELD_FLOAT),	

	// Function Pointers
	DEFINE_FUNCTION( CBreakableSurface, BreakThink ),
	DEFINE_FUNCTION( CBreakableSurface, SurfaceTouch ),

	DEFINE_INPUTFUNC( CBreakableSurface, FIELD_VECTOR,	"Shatter", InputShatter ),
...
IMPLEMENT_SERVERCLASS_ST(CBreakableSurface, DT_BreakableSurface)
	...
	SendPropVector(SENDINFO(m_vNormal), -1, SPROP_NOSCALE),
...
void CBreakableSurface::SurfaceTouch( CBaseEntity *pOther )
{
	...
	// After all panes have been broken, tell client to update all the edges
	CEntityMessageFilter filter( this, "CBreakableSurface" );
	filter.MakeReliable();

	MessageBegin( filter, 0 );
		WRITE_BYTE( WIN_MSG_BREAK_UPDATE_EDGES );
	MessageEnd();
}
...
void CBreakableSurface::TraceAttack( const CTakeDamageInfo &info, const Vector &vecDir, trace_t *ptr )
{
	...
	{
		// Fire output every time we break, not just the first time.
		m_OnBreak.FireOutput( info.GetAttacker(), this );
	}
	...
	if (info.GetDamageType() & (DMG_BULLET | DMG_CLUB))
	{
		...
		ShatterPane(nWidth, nHeight,vecDir*500,ptr->endpos);
...
void CBreakableSurface::Die( CBaseEntity *pBreaker, const Vector &vAttackDir )
{
	...
	// Send size data immediately
	CEntityMessageFilter filter( this, "CBreakableSurface" );
	filter.MakeReliable();

	MessageBegin( filter, 0 );
	WRITE_BYTE( WIN_MSG_BREAK_UPDATE_SIZE );
	WRITE_BYTE( m_nNumWide );
	WRITE_BYTE( m_nNumHigh );
	MessageEnd();
...
void CBreakableSurface::BreakThink(void)
{
	...
	for (w=0;w<m_nNumWide;w++)
	{
		for (int h=0;h<m_nNumHigh;h++)
		{
			if (!IsBroken(w,h))
			{
				m_flSupport[w][h] = flSupport[w][h]/WINDOW_MAX_SUPPORT;
...
void CBreakableSurface::BreakAllPanes(void)
{
	...
	for (int width=0;width<m_nNumWide;width++)
	{
		...
		{
			m_flSupport[width][height] = WINDOW_PANE_BROKEN;
		}
	}
	...
	MessageBegin( filter, 0 );
	WRITE_BYTE( WIN_MSG_BREAK_ALL_PANES );
	MessageEnd();
...
void CBreakableSurface::BreakPane(int nWidth, int nHeight)
{
	...
	m_nNumBrokenPanes++;
	m_flSupport[nWidth][nHeight] = WINDOW_PANE_BROKEN;

	CEntityMessageFilter filter( this, "CBreakableSurface" );
	filter.MakeReliable();

	MessageBegin( filter, 0 );
		WRITE_BYTE( WIN_MSG_BREAK_ONE_PANE );
		WRITE_BYTE( nWidth );
		WRITE_BYTE( nHeight );
	MessageEnd();
...
// Output  :
//------------------------------------------------------------------------------
void CBreakableSurface::ShatterPane(int nWidth, int nHeight, const Vector &vForce, const Vector &vForcePos)
{
	// Check parameter range
	if (nWidth < 0  || nWidth  >= m_nNumWide) return;
	if (nHeight < 0 || nHeight >= m_nNumHigh) return;
...
void CBreakableSurface::Spawn(void)
{
	...
	// We want an OnUpdateClientData call
	CBaseEntity::AddToUpdateClientDataList( this );
}

void CBreakableSurface::OnUpdateClientData( CBasePlayer *player )
{
	BaseClass::OnUpdateClientData( player );

	// Re-send shatter info if this player is issuing an impulse DEMO_RESTART_HUD_IMPULSE(204) (demo recording restart)
	if ( player->GetImpulse() != DEMO_RESTART_HUD_IMPULSE )
		return;
	...
	for ( x = 0; x < m_nNumWide; x++ )
	{
		for ( y = 0; y < m_nNumHigh; y++ )
		{
			if ( m_flSupport[x][y] == WINDOW_PANE_BROKEN )
			{
				MessageBegin( filter, 0 );
					WRITE_BYTE( WIN_MSG_BREAK_ONE_PANE );
					WRITE_BYTE( x );
