...
bool CBaseServerVehicle::Initialize( const char *pScriptName )
{
	...
	while ( !pParse->Finished() )
	{
		...
		if ( !Q_stricmp( pBlock, "vehicle_sounds" ) )
		{
...
const char *vehiclesound_gear_parsenames[VS_NUM_GEAR_SOUNDS] =
{
	"engine_loop",
	"foot_off",
	"foot_off_slow",
...
void CVehicleSoundsParser::ParseKeyValue( void *pData, const char *pKey, const char *pValue )
{
	...
	else
	{
		...
		if ( m_iCurrentGear >= 0 )
		{
			...
			for ( int i = 0; i < VS_NUM_GEAR_SOUNDS; i++ )
			{
				if ( !Q_stricmp( pKey, vehiclesound_gear_parsenames[i] ) )
				{
			...
			// Check gear keys
			if ( !Q_stricmp( pKey, "max_speed" ) )
			{
				pSounds->pGears[m_iCurrentGear].flMaxSpeed = atof(pValue);
				return;
			}
			if ( !Q_stricmp( pKey, "speed_approach_factor" ) )
			{
		...
		for ( int i = 0; i < VS_NUM_SOUNDS; i++ )
		{
			if ( !Q_stricmp( pKey, vehiclesound_parsenames[i] ) )
			{
