...
int ZOMBIE_AE_ATTACK_RIGHT  = 0x01;
int ZOMBIE_AE_ATTACK_LEFT   = 0x02;
int ZOMBIE_AE_ATTACK_BOTH   = 0x03;
int ZOMBIE_AE_SWATITEM      = 10;
int ZOMBIE_AE_STARTSWAT     = 11;
int ZOMBIE_AE_STEP_LEFT     = 16; // Footsteps sounds
int ZOMBIE_AE_STEP_RIGHT    = 17;
int ZOMBIE_AE_SCUFF_LEFT    = 18; // foot drags, scuff sound
int ZOMBIE_AE_SCUFF_RIGHT   = 19;
int ZOMBIE_AE_ATTACK_SCREAM = 20;
int ZOMBIE_AE_GET_UP        = 21; // for scripted get-ups
int ZOMBIE_AE_POUND         = 22; // pounding walls/doors/obstacles
...
{
	if ( pEvent->event == ZOMBIE_AE_POUND )
	{
		PoundSound();
		return;
	}

	if ( pEvent->event == ZOMBIE_AE_STEP_LEFT )
	{
		FootstepSound( false );
		return;
	}

	if ( pEvent->event == ZOMBIE_AE_STEP_RIGHT )
	{
		FootstepSound( true );
		return;
	}

	if ( pEvent->event == ZOMBIE_AE_GET_UP )
	{
	...
	if ( pEvent->event == ZOMBIE_AE_SCUFF_LEFT )
	{
		FootscuffSound( false );
		return;
	}

	if ( pEvent->event == ZOMBIE_AE_SCUFF_RIGHT )
	{
		FootscuffSound( true );
		return;
	}
	...
	if ( pEvent->event == ZOMBIE_AE_STARTSWAT )
	{
		AttackSound();
		return;
	}

	if ( pEvent->event == ZOMBIE_AE_ATTACK_SCREAM )
	{
		AttackSound();
		return;
	}

	if ( pEvent->event == ZOMBIE_AE_SWATITEM )
	{
	...
	if ( pEvent->event == ZOMBIE_AE_ATTACK_RIGHT )
	{
		...
		ClawAttack( ZOMBIE_MELEE_REACH, sk_zombie_dmg_one_slash.GetFloat(), qa, vec );
		return;
	}
	...
	if ( pEvent->event == ZOMBIE_AE_ATTACK_BOTH )
	{
		Vector forward;
