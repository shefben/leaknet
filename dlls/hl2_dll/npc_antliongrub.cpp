...
BEGIN_DATADESC( CNPC_AntlionGrub )
	...
	DEFINE_SOUNDPATCH( CNPC_AntlionGrub, m_pMovementSound ),
	DEFINE_SOUNDPATCH( CNPC_AntlionGrub, m_pVoiceSound ),
	DEFINE_SOUNDPATCH( CNPC_AntlionGrub, m_pHealSound ),
	DEFINE_FIELD( CNPC_AntlionGrub, m_pGlowSprite,			FIELD_CLASSPTR ),
	DEFINE_FIELD( CNPC_AntlionGrub, m_flNextVoiceChange,	FIELD_TIME ),
	DEFINE_FIELD( CNPC_AntlionGrub, m_flSquashTime,			FIELD_TIME ),
	DEFINE_FIELD( CNPC_AntlionGrub, m_flNearTime,			FIELD_TIME ),
	DEFINE_FIELD( CNPC_AntlionGrub, m_flHealthTime,			FIELD_TIME ),
	DEFINE_FIELD( CNPC_AntlionGrub, m_flEnemyHostileTime,	FIELD_TIME ),
	DEFINE_FIELD( CNPC_AntlionGrub, m_bSquashed,			FIELD_BOOLEAN ),
	DEFINE_FIELD( CNPC_AntlionGrub, m_bMoving,				FIELD_BOOLEAN ),
	DEFINE_FIELD( CNPC_AntlionGrub, m_bSquashValid,			FIELD_BOOLEAN ),
	DEFINE_FIELD( CNPC_AntlionGrub, m_bHealing,				FIELD_BOOLEAN ),
	DEFINE_FIELD( CNPC_AntlionGrub, m_nHealthReserve,		FIELD_INTEGER ),
	DEFINE_FIELD( CNPC_AntlionGrub, m_nGlowSpriteHandle,	FIELD_INTEGER ),

	// Functions
	DEFINE_ENTITYFUNC( CNPC_AntlionGrub, GrubTouch ),
...
void CNPC_AntlionGrub::Precache( void )
{
	...
	enginesound->PrecacheSound( "npc/antlion_grub/voice2.wav" );
...
void CNPC_AntlionGrub::BroadcastAlert( void )
{
	...
	for ( CEntitySphereQuery sphere( GetAbsOrigin(), ANTLIONGRUB_SQUEAL_DIST ); (pEntity = sphere.GetCurrentEntity()) != NULL; sphere.NextEntity() )
	{
		...
		{
			pNPC->HandleInteraction( g_interactionAntlionGrubAlert, NULL, this );
		}
	}
}
...
void CNPC_AntlionGrub::PrescheduleThink( void )
{
	...
	if ( m_pGlowSprite != NULL )
	{
		...
		if ( FBitSet( m_pGlowSprite->m_fEffects, EF_NODRAW ) == false )
		{
