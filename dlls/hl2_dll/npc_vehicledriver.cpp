...
BEGIN_DATADESC( CNPC_VehicleDriver )
	DEFINE_KEYFIELD( CNPC_VehicleDriver, m_iszVehicleName, FIELD_STRING, "vehicle" ),
//	DEFINE_FIELD( CNPC_VehicleDriver, m_hVehicle,			FIELD_EHANDLE ),
	// DEFINE_FIELD( CNPC_VehicleDriver, m_pVehicleInterface, FIELD_POINTER ),
	DEFINE_FIELD( CNPC_VehicleDriver, m_hVehicleEntity,		FIELD_EHANDLE ),
	// DEFINE_FIELD( CNPC_VehicleDriver, m_Waypoints,		FIELD_???? ),
	// DEFINE_FIELD( CNPC_VehicleDriver, m_pCurrentWaypoint, FIELD_POINTER ),
	// DEFINE_FIELD( CNPC_VehicleDriver, m_pNextWaypoint,	FIELD_POINTER ),
	DEFINE_FIELD( CNPC_VehicleDriver, m_vecDesiredVelocity, FIELD_VECTOR ),
	DEFINE_FIELD( CNPC_VehicleDriver, m_vecDesiredPosition, FIELD_POSITION_VECTOR ),
	DEFINE_FIELD( CNPC_VehicleDriver, m_vecPrevPoint,		FIELD_POSITION_VECTOR ),
	DEFINE_FIELD( CNPC_VehicleDriver, m_vecPrevPrevPoint,	FIELD_POSITION_VECTOR ),
	DEFINE_FIELD( CNPC_VehicleDriver, m_vecPostPoint,		FIELD_POSITION_VECTOR ),
	DEFINE_FIELD( CNPC_VehicleDriver, m_vecPostPostPoint,	FIELD_POSITION_VECTOR ),
	DEFINE_FIELD( CNPC_VehicleDriver, m_flDistanceAlongSpline, FIELD_FLOAT ),
	DEFINE_KEYFIELD( CNPC_VehicleDriver, m_flDriversMaxSpeed, FIELD_FLOAT, "drivermaxspeed" ),
	DEFINE_KEYFIELD( CNPC_VehicleDriver, m_flDriversMinSpeed, FIELD_FLOAT, "driverminspeed" ),
	DEFINE_FIELD( CNPC_VehicleDriver, m_flMaxSpeed,			FIELD_FLOAT ),
	DEFINE_FIELD( CNPC_VehicleDriver, m_flGoalSpeed,		FIELD_FLOAT ),
	//DEFINE_KEYFIELD( CNPC_VehicleDriver, m_flInitialSpeed,	FIELD_FLOAT ),
	DEFINE_FIELD( CNPC_VehicleDriver, m_flSteering,			FIELD_FLOAT ),

	// Inputs
	DEFINE_INPUTFUNC( CNPC_VehicleDriver, FIELD_FLOAT, "SetDriversMaxSpeed", InputSetDriversMaxSpeed ),
	DEFINE_INPUTFUNC( CNPC_VehicleDriver, FIELD_FLOAT, "SetDriversMinSpeed", InputSetDriversMinSpeed ),

	DEFINE_INPUTFUNC( CNPC_VehicleDriver, FIELD_VOID, "StartForward", InputStartForward ),
	DEFINE_INPUTFUNC( CNPC_VehicleDriver, FIELD_VOID, "Stop", InputStop ),
	DEFINE_INPUTFUNC( CNPC_VehicleDriver, FIELD_VOID, "StartFiring", InputStartFiring ),
	DEFINE_INPUTFUNC( CNPC_VehicleDriver, FIELD_VOID, "StopFiring", InputStopFiring ),
	DEFINE_INPUTFUNC( CNPC_VehicleDriver, FIELD_STRING, "GotoPathCorner", InputGotoPathCorner ),
END_DATADESC()
...
void CNPC_VehicleDriver::OnRestore()
{
	if ( m_hVehicleEntity )
	{
		m_pVehicleInterface = m_hVehicleEntity->GetServerVehicle();
	...
	case TASK_VEHICLEDRIVER_GET_PATH:
		{
			...
			NDebugOverlay::Box( m_pGoalEnt->GetAbsOrigin(), -Vector(50,50,50), Vector(50,50,50), 255,255,255, true, 5);

	  		AI_NavGoal_t goal( GOALTYPE_PATHCORNER, m_pGoalEnt->GetLocalOrigin(), ACT_WALK, AIN_DEF_TOLERANCE, AIN_YAW_TO_DEST);
	...
	case TASK_RANGE_ATTACK1:
		{
			...
			if ( pEnemy )
			{
				...
				Vector vecTarget = GetEnemy()->BodyTarget( GetAbsOrigin() );
...
bool CNPC_VehicleDriver::OverridePathMove( float flInterval )
{
	...
	if ( BoxOnPlaneSide( GetAbsMins(), GetAbsMaxs(), &m_pCurrentWaypoint->planeWaypoint ) == 3 )
	{
		if ( WaypointReached() )
			return true;
	}
	...
	if ( m_pNextWaypoint && BoxOnPlaneSide( GetAbsMins(), GetAbsMaxs(), &m_pNextWaypoint->planeWaypoint ) == 3 )
	{
		if ( WaypointReached() )
			return true;
	}
...
void CNPC_VehicleDriver::CheckForTeleport( void )
{
	...
	CPathTrack *pTrack = dynamic_cast<CPathTrack *>( m_pGoalEnt );
	if ( !pTrack )
		return;
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_VehicleDriver::TranslateEnemyChasePosition( CBaseEntity *pEnemy, Vector &chasePosition, float &tolerance )
{
	tolerance = 48;
}
