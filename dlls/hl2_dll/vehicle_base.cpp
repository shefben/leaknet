...
void CPropVehicleDriveable::Spawn( void )
{
	...
	if( !m_pServerVehicle->Initialize( STRING( m_vehicleScript ) ) )
	{
...
void CPropVehicleDriveable::Use( CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value )
{
	...
	int iEntryAnim = m_pServerVehicle ? m_pServerVehicle->GetEntryAnimForPoint( pPlayer->EyePosition() ) : ACTIVITY_NOT_AVAILABLE;
...
{
	int iSequence = m_pServerVehicle ? m_pServerVehicle->GetExitAnimToUse() : ACTIVITY_NOT_AVAILABLE;
