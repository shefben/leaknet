...
static AI_StandoffParams_t AI_DEFAULT_CITIZEN_STANDOFF_PARAMS = { AIHCR_MOVE_ON_COVER, true, true, 2.5, 1, 3, 25 };
...
void CNPC_Citizen::SelectCitizenModel()
{
	if( HasSpawnFlags( SF_CITIZEN_RANDOM_HEAD ) )
	{
		...
		case 2:
			...
			break;
		case 3:
			SetModelName(MAKE_STRING("models/humans/male_04.mdl"));
			break;
		case 4:
			SetModelName(MAKE_STRING("models/humans/male_05.mdl"));
			break;
		case 5:
...
*/
			...
			{
				// VXP: e3-03 hack. prevent SW citizens from running inside during closing scene.
				if ( !Q_strnicmp( gpGlobals->mapname.ToCStr(), "e3_c17_", 7 ) )
				{
					return SCHED_RELOAD;
				}
...
void CNPC_Citizen::PrescheduleThink( void )
{
	...
	if( IsSelected() )
	{
		...
		UTIL_TraceHull( pPlayer->EyePosition(), pForward * 2048, VEC_HULL_MIN, VEC_HULL_MAX, MASK_SHOT, this, COLLISION_GROUP_NONE, &tr );

	//	data.m_vOrigin = tr.endpos;
		data.m_vOrigin = GetCommandGoal();
