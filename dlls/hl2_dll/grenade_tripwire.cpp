...
#define GRENADETRIPWIRE_FLYSOUND	"weapons/incoming1.wav"
...
BEGIN_DATADESC( CTripwireGrenade )
	...
	DEFINE_FIELD( CTripwireGrenade, m_flPowerUp,		FIELD_TIME ),
	DEFINE_FIELD( CTripwireGrenade, m_nMissileCount,	FIELD_INTEGER ),
	DEFINE_FIELD( CTripwireGrenade, m_vecDir,			FIELD_VECTOR ),
	DEFINE_FIELD( CTripwireGrenade, m_vTargetPos,		FIELD_POSITION_VECTOR ),
	DEFINE_FIELD( CTripwireGrenade, m_vTargetOffset,	FIELD_VECTOR ),
	DEFINE_FIELD( CTripwireGrenade, m_pRope,			FIELD_CLASSPTR ),
	DEFINE_FIELD( CTripwireGrenade, m_pHook,			FIELD_CLASSPTR ),

	// Function Pointers
	DEFINE_FUNCTION( CTripwireGrenade, WarningThink ),
	DEFINE_FUNCTION( CTripwireGrenade, PowerupThink ),
	DEFINE_FUNCTION( CTripwireGrenade, RopeBreakThink ),
	DEFINE_FUNCTION( CTripwireGrenade, FireThink ),
...
void CTripwireGrenade::Precache( void )
{
	...
	//<<CHECK>>
	enginesound->PrecacheSound(GRENADETRIPWIRE_FLYSOUND);
...
void CTripwireGrenade::FireMissile(const Vector &vTargetPos)
{
	...
	CGrenadeHomer *pGrenade = CGrenadeHomer::CreateGrenadeHomer( MAKE_STRING(GRENADETRIPWIRE_MISSILEMDL), MAKE_STRING(GRENADETRIPWIRE_FLYSOUND),  GetAbsOrigin(), vec3_angle, edict() );
...
BEGIN_DATADESC( CTripwireHook )
	...
	DEFINE_FIELD( CTripwireHook, m_hGrenade, FIELD_EHANDLE ),
	DEFINE_FIELD( CTripwireHook, m_bAttached, FIELD_BOOLEAN ),
