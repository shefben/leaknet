...
//	DEFINE_FIELD( CFourWheelVehiclePhysics, m_pOuter, FIELD_EHANDLE ),
//											m_pOuterServerVehicle;

	// Quiet down classcheck
	// DEFINE_FIELD( CFourWheelVehiclePhysics, m_controls, vehicle_controlparams_t ),

	// Controls
	DEFINE_FIELD( CFourWheelVehiclePhysics, m_controls.throttle, FIELD_FLOAT ),
	DEFINE_FIELD( CFourWheelVehiclePhysics, m_controls.steering, FIELD_FLOAT ),
	DEFINE_FIELD( CFourWheelVehiclePhysics, m_controls.brake, FIELD_FLOAT ),
	DEFINE_FIELD( CFourWheelVehiclePhysics, m_controls.boost, FIELD_FLOAT ),
	DEFINE_FIELD( CFourWheelVehiclePhysics, m_controls.handbrake, FIELD_BOOLEAN ),
	DEFINE_FIELD( CFourWheelVehiclePhysics, m_controls.brakepedal, FIELD_BOOLEAN ),
	DEFINE_FIELD( CFourWheelVehiclePhysics, m_controls.bHasBrakePedal, FIELD_BOOLEAN ),

	// This has to be handled by the containing class owing to 'owner' issues
//	DEFINE_PHYSPTR( CFourWheelVehiclePhysics, m_pVehicle ),

	DEFINE_FIELD( CFourWheelVehiclePhysics, m_nSpeed, FIELD_INTEGER ),
	DEFINE_FIELD( CFourWheelVehiclePhysics, m_nLastSpeed, FIELD_INTEGER ),
	DEFINE_FIELD( CFourWheelVehiclePhysics, m_nRPM, FIELD_INTEGER ),
	DEFINE_FIELD( CFourWheelVehiclePhysics, m_fLastBoost, FIELD_FLOAT ),
	DEFINE_FIELD( CFourWheelVehiclePhysics, m_nBoostTimeLeft, FIELD_INTEGER ),
	DEFINE_FIELD( CFourWheelVehiclePhysics, m_nHasBoost, FIELD_INTEGER ),

	DEFINE_FIELD( CFourWheelVehiclePhysics, m_maxThrottle, FIELD_FLOAT ),
	DEFINE_FIELD( CFourWheelVehiclePhysics, m_flMaxRevThrottle, FIELD_FLOAT ),
	DEFINE_FIELD( CFourWheelVehiclePhysics, m_flThrottleReduction, FIELD_FLOAT ),
	DEFINE_FIELD( CFourWheelVehiclePhysics, m_flMaxSpeed, FIELD_FLOAT ),
	DEFINE_FIELD( CFourWheelVehiclePhysics, m_actionSpeed, FIELD_FLOAT ),

	// This has to be handled by the containing class owing to 'owner' issues
//	DEFINE_PHYSPTR_ARRAY( CFourWheelVehiclePhysics, m_pWheels ),

	DEFINE_FIELD( CFourWheelVehiclePhysics, m_wheelCount, FIELD_INTEGER ),

	DEFINE_ARRAY( CFourWheelVehiclePhysics, m_wheelPosition, FIELD_VECTOR, 4 ),
	DEFINE_ARRAY( CFourWheelVehiclePhysics, m_wheelRotation, FIELD_VECTOR, 4 ),
	DEFINE_ARRAY( CFourWheelVehiclePhysics, m_wheelBaseHeight, FIELD_FLOAT, 4 ),
	DEFINE_ARRAY( CFourWheelVehiclePhysics, m_wheelTotalHeight, FIELD_FLOAT, 4 ),
	DEFINE_ARRAY( CFourWheelVehiclePhysics, m_poseParameters, FIELD_INTEGER, 12 ),
	DEFINE_FIELD( CFourWheelVehiclePhysics, m_actionValue, FIELD_FLOAT ),
	DEFINE_KEYFIELD( CFourWheelVehiclePhysics, m_actionScale, FIELD_FLOAT, "actionScale" ),
	DEFINE_FIELD( CFourWheelVehiclePhysics, m_debugRadius, FIELD_FLOAT ),
	DEFINE_FIELD( CFourWheelVehiclePhysics, m_throttleRate, FIELD_FLOAT ),
	DEFINE_FIELD( CFourWheelVehiclePhysics, m_throttleStartTime, FIELD_FLOAT ),
	DEFINE_FIELD( CFourWheelVehiclePhysics, m_throttleActiveTime, FIELD_FLOAT ),
	DEFINE_FIELD( CFourWheelVehiclePhysics, m_turboTimer, FIELD_FLOAT ),

	DEFINE_FIELD( CFourWheelVehiclePhysics, m_flVehicleVolume, FIELD_FLOAT ),
	DEFINE_FIELD( CFourWheelVehiclePhysics, m_bIsOn, FIELD_BOOLEAN ),
	DEFINE_FIELD( CFourWheelVehiclePhysics, m_bLastThrottle, FIELD_BOOLEAN ),
	DEFINE_FIELD( CFourWheelVehiclePhysics, m_bLastBoost, FIELD_BOOLEAN ),
	DEFINE_FIELD( CFourWheelVehiclePhysics, m_bLastSkid, FIELD_BOOLEAN ),

	DEFINE_FIELD( CFourWheelVehiclePhysics, m_nTurnLeftCount, FIELD_INTEGER ),
	DEFINE_FIELD( CFourWheelVehiclePhysics, m_nTurnRightCount, FIELD_INTEGER ),
...
//-----------------------------------------------------------------------------
// Set the vehicle noises
//-----------------------------------------------------------------------------
void CFourWheelVehiclePhysics::SetVehicleSounds( const char *pStartSound, const char *pRunningSound, const char *pStopSound, const char *pTurboSound )
{
	/*
	m_vehicleSounds.iszSound[VS_ENGINE_START] = pStartSound ? MAKE_STRING( pStartSound) : NULL_STRING;
	m_vehicleSounds.iszSound[VS_ENGINE_LOOP] = pRunningSound ? MAKE_STRING( pRunningSound ) : NULL_STRING;
	m_vehicleSounds.iszSound[VS_ENGINE_STOP] = pStopSound ? MAKE_STRING( pStopSound ) : NULL_STRING;
	m_vehicleSounds.iszSound[VS_TURBO] = pTurboSound ? MAKE_STRING( pTurboSound ) : NULL_STRING;
	*/
}
...
//----------------------------------------------------
void CFourWheelVehiclePhysics::PlaceWheelDust( int wheelIndex )
{
	...
	float flSize = RemapVal( m_nSpeed, DUST_SPEED, m_flMaxSpeed, 2, 16 );
	flSize = clamp( flSize, 2, 16 );
	g_pEffects->Dust( vecPos, vecVel, flSize, 5 );
}
...
bool CFourWheelVehiclePhysics::Think()
{
	...
	// Make the steering wheel match the input
...
//-----------------------------------------------------------------------------
void CFourWheelVehiclePhysics::UpdateDriverControls( int nButtons, float flFrameTime )
{
	// Get vehicle data.
	const vehicle_operatingparams_t &carState = m_pVehicle->GetOperatingParams();
	const vehicleparams_t &vehicleData = m_pVehicle->GetVehicleParams();
	...
	if ( !m_controls.handbrake && !m_controls.brakepedal && bThrottle && !m_bLastThrottle )
	{
		...
		m_pOuterServerVehicle->StopSoundForGear( VS_FOOT_OFF_GAS );
		m_pOuterServerVehicle->StopSoundForGear( VS_FOOT_OFF_GAS_SLOW );

		// Start the new gear's engine loop
		m_pOuterServerVehicle->StartGearEngineLoop( VS_ENGINE_LOOP );
	}
	// throttle up now but not before??
	else if ( !bThrottle && m_bLastThrottle )
	{
		m_throttleActiveTime = gpGlobals->curtime - m_throttleStartTime;
		m_bLastThrottle = false;
		CPASAttenuationFilter filter( m_pOuter );

		// player depressed throttle for > 2.5s, play normal "throttle off" sound
		if ( m_throttleActiveTime > 2.5 )
		{
			m_pOuterServerVehicle->PlaySoundForGear( VS_FOOT_OFF_GAS );
		}
		else //if ( m_throttleActiveTime > 1.0 )	// didn't hold throttle long, so play slow throttle off sound
		{
			m_pOuterServerVehicle->PlaySoundForGear( VS_FOOT_OFF_GAS_SLOW );
		}
	}

	float flSpeedPercentage = clamp( m_nSpeed / m_flMaxSpeed, 0, 1 );
	m_pOuterServerVehicle->SoundUpdate( flFrameTime, flSpeedPercentage, bThrottle, (m_controls.throttle < 0) );
}
...
void CFourWheelVehiclePhysics::SetDisableEngine( bool bDisable )
{
	...
	// Adjust engine sounds accordingly.
	if ( IsEngineDisabled() )
	{
		m_pOuterServerVehicle->SoundShutdown();
	}
