...
{
	CAI_BaseNPC::gm_ClassScheduleIdSpace.Init( CAI_BaseNPC::GetSchedulingSymbols() );
...
bool CAI_SchedulesManager::LoadSchedulesFromBuffer( const char *prefix, const char *pfile, CAI_ClassScheduleIdSpace *pIdSpace )
{
	char token[1024];
	...
	while (!_stricmp("Schedule",token))
	{
		...
		pfile = engine->COM_ParseFile(pfile, token);
		while ((token[0]!='\0') && (Q_stricmp("Interrupts",token)))
		{
			...
			else if (!_stricmp("Interrupts",token) || !strnicmp("TASK_",token,5) )
			{
		...
		// ==========================
		pfile = engine->COM_ParseFile(pfile, token);
		while ((token[0]!='\0') && (Q_stricmp("Schedule",token)))
		{
			// Convert generic ID to sub-class specific enum
...
bool CAI_SchedulesManager::LoadSchedules( const char *prefix, CAI_ClassScheduleIdSpace *pIdSpace  )
{
	...
	char *pfile = (char*)engine->COM_LoadFile(sz, 2, NULL);
