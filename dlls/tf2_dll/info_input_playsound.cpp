...
BEGIN_DATADESC( CInfoInputPlaySound )
	...
	// variables
	DEFINE_KEYFIELD( CInfoInputPlaySound, m_iszSound, FIELD_SOUNDNAME, "Sound" ),
	DEFINE_KEYFIELD( CInfoInputPlaySound, m_flVolume, FIELD_FLOAT, "Volume" ),
	DEFINE_KEYFIELD( CInfoInputPlaySound, m_flAttenuation, FIELD_FLOAT, "Attenuation" ),
	DEFINE_KEYFIELD( CInfoInputPlaySound, m_iszTestVolumeName, FIELD_STRING, "TestVolume" ),

	// inputs
	DEFINE_INPUTFUNC( CInfoInputPlaySound, FIELD_STRING, "SetSound", InputSetSound ),
	DEFINE_INPUTFUNC( CInfoInputPlaySound, FIELD_VOID, "PlaySoundToAll", InputPlaySoundToAll ),
	DEFINE_INPUTFUNC( CInfoInputPlaySound, FIELD_VOID, "PlaySoundToTeam1", InputPlaySoundToTeam1 ),
	DEFINE_INPUTFUNC( CInfoInputPlaySound, FIELD_VOID, "PlaySoundToTeam2", InputPlaySoundToTeam2 ),
	DEFINE_INPUTFUNC( CInfoInputPlaySound, FIELD_EHANDLE, "PlaySoundToPlayer", InputPlaySoundToPlayer ),
...
void CInfoInputPlaySound::Precache( void )
{
	...
	{
		enginesound->PrecacheSound( STRING(m_iszSound) );
	}
}
...
void CInfoInputPlaySound::Activate( void )
{
	...
	{
		m_hTestVolume = gEntList.FindEntityByName( NULL, STRING(m_iszTestVolumeName), NULL );
		if ( !m_hTestVolume )
		{
...
void CInfoInputPlaySound::PlaySoundToPlayer( CBaseTFPlayer *pPlayer )
{
	...
	if ( HasSpawnFlags( SF_PLAYSOUND_USE_THIS_ORIGIN ) )
	{
		...
		EmitSound( filter, entindex(), CHAN_VOICE, STRING(m_iszSound), m_flVolume, m_flAttenuation, 0, PITCH_NORM, &(GetAbsOrigin()) );
	}
	else
	{
		CSingleUserRecipientFilter filter( pPlayer );
		EmitSound( filter, pPlayer->entindex(), CHAN_VOICE, STRING(m_iszSound), m_flVolume, m_flAttenuation);
	}
}
