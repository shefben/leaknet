...
#define TANK_START_SOUND	"vehicles/diesel_start1.wav"
#define TANK_RUNNING_SOUND	"vehicles/diesel_loop2.wav"
#define TANK_TURBO_SOUND	"vehicles/diesel_nitroboost.wav"
...
void CVehicleTank::Precache()
{
	...
	enginesound->PrecacheSound( TANK_START_SOUND );
	enginesound->PrecacheSound( TANK_RUNNING_SOUND );
	enginesound->PrecacheSound( TANK_TURBO_SOUND );
...
{
	SetVehicleSounds( TANK_START_SOUND, TANK_RUNNING_SOUND, NULL, TANK_TURBO_SOUND );
...
void CVehicleTank::TurretThink()
{
	...
	if ( m_flTurretYaw != flStartYaw || m_flTurretPitch != flStartPitch )
		NetworkStateChanged();
