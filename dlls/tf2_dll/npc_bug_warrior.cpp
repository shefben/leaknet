...
BEGIN_DATADESC( CNPC_Bug_Warrior )
	...
	DEFINE_FIELD( CNPC_Bug_Warrior, m_flIdleDelay,		FIELD_FLOAT ),
	...
	case TASK_WBUG_GET_PATH_TO_PATROL:
		{
			...
			m_pHintNode = CAI_Hint::FindHint( this, HINT_BUG_PATROL_POINT, bits_HINT_NODE_RANDOM, 4096 ); 
			if( !m_pHintNode )
			{
...
void CNPC_Bug_Warrior::HandleAnimEvent( animevent_t *pEvent )
{
	...
	case BUG_WARRIOR_AE_MELEE_HIT1:
		{
			QAngle viewPunch = QAngle( 20.0f, 0.0f, 0.0f );
			Vector shove = Vector( -350.0f, 1.0f, 1.0f );
			MeleeAttack( BUG_WARRIOR_MELEE1_RANGE, npc_bug_warrior_swipe_damage.GetFloat(), viewPunch, shove );
			return;
		}
		break;
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_Bug_Warrior::PainSound( void )
{
