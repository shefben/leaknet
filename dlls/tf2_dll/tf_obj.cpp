...
BEGIN_DATADESC( CBaseObject )
	// keys 
	DEFINE_KEYFIELD_NOT_SAVED( CBaseObject, m_bInvulnerable,		FIELD_BOOLEAN, "Invulnerable" ),
	DEFINE_KEYFIELD_NOT_SAVED( CBaseObject, m_flRepairMultiplier,	FIELD_FLOAT, "RepairMult" ),
	DEFINE_KEYFIELD_NOT_SAVED( CBaseObject, m_iszUnderAttackSound,	FIELD_STRING, "AttackNotify" ), 
	DEFINE_KEYFIELD_NOT_SAVED( CBaseObject, m_flMinDisableHealth,	FIELD_FLOAT, "MinDisabledHealth" ),
	DEFINE_KEYFIELD_NOT_SAVED( CBaseObject, m_SolidToPlayers,		FIELD_INTEGER, "SolidToPlayer" ),
	DEFINE_KEYFIELD_NOT_SAVED( CBaseObject, m_iszDisabledModel,		FIELD_STRING, "DisabledModel" ),
	DEFINE_KEYFIELD_NOT_SAVED( CBaseObject, m_bCantDie,				FIELD_BOOLEAN, "CantDie" ),

	// Inputs
	DEFINE_INPUTFUNC( CBaseObject, FIELD_INTEGER, "SetHealth", InputSetHealth ),
	DEFINE_INPUTFUNC( CBaseObject, FIELD_INTEGER, "AddHealth", InputAddHealth ),
	DEFINE_INPUTFUNC( CBaseObject, FIELD_INTEGER, "RemoveHealth", InputRemoveHealth ),
	DEFINE_INPUTFUNC( CBaseObject, FIELD_INTEGER, "SetMinDisabledHealth", InputSetMinDisabledHealth ),
	DEFINE_INPUTFUNC( CBaseObject, FIELD_INTEGER, "SetSolidToPlayer", InputSetSolidToPlayer ),

	// Outputs
	DEFINE_OUTPUT( CBaseObject, m_OnDestroyed, "OnDestroyed" ),
	DEFINE_OUTPUT( CBaseObject, m_OnDamaged, "OnDamaged" ),
	DEFINE_OUTPUT( CBaseObject, m_OnRepaired, "OnRepaired" ),
	DEFINE_OUTPUT( CBaseObject, m_OnBecomingDisabled, "OnDisabled" ),
	DEFINE_OUTPUT( CBaseObject, m_OnBecomingReenabled, "OnReenabled" ),
	DEFINE_OUTPUT( CBaseObject, m_OnObjectHealthChanged, "OnObjectHealthChanged" )
END_DATADESC()
...
void CBaseObject::SetObjectCollisionBox( void )
{
	...
	SetAbsMins( vecWorldMins );
	SetAbsMaxs( vecWorldMaxs );
}
...
void CBaseObject::StartPlacement( CBaseTFPlayer *pPlayer )
{
	...
	// Set my build size
	WorldSpaceAABB( &m_vecBuildMins, &m_vecBuildMaxs );
...
//-----------------------------------------------------------------------------
bool CBaseObject::CheckBuildPoint( Vector vecPoint, const Vector &vecTrace, Vector *vecOutPoint )
{
	trace_t tr;
	...
	if (pSurfaceProp->maxSpeedFactor < 1.0f)
		bClear = false;
...
void CBaseObject::TraceAttack( const CTakeDamageInfo &inputInfo, const Vector &vecDir, trace_t *ptr )
{
	...
	SpawnBlood( ptr->endpos, BloodColor(), info.GetDamage() );
			...
			*strchr(szRowName,',') = '\0';
			...
			if( g_CompletedRows.Find(szRowName) == g_CompletedRows.InvalidIndex() )
			{
				...
				for( int ColumnIdx = g_UniqueColumns.First(); ColumnIdx != g_UniqueColumns.InvalidIndex(); ColumnIdx = g_UniqueColumns.Next( ColumnIdx ) )
				{
					...
					// VXP: NOTE: No, I'd prefer to use idx, thanks
					for( int idx2 = g_DamageMap.First(); idx2 != g_DamageMap.InvalidIndex(); idx2 = g_DamageMap.Next(idx2) )
					{
						if( !_stricmp( g_DamageMap.GetElementName(idx2), szRowNameCommaColumn ) )
						{
							nDamageAmount = g_DamageMap[idx2];
							break;
						}
					}
