...
void CWalkerStrider::Fire()
{
	...
	m_flAnimTime = m_flPrevAnimTime = m_flCycle = 0;
		...
		else
		{
			...
			if ( gpGlobals->curtime >= m_flNextShootTime )
			{
				...
				if ( pDriver )
				{
					...
					CEntityMessageFilter effectFilter( this, "CWalkerStrider" );
					MessageBegin( effectFilter, 0 );
