...
void SendProxy_MeteorTargetPositions( const void *pStruct, const void *pData, DVariant *pOut, int iElement, int objectID )
{
...
void SendProxy_MeteorTargetRadii( const void *pStruct, const void *pData, DVariant *pOut, int iElement, int objectID )
{
	CEnvMeteorSpawnerShared *pMeteorSpawner = ( CEnvMeteorSpawnerShared* )pData;
...
BEGIN_DATADESC( CEnvMeteorSpawner )
	...
	// Key Fields.
	DEFINE_KEYFIELD( CEnvMeteorSpawner, m_SpawnerShared.m_iMeteorType, FIELD_INTEGER, "MeteorType" ),
	DEFINE_KEYFIELD( CEnvMeteorSpawner, m_SpawnerShared.m_bSkybox, FIELD_BOOLEAN, "SpawnInSkybox" ),
	DEFINE_KEYFIELD( CEnvMeteorSpawner, m_SpawnerShared.m_flMinSpawnTime, FIELD_FLOAT, "SpawnIntervalMin" ),
	DEFINE_KEYFIELD( CEnvMeteorSpawner, m_SpawnerShared.m_flMaxSpawnTime, FIELD_FLOAT, "SpawnIntervalMax" ),
	DEFINE_KEYFIELD( CEnvMeteorSpawner, m_SpawnerShared.m_nMinSpawnCount, FIELD_INTEGER, "SpawnCountMin" ),
	DEFINE_KEYFIELD( CEnvMeteorSpawner, m_SpawnerShared.m_nMaxSpawnCount, FIELD_INTEGER, "SpawnCountMax" ),
	DEFINE_KEYFIELD( CEnvMeteorSpawner, m_SpawnerShared.m_flMinSpeed, FIELD_FLOAT, "MeteorSpeedMin" ),
	DEFINE_KEYFIELD( CEnvMeteorSpawner, m_SpawnerShared.m_flMaxSpeed, FIELD_FLOAT, "MeteorSpeedMax" ),
	DEFINE_KEYFIELD( CEnvMeteorSpawner, m_SpawnerShared.m_flMeteorDamageRadius, FIELD_FLOAT, "MeteorDamageRadius" ),
	DEFINE_KEYFIELD( CEnvMeteorSpawner, m_fDisabled, FIELD_BOOLEAN,	"StartDisabled" ),

	// Function Pointers.
	DEFINE_FUNCTION( CEnvMeteorSpawner, MeteorSpawnerThink ),

	// Inputs
	DEFINE_INPUTFUNC( CEnvMeteorSpawner, FIELD_VOID, "Enable", InputEnable ),
	DEFINE_INPUTFUNC( CEnvMeteorSpawner, FIELD_VOID, "Disable", InputDisable ),
...
// Meteor Models
const char *strResourceMeteorModels[2] =
{
...
{
	// Set the networking to manual mode.
											    ...
											    Vector &vecTriggerMaxs )
{
	...
	if ( pEntity )
	{
		if ( pEntity->pev )
		{
			VectorCopy( pEntity->GetAbsMins(), vecTriggerMins );
			VectorCopy( pEntity->GetAbsMaxs(), vecTriggerMaxs );
		}
	}
}
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CEnvMeteorSpawner::ShouldTransmit( const edict_t *recipient, 
...
BEGIN_DATADESC( CEnvMeteorTarget )
	...
	// Key Fields.
	DEFINE_KEYFIELD( CEnvMeteorTarget, m_flRadius, FIELD_FLOAT, "EffectRadius" ),
...
BEGIN_DATADESC( CEnvMeteor )
	...
	// Function Pointers.
	DEFINE_FUNCTION( CEnvMeteor, MeteorSkyboxThink ),
	DEFINE_FUNCTION( CEnvMeteor, MeteorWorldThink ),
...
BEGIN_DATADESC( CShootingStarSpawner )
	...
	// keys 
	DEFINE_KEYFIELD_NOT_SAVED( CShootingStarSpawner, m_flSpawnInterval, FIELD_FLOAT, "SpawnInterval" ),
	DEFINE_KEYFIELD_NOT_SAVED( CShootingStarSpawner, m_bSkybox, FIELD_BOOLEAN, "SpawnInSkybox" ),
...
// Purpose:
//-----------------------------------------------------------------------------
bool CShootingStarSpawner::ShouldTransmit( const edict_t *recipient, 
										   const void *pvs, int clientArea )
{
