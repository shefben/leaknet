...
void SendProxy_TeleportStationElement( const void *pStructBase, const void *pData, DVariant *pOut, int iElement, int objectID )
{
	// Get the EHANDLE.
	EHANDLE hEnt;
	...
	SendProxy_EHandleToInt( pStructBase, &hEnt, pOut, iElement, objectID );
}


void SignalChangeInMCVSelectionPanels()
{
	FOR_EACH_LL( g_MCVSelectionPanels, i )
		g_MCVSelectionPanels[i]->NetworkStateChanged();
}
...
IMPLEMENT_SERVERCLASS_ST( CObjMCVSelectionPanel, DT_MCVSelectionPanel )
	SendPropVirtualArray( 
		...
		SendPropEHandle( "teleport_station_element", 0, 0, SendProxy_TeleportStationElement ),
		"teleport_stations" )
END_SEND_TABLE()
...
void CObjMCVSelectionPanel::SetTransmit( CCheckTransmitInfo *pInfo )
{
	...
	{
		CVehicleTeleportStation::GetDeployedTeleportStation( i )->SetTransmit( pInfo );
	}
}
