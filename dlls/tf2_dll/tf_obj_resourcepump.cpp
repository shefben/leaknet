...
BEGIN_DATADESC( CObjectResourcePump )
	...
	DEFINE_THINKFUNC( CObjectResourcePump, ResourcePumpThink ),
...
void CObjectResourcePump::SetupPump( void )
{
	...
	while ((pEntity = gEntList.FindEntityByClassname( pEntity, "trigger_resourcezone" )) != NULL)
	{
		...
		if ( ( GetAbsOrigin().x > pZone->GetAbsMins().x && GetAbsOrigin().x < pZone->GetAbsMaxs().x ) &&
			 ( GetAbsOrigin().y > pZone->GetAbsMins().y && GetAbsOrigin().y < pZone->GetAbsMaxs().y ) &&
			 ( GetAbsOrigin().z > pZone->GetAbsMins().z && GetAbsOrigin().z < pZone->GetAbsMaxs().z ) )
		{
