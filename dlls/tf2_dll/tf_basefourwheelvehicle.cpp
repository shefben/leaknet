...
ConVar road_feel( "road_feel", "0.1", FCVAR_SERVER | FCVAR_REPLICATED );
extern ConVar tf_fastbuild;

BEGIN_DATADESC( CBaseTFFourWheelVehicle )

	DEFINE_EMBEDDED( CBaseTFFourWheelVehicle, m_VehiclePhysics ),

	DEFINE_INPUTFUNC( CBaseTFFourWheelVehicle, FIELD_FLOAT, "Throttle", InputThrottle ),
	DEFINE_INPUTFUNC( CBaseTFFourWheelVehicle, FIELD_FLOAT, "Steer", InputSteering ),
	DEFINE_INPUTFUNC( CBaseTFFourWheelVehicle, FIELD_FLOAT, "Action", InputAction ),
	DEFINE_INPUTFUNC( CBaseTFFourWheelVehicle, FIELD_VOID, "TurnOn",	InputTurnOn ),
	DEFINE_INPUTFUNC( CBaseTFFourWheelVehicle, FIELD_VOID, "TurnOff", InputTurnOff ),
...
//-----------------------------------------------------------------------------
void CBaseTFFourWheelVehicle::SetVehicleSounds( const char *pStartSound, const char *pRunningSound, const char *pStopSound, const char *pTurboSound )
{
	m_VehiclePhysics.SetVehicleSounds( pStartSound, pRunningSound, pStopSound, pTurboSound );
}
...
void CBaseTFFourWheelVehicle::FinishedBuilding( void )
{
	...
	Q_snprintf( pScriptName, 128, "scripts/vehicles/%s.txt", GetClassname() );
...
void CBaseTFFourWheelVehicle::PlayerControlInit( CBasePlayer *pPlayer )
{
	...
	inputdata_t emptyInput = inputdata_t();
	InputTurnOn( emptyInput );
