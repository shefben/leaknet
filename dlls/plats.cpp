...
//Defines for default sound values for plats
#define	DEFAULT_PLAT_MOVING_NOISE	"common/null.wav"
#define	DEFAULT_PLAT_ARRIVE_NOISE	"common/null.wav"
...
BEGIN_DATADESC( CBasePlatTrain )
	...
	DEFINE_KEYFIELD( CBasePlatTrain, m_NoiseMoving, FIELD_SOUNDNAME, "noise1" ),
	DEFINE_KEYFIELD( CBasePlatTrain, m_NoiseArrived, FIELD_SOUNDNAME, "noise2" ),

#ifdef HL1_DLL
	DEFINE_KEYFIELD( CBasePlatTrain, m_MoveSound, FIELD_INTEGER, "movesnd" ),
	DEFINE_KEYFIELD( CBasePlatTrain, m_StopSound, FIELD_INTEGER, "stopsnd" ),

#endif 
	DEFINE_SOUNDPATCH( CBasePlatTrain, m_pMovementSound ),

	DEFINE_KEYFIELD( CBasePlatTrain, m_volume, FIELD_FLOAT, "volume" ),

	DEFINE_FIELD( CBasePlatTrain, m_flTWidth, FIELD_FLOAT ),
	DEFINE_FIELD( CBasePlatTrain, m_flTLength, FIELD_FLOAT ),
	DEFINE_KEYFIELD( CBasePlatTrain, m_flLip, FIELD_FLOAT, "lip" ),
	DEFINE_KEYFIELD( CBasePlatTrain, m_flWait, FIELD_FLOAT, "wait" ),
	DEFINE_KEYFIELD( CBasePlatTrain, m_flHeight, FIELD_FLOAT, "height" ),
...
// set the plat's "in-motion" sound
	...
	case	0:
		...
		break;
	case	1:
		enginesound->PrecacheSound( "plats/bigmove1.wav" );
		m_NoiseMoving = MAKE_STRING("plats/bigmove1.wav");
		break;
	case	2:
		enginesound->PrecacheSound( "plats/bigmove2.wav" );
		m_NoiseMoving = MAKE_STRING("plats/bigmove2.wav");
		break;
	case	3:
		enginesound->PrecacheSound( "plats/elevmove1.wav" );
		m_NoiseMoving = MAKE_STRING("plats/elevmove1.wav");
		break;
	case	4:
		enginesound->PrecacheSound( "plats/elevmove2.wav" );
		m_NoiseMoving = MAKE_STRING("plats/elevmove2.wav");
		break;
	case	5:
		enginesound->PrecacheSound( "plats/elevmove3.wav" );
		m_NoiseMoving = MAKE_STRING("plats/elevmove3.wav");
		break;
	case	6:
		enginesound->PrecacheSound( "plats/freightmove1.wav" );
		m_NoiseMoving = MAKE_STRING("plats/freightmove1.wav");
		break;
	case	7:
		enginesound->PrecacheSound( "plats/freightmove2.wav" );
		m_NoiseMoving = MAKE_STRING("plats/freightmove2.wav");
		break;
	case	8:
		enginesound->PrecacheSound( "plats/heavymove1.wav" );
		m_NoiseMoving = MAKE_STRING("plats/heavymove1.wav");
		break;
	case	9:
		enginesound->PrecacheSound( "plats/rackmove1.wav" );
		m_NoiseMoving = MAKE_STRING("plats/rackmove1.wav");
		break;
	case	10:
		enginesound->PrecacheSound( "plats/railmove1.wav" );
		m_NoiseMoving = MAKE_STRING("plats/railmove1.wav");
		break;
	case	11:
		enginesound->PrecacheSound( "plats/squeekmove1.wav" );
		m_NoiseMoving = MAKE_STRING("plats/squeekmove1.wav");
		break;
	case	12:
		enginesound->PrecacheSound( "plats/talkmove1.wav" );
		m_NoiseMoving = MAKE_STRING("plats/talkmove1.wav");
		break;
	case	13:
...
// set the plat's 'reached destination' stop sound
	...
	case	0:
		...
		break;
	case	1:
		enginesound->PrecacheSound( "plats/bigstop1.wav" );
		m_NoiseArrived = MAKE_STRING("plats/bigstop1.wav");
		break;
	case	2:
		enginesound->PrecacheSound( "plats/bigstop2.wav" );
		m_NoiseArrived = MAKE_STRING("plats/bigstop2.wav");
		break;
	case	3:
		enginesound->PrecacheSound( "plats/freightstop1.wav" );
		m_NoiseArrived = MAKE_STRING("plats/freightstop1.wav");
		break;
	case	4:
		enginesound->PrecacheSound( "plats/heavystop2.wav" );
		m_NoiseArrived = MAKE_STRING("plats/heavystop2.wav");
		break;
	case	5:
		enginesound->PrecacheSound( "plats/rackstop1.wav" );
		m_NoiseArrived = MAKE_STRING("plats/rackstop1.wav");
		break;
	case	6:
		enginesound->PrecacheSound( "plats/railstop1.wav" );
		m_NoiseArrived = MAKE_STRING("plats/railstop1.wav");
		break;
	case	7:
		enginesound->PrecacheSound( "plats/squeekstop1.wav" );
		m_NoiseArrived = MAKE_STRING("plats/squeekstop1.wav");
		break;
	case	8:
...
BEGIN_DATADESC( CFuncPlat )
	...
	DEFINE_FIELD( CFuncPlat, m_sNoise, FIELD_STRING ),

	// Function Pointers
	DEFINE_FUNCTION( CFuncPlat, PlatUse ),
	DEFINE_FUNCTION( CFuncPlat, CallGoDown ),
	DEFINE_FUNCTION( CFuncPlat, CallHitTop ),
	DEFINE_FUNCTION( CFuncPlat, CallHitBottom ),

	// Inputs
	DEFINE_INPUTFUNC( CFuncPlat, FIELD_VOID, "Toggle", InputToggle ),
	DEFINE_INPUTFUNC( CFuncPlat, FIELD_VOID, "GoUp", InputGoUp ),
	DEFINE_INPUTFUNC( CFuncPlat, FIELD_VOID, "GoDown", InputGoDown ),
...
void CFuncPlat::Setup( void )
{
	...
	{
		m_vecPosition2.z = GetLocalOrigin().z - EntitySpaceSize().z + 8;
	}
...
void CPlatTrigger::SpawnInsideTrigger( CFuncPlat *pPlatform )
{
	...
	Vector vecTMin = m_pPlatform->EntitySpaceMins() + Vector ( 25 , 25 , 0 );
	Vector vecTMax = m_pPlatform->EntitySpaceMaxs() + Vector ( 25 , 25 , 8 );
	vecTMin.z = vecTMax.z - ( m_pPlatform->m_vecPosition1.z - m_pPlatform->m_vecPosition2.z + 8 );
	if (m_pPlatform->EntitySpaceSize().x <= 50)
	{
		vecTMin.x = (m_pPlatform->EntitySpaceMins().x + m_pPlatform->EntitySpaceMaxs().x) / 2;
		vecTMax.x = vecTMin.x + 1;
	}
	if (m_pPlatform->EntitySpaceSize().y <= 50)
	{
		vecTMin.y = (m_pPlatform->EntitySpaceMins().y + m_pPlatform->EntitySpaceMaxs().y) / 2;
...
void CFuncPlat::HitBottom( void )
{
	...
	{
		CPASAttenuationFilter filter( this );
		EmitSound( filter, entindex(), CHAN_WEAPON, (char*)STRING(m_NoiseArrived), m_volume, ATTN_NORM );
	}
...
BEGIN_DATADESC( CFuncTrain )
	...
	DEFINE_FIELD( CFuncTrain, m_hEnemy, FIELD_EHANDLE ),
	DEFINE_FIELD( CFuncTrain, m_iszLastTarget, FIELD_STRING ),
	DEFINE_FIELD( CFuncTrain, m_flNextBlockTime, FIELD_TIME ),

	DEFINE_KEYFIELD( CFuncTrain, m_flBlockDamage, FIELD_FLOAT, "dmg" ),

	// Function Pointers
	DEFINE_FUNCTION( CFuncTrain, Wait ),
	DEFINE_FUNCTION( CFuncTrain, Next ),

	// Inputs
	DEFINE_INPUTFUNC( CFuncTrain, FIELD_VOID, "Toggle", InputToggle ),
	DEFINE_INPUTFUNC( CFuncTrain, FIELD_VOID, "Start",	InputStart ),
	DEFINE_INPUTFUNC( CFuncTrain, FIELD_VOID, "Stop",	InputStop ),
...
void CFuncTrain::Use( CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value )
{
	...
	else
	{
		...
		{
			CPASAttenuationFilter filter( this );
			EmitSound( filter, entindex(), CHAN_VOICE, (char*) STRING( m_NoiseArrived ), m_volume, ATTN_NORM );
		}
	}
}
...
void CFuncTrain::Next( void )
{
	...
	if ( m_hCurrentTarget->HasSpawnFlags( SF_CORNER_TELEPORT ) )
	{
		...
		UTIL_SetOrigin( this, pTarg->GetLocalOrigin() - (EntitySpaceMins() + EntitySpaceMaxs())* 0.5);
	...
	else
	{
		...
		CLEARBITS( m_fEffects, EF_NOINTERP );
		SetMoveDone( &CFuncTrain::Wait );
		LinearMove ( pTarg->GetLocalOrigin() - (EntitySpaceMins() + EntitySpaceMaxs())* 0.5, m_flSpeed );
	}
}
...
void CFuncTrain::Activate( void )
{
	...
	if ( m_activated == false )
	{
		...
		UTIL_SetOrigin( this, m_hCurrentTarget->GetLocalOrigin() - (EntitySpaceMins() + EntitySpaceMaxs()) * 0.5 );
...
BEGIN_DATADESC( CFuncTrackTrain )
	...
	DEFINE_KEYFIELD( CFuncTrackTrain, m_length, FIELD_FLOAT, "wheels" ),
	DEFINE_KEYFIELD( CFuncTrackTrain, m_height, FIELD_FLOAT, "height" ),
	DEFINE_KEYFIELD( CFuncTrackTrain, m_maxSpeed, FIELD_FLOAT, "startspeed" ),
	DEFINE_KEYFIELD( CFuncTrackTrain, m_flBank, FIELD_FLOAT, "bank" ),
	DEFINE_KEYFIELD( CFuncTrackTrain, m_flBlockDamage, FIELD_FLOAT, "dmg" ),
	DEFINE_KEYFIELD( CFuncTrackTrain, m_iszSoundMove, FIELD_SOUNDNAME, "MoveSound" ),
	DEFINE_KEYFIELD( CFuncTrackTrain, m_iszSoundStart, FIELD_SOUNDNAME, "StartSound" ),
	DEFINE_KEYFIELD( CFuncTrackTrain, m_iszSoundStop, FIELD_SOUNDNAME, "StopSound" ),
	...
	DEFINE_FIELD( CFuncTrackTrain, m_vecOldOrigin, FIELD_POSITION_VECTOR ),

	DEFINE_FIELD( CFuncTrackTrain, m_bSoundPlaying, FIELD_BOOLEAN ),

	// Inputs
	DEFINE_INPUTFUNC( CFuncTrackTrain, FIELD_VOID, "Stop", InputStop ),
	DEFINE_INPUTFUNC( CFuncTrackTrain, FIELD_VOID, "StartForward", InputStartForward ),
	DEFINE_INPUTFUNC( CFuncTrackTrain, FIELD_VOID, "StartBackward", InputStartBackward ),
	DEFINE_INPUTFUNC( CFuncTrackTrain, FIELD_VOID, "Toggle", InputToggle ),
	DEFINE_INPUTFUNC( CFuncTrackTrain, FIELD_VOID, "Resume", InputResume ),
	DEFINE_INPUTFUNC( CFuncTrackTrain, FIELD_VOID, "Reverse", InputReverse ),
	DEFINE_INPUTFUNC( CFuncTrackTrain, FIELD_FLOAT, "SetSpeed", InputSetSpeed ),
	DEFINE_INPUTFUNC( CFuncTrackTrain, FIELD_FLOAT, "SetSpeedDir", InputSetSpeedDir ),

	// Function Pointers
	DEFINE_FUNCTION( CFuncTrackTrain, Next ),
	DEFINE_FUNCTION( CFuncTrackTrain, Find ),
	DEFINE_FUNCTION( CFuncTrackTrain, NearestPath ),
	DEFINE_FUNCTION( CFuncTrackTrain, DeadEnd ),
...
int CFuncTrackTrain::DrawDebugTextOverlays( void )
{
	...
	if (m_debugOverlays & OVERLAY_TEXT_BIT) 
	{
		...
		Q_snprintf( tempstr,sizeof(tempstr), "current speed: %g", (double)m_flSpeed );
		NDebugOverlay::EntityText( entindex(), nOffset, tempstr, 0 );
		nOffset++;
...
void CFuncTrackTrain::SoundStop( void )
{
	...
	if ( m_bSoundPlaying )
	{
		...
		{
			CPASAttenuationFilter filter( this );
			EmitSound( filter, entindex(), CHAN_ITEM, STRING( m_iszSoundStop ), m_flVolume, ATTN_NORM, 0, 100 );
		}
	}
...
{
	if ( ( !m_iszSoundMove ) && ( !m_iszSoundStart ) )
	{
		return;
	}
	...
	if (!m_bSoundPlaying)
	{
		...
		{
			EmitSound( filter, entindex(), CHAN_ITEM, STRING( m_iszSoundStart ), m_flVolume, ATTN_NORM, 0, 100 );
		}

		if ( m_iszSoundMove != NULL_STRING )
		{
			EmitSound( filterReliable, entindex(), CHAN_STATIC, STRING( m_iszSoundMove ), m_flVolume, ATTN_NORM, 0, ( int )flpitch );
		}

		m_bSoundPlaying = true;
	} 
	else if ( m_iszSoundMove != NULL_STRING )
	{
		// update pitch
		EmitSound( filterReliable, entindex(), CHAN_STATIC, STRING( m_iszSoundMove ), m_flVolume, ATTN_NORM, SND_CHANGE_PITCH, (int) flpitch);
	}
}
...
void CFuncTrackTrain::ArriveAtNode( CPathTrack *pNode )
{
	...
	if ( m_spawnflags & SF_TRACKTRAIN_NOCONTROL )
	{
		...
		if ( pNode->m_flSpeed != 0 )
		{
			...
			DevMsg( 2, "TrackTrain %s speed to %4.2f\n", GetDebugName(), m_maxSpeed );
		}
	}
}
...
void CFuncTrackTrain::Next( void )
{
	...
	nextPos.z -= m_height;
	CPathTrack *pnext = m_ppath->LookAhead( nextPos, m_flSpeed * 0.1, 1 );
	nextPos.z += m_height;
	...
	if ( !HasSpawnFlags( SF_TRACKTRAIN_FIXED_ORIENTATION ) )
	{
		...
		if ( !pnext || (vecFaceDir.x == 0 && vecFaceDir.y == 0) )
			angles = curAngles;
		...
		QAngle vecAngVel( vx * 10, vy * 10, GetLocalAngularVelocity().z );
	...
	if ( pnext )
	{
		if ( pnext != m_ppath )
		{
			...
			//
			CPathTrack *pFire;
			bool movingForward = ( m_flSpeed >= 0 ) ? true : false;
			pFire = movingForward ? pnext : m_ppath;
			m_ppath = pnext;
			...
			//
			CPathTrack *pTeleport = pFire->GetNext();
...
{
	Vector offset = pControls->GetLocalOrigin() - m_vecOldOrigin;
...
void CFuncTrackTrain::Find( void )
{
	...
	// relink to make the object solid again
	Relink();
...
void CFuncTrackTrain::OnRestore( void )
{
	...
	{
		SetMoveDoneTime( 0.1 );	
		SetMoveDone( &CFuncTrackTrain::NearestPath );
		SetThink( NULL );
	}
}
...
void CFuncTrackTrain::Spawn( void )
{
	...
	m_vecOldOrigin = GetLocalOrigin();

	m_controlMins = EntitySpaceMins();
	m_controlMaxs = EntitySpaceMaxs();
	m_controlMaxs.z += 72;
...
void CFuncTrackTrain::Precache( void )
{
	...
	if ( m_iszSoundMove != NULL_STRING )
	{
		enginesound->PrecacheSound( STRING( m_iszSoundMove ) );
	}

	if ( m_iszSoundStart != NULL_STRING )
	{
		enginesound->PrecacheSound( STRING( m_iszSoundStart ) );
	}

	if ( m_iszSoundStop != NULL_STRING )
	{
		enginesound->PrecacheSound( STRING( m_iszSoundStop ) );
	}
}
...
BEGIN_DATADESC( CFuncTrainControls )
	...
	// Function Pointers
	DEFINE_FUNCTION( CFuncTrainControls, Find ),
...
BEGIN_DATADESC( CFuncTrackChange )
	...
	DEFINE_GLOBAL_KEYFIELD( CFuncTrackChange, m_trackTopName, FIELD_STRING, "toptrack" ),
	DEFINE_GLOBAL_KEYFIELD( CFuncTrackChange, m_trackBottomName, FIELD_STRING, "bottomtrack" ),
	DEFINE_GLOBAL_KEYFIELD( CFuncTrackChange, m_trainName, FIELD_STRING, "train" ),
	...
	// Function Pointers
	DEFINE_FUNCTION( CFuncTrackChange, Find ),
...
{
	// Can't trigger sound
	enginesound->PrecacheSound( "buttons/button11.wav" );
...
void CFuncTrackChange::OnRestore( void )
{
	BaseClass::OnRestore();
	SetMoveDoneTime( 1.0 );
	...
	if ( target )
	{
		...
		if ( target )
		{
			...
			if ( target )
			{
				...
				Vector center = (GetAbsMins() + GetAbsMaxs()) * 0.5;
...
BEGIN_DATADESC( CFuncTrackAuto )
	DEFINE_INPUTFUNC( CFuncTrackAuto, FIELD_VOID, "Trigger", TriggerTrackChange ),
END_DATADESC()
