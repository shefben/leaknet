...
//
// Purpose: Implements all assistant classes and members needed to support
//			CBaseEntity and its derived classes
/*
...
some of these can overlap.  all the data descriptions usable are:
	...
	DEFINE_FIELD(		type,	name,	fieldtype )
	DEFINE_KEYFIELD(	type,	name,	fieldtype,	mapname )
	DEFINE_KEYFIELD_NOTSAVED(	type,	name,	fieldtype,	mapname )
	DEFINE_ARRAY(		type,	name,	fieldtype,	count )
	DEFINE_GLOBAL_FIELD(type,	name,	fieldtype )
	DEFINE_CUSTOM_FIELD(type,	name,	datafuncs,	mapname )
	DEFINE_GLOBAL_KEYFIELD(type,name,	fieldtype,	mapname )
...
OUTPUTS:
 	...
 	DEFINE_OUTPUT(		type,	outputvar,	outputname )
...
typedescription_t CEventAction::m_SaveData[] =
{
	DEFINE_FIELD( CEventAction, m_iTarget, FIELD_STRING ),
	DEFINE_FIELD( CEventAction, m_iTargetInput, FIELD_STRING ),
	DEFINE_FIELD( CEventAction, m_iParameter, FIELD_STRING ),
	DEFINE_FIELD( CEventAction, m_flDelay, FIELD_FLOAT ),
	DEFINE_FIELD( CEventAction, m_nTimesToFire, FIELD_INTEGER ),
	DEFINE_FIELD( CEventAction, m_iIDStamp, FIELD_INTEGER ),

	// This is dealt with by the Restore method
	// DEFINE_FIELD( CEventAction, m_pNext, CEventAction ),
};
...
CMemoryPool g_EntityListPool( max(sizeof(CEventAction),sizeof(CMultiInputVar::inputitem_t)), 512, CMemoryPool::GROW_FAST );
...
typedescription_t CBaseEntityOutput::m_SaveData[] = 
{
	DEFINE_CUSTOM_FIELD( CBaseEntityOutput, m_Value, variantFuncs ),

	// This is saved manually by CBaseEntityOutput::Save
	// DEFINE_FIELD( CBaseEntityOutput, m_ActionList, CEventAction ),
};
...
int CBaseEntityOutput::Save( ISave &save )
{
	...
	if ( !save.WriteFields( "Value", this, NULL, m_SaveData, ARRAYSIZE(m_SaveData) ) )
		return 0;

	for ( CEventAction *ev = m_ActionList; ev != NULL; ev = ev->m_pNext )
	{
		if ( !save.WriteFields( "EntityOutput", ev, NULL, ev->m_SaveData, ARRAYSIZE(ev->m_SaveData) ) )
			return 0;
	}
...
int CBaseEntityOutput::Restore( IRestore &restore, int elementCount )
{
	...
	if ( !restore.ReadFields( "Value", this, NULL, m_SaveData, ARRAYSIZE(m_SaveData) ) )
		return 0;
	...
	for ( int i = 0; i < elementCount; i++ )
	{
		...
		if ( !restore.ReadFields( "EntityOutput", ev, NULL, ev->m_SaveData, ARRAYSIZE(ev->m_SaveData) ) )
			return 0;
...
//-----------------------------------------------------------------------------
DEFINE_FIXEDSIZE_ALLOCATOR( CEventQueue::PrioritizedEvent_t, 128, CMemoryPool::GROW_SLOW );
...
void CEventQueue::Clear( void )
{
	...
	PrioritizedEvent_t *pe = m_Events.m_pNext;
	
	while ( pe != NULL )
	{
		PrioritizedEvent_t *next = pe->m_pNext;
		delete pe;
		pe = next;
	}
...
{
	// build the new event
	PrioritizedEvent_t *newEvent = new PrioritizedEvent_t;
...
//-----------------------------------------------------------------------------
void CEventQueue::AddEvent( PrioritizedEvent_t *newEvent )
{
	// loop through the actions looking for a place to insert
	PrioritizedEvent_t *pe;
...
void CEventQueue::RemoveEvent( PrioritizedEvent_t *pe )
{
	Assert( pe->m_pPrev );
...
void CEventQueue::ServiceEvents( void )
{
	...
	while ( pe != NULL && pe->m_flFireTime <= gpGlobals->curtime )
	{
		...
		if ( pe->m_iTarget != NULL_STRING )
		{
			...
			while ( 1 )
			{
				target = gEntList.FindEntityByName( target, pe->m_iTarget, pe->m_pActivator );
				if ( !target )
					break;
...
void CEventQueue::CancelEvents( CBaseEntity *pCaller )
{
	...
	PrioritizedEvent_t *pCur = m_Events.m_pNext;
	...
	while (pCur != NULL)
	{
		...
		if (pCur->m_pCaller == pCaller)
		{
			...
			if (!_stricmp(STRING(pCur->m_pCaller->GetEntityName()), STRING(pCaller->GetEntityName())) &&
				!_stricmp(pCur->m_pCaller->GetClassname(), pCaller->GetClassname()))
			{
		...
		PrioritizedEvent_t *pCurSave = pCur;
		pCur = pCur->m_pNext;
...
// save data description for the event queue
typedescription_t CEventQueue::m_SaveData[] = 
{
	// These are saved explicitly in CEventQueue::Save below
	// DEFINE_FIELD( CEventQueue, m_Events, PrioritizedEvent_t ),

	DEFINE_FIELD( CEventQueue, m_iListCount, FIELD_INTEGER ),	// this value is only used during save/restore
};


// save data for a single event in the queue
typedescription_t CEventQueue::PrioritizedEvent_t::m_SaveData[] =
{
	DEFINE_FIELD( CEventQueue::PrioritizedEvent_t, m_flFireTime, FIELD_TIME ),
	DEFINE_FIELD( CEventQueue::PrioritizedEvent_t, m_iTarget, FIELD_STRING ),
	DEFINE_FIELD( CEventQueue::PrioritizedEvent_t, m_iTargetInput, FIELD_STRING ),
	DEFINE_FIELD( CEventQueue::PrioritizedEvent_t, m_pActivator, FIELD_EHANDLE ),
	DEFINE_FIELD( CEventQueue::PrioritizedEvent_t, m_pCaller, FIELD_EHANDLE ),
	DEFINE_FIELD( CEventQueue::PrioritizedEvent_t, m_pEntTarget, FIELD_EHANDLE ),
	DEFINE_FIELD( CEventQueue::PrioritizedEvent_t, m_iOutputID, FIELD_INTEGER ),
	DEFINE_CUSTOM_FIELD( CEventQueue::PrioritizedEvent_t, m_VariantValue, variantFuncs ),

//	DEFINE_FIELD( CEventQueue::PrioritizedEvent_t, m_pNext, FIELD_??? ),
//	DEFINE_FIELD( CEventQueue::PrioritizedEvent_t, m_pPrev, FIELD_??? ),
};
...
int CEventQueue::Save( ISave &save )
{
	...
	// save that value out to disk, so we know how many to restore
	if ( !save.WriteFields( "EventQueue", this, NULL, m_SaveData, ARRAYSIZE(m_SaveData) ) )
		return 0;
	
	// cycle through all the events, saving them all
	for ( pe = m_Events.m_pNext; pe != NULL; pe = pe->m_pNext )
	{
		if ( !save.WriteFields( "PEvent", pe, NULL, pe->m_SaveData, ARRAYSIZE(pe->m_SaveData) ) )
			return 0;
	}
...
int CEventQueue::Restore( IRestore &restore )
{
	...
	PrioritizedEvent_t tmpEvent;

	// load the number of items saved
	if ( !restore.ReadFields( "EventQueue", this, NULL, m_SaveData, ARRAYSIZE(m_SaveData) ) )
		return 0;
	
	for ( int i = 0; i < m_iListCount; i++ )
	{
		if ( !restore.ReadFields( "PEvent", &tmpEvent, NULL, tmpEvent.m_SaveData, ARRAYSIZE(tmpEvent.m_SaveData) ) )
			return 0;
...
void variant_t::Set( fieldtype_t ftype, void *data )
{
	...
	{
		memcpy( matVal, data, 16 * sizeof(float) );
		break;
	}
	case FIELD_MATRIX3X4_WORLDSPACE:
	{
		memcpy( matVal, data, sizeof(matrix3x4_t) );
		break;
	}
...
void variant_t::SetOther( void *data )
{
	...
	case FIELD_VMATRIX_WORLDSPACE:
	{
		memcpy( data, matVal, 16 * sizeof(float) );
		break;
	}
	case FIELD_MATRIX3X4_WORLDSPACE:
	{
		memcpy( data, matVal, sizeof(matrix3x4_t) );
		break;
	}
	...
	switch ( fieldType )
	{
		...
		case FIELD_STRING:
		{
			switch ( newType )
			{
				...
				{
					VMatrix mat;
					if (sscanf(STRING(iszVal), "{%f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f}", 
				...
				{
					matrix3x4_t matrix;
					if (sscanf(STRING(iszVal), "{%f %f %f %f %f %f %f %f %f %f %f %f}", 
						&matrix[0][0], &matrix[0][1], &matrix[0][2], &matrix[0][3],
						&matrix[1][0], &matrix[1][1], &matrix[1][2], &matrix[1][3],
						&matrix[2][0], &matrix[2][1], &matrix[2][2], &matrix[2][3] )!= 12 )
					{
...
const char *variant_t::ToString( void ) const
{
	...
	switch (fieldType)
	{
		...
		case FIELD_VMATRIX_WORLDSPACE:
		{
			Q_snprintf(szBuf,sizeof(szBuf), "{%g %g %g %g %g %g %g %g %g %g %g %g %g %g %g %g}", 
				(double)matVal[0], (double)matVal[1], (double)matVal[2], (double)matVal[3],
				(double)matVal[4], (double)matVal[5], (double)matVal[6], (double)matVal[7],
				(double)matVal[8], (double)matVal[9], (double)matVal[10], (double)matVal[11],
				(double)matVal[12], (double)matVal[13], (double)matVal[14], (double)matVal[15]
				);
			return(szBuf);
		}
		...
		case FIELD_MATRIX3X4_WORLDSPACE:
		{
			Q_snprintf(szBuf,sizeof(szBuf), "{%g %g %g %g %g %g %g %g %g %g %g %g}", 
				(double)matVal[0], (double)matVal[1], (double)matVal[2], (double)matVal[3],
				(double)matVal[4], (double)matVal[5], (double)matVal[6], (double)matVal[7],
				(double)matVal[8], (double)matVal[9], (double)matVal[10], (double)matVal[11]);
			return(szBuf);
...
{
	DEFINE_FIELD( variant_t, bVal, FIELD_BOOLEAN ),
};
typedescription_t variant_t::m_SaveInt[] =
{
	DEFINE_FIELD( variant_t, iVal, FIELD_INTEGER ),
};
typedescription_t variant_t::m_SaveFloat[] =
{
	DEFINE_FIELD( variant_t, flVal, FIELD_FLOAT ),
};
typedescription_t variant_t::m_SaveEHandle[] =
{
	DEFINE_FIELD( variant_t, eVal, FIELD_EHANDLE ),
};
typedescription_t variant_t::m_SaveString[] =
{
	DEFINE_FIELD( variant_t, iszVal, FIELD_STRING ),
};
typedescription_t variant_t::m_SaveColor[] =
{
	DEFINE_FIELD( variant_t, rgbaVal, FIELD_COLOR32 ),
};
...
//	DEFINE_ARRAY( variant_t, vecVal, FIELD_FLOAT, 3 ),

	DEFINE_FIELD( variant_savevector_t, vecSave, FIELD_VECTOR ),
};
typedescription_t variant_t::m_SavePositionVector[] =
{
	DEFINE_FIELD( variant_savevector_t, vecSave, FIELD_POSITION_VECTOR ),
};
...
{
	// Just here to shut up ClassCheck
//	DEFINE_ARRAY( variant_t, matVal, FIELD_FLOAT, 16 ),

	DEFINE_FIELD( variant_savevmatrix_t, matSave, FIELD_VMATRIX ),
};
typedescription_t variant_t::m_SaveVMatrixWorldspace[] =
{
	DEFINE_FIELD( variant_savevmatrix_t, matSave, FIELD_VMATRIX_WORLDSPACE ),
};
...
{
	DEFINE_FIELD( variant_savevmatrix3x4_t, matSave, FIELD_MATRIX3X4_WORLDSPACE ),
};
...
class CVariantSaveDataOps : public CDefSaveRestoreOps
{
	...
	virtual void Save( const SaveRestoreFieldInfo_t &fieldInfo, ISave *pSave )
	{
		...
		case FIELD_VMATRIX:
		{
			variant_savevmatrix_t temp;
			var->GetVMatrix( &temp.matSave );
			pSave->WriteFields( fieldInfo.pTypeDesc->fieldName, &temp, NULL, variant_t::m_SaveVMatrix, 1 );
			break;
		}
		case FIELD_VMATRIX_WORLDSPACE:
		{
			variant_savevmatrix_t temp;
			var->GetVMatrix( &temp.matSave );
			pSave->WriteFields( fieldInfo.pTypeDesc->fieldName, &temp, NULL, variant_t::m_SaveVMatrixWorldspace, 1 );
			break;
		}

		case FIELD_MATRIX3X4_WORLDSPACE:
		{
			variant_savevmatrix3x4_t temp;
			var->GetMatrix3x4( &temp.matSave );
			pSave->WriteFields( fieldInfo.pTypeDesc->fieldName, &temp, NULL, variant_t::m_SaveMatrix3x4Worldspace, 1 );
			break;
		}
	...
	virtual void Restore( const SaveRestoreFieldInfo_t &fieldInfo, IRestore *pRestore )
	{
		...
		case FIELD_VOID:
			break;
		case FIELD_BOOLEAN:	
			pRestore->ReadFields( fieldInfo.pTypeDesc->fieldName, &tmpVar, NULL, variant_t::m_SaveBool, 1 );
			break;
		case FIELD_INTEGER:	
			pRestore->ReadFields( fieldInfo.pTypeDesc->fieldName, &tmpVar, NULL, variant_t::m_SaveInt, 1 );
			break;
		case FIELD_FLOAT:
			pRestore->ReadFields( fieldInfo.pTypeDesc->fieldName, &tmpVar, NULL, variant_t::m_SaveFloat, 1 );
			break;
		case FIELD_EHANDLE:
			pRestore->ReadFields( fieldInfo.pTypeDesc->fieldName, &tmpVar, NULL, variant_t::m_SaveEHandle, 1 );
			break;
		case FIELD_STRING:
			pRestore->ReadFields( fieldInfo.pTypeDesc->fieldName, &tmpVar, NULL, variant_t::m_SaveString, 1 );
			break;
		case FIELD_COLOR32:
			pRestore->ReadFields( fieldInfo.pTypeDesc->fieldName, &tmpVar, NULL, variant_t::m_SaveColor, 1 );
			break;
		case FIELD_VECTOR:
		{
			variant_savevector_t Temp;
			pRestore->ReadFields( fieldInfo.pTypeDesc->fieldName, &Temp, NULL, variant_t::m_SaveVector, 1 );
			tmpVar.SetVector3D(Temp.vecSave);
			break;
		}
		case FIELD_POSITION_VECTOR:
		{
			...
			tmpVar.SetPositionVector3D(Temp.vecSave);
			break;
		}
		case FIELD_VMATRIX:
		{
			variant_savevmatrix_t temp;
			pRestore->ReadFields( fieldInfo.pTypeDesc->fieldName, &temp, NULL, variant_t::m_SaveVMatrix, 1 );
			tmpVar.SetVMatrix( temp.matSave );
			break;
		}
		case FIELD_VMATRIX_WORLDSPACE:
		{
			variant_savevmatrix_t temp;
			pRestore->ReadFields( fieldInfo.pTypeDesc->fieldName, &temp, NULL, variant_t::m_SaveVMatrixWorldspace, 1 );
			tmpVar.SetVMatrix( temp.matSave );
			break;
		}
		case FIELD_MATRIX3X4_WORLDSPACE:
		{
			variant_savevmatrix3x4_t temp;
			pRestore->ReadFields( fieldInfo.pTypeDesc->fieldName, &temp, NULL, variant_t::m_SaveMatrix3x4Worldspace, 1 );
			tmpVar.SetMatrix3x4( temp.matSave );
			break;
		}
		default:
			Warning( "Bad type %d in saved variant_t\n", var->FieldType() );
		...
		var->Set( (_fieldtypes)type, &tmpVar );
	}
...
CMemoryPool g_EntListMemPool( sizeof(entitem_t), 256, false );
