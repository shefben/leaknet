...
{
	PrecacheModel( CS_PLAYER_MODEL );
	PrecacheModel( CS_TERRORIST_MODEL );
	BaseClass::Precache();
}
...
void CCSPlayer::CSWeaponDrop( CBaseCombatWeapon *pWeapon )
{
	if ( pWeapon )
	{
		// tyabus: don't let players drop the knife
		if (!Q_strcmp(pWeapon->GetClassname(), "weapon_knife"))
			return;
		...
		pWeapon->AddSolidFlags( FSOLID_NOT_STANDABLE | FSOLID_TRIGGER );
	}
}
