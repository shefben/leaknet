	...
	if ( c == '\"' )
	{
		...
		while ( true )
		{
			...
			if ( c == '\\' && m_bHasEscapeSequences )
			{
				...
				switch ( c )
				{
					case 0  :	DevMsg(1, "KeyValues::ReadToken unexpected EOF after \\ in file %s.\n", s_LastFileLoadingFrom);
								return NULL;
...
KeyValues *KeyValues::CreateNewKey()
{
	...
	itoa(newID, buf, 10);
#elif _LINUX
	Q_snprintf( buf, sizeof( buf ), "%d", newID ); // VXP: TODO: Probably make this the main option
#endif // _LINUX
...
#elif _LINUX
			DevMsg( "KeyValues::GetInt(): Implement _wtoi" ); // VXP: TODO: Implement _wtoi
			return 0;
#endif // _LINUX
		case TYPE_FLOAT:
...
#elif _LINUX
	DevMsg( "KeyValues::GetWString(): Implement me!\n" ); // VXP: TODO: Implement wide char functions
#endif // _LINUX
	return defaultValue;
}
...
//#include "tier0/memdbgon.h" // VXP: Why is this here?
