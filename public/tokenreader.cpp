...
const char *TokenReader::Error(char *error, ...)
{
	...
	sprintf(szErrorBuf, "File %s, line %d: ", m_szFilename, m_nLine);
	strcat(szErrorBuf, error);
	m_nErrorCount++;
	return(szErrorBuf);
}
...
trtoken_t TokenReader::NextTokenDynamic(char **ppszStore)
{
	...
	*ppszStore = new char [strlen(szTempBuffer) + 1];
	strcpy(*ppszStore, szTempBuffer);
...
// Output : 
//-----------------------------------------------------------------------------
trtoken_t TokenReader::PeekTokenType(char *pszStore)
{
	if (!m_bStuffed)
	{
