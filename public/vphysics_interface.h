...
// VXP: Putting that so we don't have to worry about infinite values in IVP's set_mass
const float VPHYSICS_MIN_MASS = 0.1f;
const float VPHYSICS_MAX_MASS = 5e4f;
const float VPHYSICS_MAX_INERTIA = VPHYSICS_MAX_MASS; // VXP: Because why not
...
#define VPHYSICS_INTERFACE_VERSION	"VPhysics029"
class IPhysics
{
public:
...
public:
	...
	virtual void GetControlVelocity( Vector &outVel ) = 0;
...
public:
	...
	virtual void MaxSpeed( const Vector &maxVelocity, const AngularImpulse &maxAngularVelocity ) = 0;
	virtual void GetControlVelocity( Vector &outVel ) = 0;
	virtual void StepUp( float height ) = 0;
...
public:
	...
	virtual void			EnableCollisions( IPhysicsObject *pObject1, IPhysicsObject *pObject2 ) = 0;
	// install a function to filter collisions/penentration
	virtual void			SetCollisionSolver( IPhysicsCollisionSolver *pSolver ) = 0;
	virtual bool			ShouldCollide( IPhysicsObject *pObject1, IPhysicsObject *pObject2 ) = 0;
	...
	// UNDONE: Expose spatial callback oriented controllers (you could implement AI-based or more complex fluid with this)
	//			Physics trigger / phantom object
	// UNDONE: Expose performance scalability options
	// UNDONE: Statistics API
};
