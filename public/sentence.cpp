...
void CSentence::ParseCloseCaption( CUtlBuffer& buf )
{
	...
	while ( 1 )
	{
		...
		while ( 1 )
		{
			...
			bool unicode = false;
			if ( !Q_stricmp( cc_type, "unicode" ) )
			{
				unicode = true;
			}
			else if ( Q_stricmp( cc_type, "char" ) )
			{
				Assert( 0 );
			}
...
void CSentence::ParseDataVersionOnePointZero( CUtlBuffer& buf )
{
	...
	while ( 1 )
	{
		...
		if ( !Q_stricmp( section, "PLAINTEXT" ) )
		{
			ParsePlaintext( buf );
		}
		else if ( !Q_stricmp( section, "WORDS" ) )
		{
			ParseWords( buf );
		}
		...
		else if ( !Q_stricmp( section, "CLOSECAPTION" ) )
		{
			ParseCloseCaption( buf );
		}
		else if ( !Q_stricmp( section, "OPTIONS" ) )
		{
			ParseOptions( buf );
		}
	}
