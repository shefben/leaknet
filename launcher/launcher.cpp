...
static bool LoadAppSystems( )
{
	...
	s_hMatSystemModule = FileSystem_LoadModule("materialsystem" DLL_EXT_STRING);
	if (!s_hMatSystemModule)
	{
		Error( "Unable to load materialsystem" DLL_EXT_STRING "\n" );
		return false;
	}
	...
	// Load up the engine
	s_hEngineModule = FileSystem_LoadModule( "engine" DLL_EXT_STRING );
	if ( !s_hEngineModule )
	{
		Error( "Unable to load engine" DLL_EXT_STRING "\n" );
		return false;
	}
	...
	// Load up vgui2
	s_hVGuiModule = FileSystem_LoadModule( "vgui2" DLL_EXT_STRING );
	if ( !s_hVGuiModule )
	{
		Error( "Unable to load vgui2" DLL_EXT_STRING "\n" );
		return false;
	}
	...
	s_hVguiMatSurfaceModule = FileSystem_LoadModule("vguimatsurface" DLL_EXT_STRING);
	if (!s_hVguiMatSurfaceModule)
	{
		Error( "Unable to load vguimatsurface" DLL_EXT_STRING "\n" );
		return false;
	}
