...
void CClassMenu::Update( int *validClasses, int numClasses)
{
	...
	if (validClasses[teamNumber] == -1) // civilian button only
	{
		...
		{
			if( !Q_stricmp(m_pClassButtons[i]->GetName(), "civilian") )
			{
...
{
	static char classPanel[ MAX_PATH ];
	Q_snprintf( classPanel, sizeof( classPanel ), "classes/short_%s.res", className);
