...
{
	m_NavGroup.SetCurrentDefaultButton(defaultButton);
...
//-----------------------------------------------------------------------------
void EditablePanel::LoadControlSettingsAndUserConfig(const char *dialogResourceName, const char* pathID, int dialogID)
{
	LoadControlSettings(dialogResourceName, pathID);
...
{
	if (!strcmp(data->GetName(), "BuildDialog"))
	{
	...
	else if (!strcmp(data->GetName(), "ControlFactory"))
	{
...
// Input  : *panel - 
//-----------------------------------------------------------------------------
bool EditablePanel::RequestFocusNext(Panel *panel)
{
...
// Input  : *panel - 
//-----------------------------------------------------------------------------
bool EditablePanel::RequestFocusPrev(Panel *panel)
{
...
//-----------------------------------------------------------------------------
void EditablePanel::OnRequestFocus(Panel *subFocus, Panel *defaultPanel)
{
	if (!subFocus->IsPopup())
    {
		defaultPanel = m_NavGroup.SetCurrentFocus(subFocus, defaultPanel);
    }
	BaseClass::OnRequestFocus(this, defaultPanel);
}
...
{
	if (!strcmp(controlName, "Label"))
	{
		return new Label(NULL, NULL, "Label");
	}
	else if (!strcmp(controlName, "Button"))
	{
		return new Button(NULL, NULL, "Button");
	}
	else if (!strcmp(controlName, "CheckButton"))
	{
		return new CheckButton(NULL, NULL, "CheckButton");
	}
	else if (!strcmp(controlName, "ComboBox"))
	{
		return new ComboBox(NULL, NULL, 5, true);
	}
	else if (!strcmp(controlName, "Menu"))
	{
		return new Menu(NULL, "Menu");
	}
	else if (!strcmp(controlName, "MenuItem"))
	{
		return new MenuItem(NULL, NULL, "MenuItem");
	}
	else if (!strcmp(controlName, "MessageBox"))
	{
		return new MessageBox("MessageBox", "MessageBoxText");
	}
	else if (!strcmp(controlName, "ProgressBar"))
	{
		return new ProgressBar(NULL, NULL);
	}
	else if (!strcmp(controlName, "RadioButton"))
	{
		return new RadioButton(NULL, NULL, "RadioButton");
	}
	else if (!strcmp(controlName, "ScrollBar"))
	{
		return new ScrollBar(NULL, NULL, false);
	}
	else if (!strcmp(controlName, "TextEntry"))
	{
		return new TextEntry(NULL, NULL);
	}
	else if (!strcmp(controlName, "RichText"))
	{
		return new RichText(NULL, NULL);
	}
	else if (!strcmp(controlName, "ToggleButton"))
	{
		return new ToggleButton(NULL, NULL, "ToggleButton");
	}
	else if (!strcmp(controlName, "ImagePanel"))
	{
		return new ImagePanel(NULL, "ResourceImagePanel");
	}
	else if (!strcmp(controlName, "AnimatingImagePanel"))
	{
		return new AnimatingImagePanel(NULL, NULL);
	}
	else if (!strcmp(controlName, "Panel"))
	{
		return new Panel(NULL, NULL);
	}
	else if (!strcmp(controlName, "Divider"))
	{
		return new Divider(NULL, "Divider");
	}
    else if (!strcmp(controlName, "URLLabel"))
    {
        return new URLLabel(NULL, NULL, "URLLabel", NULL);
    }
	else if (!strcmp(controlName, "EditablePanel"))
	{
...
// Purpose: Message map
//-----------------------------------------------------------------------------
MessageMapItem_t EditablePanel::m_MessageMap[] =
{
	MAP_MESSAGE( EditablePanel, "FindDefaultButton", OnFindDefaultButton ),
	MAP_MESSAGE_PTR( EditablePanel, "DefaultButtonSet", OnDefaultButtonSet, "button" ),
	MAP_MESSAGE_PTR( EditablePanel, "CurrentDefaultButtonSet", OnCurrentDefaultButtonSet, "button" ),
};

IMPLEMENT_PANELMAP(EditablePanel, Panel);
