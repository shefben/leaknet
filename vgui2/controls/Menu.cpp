...
//-----------------------------------------------------------------------------
int Menu::AddMenuItemCharCommand(MenuItem *item, const char *command, Panel *target, KeyValues *userData)
{
...
// Output:  itemID - ID of this item
//-----------------------------------------------------------------------------
int Menu::AddMenuItemKeyValuesCommand( MenuItem *item, KeyValues *message, Panel *target, KeyValues *userData  )
{
...
// Output:  itemID - ID of this item
//-----------------------------------------------------------------------------
int Menu::AddMenuItem( const char *itemName, const char *itemText, const char *command, Panel *target, KeyValues *userData  )
{
	MenuItem *item = new MenuItem(this, itemName, itemText );
	return AddMenuItemCharCommand(item, command, target, userData);
}

int Menu::AddMenuItem( const char *itemName, const wchar_t *wszItemText, const char *command, Panel *target, KeyValues *userData  )
{
...
// Output:  itemID - ID of this item
//-----------------------------------------------------------------------------
int Menu::AddMenuItem( const char *itemText, const char *command, Panel *target, KeyValues *userData  )
{
...
//							ptr to the menu that opens on selecting the item
//-----------------------------------------------------------------------------
int Menu::AddMenuItem( const char *itemName, const char *itemText, KeyValues *message, Panel *target, KeyValues *userData  )
{
	MenuItem *item = new MenuItem(this, itemName, itemText );
	return AddMenuItemKeyValuesCommand(item, message, target, userData);
}

int Menu::AddMenuItem( const char *itemName, const wchar_t *wszItemText, KeyValues *message, Panel *target, KeyValues *userData  )
{
	MenuItem *item = new MenuItem(this, itemName, wszItemText );
	return AddMenuItemKeyValuesCommand(item, message, target, userData);
}
...
//							ptr to the menu that opens on selecting the item
//-----------------------------------------------------------------------------
int Menu::AddMenuItem( const char *itemText, KeyValues *message, Panel *target, KeyValues *userData  )
{
...
//							ptr to the menu that opens on selecting the item
//-----------------------------------------------------------------------------
int Menu::AddMenuItem( const char *itemText, Panel *target , KeyValues *userData )
{
...
//			*target - Target panel of the command
//-----------------------------------------------------------------------------
int Menu::AddCheckableMenuItem( const char *itemName, const char *itemText, const char *command, Panel *target , KeyValues *userData )
{
	MenuItem *item = new MenuItem(this, itemName, itemText, NULL, true);
	return AddMenuItemCharCommand(item, command, target, userData);
}

int Menu::AddCheckableMenuItem( const char *itemName, const wchar_t *wszItemText, const char *command, Panel *target , KeyValues *userData )
{
...
//							ptr to the menu that opens on selecting the item
//-----------------------------------------------------------------------------
int Menu::AddCheckableMenuItem( const char *itemText, const char *command, Panel *target, KeyValues *userData  )
{
...
//							ptr to the menu that opens on selecting the item
//-----------------------------------------------------------------------------
int Menu::AddCheckableMenuItem( const char *itemName, const char *itemText, KeyValues *message, Panel *target, KeyValues *userData  )
{
	MenuItem *item = new MenuItem(this, itemName, itemText, NULL, true);
	return AddMenuItemKeyValuesCommand(item, message, target, userData);
}

int Menu::AddCheckableMenuItem( const char *itemName, const wchar_t *wszItemText, KeyValues *message, Panel *target, KeyValues *userData  )
{
	MenuItem *item = new MenuItem(this, itemName, wszItemText, NULL, true);
	return AddMenuItemKeyValuesCommand(item, message, target, userData);
}
...
//							ptr to the menu that opens on selecting the item
//-----------------------------------------------------------------------------
int Menu::AddCheckableMenuItem( const char *itemText, KeyValues *message, Panel *target, KeyValues *userData  )
{
...
//							ptr to the menu that opens on selecting the item
//-----------------------------------------------------------------------------
int Menu::AddCheckableMenuItem( const char *itemText, Panel *target, KeyValues *userData  )
{
...
//							ptr to the menu that opens on selecting the item
//-----------------------------------------------------------------------------
int Menu::AddCascadingMenuItem( const char *itemName, const char *itemText, const char *command, Panel *target, Menu *cascadeMenu , KeyValues *userData )
{
	MenuItem *item = new MenuItem(this, itemName, itemText, cascadeMenu );
	return AddMenuItemCharCommand(item, command, target, userData);
}

int Menu::AddCascadingMenuItem( const char *itemName, const wchar_t *wszItemText, const char *command, Panel *target, Menu *cascadeMenu , KeyValues *userData )
{
...
//							ptr to the menu that opens on selecting the item
//-----------------------------------------------------------------------------
int Menu::AddCascadingMenuItem( const char *itemText, const char *command, Panel *target, Menu *cascadeMenu , KeyValues *userData )
{
...
//							ptr to the menu that opens on selecting the item
//-----------------------------------------------------------------------------
int Menu::AddCascadingMenuItem( const char *itemName, const char *itemText, KeyValues *message, Panel *target, Menu *cascadeMenu , KeyValues *userData )
{
	MenuItem *item = new MenuItem( this, itemName, itemText, cascadeMenu);
	return AddMenuItemKeyValuesCommand(item, message, target, userData);
}

int Menu::AddCascadingMenuItem( const char *itemName, const wchar_t *wszItemText, KeyValues *message, Panel *target, Menu *cascadeMenu , KeyValues *userData )
{
	MenuItem *item = new MenuItem( this, itemName, wszItemText, cascadeMenu);
	return AddMenuItemKeyValuesCommand(item, message, target, userData);
}
...
//							ptr to the menu that opens on selecting the item
//-----------------------------------------------------------------------------
int Menu::AddCascadingMenuItem( const char *itemText, KeyValues *message, Panel *target, Menu *cascadeMenu , KeyValues *userData )
{
...
//							ptr to the menu that opens on selecting the item
//-----------------------------------------------------------------------------
int Menu::AddCascadingMenuItem( const char *itemText, Panel *target, Menu *cascadeMenu , KeyValues *userData )
{
...
//			*message - pointer to the message to send when the item is selected
//-----------------------------------------------------------------------------
void Menu::UpdateMenuItem(int itemID, const char *itemText, KeyValues *message, KeyValues *userData)
{
...
void Menu::UpdateMenuItem(int itemID, const wchar_t *wszItemText, KeyValues *message, KeyValues *userData)
{
	Assert( m_MenuItems.IsValidIndex(itemID) );
	if ( m_MenuItems.IsValidIndex(itemID) )
	{
...
void Menu::PerformLayout()
{
	...
	Panel *check = GetParent();

	if ( check )
	{
		// move the menu to the correct position if it is a cascading menu.
		MenuItem *parent = dynamic_cast<MenuItem *>( check );
		if (parent)
		{
...
void Menu::PositionCascadingMenu()
{
	...
	GetParent()->LocalToScreen(parentX, parentY);
...
void Menu::OnKeyCodeTyped(KeyCode code)
{
	...
	if (alt)
	{
		if (GetParent())
		{
			GetParent()->OnKeyCodeTyped(code); // chain up.
		}
	...
	case KEY_LEFT:
		{
			...
			if (dynamic_cast<MenuItem *>(GetParent()))
			{
				SetVisible(false);
			}
			else if (GetParent())
...
void Menu::OnKillFocus()
{
	...
	if (!input()->GetFocus() || !ipanel()->HasParent(input()->GetFocus(), GetVPanel()))
	{
		...
		MenuItem *item = dynamic_cast<MenuItem *>(GetParent());
		...
		if ( (item) && (input()->GetFocus() == item->GetParent()->GetVPanel()) )
		{
...
void Menu::OnMenuItemSelected(Panel *panel)
{
	...
	if (item)
	{
		...
		Menu *parentMenu = 	dynamic_cast<Menu *>(item->GetParent());
		if (parentMenu)
		{
	...
	if (GetParent())
	{
		...
		ivgui()->PostMessage(GetParent()->GetVPanel(), kv, GetVPanel());
	}
}
...
void Menu::OnKeyCodePressed(KeyCode code)
{
	...
	if (GetParent())
	{
		ivgui()->PostMessage(GetParent()->GetVPanel(), new KeyValues("KeyModeSet"), GetVPanel());
	}
}
...
MessageMapItem_t Menu::m_MessageMap[] =
{
	MAP_MESSAGE( Menu, "KeyModeSet", OnKeyModeSet ),
	MAP_MESSAGE_PTR( Menu, "MenuItemSelected", OnMenuItemSelected, "panel" ),
	MAP_MESSAGE( Menu, "ScrollBarSliderMoved", OnSliderMoved ),
	MAP_MESSAGE_INT( Menu, "CursorEnteredMenuItem", OnCursorEnteredMenuItem, "VPanel" ),
	MAP_MESSAGE_INT( Menu, "CursorExitedMenuItem", OnCursorExitedMenuItem, "VPanel" ),

};
IMPLEMENT_PANELMAP(Menu, Panel);
