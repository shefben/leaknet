...
void CLabeledSlider::DrawTickLabels()
{
	...
	if ( m_szTickCaptions[ 0 ][ 0 ] )
	{
		...
		surface()->DrawPrintText( (const wchar_t *)m_szTickCaptions[ 0 ], strlen( m_szTickCaptions[ 0 ] ) );
	}
	...
	if ( m_szTickCaptions[ 1 ][ 0 ] )
	{
		...
		surface()->DrawPrintText((const wchar_t *) m_szTickCaptions[ 1 ], strlen( m_szTickCaptions[ 1 ] ) );
	}
}
