	...
	if (_defaultButton != (bool)state)
	{
		...
		if (state)
		{
			...
			if (GetParent())
			{
				...
				ivgui()->PostMessage(GetParent()->GetVPanel(), msg, GetVPanel());
			}
		}
...
{
	if (!strcmp(outputData->GetName(), "GetState"))
	{
		outputData->SetInt("state", IsSelected());
		return true;
	}
	else if ( !strcmp( outputData->GetName(), "GetCommand" ))
	{
		if ( _actionMessage )
		{
...
const char *Button::GetDescription( void )
{
	...
	Q_sprintf_safe(buf, "%s, string command, int default", BaseClass::GetDescription());
	return buf;
}
...
// Purpose: Message map
//-----------------------------------------------------------------------------
MessageMapItem_t Button::m_MessageMap[] =
{
	MAP_MESSAGE( Button, "PressButton", DoClick ),
	MAP_MESSAGE( Button, "Hotkey", DoClick ),
	MAP_MESSAGE_INT( Button, "SetState", OnSetState, "state" ),
	MAP_MESSAGE_INT( Button, "SetAsDefaultButton", SetAsDefaultButton, "state" ),
	MAP_MESSAGE_INT( Button, "SetAsCurrentDefaultButton", SetAsCurrentDefaultButton, "state" ),
};
IMPLEMENT_PANELMAP( Button, Label );
