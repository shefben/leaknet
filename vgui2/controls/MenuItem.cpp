...
//-----------------------------------------------------------------------------
MenuItem::MenuItem(Panel *parent, const char *panelName, const char *text, Menu *cascadeMenu, bool checkable) : Button(parent, panelName, text)
{
...
//			MenuItems cannot be both checkable and trigger a cascade menu.
//-----------------------------------------------------------------------------
MenuItem::MenuItem(Panel *parent, const char *panelName, const wchar_t *wszText, Menu *cascadeMenu, bool checkable) : Button(parent, panelName, wszText)
{
	m_pCascadeMenu = cascadeMenu;
	m_bCheckable = checkable;
	SetButtonActivationType(ACTIVATE_ONRELEASED);
...
void MenuItem::OnCursorMoved(int x, int y)
{
	...
	// highlight this item
	if (static_cast<Menu *>(GetParent())->GetMenuMode() == Menu::KEYBOARD)
	{
		OnCursorEntered();
	}
...
{
	if (static_cast<Menu *>(GetParent())->GetMenuMode() == Menu::KEYBOARD && m_pCascadeMenu)
	{
		return;
	}
...
void MenuItem::ArmItem()
{
	...
	static_cast<Menu *>(GetParent())->CloseOtherMenus(this);
	// arm the menuItem.
...
void MenuItem::FireActionSignal()
{
	...
	//	ivgui()->PostMessage(GetParent()->GetVPanel(), new KeyValues("MenuItemSelected"), GetVPanel());		
...
// Purpose: 
//-----------------------------------------------------------------------------
void MenuItem::SetUserData(KeyValues *kv)
{
	if (kv)
	{
		if ( m_pUserData )
...
MessageMapItem_t MenuItem::m_MessageMap[] =
{
	MAP_MESSAGE( MenuItem, "KeyModeSet", OnKeyModeSet ),
	MAP_MESSAGE( MenuItem, "MenuClose", OnKillFocus ),
	MAP_MESSAGE( MenuItem, "ArmItem", ArmItem ),
	MAP_MESSAGE( MenuItem, "DisarmItem", DisarmItem ),
};
IMPLEMENT_PANELMAP(MenuItem, Button);
