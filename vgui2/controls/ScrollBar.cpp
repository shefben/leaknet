...
ScrollBar::ScrollBar(Panel *parent, const char *panelName, bool vertical) : Panel(parent, panelName)
{
	...
	if (vertical)
	{
		...
		_button[0]->SetTextInset(1, 1);
		_button[1]->SetTextInset(1, -1);
	}
	else
	{
		...
		_button[0]->SetTextInset(1, 0);
		_button[1]->SetTextInset(1, 0);
...
// Purpose: message map
//-----------------------------------------------------------------------------
MessageMapItem_t ScrollBar::m_MessageMap[] =
{
	MAP_MESSAGE_FLOAT( ScrollBar, "ScrollBarSliderMoved", OnSliderMoved, "position" ),
};
IMPLEMENT_PANELMAP(ScrollBar, Panel);
