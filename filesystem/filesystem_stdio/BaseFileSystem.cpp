...
FileHandle_t CBaseFileSystem::Open( const char *pFileName, const char *pOptions, const char *pathID )
{
	...
	// VXP: Call createdirhierarchy upon opening for write.
	char tempPathName[MAX_PATH];
	Q_snprintf( tempPathName, MAX_PATH, "%s", pFileName );
	StripFilename( tempPathName );

	if ( Q_strncmp( tempPathName, "", MAX_PATH ) != 0 )
	{
		CreateDirHierarchy( tempPathName, pathID );
	}
