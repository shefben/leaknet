...
bool MaterialSystemApp::InitMaterialSystem()
{
	...
	if(!pShaderDLL)
	{
		strcpy(defaultShaderDLL, "shaderapidx9" DLL_EXT_STRING);
		pShaderDLL = defaultShaderDLL;
	}

	m_pFileSystemDLL = Sys_LoadModule( "filesystem_stdio" DLL_EXT_STRING );
	if( !m_pFileSystemDLL )
	{
		return Sys_Error( "Can't load file system DLL (filesystem_stdio%s)", DLL_EXT_STRING );
	}
