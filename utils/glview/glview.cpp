...
void ReadPolyFileType(char *name, int nList, BOOL drawLines)
{
	FILE	*f;
	int		i, j, numverts;
	float	v[8];
...
void ReadPHYFile(char *name, int nList)
{
...
void ReadPolyFile (char *name)
{
	char ext[4];
	ExtractFileExtension( name, ext, 4 );

	if ( !Q_stricmp( ext, "phy" ) )
	{
		CmdLib_InitFileSystem( name, true );
					...
					,LPSTR lpCmdLine, int nCmdShow)
{
	...
	// This is ugly, but gets the job done for checking for "-portal"
	strcpy(szTempCmd, lpCmdLine);
	pStart = szTempCmd;

	// Is first command letter a '-'?
	if (pStart && pStart[0] == '-')
	{
		// Look for any space
		...
		// If we found a space (not at end), then terminate this string
		if (pStart && pStart[0])
			*pStart++ = '\0';
	};
