...
private:
	...
	LONG			CreateWorkspaceKey( PHKEY phKey );
...
{
	if ( !_stricmp( name, "expressionfiles" ) )
	{
		return EXPRESSION;
	}
	else if ( !_stricmp( name, "choreodatafiles" ) )
	{
		return CHOREODATA;
	}
	else if ( !_stricmp( name, "modelfiles" ) )
	{
		return MODELDATA;
	}
	return -1;
...
LONG CWorkspaceFiles::CreateWorkspaceKey( PHKEY phKey )
{
	DWORD disp;

	return RegCreateKeyEx(
		HKEY_CURRENT_USER,	// handle of open key 
		"Software\\LeakNet\\faceposer\\workspace", // address of name of subkey to open 
