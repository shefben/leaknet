...
bool LoadStudioModel( char const* pModelName, char const* pEntityType, CUtlBuffer& buf )
{
	...
	// load the model
	if ( ( fp = fopen( pFileNameModDir, "rb" ) ) == NULL )
		if ( ( fp = fopen( pFileNameBaseGameDir, "rb" ) ) == NULL ) // VXP: If we cannot get the model from the mod directory - try the base game directory (hl2)
			return false;
		else
