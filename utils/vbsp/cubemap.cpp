...
//	"$fallbackmaterial",	// Used by all materials. VXP: Not used in LeakNet
...
static bool LoadSrcVTFFiles( IVTFTexture *pSrcVTFTextures[6], const char *pSkyboxBaseName )
{
	...
	for( i = 0; i < 6; i++ )
	{
		...
		// VXP: Gather $basetexture from a skybox material
		char basetextureVal[512];
		bool foundVal = GetValueFromMaterial( srcVMTFileName, "$basetexture", basetextureVal, 511 );

		// VXP: Check in mod directory first, in hl2 directory next
		// VXP: FIXME: This should be integrated with a filesystem!
		for ( int j = 0; j < (sizeof( dirs ) / sizeof( dirs[0] )); j++ )
		{
			...
			if ( foundVal )
			{
				...
				sprintf( srcVTFFileName, "%smaterials/%s.vtf", dirs[j], basetextureVal );
			}
			else
			{
				// VXP: We can't find a $basetexture val in material file.
			...
			// VXP: If we've found a texture - break from searching
...
void Cubemap_FixupBrushSidesMaterials( void )
{
	...
	for( cubemapID = 0; cubemapID < g_nCubemapSamples; cubemapID++ )
	{
		...
		for( i = 0; i < brushSidesVector.Count(); i++ )
		{
			...
			// VXP: Cubemap_CreateMaterialForBrushSide
