...
ChunkFileResult_t LoadSolidKeyCallback(const char *szKey, const char *szValue, LoadEntity_t *pLoadEntity);
...
ChunkFileResult_t LoadSolidCallback(CChunkFile *pFile, LoadEntity_t *pLoadEntity)
{
	...
	g_MapError.BrushState( b->entitynum, b->brushnum );
	...
	//
	pFile->PushHandlers(&Handlers);
	ChunkFileResult_t eResult = pFile->ReadChunk((KeyHandler_t)LoadSolidKeyCallback, pLoadEntity);
	pFile->PopHandlers();
...
ChunkFileResult_t LoadSolidKeyCallback(const char *szKey, const char *szValue, LoadEntity_t *pLoadEntity)
{
	// UNDONE: flag errors by ID, rather than by index
	//if (!_stricmp(szKey, "id"))
	//{
	//	g_MapError.BrushState( atoi( szValue ));
	//}
