...
void LoadMaterialSystemInterface( CreateInterfaceFn fileSystemFactory )
{
	...
	CreateInterfaceFn clientFactory = Sys_GetFactory( "materialsystem" DLL_EXT_STRING );
	if ( clientFactory )
	{
		g_pMaterialSystem = (IMaterialSystem *)clientFactory( MATERIAL_SYSTEM_INTERFACE_VERSION, NULL );
		if ( !g_pMaterialSystem )
		{
			Error( "Could not get the material system interface from materialsystem%s ( %s )", DLL_EXT_STRING, __FILE__ );
		}
	}
	else
	{
