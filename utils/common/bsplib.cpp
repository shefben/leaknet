...
void AddBufferToPack( const char *relativename, void *data, int length, bool bTextMode )
{
	GetPakFile().AddBufferToPack( relativename, data, length, bTextMode );
}
...
//-----------------------------------------------------------------------------
static void UnserializeOcclusionLumpV1( CUtlBuffer &buf )
{
...
static void LoadOcclusionLump()
{
	...
	{
	case 1:
		UnserializeOcclusionLumpV1( buf );
		break;
...
//		ParseEntities ();
    ...
    totalmemory += ArrayUsage( "occluder vert indices",g_OccluderVertexIndices.Count(),0, sizeof( g_OccluderVertexIndices[0] ) );
	...
	Msg( "=== Total BSP file data space used: %d bytes ===\n", totalmemory );
