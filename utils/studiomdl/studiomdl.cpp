...
#if 0
				normal[k][0] += 0.5f * ( 2.0f * ( ( float )rand() ) / ( float )VALVE_RAND_MAX ) - 1.0f;
				normal[k][1] += 0.5f * ( 2.0f * ( ( float )rand() ) / ( float )VALVE_RAND_MAX ) - 1.0f;
				normal[k][2] += 0.5f * ( 2.0f * ( ( float )rand() ) / ( float )VALVE_RAND_MAX ) - 1.0f;
...
void Cmd_ScreenAlign ( void )
{
	if (GetToken (false))
	{
		...
		if( GetToken( false ) )
		{
			...
			else if( !Q_stricmp( "cylinder", token ) )
			{
...
void ParseScript (void)
{
	while (1)
	{
		...
		*/
		...
		else if (!_stricmp( token, "$definebone" ))
		{
			Cmd_DefineBone( );
		}
		...
		else if ( !_stricmp( token, "$skipboneinbbox" ) )
		{
			Cmd_SkipBoneInBBox( );
		}
		else
