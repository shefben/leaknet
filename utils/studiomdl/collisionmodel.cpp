...
int FindLocalBoneNamed( const s_source_t *pSource, const char *pName )
{
	if ( pName )
	{
		...
		{
			if ( !Q_stricmp( pName, pSource->localBone[i].name ) )
				return i;
		}
...
int CJointedModel::BoneIndex( const char *pName )
{
	...
	{
		if ( !Q_stricmp( m_pModel->localBone[boneIndex].name, pName ) )
			return boneIndex;
	}
...
int CJointedModel::CollisionIndex( const char *pName )
{
	...
	while ( pList )
	{
		if ( !Q_stricmp( pName, pList->m_name ) )
			return index;
	...
	while ( swapped )
	{
		...
		for ( i = 0; i < m_collisionCount; i++ )
		{
			...
			for ( j = 0; j < m_collisionCount; j++ )
			{
				...
				if ( !Q_stricmp( pPhys->m_parent, pArray[j]->m_name ) )
					break;
			}
...
CPhysCollisionModel *FindObjectInList( CPhysCollisionModel *pHead, const char *pName )
{
	while ( pHead )
	{
		if ( !Q_stricmp( pName, pHead->m_name ) )
			break;
		pHead = pHead->m_pNext;
	}
...
void BuildRagdollConstraint( CPhysCollisionModel *pPhys, constraint_ragdollparams_t &ragdoll )
{
	...
	while ( pList )
	{
		...
		else if ( (!pListModel->m_parent || g_JointedModel.CollisionIndex(pListModel->m_parent) < 0) && Q_stricmp( pList->m_pJointName, g_JointedModel.m_rootName ) )
		{
