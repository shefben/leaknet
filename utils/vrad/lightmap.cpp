...
void CreateDirectLights (void)
{
	...
	for (i=0 ; i<(unsigned)num_entities ; i++)
	{
		...
		{	// point down angle
			...
			if (angle == ANGLE_UP)
			{
				dl->light.normal[0] = dl->light.normal[1] = 0;
				dl->light.normal[2] = 1;
			}
			...
			else
			{
				...
				dl->light.normal[2] = 0;
				dl->light.normal[0] = (float)cos (angle/180*M_PI);
				dl->light.normal[1] = (float)sin (angle/180*M_PI);
			}
			...
			dl->light.normal[2] = (float)sin(angle/180*M_PI);
			dl->light.normal[0] *= (float)cos(angle/180*M_PI);
			dl->light.normal[1] *= (float)cos(angle/180*M_PI);
		}

		if (!strcmp (name, "light_spot") || !strcmp( name, "light_glspot" ) ) // VXP: light_spot and light_glspot are the same CLight entity
		{
			ParseLightSpot( e, dl );
		}
...
float GatherSampleLight( directlight_t *dl, int facenum, 
	Vector const& pos, Vector const& normal, Vector& delta, 
	float *scale, int iThread )
{
	float			dot, dot2;
	float			dist;
	...
	else if (dl->light.type == emit_skyambient)
	{
		...
		for (j = 0; j < NUMVERTEXNORMALS; j++)
		{
			...
			// search back to see if we can hit a sky brush
			VectorScale( r_avertexnormals[j], -MAX_TRACE_LENGTH, delta );
			VectorAdd( pos, delta, delta );
			...
			ambient_intensity += dot;
		}

		*scale = 1.0;
		return (ambient_intensity / NORMALFORMFACTOR);
	}
/*
	else if (dl->light.type == emit_radient)
	{
		if (dl->texdata == texinfo[dfaces[facenum].texinfo].texdata)
		{
			return 1.0;
		}
		return 0;
	}
	else if (dl->light.type == emit_baselight)
	{
		if (dl->texdata == texinfo[dfaces[facenum].texinfo].texdata)
		{
			return 1.0;
...
*/
	else
	{
		...
		{
#if 0
			// cast onto texture plane, clamp to size, move, resample
			Vector coord;

			// FIXME: this domain is crap.  The results depend on how the light it cut up.
			WorldToCoord( gpLightinfo[dl->facenum], pos, coord );

			coord[0] /= (gpLightinfo[dl->facenum]->texsize[0]) * 16;
			coord[1] /= (gpLightinfo[dl->facenum]->texsize[1]) * 16;

			if (coord[0] < 0.05) coord[0] = 0;
			if (coord[1] < 0.05) coord[1] = 0;
			if (coord[0] > 0.95) coord[0] = 0.95;
			if (coord[1] > 0.95) coord[1] = 0.95;

			coord[0] *= (gpLightinfo[dl->facenum]->texsize[0]) * 16;
			coord[1] *= (gpLightinfo[dl->facenum]->texsize[1]) * 16;

			CoordToWorld( gpLightinfo[dl->facenum], coord[0], coord[1], src );
#endif
			src.Init( 0, 0, 0 );
		}
	...
	for (directlight_t *dl = activelights; dl != NULL; dl = dl->next)
	{
		...
		if( g_pIncremental && (dl->light.style == 0) )
		{
			g_pIncremental->AddLightToFace( dl->m_IncrementalID, info.m_FaceNum, sampleIdx, 
				info.m_LightmapSize, falloff * dot, info.m_iThread );
		}
		...
		{
			dot = DotProduct( info.m_PointNormal[n], delta );
			if (dot > 0)
			{
				VectorMA( pLightmaps[n][sampleIdx], falloff * dot, dl->light.intensity, pLightmaps[n][sampleIdx] );
			}
		}
	}
}
...
static void ResampleLightAtPoint( SampleInfo_t& info, int lightStyleIndex, int flags, Vector* pLightmap )
{
	...
	for (directlight_t *dl = activelights; dl != NULL; dl = dl->next)
	{
		...
		for( int n = 1; n < info.m_NormalCount; ++n)
		{
			...
			if (dot > 0)
			{
				VectorMA( pLightmap[n], falloff * dot, dl->light.intensity, pLightmap[n] );
			}
		}
	}
}
...
//			StaticDispMgr()->SetLightmapWorldSpaceTransformData( facenum, l );
		...
		for (dl = activelights; dl != NULL; dl = dl->next)
		{
			...
			float dot = GatherSampleLight( dl, facenum, spot, pointnormal[0], delta, &falloff, iThread );
			...
			if (dot > 0)
			{
				...
				// The first sample is for non-bumped lighting.
				// The other sample are for bumpmapping.
				float flContribution = falloff * dot;
				...
				{
					dot = DotProduct( pointnormal[n], delta );
					if (dot > 0)
					{
						VectorMA( fl->light[k][n][i], falloff * dot, dl->light.intensity, fl->light[k][n][i] );
					}
				}
			}
		}
		...
		for (i=0 ; i<fl->numsamples ; i++)
		{
			...
			if (gradient[i] > 0.0625)
			{
				...
				for (dl = activelights; dl != NULL; dl = dl->next)
				{
					...
					float dot = GatherSampleLight( dl, facenum, spot2, l.facenormal, delta, &falloff, iThread );

					if (dot > 0)
					{
						// accumulate the ambient light here and multiply by the number of "active" subsamples below!!
						VectorMA( ambientLight, ( falloff * dot ), dl->light.intensity, ambientLight );
					...
					for (t = 0; t < subsample; t++)
					{
						...
						if( PointInWinding( spot2, fl->sample[i].w ) )
						{
							...
							for (dl = activelights; dl != NULL; dl = dl->next)
							{
								...
								if (dot > 0)
								{
									VectorMA( fl->light[0][0][i], falloff * dot, dl->light.intensity, fl->light[0][0][i] );
								}
							}
...
//		if (patch->parent == -1)
	...
	for( patch = &patches.Element( facePatches.Element( facenum ) ); patch; patch = pNextPatch )
	{
		...
		if ( patch->child1 != patches.InvalidIndex() )
		{
			...
			VectorCopy( patch->totallight, patch->directlight );
		}
	}
