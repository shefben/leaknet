...
// VXP: FIXME: Make the usage of this a bit more simple
//-----------------------------------------------------------------------------

static char const* ConstructFileName( char const* pModelName, char *directory )
{
	static char buf[1024];
	sprintf( buf, "%s%s", directory, pModelName );
	return buf;
}
...
bool CVradStaticPropMgr::LoadStudioModelVtx( char const* pModelName, CUtlBuffer& buf )
{
	...
	char const* pFileNameModDir = ConstructFileName( pModelName, gamedir );
	char const *pFileNameBaseGameDir = ConstructFileName( pModelName, basegamedir );

	strcpy( tempFileName, pFileNameModDir );
	...
	// VXP: Continue trying to get the model, but now from the basegamedir (hl2)
	strcpy( tempFileName, pFileNameBaseGameDir );
	pTex = strrchr( tempFileName, '.' );
	assert( pTex );
	strcpy( pTex, ".dx80.vtx" );
...
bool LoadStudioModel( char const* pModelName, CUtlBuffer& buf )
{
	...
	char const* pFileNameModDir = ConstructFileName( pModelName, gamedir );
	char const *pFileNameBaseGameDir = ConstructFileName( pModelName, basegamedir );
	char pFilenameFound[256]; // VXP: Store the correct filename for the next warning messages

	if (!LoadFile( pFileNameModDir, buf ))
	{
		Warning( "Warning! Unable to load model \"%s\"\n", pFileNameModDir );
		if ( !LoadFile( pFileNameBaseGameDir, buf ) )
		{
			Warning( "Error! Unable to load model \"%s\"\n", pFileNameBaseGameDir );
			return false;
		}
		else
		{
			Q_strncpy( pFilenameFound, pFileNameBaseGameDir, 256 );
		}
	}
	else
	{
		Q_strncpy( pFilenameFound, pFileNameModDir, 256 );
	}
...
bool LoadStudioCollisionModel( char const* pModelName, CUtlBuffer& buf )
{
	...
	// No luck, gotta build it	
	// Construct the file name...
	char const* pFileNameModDir = ConstructFileName( tmp, gamedir );
	char const *pFileNameBaseGameDir = ConstructFileName( tmp, basegamedir );
	if (!LoadFile( pFileNameModDir, buf ))
	{
		if ( !LoadFile( pFileNameBaseGameDir, buf ) )
		{
