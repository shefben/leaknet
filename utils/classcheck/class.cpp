...
void CClass::SetBaseClass( const char *name )
{
	...
	else if ( _stricmp( m_szBaseClass, name ) )
	{
...
void CClass::CheckChildOfBaseEntity( const char *baseentityclass )
{
	...
	if ( !_stricmp( m_szName, baseentityclass ) )
	{
	...
	while ( base )
	{
		...
		// Check name
		if ( !_stricmp( base->m_szName, baseentityclass ) )
		{
			m_bDerivedFromCBaseEntity = true;
			return;
		}
...
bool CClass::CheckForMissingTypeDescriptionFields( int& missingcount, bool createtds )
{
	...
	for ( int i = 0; i < m_nVarCount; i++ )
	{
		...
		if ( var->m_bIsArray && 
			(
				_stricmp( var->m_szType, "char" ) ||
				_stricmp( t, "FIELD_STRING" )
			) )
		{
			if ( *t )
			{
				vprint( 0, "\tDEFINE_ARRAY( %s, %s, %s, %s ),\n", m_szName, goodname, t, var->m_szArraySize );
			}
			else
			{
				vprint( 0, "\t// DEFINE_ARRAY( %s, %s, %s, %s ),\n", m_szName, goodname, var->m_szType, var->m_szArraySize );
			}
		}
		else
		{
			if ( *t )
			{
				vprint( 0, "\tDEFINE_FIELD( %s, %s, %s ),\n", m_szName, goodname, t );
			}
			else
			{
				vprint( 0, "\t// DEFINE_FIELD( %s, %s, %s ),\n", m_szName, goodname, var->m_szType );
			}
		}
	}
...
bool CClass::ParseNetworkVar( char *&input, int protection )
{
	...
	if ( !strcmp( com_token, "CNetworkVar" ) || !strcmp( com_token, "CNetworkVarEmbedded" ) )
	{
		input = CC_ParseToken( input );
		Assert( !strcmp( com_token, "(") );
	...
	if ( !strcmp( com_token, "CNetworkVector" ) || !strcmp( com_token, "CNetworkQAngle" ) )
	{
		input = CC_ParseToken( input );
		Assert( !strcmp( com_token, "(") );
	...
	if ( !strcmp( com_token, "CNetworkArray" ) )
	{
		input = CC_ParseToken( input );
		Assert( !strcmp( com_token, "(") );
...
bool CClass::ParseClassMember( char *&input, int protection )
{
	...
	if ( !_stricmp( var.m_pName, "SHARED_CLASSNAME" ) )
	{
		input = CC_ParseToken( input );
	...
	do
	{
		...
		else if ( !_stricmp( com_token, ":" ) && !isfunction )
		{
		...
		{
			if ( !_stricmp( var.m_pName, "typedef" ) ||
				 !_stricmp( var.m_pName, "enum" ) ||
				 !_stricmp( var.m_pName, "friend" ) )
			{
				skipvar = true;
			}
			AppendType( var.m_pName, var.m_pType );
	...
	*/
	...
	else
	{
		...
		do
		{
			...
			else if ( !_stricmp( var.m_pName, "BaseClass" ) )
			{
...
{
	if ( _stricmp( com_token, "struct" ) && _stricmp( com_token, "class" ) )
		return false;
...
bool CClass::ParseProtection( char *&input, int &protection )
{
	...
	else if ( !_stricmp( com_token, "protected" ) )
	{
		protection = 1;
		input = CC_ParseToken( input );
		assert( !_stricmp( com_token, ":" ) );
	...
	else if ( !_stricmp( com_token, "private" ) )
	{
		protection = 2;
		input = CC_ParseToken( input );
		assert( !_stricmp( com_token, ":" ) );
