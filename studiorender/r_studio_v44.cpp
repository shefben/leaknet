...
/*
=================
R_StudioSetupModel_v44
	v44+ version - based on the body part, figure out which mesh it should be using.
inputs:
outputs:
	pstudiomesh
	pmdl
=================
*/
int R_StudioSetupModel_v44( int bodypart, int entity_body, mstudiomodel_v44_t **ppSubModel,
	const studiohdr_v44_t *pStudioHdr )
{
	int index;
	mstudiobodyparts_v44_t   *pbodypart;

	if (bodypart > pStudioHdr->numbodyparts)
	{
		ConDMsg ("R_StudioSetupModel_v44: no such bodypart %d\n", bodypart);
		bodypart = 0;
	}
...
// world. (v44+ version)
//-----------------------------------------------------------------------------
static void ScreenAlignBone_v44( matrix3x4_t *pPoseToWorld, mstudiobone_v44_t *pCurBone,
	const Vector& vecViewOrigin, const matrix3x4_t &boneToWorld )
{
	...
	if( pCurBone->flags & BONE_SCREEN_ALIGN_SPHERE_V44 )
	{
		vX = vecViewOrigin - vT;
		VectorNormalize(vX);
		vZ = Vector(0,0,1);
	...
	{
		Assert( pCurBone->flags & BONE_SCREEN_ALIGN_CYLINDER_V44 );
...
//-----------------------------------------------------------------------------
// Computes PoseToWorld from BoneToWorld (v44+ version)
//-----------------------------------------------------------------------------
void ComputePoseToWorld_v44( matrix3x4_t *pPoseToWorld, studiohdr_v44_t *pStudioHdr, int boneMask, const Vector& vecViewOrigin, const matrix3x4_t *pBoneToWorld )
{
	if ( pStudioHdr->flags & STUDIOHDR_FLAGS_STATIC_PROP_V44 )
	{
	...
	if ( !pStudioHdr->pLinearBones() )
	{
		...
		{
			mstudiobone_v44_t *pCurBone = pStudioHdr->pBone( i );
	...
	{
		mstudiolinearbone_v44_t *pLinearBones = pStudioHdr->pLinearBones();
		...
		for (int i = 0; i < pStudioHdr->numbones; i++)
		{
			...
			// Pretransform (v44+ version)
			if( !( pCurBone->flags & ( BONE_SCREEN_ALIGN_SPHERE_V44 | BONE_SCREEN_ALIGN_CYLINDER_V44 )))
			{
				ConcatTransforms( pBoneToWorld[ i ], pCurBone->poseToBone, pPoseToWorld[ i ] );
			}
			else
			{
				...
				ScreenAlignBone_v44( &pPoseToWorld[i], pCurBone, vecViewOrigin, pBoneToWorld[i] );
			}
#endif
}
