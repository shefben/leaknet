...
void CStudioRender::R_StudioEyeballPosition( const mstudioeyeball_t *peyeball, eyeballstate_t *pstate )
{
	...
	if (1)
	{
		...
		// VXP: NOTE: Also can do asin( clamp( peyeball->uppertarget[0] / peyeball->radius, -1.0f, 1.0f ) );,
		// but this is prettier and hopefully faster (due to division is calculating only one time)
...
{
	// v44+ safety: Check if eyeball data is valid
	if (!pState || !pState->peyeball)
	{
		// Set identity-like matrix to prevent rendering issues
...
void R_MouthLighting( int count, const Vector *psrcverts, const Vector *psrcnorms, Vector4D *pdestlightvalues )
{
	...
	if (StudioHdr_GetNumMouths(m_pStudioHdr) < 1) return;

	mstudiomouth_t *pMouth = StudioHdr_GetMouth(m_pStudioHdr, 0); // FIXME: this needs to get the mouth index from the shader
