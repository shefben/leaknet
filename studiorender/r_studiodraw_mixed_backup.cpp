...
#include <malloc.h>
// v44+ independent model system includes
#include "../engine/modelloader_v44.h"  // For model_v44_t structure
#include <xmmintrin.h>
...
void CStudioRender::R_StudioAbsBB ( int sequence, Vector& origin )
{
	...
	// Use version-safe seqdesc accessors for v44+ (baseptr shifts all fields by 4 bytes)
	const Vector& bbmin = StudioSeqdesc_GetBBMin(m_pStudioHdr, sequence);
	const Vector& bbmax = StudioSeqdesc_GetBBMax(m_pStudioHdr, sequence);

	m_pMaterialSystem->Bind( m_pMaterialAdditiveVertexColorVertexAlpha );

	for (j = 0; j < 8; j++)
	{
		p[j][0] = (j & 1) ? bbmin[0] : bbmax[0];
		p[j][1] = (j & 2) ? bbmin[1] : bbmax[1];
		p[j][2] = (j & 4) ? bbmin[2] : bbmax[2];
...
//	float		lv;
	...
	int numBones = StudioHdr_GetNumBones(m_pStudioHdr);
...
// short bone[4] (16-bit indices) instead of char bone[3] (8-bit indices)
//-----------------------------------------------------------------------------
...
// Note: v37 uses short bone[4] which has different memory layout than char bone[3]
// For simplicity, we use the non-SSE C implementation since the bone index
// layout is different and would require separate SSE assembly paths.
//-----------------------------------------------------------------------------
...
{
	// Use the non-SSE C implementation - bone indices are short[] not char[] so
	// the SSE assembly paths would need to be rewritten for v37
...
public:
	...
	{
		Vector color;
		Vector4D *pStudioTangentS = NULL;  // MUST initialize to NULL
		...
		// =========================================================================
		// v37 MODEL RENDERING PATH - uses 64-byte mstudiovertex_v37_t stride
		// This is COMPLETELY SEPARATE from v44+ to avoid stride issues
		// =========================================================================
		if (pStudioHdr->IsV37())
		{
			// Get v37 vertex data with proper 64-byte stride
			mstudiovertex_v37_t *pVertices_v37 = pmesh->Vertex_v37(0);
			if (!pVertices_v37)
			{
				DevWarning("CRITICAL: v37 pVertices is NULL for %s, mesh will be invisible!\n", pStudioHdr->name);
				return;
			}

			// Tangent data for v37 (stored separately, uses index offset)
			if (nHasTangentSpace)
			{
				pStudioTangentS = pmesh->TangentS(0);
			}

			// Precache v37 data with correct 64-byte stride
			for ( int i = 0; i < 4; ++i )
			{
				ntemp[i] = pGroupToMesh[i];
				if (nHasSSE)
				{
					char *pMem = (char*)&pVertices_v37[ntemp[i]];
			...
			for ( int j=0; j < numVertices; ++j )
			{
				...
				// v37: Use mstudiovertex_v37_t with 64-byte stride
				mstudiovertex_v37_t &vert = pVertices_v37[n];

				ntemp[idx] = pGroupToMesh[j+4];

				// v37: Use ComputeSkinMatrix_v37 with mstudioboneweight_v37_t (32 bytes, short bone[4])
				if (nHasSSE)
				{
				...
				if (nHasSSE)
				{
					_mm_prefetch( (char*)&pVertices_v37[ntemp[idx]], _MM_HINT_T0 );
					_mm_prefetch( (char*)&pVertices_v37[ntemp[idx]] + 32, _MM_HINT_T0 );
					if (nHasTangentSpace && pStudioTangentS)
					{
		...
		// =========================================================================
		// v44+ MODEL RENDERING PATH - uses 48-byte mstudiovertex_t stride
		// =========================================================================
		else
		{
			// Get v44+ vertex data using SetupVertexDataForMesh
			...
			if (pVertices == NULL)
			{
				DevWarning("CRITICAL: v44+ pVertices is NULL for %s (v%d), mesh will be invisible!\n", pStudioHdr->name, pStudioHdr->version);
				return;
			}
			...
			// Precache v44+ data with correct 48-byte stride
			for ( int i = 0; i < 4; ++i )
			{
				ntemp[i] = pGroupToMesh[i];
				if (nHasSSE)
			...
			for ( int j=0; j < numVertices; ++j )
			{
				...
				// v44+: Use mstudiovertex_t with 48-byte stride
				mstudiovertex_t &vert = pVertices[n];

				ntemp[idx] = pGroupToMesh[j+4];

				// v44+: Use ComputeSkinMatrix with mstudioboneweight_t (16 bytes, char bone[3])
				if (nHasSSE)
				{
		...
		bool bNeedsTangentSpace )
{
	...
	Vector *pSrcNorm;
	Vector4D *pSrcTangentS = NULL;
	Vector4D *pStudioTangentS = NULL;  // MUST initialize to NULL - used in v44+ paths
	VectorAligned norm, pos;
	Vector4DAligned tangentS;
	...
	else if (m_pStudioHdr->version >= STUDIO_VERSION_44)
	{
		...
		if (pModel44)
		{
			...
			{
				// CRITICAL FIX: Add model's base vertex offset to global VVD buffer
				// pVertexData points to global VVD base, vertexindex is model's byte offset
				int modelVertexBase = pModel44->vertexindex / sizeof(mstudiovertex_t);
				pVertices = (mstudiovertex_t*)pModel44->vertexdata.pVertexData + modelVertexBase + pmesh->vertexoffset;
			}
			else
			{
			...
			{
				// CRITICAL FIX: Add model's base tangent offset to global VVD buffer
				int modelTangentBase = pModel44->tangentsindex / sizeof(Vector4D);
				pStudioTangentS = (Vector4D*)pModel44->vertexdata.pTangentData + modelTangentBase + pmesh->vertexoffset;
				if (pStudioTangentS) Assert( pStudioTangentS->w == -1.0f || pStudioTangentS->w == 1.0f );
			}
		}
		else
	...
	StudioModelLighting_t lighting, IMaterial *pMaterial )
{
	...
	// v44+ safety: Check if eyeball is valid before proceeding
	if (!peyeball)
	{
		DevWarning("R_StudioDrawEyeball: NULL eyeball for materialParam %d in %s\n", materialParam, m_pStudioHdr->name);
		return 0;
	}
...
//	MEASURECODE( "R_StudioDrawPoints" );
	...
	int numBonesForMaterial = StudioHdr_GetNumBones(m_pStudioHdr);
	for ( i = 0; i < numBonesForMaterial; i++)
	{
...
//	int numMeshes = SortMeshes( pIndices, ppMaterials, pskinref, vforward, r_origin );
	...
	//-----------------------------------------------------------------------------
	// DETECT v44+ MODELS FROM INDEPENDENT SYSTEM
	// Check if this is a v44+ model and route to independent rendering system
	//-----------------------------------------------------------------------------
	if (m_pCurrentV44Model != NULL)
	{
		// Use completely independent v44+ rendering
		Con_DPrintf("Routing v44+ model to independent rendering system: %s\n", m_pCurrentV44Model->name);
		return R_StudioDrawPoints_v44(m_pCurrentV44Model, skin, pClientEntity, ppMaterials, pMaterialFlags, ppColorMeshes);
	}

	//-----------------------------------------------------------------------------
	// ORIGINAL v37 RENDERING PATH (unchanged)
	//-----------------------------------------------------------------------------
	// draw each mesh - all remaining models are v37 since v44+ returned early
	...
	for ( i = 0; i < numMeshes; ++i)
	{
		...
		if( !pMaterial )
			...
			// Handle v37 mesh rendering (all models here are v37 since v44+ returned early)
			switch (pmesh->material) // material type for v37
			{
			case 1:	// eyeballs
				if( m_bDrawTranslucentSubModels )
					break;
...
//-----------------------------------------------------------------------------
// COMPLETELY INDEPENDENT v44+ MODEL RENDERING SYSTEM
// Renders v44+ models using the independent model system
//-----------------------------------------------------------------------------

int CStudioRender::R_StudioDrawPoints_v44( model_v44_t *pModel_v44, int skin, void *pClientEntity,
	IMaterial **ppMaterials, int *pMaterialFlags, IMesh **ppColorMeshes )
{
	if (!pModel_v44 || !pModel_v44->pStudioHdr)
	{
		Warning("R_StudioDrawPoints_v44: NULL model or studio header\n");
		return 0;
	}

	int numTrianglesRendered = 0;
	studiohdr_v44_t *pStudioHdr_v44 = pModel_v44->pStudioHdr;

	// Get skin reference array
	short *pskinref = pStudioHdr_v44->pSkinref(0);
	if (skin > 0 && skin < pStudioHdr_v44->numskinfamilies)
	{
		pskinref += (skin * pStudioHdr_v44->numskinref);
	}

	// v44+ models use the same hardware mesh data as v37 (m_pStudioMeshes is already set up)
	// The studiomeshdata_t was created by R_StudioCreateStaticMeshes from VTX data
	// and contains the hardware mesh buffers ready for rendering.

	// Iterate through body parts
	for (int bodyPartId = 0; bodyPartId < pStudioHdr_v44->numbodyparts; bodyPartId++)
	{
		mstudiobodyparts_t *pBodyPart = pStudioHdr_v44->pBodypart(bodyPartId);
		if (!pBodyPart)
			continue;

		// Iterate through models in this body part
		for (int modelId = 0; modelId < pBodyPart->nummodels; modelId++)
		{
			mstudiomodel_v44_t *pStudioModel = pBodyPart->pModel_v44(modelId);
			if (!pStudioModel)
				continue;

			// Iterate through meshes in this model
			for (int meshId = 0; meshId < pStudioModel->nummeshes; meshId++)
			{
				mstudiomesh_v44_t *pMesh_v44 = pStudioModel->pMesh(meshId);
				if (!pMesh_v44)
					continue;

				// Get the pre-built studiomeshdata using the mesh's unique ID
				// This was created by R_StudioCreateStaticMeshes from VTX data
				studiomeshdata_t *pMeshData = &m_pStudioMeshes[pMesh_v44->meshid];
				if (!pMeshData)
					continue;

				// Get material for this mesh
				IMaterial* pMaterial = R_StudioSetupSkin(pskinref[pMesh_v44->material], ppMaterials, pClientEntity);
				if (!pMaterial)
					continue;

				// Check material flags
				int materialFlags = (!m_pForcedMaterial) ? pMaterialFlags[pskinref[pMesh_v44->material]] : 0;
				bool translucent = pMaterial->IsTranslucent();
				...
				// Set up flex data
				m_VertexCache.SetMesh( meshId );

				// Render the mesh using the hardware mesh data
				// The studiomeshdata contains the pre-built vertex/index buffers from VTX
				numTrianglesRendered += R_StudioDrawMesh_v44( pMesh_v44, pMeshData, lighting, pMaterial, ppColorMeshes );
			}
		}
	}
...
// Simple v44+ mesh renderer for testing
int CStudioRender::R_StudioDrawMesh_v44_Simple( mstudiomesh_v44_t *pMesh_v44, IMaterial *pMaterial, StudioModelLighting_t lighting )
{
	if (!pMesh_v44)
		return 0;

	// Get the model this mesh belongs to
	mstudiomodel_v44_t *pModel_v44 = pMesh_v44->pModel();
	if (!pModel_v44)
	{
		Warning("R_StudioDrawMesh_v44_Simple: Failed to get model from mesh\n");
		return 0;
	}

	// Verify vertex data access
	if (!pModel_v44->vertexdata.pVertexData)
	{
		Warning("R_StudioDrawMesh_v44_Simple: No vertex data in model\n");
		return 0;
	}

	// Get vertex data for this mesh
	// CRITICAL FIX: Add model's base vertex offset to global VVD buffer
	int modelVertexBase = pModel_v44->vertexindex / sizeof(mstudiovertex_t);
	mstudiovertex_t *pVertices = (mstudiovertex_t*)pModel_v44->vertexdata.pVertexData + modelVertexBase + pMesh_v44->vertexoffset;

	Con_DPrintf("R_StudioDrawMesh_v44_Simple: Mesh has %d vertices at offset %d (model base: %d)\n",
		pMesh_v44->numvertices, pMesh_v44->vertexoffset, modelVertexBase);

	// For now, just return success - this validates that vertex data access works
	// TODO: Implement actual mesh rendering when VTX data is loaded
	return pMesh_v44->numvertices / 3; // Estimate triangles
}

//-----------------------------------------------------------------------------
// v44+ Model Rendering Functions
// Separate rendering pipeline for v44+ models to avoid casting issues
//-----------------------------------------------------------------------------

int CStudioRender::R_StudioDrawMesh_v44( mstudiomesh_v44_t* pmesh_v44, studiomeshdata_t* pMeshData,
									 StudioModelLighting_t lighting, IMaterial *pMaterial,
									 IMesh **ppColorMeshes )
{
	MEASURE_TIMED_STAT( STUDIO_STATS_DRAW_MESH );
	...
	for ( int j = 0; j < pMeshData->m_NumGroup; ++j )
	{
			 ...
			 !m_Config.bNormals )
		{
			if (!m_Config.bNoHardware)
			{
				numTrianglesRendered += R_StudioDrawStaticMesh_v44( pmesh_v44, pGroup, lighting, m_AlphaMod, pMaterial, ppColorMeshes );
			}
		}
		else
		{
			if (!m_Config.bNoSoftware)
			{
				numTrianglesRendered += R_StudioDrawDynamicMesh_v44( pmesh_v44, pGroup, lighting, m_AlphaMod, pMaterial );
			}
		}
	}
...
int CStudioRender::R_StudioDrawEyeball_v44( mstudiomesh_v44_t* pmesh_v44, studiomeshdata_t* pMeshData,
									    StudioModelLighting_t lighting, IMaterial *pMaterial )
{
	// For now, use the regular eyeball rendering
	// TODO: Implement v44+-specific eyeball rendering if needed
	return R_StudioDrawEyeball( (mstudiomesh_t*)pmesh_v44, pMeshData, lighting, pMaterial );
}

int CStudioRender::R_StudioDrawStaticMesh_v44( mstudiomesh_v44_t* pmesh_v44,
											  studiomeshgroup_t* pGroup, StudioModelLighting_t lighting,
											  float r_blend, IMaterial* pMaterial, IMesh **ppColorMeshes )
{
	// For now, delegate to regular static mesh rendering
	// The key difference is that we have proper v44+ mesh pointer type
	return R_StudioDrawStaticMesh( (mstudiomesh_t*)pmesh_v44, pGroup, lighting, r_blend, pMaterial, ppColorMeshes );
}

int CStudioRender::R_StudioDrawDynamicMesh_v44( mstudiomesh_v44_t* pmesh_v44,
											   studiomeshgroup_t* pGroup, StudioModelLighting_t lighting,
											   float r_blend, IMaterial* pMaterial )
{
	int numTrianglesRendered = 0;
	...
	if (!doFlex && !swSkin && !m_Config.bNormals )
	{
		return R_StudioDrawStaticMesh_v44( pmesh_v44, pGroup, lighting, r_blend, pMaterial, NULL );
	}
	...
	if (doFlex)
	{
		R_StudioFlexVerts_v44( pmesh_v44 );
	}

	// Use the hardware if we're not flexing and don't need to do software
	if ( !doSoftwareLighting && !doFlex && !swSkin )
	{
		numTrianglesRendered = R_StudioDrawGroupHWSkin( pGroup, m_pMaterialSystem->GetDynamicMesh( true ), NULL );
	}
	else
	{
		numTrianglesRendered = R_StudioDrawGroupSWSkin( pGroup, m_pMaterialSystem->GetDynamicMesh( false ) );
	}
	...
	if( m_Config.bNormals )
	{
		...
		CMeshBuilder meshBuilder;
		IMesh* pMesh = m_pMaterialSystem->GetDynamicMesh( false );
		meshBuilder.Begin( pMesh, MATERIAL_LINES, pGroup->m_NumVertices );

		R_StudioSoftwareProcessMesh_Normals_v44( pmesh_v44, meshBuilder, pGroup->m_NumVertices,
			pGroup->m_pGroupIndexToMeshIndex, lighting, doFlex, r_blend, false );
		meshBuilder.End( );
...
// v44+ version of flex vertex processing
void CStudioRender::R_StudioFlexVerts_v44( mstudiomesh_v44_t* pmesh_v44 )
{
	// For now, delegate to regular flex processing
	// TODO: Implement v44+-specific flex processing if needed
	R_StudioFlexVerts( (mstudiomesh_t*)pmesh_v44 );
}

// v44+ version of software mesh processing
void CStudioRender::R_StudioSoftwareProcessMesh_Normals_v44( mstudiomesh_v44_t* pmesh_v44, CMeshBuilder& meshBuilder,
		int numVertices, unsigned short* pGroupToMesh, StudioModelLighting_t lighting, bool doFlex, float r_blend,
		bool bNeedsTangentSpace )
{
	// This is where we properly handle v44+ vertex data access
	// Use the proper v44+ vertex data access methods

	// Get the model's vertex data using proper v44+ accessors
	mstudiomodel_v44_t* pModel44 = pmesh_v44->pModel();
	if (!pModel44 || !pModel44->vertexdata.pVertexData)
	{
		DevWarning("v44+ model has NULL vertex data in normals processing: %s\n", m_pStudioHdr->name);
		return;
	}

	// Access vertex data through the proper v44+ vertex data structure
	// CRITICAL FIX: Add model's base vertex offset to global VVD buffer
	int modelVertexBase = pModel44->vertexindex / sizeof(mstudiovertex_t);
	mstudiovertex_t *pVertices = (mstudiovertex_t*)pModel44->vertexdata.pVertexData + modelVertexBase + pmesh_v44->vertexoffset;
	Vector4D *pStudioTangentS = NULL;
	if (pModel44->vertexdata.pTangentData)
	{
		// CRITICAL FIX: Add model's base tangent offset to global VVD buffer
		int modelTangentBase = pModel44->tangentsindex / sizeof(Vector4D);
		pStudioTangentS = (Vector4D*)pModel44->vertexdata.pTangentData + modelTangentBase + pmesh_v44->vertexoffset;
	}

	// Continue with regular processing now that we have proper vertex pointers
	if (pVertices && pStudioTangentS)
	{
		// Process the vertices for normal visualization
		for (int i = 0; i < numVertices; ++i)
		{
			int vertexIndex = pGroupToMesh[i];
			mstudiovertex_t* pVertex = &pVertices[vertexIndex];

			meshBuilder.Position3fv( pVertex->m_vecPosition.Base() );
			meshBuilder.AdvanceVertex();

			Vector normalEnd = pVertex->m_vecPosition + pVertex->m_vecNormal * 10.0f;
			meshBuilder.Position3fv( normalEnd.Base() );
			meshBuilder.AdvanceVertex();
		}
	}
}
