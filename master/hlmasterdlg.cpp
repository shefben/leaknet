...
#define UPDATE_STATS_TIME 15.0f
...
void CHLMasterDlg::Peer_GetHeartbeat2( void )
{
	...
	if ( !islan && !m_bAllowOldProtocols && protocol != PROTOCOL_VERSION ) 
	{
		return;
	}
...
int CHLMasterDlg::RunModalLoop(DWORD dwFlags)
{
	...
	for (;;)
	{
		...
		if ( (float)(tCurrent - tLast)/CLOCKS_PER_SEC >= UPDATE_STATS_TIME )
		{
			tLast = tCurrent;
			UpdateCount();
		}
...
BOOL CHLMasterDlg::OnInitDialog()
{
	...
	sprintf(szText, "HLMaster %s - protocol v%i", AdrToString(net_local_adr), PROTOCOL_VERSION);
...
void CHLMasterDlg::Packet_Heartbeat2 (void)
{
	...
	//UTIL_VPrintf("protocol: %i challenge: %i num: %i maxplayers: %i bots: %i dedicated: %i password: %i secure: %i, gamedir: %s, map: %s, os: %s, lan: %i, proxy: %i, isproxytarget: %i, version: %s\n", protocol, challenge, num, maxplayers, bots, dedicated, password, secure, gamedir, map, os, islan, isproxy, isproxytarget, version);

	// protocol != 1 for Sony stand-alone game support...1.1.1.0 engine license (EricS)
	if ( !m_bAllowOldProtocols && ( protocol != PROTOCOL_VERSION ) && ( protocol != 1 ) )
	{
		RejectConnection(&packet_from, "Outdated protocol.");
...
void CHLMasterDlg::PacketCommand (void)
{
	...
	{
	case S2M_HEARTBEAT2:
		UTIL_Printf("Heatbeat2 from %s\r\n", AdrToString(packet_from));
		Packet_Heartbeat2 ();
		break;
	case S2M_SHUTDOWN:
		UTIL_Printf("Shutdown from %s\r\n", AdrToString(packet_from));
		Packet_Shutdown ();
		break;
	case A2M_GET_SERVERS:
		UTIL_Printf("GetServers from %s\r\n", AdrToString(packet_from));
		Packet_GetServers ();
		break;	
	case A2M_GET_SERVERS_BATCH:
		UTIL_Printf("GetServers_batch from %s\r\n", AdrToString(packet_from));
		Packet_GetBatch ();
		break;	
	case A2M_GET_SERVERS_BATCH2:
		UTIL_Printf("GetServers_batch2 from %s\r\n", AdrToString(packet_from));
		Packet_GetBatch2 ();
		break;
	// tyabus: i don't think we need this
	/*case A2M_GETMASTERSERVERS:
		//UTIL_Printf("GetServerMasterServers from %s\r\n", AdrToString(packet_from));
		//Packet_GetMasterServers ();
		break;*/
	case A2A_PING:
		UTIL_Printf("Ping from %s\r\n", AdrToString(packet_from));
		Packet_Ping ();
		break;	
	case A2M_GET_MOTD:
		UTIL_Printf("Motd from %s\r\n", AdrToString(packet_from));
		Packet_Motd ();
		break;	
	case A2A_GETCHALLENGE:
		UTIL_Printf("GetChallenge from %s\r\n", AdrToString(packet_from));
		Packet_GetChallenge();
		break;
	case A2A_NACK:
		UTIL_Printf("NACK from %s\r\n", AdrToString(packet_from));
		break;
	case A2M_GETACTIVEMODS:
		UTIL_Printf("GetActiveMods from %s\r\n", AdrToString(packet_from));
		Packet_GetModBatch();
		break;
	case A2M_GETACTIVEMODS2:
		UTIL_Printf("GetActiveMods2 from %s\r\n", AdrToString(packet_from));
		Packet_GetModBatch2();
		break;
	case A2M_GETACTIVEMODS3:
		UTIL_Printf("GetActiveMods3 from %s\r\n", AdrToString(packet_from));
		Packet_GetModBatch3();
		break;
	case M2M_MSG:
		UTIL_Printf("MSG from %s\r\n", AdrToString(packet_from));
...
{
	UpdateData(TRUE);
	if (!m_bShowTraffic || m_bShowPackets)
		return;
...
void CHLMasterDlg::GenerateFakeServers( void )
{
	...
	for ( i = 0; i < NUM_FAKE; i++ )
	{
		...
		adr.sin_addr.S_un.S_un_b.s_b1 = (unsigned char)( rand() & 254 );
