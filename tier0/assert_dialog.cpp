...
{
	static bool bResult = ( strstr( Plat_GetCommandLine(), "-debugbreak" )    != NULL ) || \
	                      ( strstr( Plat_GetCommandLine(), "-raiseonassert" ) != NULL ) || \
...
{
	static bool bResult = ( strstr( Plat_GetCommandLine(), "-noassert" ) != NULL );
	return bResult;
}
...
CAssertDisable* CreateNewAssertDisable( const char *pFilename )
{
	...
	strncpy( pDisable->m_Filename, g_Info.m_pFilename, sizeof( pDisable->m_Filename ) - 1 );
...
{
	static bool bMPIWorker = ( strstr( Plat_GetCommandLine(), "-mpi_worker" ) != NULL );
	if ( bMPIWorker )
	{
		return false;
	}
...
DBG_INTERFACE bool DoNewAssertDialog( const char *pFilename, int line, const char *pExpression )
{
	//LOCAL_THREAD_LOCK();
