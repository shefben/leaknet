...
//-----------------------------------------------------------------------------
void LoadHudTextures( CUtlDict< CHudTexture *, int >& list, const char *psz )
{
...
void CHud::Init( void )
{
	...
	LoadHudTextures( textureList, "scripts/hud_textures.txt" );
	LoadHudTextures( textureList, "scripts/mod_textures.txt" );
...
void CHudTexture::DrawSelfCropped( int x, int y, int cropx, int cropy, int cropw, int croph, const Color& clr ) const
{
	if ( textureId == -1 )
		return;
...
CHudElement *CHud::FindElement( const char *pName )
{
	for ( int i = 0; i < m_HudList.Size(); i++ )
	{
		if ( Q_stricmp( m_HudList[i]->GetName(), pName ) == 0 )
			return m_HudList[i];
	}
