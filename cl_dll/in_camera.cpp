...
void CInput::CAM_Think( void )
{
	...
	if (m_fCameraMovingWithMouse)
	{
		...
		if (!m_fCameraDistanceMove)
		{
			...
			if (m_nCameraX>engine->GetWindowCenterX())
			{
				...
				{
					camAngles[ YAW ] += (CAM_ANGLE_MOVE)*((m_nCameraX-engine->GetWindowCenterX())/2);
				}
				if (camAngles[YAW]>c_maxyaw.GetFloat())
				{
			...
			else if (m_nCameraX<engine->GetWindowCenterX())
			{
				...
				{
					camAngles[ YAW ] -= (CAM_ANGLE_MOVE)* ((engine->GetWindowCenterX()-m_nCameraX)/2);
			...
			if (m_nCameraY > engine->GetWindowCenterY())
			{
				...
				{
					camAngles[PITCH] +=(CAM_ANGLE_MOVE)* ((m_nCameraY-engine->GetWindowCenterY())/2);
				}
				if (camAngles[PITCH]>c_maxpitch.GetFloat())
				{
			...
			else if (m_nCameraY<engine->GetWindowCenterY())
			{
				...
				{
					camAngles[PITCH] -= (CAM_ANGLE_MOVE)*((engine->GetWindowCenterY()-m_nCameraY)/2);
				}
				if (camAngles[PITCH]<c_minpitch.GetFloat())
				{
	...
	if (m_fCameraDistanceMove)
	{
		if (m_nCameraY>engine->GetWindowCenterY())
		{
			...
			{
				dist +=CAM_DIST_DELTA * ((m_nCameraY-engine->GetWindowCenterY())/2);
			}
			if (dist>c_maxdistance.GetFloat())
			{
		...
		else if (m_nCameraY<engine->GetWindowCenterY())
		{
			...
			{
				dist -= (CAM_DIST_DELTA)*((engine->GetWindowCenterY()-m_nCameraY)/2);
			}
			if (dist<c_mindistance.GetFloat())
			{
