...
void FormatTimeModeString(TimeMode mode, const CCycleCount &elapsed, char *str)
{
	switch(mode)
	{
		case Cycles:
		{
			sprintf(str, "%lu cycles", elapsed.GetCycles());
		}
		break;

		case Microseconds:
		{
			sprintf(str, "%.1fu", elapsed.GetMicrosecondsF());
		}
		break;

		case Milliseconds:
		{
			sprintf(str, "%.3fms", elapsed.GetMillisecondsF());
		}
		break;
		
		case Seconds:
		{
			sprintf(str, "%.4f seconds", elapsed.GetSeconds());
		}
		break;

		case Frametime:
		{
			// Put two %%'s because this string gets sent to sprintf again.
			sprintf(str, "%.3f%%%%", elapsed.GetSeconds() * 100.0 / gpGlobals->frametime);
		}
		break;

		default:
		{
			sprintf(str, "<INVALID TIMEMODE>");
		}
		break;
	}
}
