...
void CHudDeathNotice::MsgFunc_DeathMsg( const char *pszName, int iSize, void *pbuf )
{
	...
	const char *killer_name = g_PR->Get_Name( killer );
	const char *victim_name = g_PR->Get_Name( victim );
	if ( !killer_name )
		killer_name = "";
	if ( !victim_name )
		victim_name = "";
...
if ( strstr( victim_name, "Robin" ) )
{
	...
	if ( killedwith[0] && ( killedwith[0] > 13 ) && Q_strcmp( killedwith, "d_world" ) && !rgDeathNoticeList[i].iTeamKill )
	{
		Msg( " with " );
