...
#define WIN_MSG_BREAK_ONE_PANEL		0
#define WIN_MSG_BREAK_ALL_PANELS	1
#define WIN_MSG_BREAK_UPDATE_SIZE	2
#define WIN_MSG_BREAK_UPDATE_EDGES	3

#define MAX_PANEL_WIDTH 16
...
CLIENTEFFECT_REGISTER_BEGIN( PrecacheEffectBreakableSurface )
CLIENTEFFECT_MATERIAL( "models/brokenglass/glassbroken_solid" )
CLIENTEFFECT_MATERIAL( "models/brokentile/tilebroken_01a" )
CLIENTEFFECT_MATERIAL( "models/brokentile/tilebroken_01b" )
CLIENTEFFECT_MATERIAL( "models/brokentile/tilebroken_01c" )
CLIENTEFFECT_MATERIAL( "models/brokentile/tilebroken_01d" )
CLIENTEFFECT_MATERIAL( "models/brokentile/tilebroken_02a" )
CLIENTEFFECT_MATERIAL( "models/brokentile/tilebroken_02b" )
CLIENTEFFECT_MATERIAL( "models/brokentile/tilebroken_02c" )
CLIENTEFFECT_MATERIAL( "models/brokentile/tilebroken_02d" )
CLIENTEFFECT_MATERIAL( "models/brokentile/tilebroken_03a" )
CLIENTEFFECT_MATERIAL( "models/brokentile/tilebroken_03b" )
CLIENTEFFECT_MATERIAL( "models/brokentile/tilebroken_03c" )
CLIENTEFFECT_MATERIAL( "models/brokentile/tilebroken_03d" )
CLIENTEFFECT_MATERIAL( "models/brokenglass/glassbroken_01a" )
CLIENTEFFECT_MATERIAL( "models/brokenglass/glassbroken_01b" )
CLIENTEFFECT_MATERIAL( "models/brokenglass/glassbroken_01c" )
CLIENTEFFECT_MATERIAL( "models/brokenglass/glassbroken_01d" )
CLIENTEFFECT_MATERIAL( "models/brokenglass/glassbroken_02a" )
CLIENTEFFECT_MATERIAL( "models/brokenglass/glassbroken_02b" )
CLIENTEFFECT_MATERIAL( "models/brokenglass/glassbroken_02c" )
CLIENTEFFECT_MATERIAL( "models/brokenglass/glassbroken_02d" )
CLIENTEFFECT_MATERIAL( "models/brokenglass/glassbroken_03a" )
CLIENTEFFECT_MATERIAL( "models/brokenglass/glassbroken_03b" )
CLIENTEFFECT_MATERIAL( "models/brokenglass/glassbroken_03c" )
CLIENTEFFECT_MATERIAL( "models/brokenglass/glassbroken_03d" )
...
private:
	...
	byte		m_nPanelBits[MAX_PANEL_WIDTH][MAX_PANEL_WIDTH];	//UNDONE: allocate this dynamically?
	CMaterialReference	m_pMaterialBox;	
	EdgeTexture_t	m_pSolid;
	EdgeTexture_t	m_pEdge[NUM_EDGE_TYPES][NUM_EDGE_STYLES];

	inline bool InLegalRange(int nWidth, int nHeight)				{ return (nWidth < m_nNumWide && nHeight < m_nNumHigh && nWidth >=0 && nHeight >= 0); }
	inline bool	IsPanelSolid(int nWidth, int nHeight)				{ return (BITS_PANEL_IS_SOLID & m_nPanelBits[nWidth][nHeight])!=0; }
	inline bool	IsPanelStale(int nWidth, int nHeight)				{ return (BITS_PANEL_IS_STALE & m_nPanelBits[nWidth][nHeight])!=0; }
	inline void	SetPanelSolid(int nWidth, int nHeight, bool value)	{ if (InLegalRange(nWidth,nHeight)) (value ? (m_nPanelBits[nWidth][nHeight] |= BITS_PANEL_IS_SOLID) : (m_nPanelBits[nWidth][nHeight] &= ~BITS_PANEL_IS_SOLID)); }
	inline void	SetPanelStale(int nWidth, int nHeight, bool value)	{ if (InLegalRange(nWidth,nHeight)) (value ? (m_nPanelBits[nWidth][nHeight] |= BITS_PANEL_IS_STALE) : (m_nPanelBits[nWidth][nHeight] &= ~BITS_PANEL_IS_STALE)); }
...
BEGIN_DATADESC( C_BreakableSurface )
	...
	DEFINE_ARRAY( C_BreakableSurface, m_nPanelBits, FIELD_CHARACTER, MAX_PANEL_WIDTH * MAX_PANEL_WIDTH ),
...
// Output  :
//------------------------------------------------------------------------------
void C_BreakableSurface::ReceiveMessage( const char *msgname, int length, void *data )
{
	BEGIN_READ( data, length );

	byte nMsg		= READ_BYTE();

	if (nMsg == WIN_MSG_BREAK_UPDATE_SIZE)
	{
		m_nNumWide		= READ_BYTE();
		m_nNumHigh		= READ_BYTE();
	}
	else if (nMsg == WIN_MSG_BREAK_ALL_PANELS)
	{
		int width;
		for (width=0;width<m_nNumWide;width++)
		{
	...
	else if (nMsg == WIN_MSG_BREAK_ONE_PANEL)
	{
		int nWidth		= READ_BYTE();
		int nHeight		= READ_BYTE();

		Assert(InLegalRange(nWidth,nHeight));

		SetPanelSolid(nWidth,nHeight,false);
		...
		SetPanelStale(nWidth,	nHeight  ,true);
		SetPanelStale(nWidth,   nHeight+1,true);
		SetPanelStale(nWidth,   nHeight-1,true);
		SetPanelStale(nWidth-1, nHeight  ,true);
		SetPanelStale(nWidth+1, nHeight  ,true);
		SetPanelStale(nWidth+1, nHeight+1,true);
		SetPanelStale(nWidth-1, nHeight+1,true);
		SetPanelStale(nWidth+1, nHeight-1,true);
		SetPanelStale(nWidth-1, nHeight-1,true);
	}
	else if (nMsg == WIN_MSG_BREAK_UPDATE_EDGES)
	{
...
void C_BreakableSurface::FindCrackedMaterial()
{
	...
	if (foundVar)
	{
		m_pCrackedMaterial.Init( pCrackName->GetStringValue() );
	}
	else
	{
...
void C_BreakableSurface::InitMaterial(WinEdge_t nEdgeType, int nEdgeStyle, char const* pMaterialName)
{
	...
	m_pEdge[nEdgeType][nEdgeStyle].m_pMaterialEdge.Init(pMaterialName);
...
void C_BreakableSurface::OnPreDataChanged( DataUpdateType_t updateType )
{
	...
	{
		// Initialize panels
		m_nNumWide = MAX_PANEL_WIDTH;
		m_nNumHigh = MAX_PANEL_WIDTH;
		for (int w=0;w<MAX_PANEL_WIDTH;w++)
		{ 
			for (int h=0;h<MAX_PANEL_WIDTH;h++)
			{
...
void C_BreakableSurface::OnDataChanged( DataUpdateType_t updateType )
{
	...
	if ( updateType == DATA_UPDATE_CREATED )
	{
		...
		m_pMaterialBox.Init( "models/brokenglass/glassbroken_solid" );
...
//-----------------------------------------------------------------------------
// returns true if it should be drawn along with the rest of the world.

//-----------------------------------------------------------------------------
bool C_BreakableSurface::IsIdentityBrush( void )
{
	// Not an identity brush if it's broken
	if (m_bIsBroken)
		return false;

	return C_BaseEntity::IsIdentityBrush();
}
...
{
	bool foundVar;
	m_EnvMapMaskVar = pMaterial->FindVar( "$envmapmask", &foundVar, false );
	if (!foundVar)
		return false;
...
void CBreakableSurfaceProxy::OnBind( C_BaseEntity *pC_BaseEntity )
{
	...
	m_EnvMapMaskVar->SetTextureValue( pEnt->m_pCurrentDetailTexture );
}
