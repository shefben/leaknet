...
public:
	...
	bool			UseTexture( TextureHandle_t h, bool bWillRedraw );
...
void CTextureAllocator::Init()
{
	...
	m_TexturePage.InitRenderTarget( TEXTURE_PAGE_SIZE, TEXTURE_PAGE_SIZE, IMAGE_FORMAT_ARGB8888, true );
}
...
bool CTextureAllocator::UseTexture( TextureHandle_t h, bool bWillRedraw )
{
	...
	if (currentFragment != INVALID_FRAGMENT_HANDLE)
	{
		...
		bool bShouldKeepTexture = (!bWillRedraw) && (info.m_Power < 8) && (info.m_Power - nCurrentPower <= 1);
		if ((nCurrentPower == info.m_Power) || bShouldKeepTexture)
		{
...
public:
	...
	// Update a shadow
	virtual void UpdateShadow( ClientShadowHandle_t handle, bool force );
	...
	void ComputeShadowTextures( const CViewSetup *pView, int leafCount, int* pLeafList );
...
private:
	enum
	{
		SHADOW_FLAGS_TEXTURE_DIRTY =	(SHADOW_FLAGS_LAST_FLAG << 1),
		SHADOW_FLAGS_BRUSH_MODEL =		(SHADOW_FLAGS_LAST_FLAG << 2), 
		SHADOW_FLAGS_USING_LOD_SHADOW = (SHADOW_FLAGS_LAST_FLAG << 3),
	};
...
static ConCommand r_shadowdir("r_shadowdir", ShadowDir_f);
static ConCommand r_shadowangles("r_shadowangles", ShadowAngles_f);
static ConCommand r_shadowcolor("r_shadowcolor", ShadowColor_f);
static ConCommand r_shadowdist("r_shadowdist", ShadowDistance_f);
static ConCommand r_shadowblobbycutoff("r_shadowblobbycutoff", ShadowBlobbyCutoff_f);

static void ShadowRestoreFunc()
{
...
bool CClientShadowMgr::Init()
{
	...
	m_SimpleShadow.Init( "decals/simpleshadow" );
	...
	m_RenderToTextureActive = (engine->GetDXSupportLevel() >= 80);
	if (m_RenderToTextureActive)
	{
		m_RenderShadow.Init( "decals/rendershadow" );
...
void CClientShadowMgr::SetupRenderToTextureShadow( ClientShadowHandle_t h )
{
	...
	Vector mins = pEnt->EntitySpaceSurroundingMins();
	Vector maxs = pEnt->EntitySpaceSurroundingMaxs();
...
{
	// Deactivate render to texture if we got a low-end board
	...
	shadow.m_ClientLeafShadowHandle = ClientLeafSystem()->AddShadow( h );
	...
	if ( flags & SHADOW_FLAGS_USE_RENDER_TO_TEXTURE )
	{
	...
	shadow.m_ShadowHandle = shadowmgr->CreateShadow( pShadowMaterial, pShadowModelMaterial, pShadowProxyData, SHADOW_CACHE_VERTS );
		...
		ClientShadowHandle_t handle, const Vector& mins, const Vector& maxs)
{
	...
	localShadowDir[0] = DotProduct( vec[0], m_SimpleShadowDir );
	localShadowDir[1] = DotProduct( vec[1], m_SimpleShadowDir );
	localShadowDir[2] = DotProduct( vec[2], m_SimpleShadowDir );
	...
	xvec -= m_SimpleShadowDir * DotProduct(	m_SimpleShadowDir, xvec );
	yvec -= m_SimpleShadowDir * DotProduct(	m_SimpleShadowDir, yvec );
	...
	BuildGeneralWorldToShadowMatrix( m_Shadows[handle].m_WorldToShadow, worldOrigin, m_SimpleShadowDir, xvec, yvec );
...
//	float shadowArea = size.x * size.y;	
	...
	float flShadowCastDistance = m_flShadowCastDist;
	C_BaseEntity *pEntity = C_BaseEntity::Instance( pEnt );
	if ( pEntity && pEntity->GetShadowCastDistance() )
	{
		flShadowCastDistance = pEntity->GetShadowCastDistance();
	}
	...
	shadowmgr->ProjectShadow( m_Shadows[handle].m_ShadowHandle, worldOrigin, 
		m_SimpleShadowDir, worldToTexture, size, maxHeight, falloffStart, MAX_FALLOFF_AMOUNT );
	...
	ClientLeafSystem()->ProjectShadow( m_Shadows[handle].m_ClientLeafShadowHandle, 
		worldOrigin, m_SimpleShadowDir, size, maxHeight );
}
...
//	DrawRenderToTextureDebugInfo( pEnt, mins, maxs );
	...
	Vector xvec;
	CrossProduct( yvec, m_SimpleShadowDir, xvec );
	...
	BuildOrthoWorldToShadowMatrix( m_Shadows[handle].m_WorldToShadow, worldOrigin, m_SimpleShadowDir, xvec, yvec );
	BuildWorldToTextureMatrix( m_Shadows[handle].m_WorldToShadow, size, worldToTexture );
	Vector2DCopy( size, m_Shadows[handle].m_WorldSize );
...
void CClientShadowMgr::AddChildBounds( matrix3x4_t &worldToBBox, C_BaseEntity* pParent, Vector &vecMins, Vector &vecMaxs )
{
	...
	while( pChild )
	{
		...
		// FIXME: Optimize this?
		if (pChild->ShadowCastType() != SHADOWS_NONE)
		{
			pChild->GetRenderBounds( vecChildMins, vecChildMaxs );
...
void CClientShadowMgr::UpdateStudioShadow( IClientEntity *pEnt, ClientShadowHandle_t handle )
{
	...
	if ((m_Shadows[handle].m_Flags & SHADOW_FLAGS_USE_RENDER_TO_TEXTURE) == 0)
...
bool CClientShadowMgr::ShouldUseParentShadow( C_BaseEntity *pBaseEntity )
{
	...
	if (pMoveParent->ShadowCastType() == SHADOWS_SIMPLE )
		return false;

	// If there's *no* shadow casting type, then we want to see if we can render into its parent
 	if (pMoveParent->ShadowCastType() == SHADOWS_NONE )
	{
...
void CClientShadowMgr::UpdateShadow( ClientShadowHandle_t handle, bool force )
{
	...
	if ( ShouldUseParentShadow( pEnt->GetBaseEntity() ) )
	{
	...
	if (force || (origin != shadow.m_LastOrigin) || (angles != shadow.m_LastAngles))
	{
		...
		model_t const* pModel = pEnt->GetModel();
...
static void ComputeBoundingSphere( IClientRenderable* pRenderable, Vector& origin, float& radius )
{
	Vector mins, maxs;
...
void CClientShadowMgr::ComputeShadowBBox( C_BaseEntity *pEntity, const Vector &vecAbsCenter, float flRadius, Vector *pAbsMins, Vector *pAbsMaxs )
{
	...
	for (int i = 0; i < 3; ++i)
	{
		...
		float flDist = flShadowCastDistance * m_SimpleShadowDir[i];

		if (m_SimpleShadowDir[i] < 0)
		{
...
//-----------------------------------------------------------------------------
static bool ComputeSeparatingPlane( IClientRenderable* pRend1, IClientRenderable* pRend2, cplane_t* pPlane )
{
	Vector min1, max1, min2, max2;
	pRend1->GetRenderBounds( min1, max1 );
	pRend2->GetRenderBounds( min2, max2 );
									...
									IClientRenderable* pSourceRenderable )
{
	...
	if (foundSeparatingPlane)
	{
		...
		float shadowDot = DotProduct( m_SimpleShadowDir, plane.normal );
	...
	IClientRenderable* pRenderable, ShadowReceiver_t type )
{
	...
	if (!pRenderable->ShouldReceiveShadows())
		return;
...
bool CClientShadowMgr::DrawShadowHierarchy( IClientEntity *pClientEntity, const ClientShadow_t &shadow, bool bChild )
{
	...
	if (pEnt && pEnt->ShadowCastType() == SHADOWS_SIMPLE)
		return false;

	if (!pEnt || pEnt->ShadowCastType() != SHADOWS_NONE)
	{
...
//-----------------------------------------------------------------------------
bool CClientShadowMgr::DrawRenderToTextureShadow( unsigned short clientShadowHandle )
{
	...
	bool bDrewTexture = false;
	bool needsRedraw = m_ShadowAllocator.UseTexture( shadow.m_ShadowTexture, bDirtyTexture );
...
public:
	...
	CVisibleShadowList();
	int FindShadows( const CViewSetup *pView, int nLeafCount, int *pLeafList );
...
{
	// Compute effective screen area (in normalized space, 0-1 is the width of the screen)....
	// FIXME: This is approximate, and only truly accurate when the view center
	// ray passes directly through the center of the sphere:
	
	// Think about a circle whose center is directly along the view center ray
	// Call this ray V. Assume view point is O, sphere center is C, Focal length is f.
	// sphere radius is r. Distance along V from O to C is d.
	// Now draw a line (T) tangent to the sphere passing through O. 
	// Finally draw a line segment (P) perpendicular to V, starting at C and ending when
	// in hits T. We want to find the length of P. Let's call this r'

	// Screen space length = s, screen space area = PI * s^2.
	// Since s/f = r'/d, area = PI * (f * r' / d)^2

	// Compute r':
	// Let a = angle between V and T.
	// sin a = r/d, cos a = r/r' (from geometry)
	// sin^2 a + cos^2 a = 1 (little math law)
	// (r/d)^2 + (r/r')^2 = 1, (rr')^2 + (rd)^2 = (r'd)^2
	// r'^2 * (r^2 - d^2) = -(rd)^2
	// r'^2 = -(rd)^2 / (r^2 - d^2)
	// area = -PI * (rf)^2 / (r^2 - d^2) = PI * (rf)^2 / (d^2 - r^2)

	Vector vecDelta;
	VectorSubtract( vecCenter, m_vecViewOrigin, vecDelta );
	float d = DotProduct( vecDelta, m_vecViewDirection );
	d *= d;	r *= r;
	if (d - r > 1e-6)
	{
		return M_PI * (r * m_flFocalLength * m_flFocalLength) / (d - r);
	}
	return 1;
}
...
bool CVisibleShadowList::EnumShadow( unsigned short clientShadowHandle )
{
	...
	if ((shadow.m_Flags & SHADOW_FLAGS_USE_RENDER_TO_TEXTURE) == 0)
		return true;
	...
	if ( s_ClientShadowMgr.ShouldUseParentShadow( pEntity ) )
		return true;
	...
	Vector vecAbsCenter;
	float flRadius;
	ComputeBoundingSphere( pRenderable, vecAbsCenter, flRadius );
	...
	if (!m_bNoView)
		...
		m_ShadowsInView[i].m_flArea = 1.0f;
...
//-----------------------------------------------------------------------------
int CVisibleShadowList::FindShadows( const CViewSetup *pView, int nLeafCount, int *pLeafList )
{
	// Compute a couple things we'll need to check for blobby shadows
	m_vecViewOrigin = vec3_origin;
	m_flFocalLength = 0;
	m_bNoView = true;
	if (pView)
	{
		// Basically, assuming that the half width == 1 at the focal length
		m_flFocalLength = 1.0 / tan(M_PI * pView->fov / 360.0);
		m_bNoView = false;
		m_vecViewOrigin = pView->origin;
		AngleVectors( pView->angles, &m_vecViewDirection );
	}
...
// Re-renders all shadow textures for shadow casters that lie in the leaf list
//-----------------------------------------------------------------------------
void CClientShadowMgr::ComputeShadowTextures( const CViewSetup *pView, int leafCount, int* pLeafList )
{
	...
	for (int i = 0; i < nCount; ++i)
	{
		...
		{
			if (DrawRenderToTextureShadow( info.m_hShadow ))
			{
				++nModelsRendered;
			}
		}
