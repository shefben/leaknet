...
private:
	...
	void					DrawBeamWithHalo( Beam_t* pbeam,int frame,int rendermode,float *color,const model_t *sprite,const model_t *halosprite);
...
bool Beam_t::ShouldCacheRenderInfo()
{
	return true;
}
...
Beam_t *CViewRenderBeams::BeamAlloc( bool bRenderable )
{
	...
	{
		pBeam->m_Handle = INVALID_CLIENT_RENDER_HANDLE;
	}
...
//	Assert( beamInfo.m_vecStart != vec3_origin );
//	Assert( beamInfo.m_vecEnd != vec3_origin );
...
// Output : Beam_t
//-----------------------------------------------------------------------------
void CViewRenderBeams::CreateBeamRingPoint( const Vector& center, float start_radius, float end_radius, int modelIndex, int haloIndex, float haloScale, float life, float width, float endWidth, float fadeLength, 
	...
	case TE_BEAMSPLINE:
		{
			...
			{
				if (!ComputeBeamEntPosition( pbeam->entity[i], pbeam->attachmentIndex[i], pbeam->attachment[i] ))
				{
...
void CViewRenderBeams::UpdateTempEntBeams( )
{
	if ( !m_pActiveBeams )
		return;
	...
	for ( Beam_t* pBeam = m_pActiveBeams; pBeam ; pBeam = pNext )
	{
		...
		if ( pBeam->m_Handle != INVALID_CLIENT_RENDER_HANDLE )
		{
			ClientLeafSystem()->RenderableMoved( pBeam->m_Handle );
		}
											...
											const model_t	*halosprite)
{
	...
	else
	{
		...
		{
			VectorScale( color, (1-fade), color);
		}
	}

	// Draw primary beam just shy of it's end so it doesn't clip
	DrawSegs( NOISE_DIVISIONS, pbeam->rgNoise, sprite, frame, rendermode, pbeam->attachment[0], 
		...
		pbeam->segments, pbeam->flags, color, 0.9*pbeam->fadeLength );
	...
	if (fade)
	{
		...
		// Draw secondary short beam to brighten the source as it comes closer
		beamScale = 0.5 * (1-fade) * pbeam->fadeLength;
		Assert( pbeam->fadeLength >= 0 );
		if (beamScale < 0)
			beamScale = 0;

		VectorScale( color2, (fade), color2);
		DrawSegs( NOISE_DIVISIONS, pbeam->rgNoise, sprite, frame, rendermode, 
			pbeam->attachment[0], pbeam->delta, pbeam->width, pbeam->endWidth, 
			pbeam->amplitude, pbeam->freq, pbeam->speed, pbeam->segments, pbeam->flags, 
			color2, beamScale );

		// ----------------
		// Draw halos
		// ----------------
		haloScale = fade * pbeam->haloScale;
		
		float tempColor[3];
		tempColor[0] = 1;
		tempColor[1] = 1;
		tempColor[2] = 1;
		VectorScale( tempColor, fade*fade, haloColor );

		// Clamp... (this can happen due to inaccuracies in fade
		if (haloColor[0] > 1.0f)
			haloColor[0] = 1.0f;
		if (haloColor[1] > 1.0f)
			haloColor[1] = 1.0f;
		...
		Vector vSource1  = pbeam->attachment[0] + (beamDir * 10);
		BeamDrawHalo( halosprite, frame, rendermode, vSource1, haloScale, haloColor );

		Vector vSource2  = pbeam->attachment[0] + (beamDir * 100);
		haloScale = 40;
		BeamDrawHalo( halosprite, frame, rendermode, vSource2, haloScale, haloColor );

		Vector vSource3  = pbeam->attachment[0] + (beamDir * 200);
		haloScale = 70;
		BeamDrawHalo( halosprite, frame, rendermode, vSource3, haloScale, haloColor );
	}
}
...
void CViewRenderBeams::DrawBeam( Beam_t *pbeam )
{
	...
	case TE_BEAMPOINTS:
		if (halosprite)
		{	
			DrawBeamWithHalo( pbeam, frame, rendermode, color, sprite, halosprite);
		}
		else
		{
			DrawSegs( NOISE_DIVISIONS, pbeam->rgNoise, sprite, frame, rendermode, 
