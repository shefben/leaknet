...
void C_VehicleMortar::ReceiveMessage( const char *msgname, int length, void *data )
{
}
...
void CVehicleMortarControlPanel::OnTickMainPanel()
{
	...
	{
		Q_snprintf( buf, 256, "Driven by %s", pRam->GetDriverPlayer()->GetPlayerName() );
	...
	if ( pRam->GetOwner() == C_BaseTFPlayer::GetLocalPlayer() )
	{
		if (nPassengerCount == nMaxPassengerCount)
		{
			...
			C_BaseTFPlayer *pPlayer = static_cast<C_BaseTFPlayer*>(pRam->GetPassenger( VEHICLE_DRIVER ));
