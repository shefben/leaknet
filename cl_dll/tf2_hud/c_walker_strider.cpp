...
void C_WalkerStrider::ReceiveMessage( const char *msgname, int length, void *data )
{
	BEGIN_READ( data, length );
	
	Vector vHitPos;
	READ_VEC3COORD( vHitPos );
...
int C_WalkerStrider::DrawModel( int flags )
{
	...
	IMaterial *pMaterial = materials->FindMaterial( STRIDER_BEAM_MATERIAL, NULL );
