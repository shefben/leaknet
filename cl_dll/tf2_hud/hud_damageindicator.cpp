...
{
	IMaterial *pMat = materials->FindMaterial( "hud/health/indicator", NULL );
...
void CHudDamageIndicator::MsgFunc_Damage( const char *pszName, int iSize, void *pbuf )
{
	...
	damage_t damage;
	damage.iScale = READ_BYTE();
	if ( damage.iScale > m_iMaximumDamage )
	{
		damage.iScale = m_iMaximumDamage;
	}
	Vector vecOrigin;
	vecOrigin.x = READ_FLOAT();
	vecOrigin.y = READ_FLOAT();
	vecOrigin.z = READ_FLOAT();
