...
//-----------------------------------------------------------------------------
void C_TFTeam::ReceiveMessage( const char *msgname, int length, void *data )
{
	...
	if ( GetLocalTeam() == this )
	{
		...
		Q_snprintf( sResourceString, sizeof( sResourceString ), "%d", iAmount );
	...
	else
	{
		...
		Q_snprintf( sAmount, sizeof( sAmount ), "%d", iAmount );
