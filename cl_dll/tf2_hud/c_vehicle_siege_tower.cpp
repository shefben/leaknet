...
{
	Msg( "Seq: %d, Play: %f, Cycle: %f, Anim %f, Done: %d\n", m_nSequence, m_flPlaybackRate, m_flCycle, m_flAnimTime,
...
void CVehicleSiegeTowerControlPanel::OnTick()
{
	...
	{
		Q_snprintf( buf, 256, "Driven by %s", pTower->GetDriverPlayer()->GetPlayerName() );
	...
	if ( pTower->GetOwner() == C_BaseTFPlayer::GetLocalPlayer() )
	{
		if ( nPassengerCount == nMaxPassengerCount )
		{
			...
			C_BaseTFPlayer *pPlayer = static_cast<C_BaseTFPlayer*>( pTower->GetPassenger( VEHICLE_DRIVER ) );
