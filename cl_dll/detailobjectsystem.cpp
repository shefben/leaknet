...
public:
	...
	virtual void RenderOpaqueDetailObjects( int nLeafCount, int *pLeafList );

	// Renders all translucent detail objects in a particular set of leaves
	virtual void RenderTranslucentDetailObjects( const Vector &viewOrigin, const Vector &viewForward, int nLeafCount, int *pLeafList );
...
private:
	...
	int CountSpritesInLeafList( int nLeafCount, int *pLeafList );
...
{
	// Detail models are assumed to be point-sized
	mins.Init( 0,0,0 );
	maxs.Init( 0,0,0 );
}

const Vector& CBaseStaticModel::EntitySpaceSurroundingMins() const
{
	return vec3_origin;
}

const Vector& CBaseStaticModel::EntitySpaceSurroundingMaxs() const
{
	return vec3_origin;
}

const Vector& CBaseStaticModel::WorldSpaceSurroundingMins() const
{
	return vec3_origin;
}

const Vector& CBaseStaticModel::WorldSpaceSurroundingMaxs() const
{
	return vec3_origin;
}

bool CBaseStaticModel::ShouldCacheRenderInfo()
{
	return true;
}


bool CBaseStaticModel::ShouldReceiveShadows()
{
	return false;
}
...
// Render baby!
//-----------------------------------------------------------------------------
const model_t* CBaseStaticModel::GetModel( )
{
	return m_pModel;
}
...
void CDetailObjectSystem::LevelInitPreEntity()
{
	...
	m_DetailSpriteMaterial.Init( "detail/detailsprites" );
	m_DetailWireframeMaterial.Init( "debug/debugspritewireframe" );
...
{
	for ( int i = 0; i < m_DetailObjectDict.Count(); ++i )
	{
		engine->UnloadModel( m_DetailObjectDict[i].m_pModel, true );
	}
...
// Renders all opaque detail objects in a particular set of leaves
//-----------------------------------------------------------------------------
void CDetailObjectSystem::RenderOpaqueDetailObjects( int nLeafCount, int *pLeafList )
{
	// FIXME: Implement!
}
...
// Count the number of detail sprites in the leaf list
//-----------------------------------------------------------------------------
int CDetailObjectSystem::CountSpritesInLeafList( int nLeafCount, int *pLeafList )
{
	...
	for ( int i = 0; i < nLeafCount; ++i )
	{
		...
		nPropCount += nDetailObjectCount * 4;
	}
...
int CDetailObjectSystem::SortSpritesBackToFront( int nLeaf, const Vector &viewOrigin, const Vector &viewForward, SortInfo_t *pSortInfo )
{
	...
	for ( int j = 0; j < nDetailObjectCount; ++j )
	{
		...
		pSortInfo[nCount].m_flDistance = DotProduct( viewForward, vecDelta );
		++nCount;
	}
...
void CDetailObjectSystem::RenderTranslucentDetailObjects( const Vector &viewOrigin, const Vector &viewForward, int nLeafCount, int *pLeafList )
{
...
bool CDetailObjectSystem::EnumerateLeaf( int leaf, int context )
{
	...
	for ( i = 0; i < detailObjectCount; ++i)
	{
		...
		{
			if ( model.IsDetailModelTranslucent() && (pCtx->m_FadeSqDist > 0) && (sqDist > pCtx->m_FadeSqDist))
			{
