...
void C_RopeKeyframe::ClientThink()
{
	...
	if( !DetectRestingState( m_bApplyWind ) )
	{
		...
		if( m_flTimeToNextGust <= 0 )
		{
			...
			m_vWindDir *= RemapVal( rand(), 0, VALVE_RAND_MAX, -1, 1 );
			
			m_flCurrentGustTimer = 0;
			m_flCurrentGustLifetime = RemapVal( rand(), 0, VALVE_RAND_MAX, 2, 3 );

			m_flTimeToNextGust = RemapVal( rand(), 0, VALVE_RAND_MAX, 2, 4 );
		}
...
bool C_RopeKeyframe::InitRopePhysics()
{
	...
	m_flTimeToNextGust = RemapVal( rand(), 0, VALVE_RAND_MAX, 1, 3 );
