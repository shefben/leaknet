...
bool CDustEffect::SimulateAndRender( Particle *pInParticle, ParticleDraw *pDraw, float &sortKey )
{
	CFuncDustParticle *pParticle = (CFuncDustParticle*)pInParticle;
	...
	else
	{
		...
		// Apply velocity.
		pInParticle->m_Pos.MulAdd( pInParticle->m_Pos, pParticle->m_vVelocity, pDraw->GetTimeDelta() );
...
void C_Func_Dust::PostDataUpdate( DataUpdateType_t updateType )
{
...
void C_Func_Dust::AttemptSpawnNewParticle()
{
	...
	for( int iTest=0; iTest < nTests; iTest++ )
	{
		...
		int contents = enginetrace->GetPointContents( vTest );
