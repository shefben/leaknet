...
void CPlayerProximityProxy::OnBind( void *pC_BaseEntity )
{
	...
	Vector delta;
	VectorSubtract( pEntity->GetAbsOrigin(), pPlayer->GetAbsOrigin(), delta );
...
void CPlayerPositionProxy::OnBind( void *pC_BaseEntity )
{
	...
	Assert( m_pResult );
	Vector res;
	VectorMultiply( pPlayer->GetAbsOrigin(), m_Factor, res ); 
...
bool CPlayerLogoProxy::Init( IMaterial *pMaterial, KeyValues *pKeyValues )
{
	...
	m_pDefaultTexture = materials->FindTexture( DEFAULT_DECAL_NAME, &found );
	if ( !found )
		return false;
...
void CPlayerLogoProxy::OnBind( void *pC_BaseEntity )
{
	...
	if ( playerindex >= engine->GetMaxClients() )
		return;
	...
	PlayerLogo logo;
	logo.crc = (unsigned int)info.logo;
	logo.texture = NULL;
	...
	if ( lookup == m_Logos.InvalidIndex() )
	{
		...
		Q_snprintf( crcfilename, sizeof( crcfilename ), "decals/downloads/%s",
			COM_BinPrintf( (byte *)&info.logo, 4 ) );

		bool found;
		texture = materials->FindTexture( crcfilename, &found, false );
		if ( texture )
		{
