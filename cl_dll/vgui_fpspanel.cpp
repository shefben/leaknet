...
void CFPSPanel::Paint() 
{
	...
	float frametimeAbs = gpGlobals->absoluteframetime;

	if ( engine->Con_IsVisible() )
		i++;

	if ( cl_showfps.GetInt() && frametimeAbs > 0.0 )
	{
		if ( cl_showfps.GetInt() == 2 )
		{
			float NewFrame = 1.f / frametimeAbs;
			...
			g_pMatSystemSurface->DrawColoredText( m_hFont, x, 2 + i * ( vgui::surface()->GetFontTall( m_hFont ) + 2 ),
												  255, 255, 255, 255,
												  "%3i fps (%3i, %3i) on %s",
												  (int)AverageFPS, low, high, engine->GetLevelName() );
		}
		else
		{
			AverageFPS = -1;
			g_pMatSystemSurface->DrawColoredText( m_hFont, x, 2 + i * ( vgui::surface()->GetFontTall( m_hFont ) + 2 ),
												  255, 255, 255, 255,
												  "%3i fps on %s",
												  (int)(1.f / frametimeAbs), engine->GetLevelName() );
		}
