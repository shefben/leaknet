...
public:
	...
	typedef struct message_s
	{
		...
		int			x, y;
		byte		r, g, b, a;
		char		text[ MAX_MESSAGECHARSPANEL_LEN ];
	...
	... )
{
	...
	va_start(argptr, fmt);
#ifdef _WIN32
	len = _vsnprintf(data, MAX_MESSAGECHARSPANEL_LEN, fmt, argptr);
#else
...
public:
	...
	int DrawStringForTime( float flTime, vgui::HFont hCustomFont, int x, int y, int r, int g, int b, int a, int messageID, const char *fmt, ... )
	{
		va_list argptr;
		char data[ MAX_MESSAGECHARSPANEL_LEN ];
		int len;
		...
		return messageCharsPanel->AddText( flTime, hCustomFont, x, y, r, g, b, a, messageID, data );
	}

	int DrawStringForTime( float flTime, vgui::HFont hCustomFont, int x, int y, int messageID, const char *fmt, ... )
	{
		int r = 192, g = 192, b = 192;

		va_list argptr;
		va_start( argptr, fmt );
		int result = DrawString( hCustomFont, x, y, r, g, b, 255, messageID, fmt, argptr );
		va_end( argptr );
		return result;
	}
	...
	int DrawString( vgui::HFont hCustomFont, int x, int y, int r, int g, int b, int a, int messageID, const char *fmt, ... )
	{
		va_list argptr;
		va_start( argptr, fmt );
		int result = DrawStringForTime( 0, hCustomFont, x, y, r, g, b, a, messageID, fmt, argptr );
		va_end( argptr );
		return result;
	}

	int DrawString( vgui::HFont hCustomFont, int x, int y, int messageID, const char *fmt, ... )
	{
		va_list argptr;
		va_start( argptr, fmt );
		int result = DrawStringForTime( 0, hCustomFont, x, y, messageID, fmt, argptr );
		va_end( argptr );
		return result;
	}
