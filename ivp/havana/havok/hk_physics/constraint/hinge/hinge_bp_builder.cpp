...
void hk_Hinge_BP_Builder::set_axis_perp_os(const hk_Vector3 &perp_os, int index)
{
    float normalizedLength = 1.0f / sqrtf(perp_os.length_squared());
    m_hinge_bp.m_axis_perp_os[ index ].x = (perp_os.x * normalizedLength);
    m_hinge_bp.m_axis_perp_os[ index ].y = (perp_os.y * normalizedLength);
    m_hinge_bp.m_axis_perp_os[ index ].z = (perp_os.z * normalizedLength);
}
