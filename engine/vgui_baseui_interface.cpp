...
void CBaseUI::Initialize( CreateInterfaceFn *factories, int count )
{
	...
	fs->GetLocalCopy("vgui2" DLL_EXT_STRING);
	m_hVGuiModule = Sys_LoadModule("vgui2" DLL_EXT_STRING);
	...
	// load the GameUI dll
	char szDllName[512];
	Q_snprintf(szDllName, sizeof(szDllName), "gameui" DLL_EXT_STRING );
...
//	staticGameConsole->Hide();
	...
	bool bIsLevelBackground = engineClient->IsLevelMainMenuBackground();
	...
	{
		if ( engineClient->GetMaxClients() <= 1 && cl.paused )
...
bool CBaseUI::IsInLevel()
{
	return staticGameUIFuncs->IsInLevel();
}
bool CBaseUI::IsInBackgroundLevel()
{
	return staticGameUIFuncs->IsInBackgroundLevel();
}
