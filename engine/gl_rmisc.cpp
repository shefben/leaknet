...
static bool LoadSrcVTFFiles( IVTFTexture *pSrcVTFTextures[6], const char *pSkyboxBaseName )
{
	...
	for( i = 0; i < 6; i++ )
	{
		char srcVMTFileName[1024];
		sprintf( srcVMTFileName, "skybox/%s%s", pSkyboxBaseName, facingName[i] );

		bool foundMat = false;
		IMaterial* pMaterial = materialSystemInterface->FindMaterial( srcVMTFileName, &foundMat );

		char srcVTFFileName[1024];
		bool foundVar = false;
		if ( foundMat )
		{
			// VXP: Material file is present.
			// Try to find a $basetexture var in it
			IMaterialVar *pBaseTextureVar = pMaterial->FindVar( "$basetexture", &foundVar );

			if ( foundVar )
			{
				// VXP: We've found a $basetexture var in material file
				char const *basetextureVal = pBaseTextureVar->GetStringValue();
				sprintf( srcVTFFileName, "materials/%s.vtf", basetextureVal );
			}
		}

		if ( !foundMat || !foundVar )
		{
			// VXP: We can't find a texture mentioned in material file, or a material file itself.
			// Fallback to load texture with the same name as material (original behavior)
...
void R_BuildCubemapSamples( int numIterations )
{
	...
	for( bounce = 0; bounce < numIterations; bounce++ )
	{
		...
		// load the vtex dll
		IVTex *ivt = NULL;
		CSysModule *pmodule = FileSystem_LoadModule( "vtex_dll" DLL_EXT_STRING );
		if ( pmodule )
		{
		...
		pmodule = FileSystem_LoadModule( "bsppack" DLL_EXT_STRING );
		if ( pmodule )
		{
			CreateInterfaceFn factory = Sys_GetFactory( pmodule );
			if ( factory )
		...
		// VXP: FIXME: We need to remove this nasty materialsrc/maps folder right here (GL#382)
...
void R_NewMap (void)
{
	...
	else
	{
		...
		{
			Con_DPrintf( "Level is lit, while previous one probably wasn't. Setting 'mat_fullbright 0'\n" );
