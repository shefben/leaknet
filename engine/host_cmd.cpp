...
==================
*/
void Host_Status_PrintClient( const char *pszStatus, qboolean bShowAddress, void (*print) (const char *fmt, ...), int j, client_t *client )
{
	int			seconds;
	int			minutes;
	int			hours = 0;
...
// Server status information.
	print ( "hostname:  %s\n", host_name.GetString() );
	print ( "version :  %s (%d)\n", gpszVersionString, build_number());
...
void Host_Map_Helper( bool editmode, bool bBackground )
{
	int		i;
	char	name[MAX_QPATH];
	...
	ContinueLoadingProgressBar( "Server", 1, 0.f );
...
/*
======================
Host_Map_Background_f

handle a 
map_background <servername>
Runs a map as the background
======================
*/
...
#endif
	{
		HostState_NewGame( host_map.GetString(), false ); // VXP: FIXME: Better use sv.name
	}
}
...
void Host_Version_f (void)
{
	...
	Msg ("LeakNet 2.0 build: " __TIME__ " " __DATE__ " (%i)\n", build_number() );
}
