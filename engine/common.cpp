...
const char *gpszAppName = "Half-Life 2";

char gpszVersionString[32] = "1.0"; 

com_breaksets com_breakset = COM_BREAKSET_INCLUDING_COLONS;  // YWB:  Ignore colons as token separators in COM_Parse

// wordbreak parsing set
static characterset_t	g_BreakSet,
						g_BreakSetIncludingColons,
						g_BreakSetIncludingColonsAndPunctuation,
						g_BreakSetWindowsDisallowed,
						g_BreakSetWindowsDisallowedIncludingPunctuation;
...
//		breaks = &g_BreakSet;
	...
	switch ( com_breakset )
	{
	case COM_BREAKSET_DEFAULT:
		breaks = &g_BreakSet;
		break;
	case COM_BREAKSET_INCLUDING_COLONS:
		breaks = &g_BreakSetIncludingColons;
		break;
	case COM_BREAKSET_INCLUDING_PUNCTUATION:
		breaks = &g_BreakSetIncludingColonsAndPunctuation;
		break;
	case COM_BREAKSET_WINDOWS_DISALLOWED:
		breaks = &g_BreakSetWindowsDisallowed;
		break;
	case COM_BREAKSET_WINDOWS_DISALLOWED_INCLUDING_PUNCTUATION:
		breaks = &g_BreakSetWindowsDisallowedIncludingPunctuation;
		break;
	default:
		breaks = &g_BreakSetIncludingColons; // VXP: Shut
		break;
	}
...
// Output : char
//-----------------------------------------------------------------------------
const char *COM_ParseFile(const char *data, char *token)
{
...
===============
*/
const char *COM_GetShortName(const char *name)
{
...
void COM_Init ( void )
{
	...
	CharacterSetBuild( &g_BreakSetIncludingColonsAndPunctuation, "{}()':,;.?!" );
	CharacterSetBuild( &g_BreakSetWindowsDisallowed, "\\/:*?\"<>|" );
	CharacterSetBuild( &g_BreakSetWindowsDisallowedIncludingPunctuation, "\\/:*?\"<>|,;.?!" );
}
