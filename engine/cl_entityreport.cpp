...
static ConVar cl_entityreport( "cl_entityreport", "0", 0, "For debugging, draw entity states to console" );
...
#define MAX_HISTORY_ENTITIES 1024
...
static ENTITYBITS s_EntityBits[ MAX_HISTORY_ENTITIES ];
...
void CEntityReportPanel::Paint() 
{
	...
	for ( int i = start; i < lastused; i++ )
	{
		...
		if ( pNet && (pClientClass = pNet->GetClientClass()) )
		{
			...
			DrawColoredText( m_hFont, left + col * colwidth, top + row * rowheight, r, g, b, a, 
				"(%i) %s", i, pClientClass->m_pNetworkName );
		...
		else
		{
			...
			DrawColoredText( m_hFont, left + col * colwidth, top + row * rowheight, r, g, b, a, 
				"(%i) %s", 
				i, 
				( effectactive && entry->deletedclientclass ) ? 
				  entry->deletedclientclass->m_pNetworkName : "unused" );
		}

		row++;
		if ( top + row * rowheight > (int)vid.height - rowheight )
		{
			row = 0;
			col++;
			// No more space anyway, give up
			if ( left + ( col + 1 ) * 200 > (int)vid.width )
				return;
		}
	}
}
