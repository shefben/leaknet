...
{
	sv.m_bIsLevelMainMenuBackground = false;
...
Counts number of bots (fakeclients).
==================
*/
void SV_CountBots( int *fakeclients )
{
	int i;
	client_t* cl;
...
================
*/
void SV_RejectConnection(netadr_t *adr, const char *fmt, ... )
{
	va_list		argptr;
	char	text[1024];
...
void SVC_Info ( qboolean bDetailed )
{
	...
	//!!!BETA mwhmwh
	sprintf(gd, "%s", host_name.GetString());
...
void SVC_InfoString ( void )
{
	...
	Info_SetValueForKey( info, "address", va( "%s", address ), MAX_SINFO );
	// Actual players
...
//	Info_SetValueForKey( info, "lan", va( "%i", Master_IsLanGame() ? 1 : 0 ), MAX_SINFO );
	...
	Info_SetValueForKey( info, "description", serverGameDLL->GetGameDescription(), MAX_SINFO );
...
void SV_ActivateServer()
{
	...
	ContinueLoadingProgressBar( "Server", 8, 0.f );

	// Activate the DLL server code
	serverGameDLL->ServerActivate( sv.edicts, sv.num_edicts, svs.maxclients );

	ContinueLoadingProgressBar( "Server", 9, 0.f );

	sv.active = true;
	// all setup is completed, any further precache statements are errors
	sv.state = ss_active;

	ContinueLoadingProgressBar( "Server", 10, 0.f );
...
================
*/
int SV_SpawnServer( const char *mapname, const char *startspot )
{
	client_t		*cl;
	edict_t			*ent;
	int				i;
	...
	ContinueLoadingProgressBar( "Server", 2, 0.f );
	...
	ContinueLoadingProgressBar( "Server", 3, 0.f );
	...
	ContinueLoadingProgressBar( "Server", 4, 0.f );
...
#if !defined( SWDS )
		// DLL CRC check.
		Q_snprintf( szDllName, sizeof( szDllName ), "bin\\client" DLL_EXT_STRING );
	...
	ContinueLoadingProgressBar( "Server", 6, 0.f );
...
//			... -
//-----------------------------------------------------------------------------
void SV_SendClientCommands (const char *fmt, ...)
{
	va_list		argptr;
	char		string[1024];
