...
#define WARNING_MEMORY				(128 * 1024 * 1024)
#define MAXIMUM_WIN_MEMORY			(256 * 1024 * 1024) // Ask for 256 MB max
#define MAXIMUM_DEDICATED_MEMORY	(64  * 1024 * 1024) // Ask for 64 MB max
...
qboolean			gfCanUseExtendedMemoryCheck;
...
void Sys_CheckOSVersion( void )
{
	...
	ZeroMemory( &vinfo, sizeof( OSVERSIONINFOEX ) );
	...
	gfCanUseExtendedMemoryCheck = ( vinfo.dwPlatformId >= VER_PLATFORM_WIN32_NT && vinfo.dwMajorVersion >= 5 );
#endif
}
...
void Sys_InitMemory( void )
{
	...
	if ( gfCanUseExtendedMemoryCheck )
	{
		MEMORYSTATUSEX	lpBuffer;
		ZeroMemory( &lpBuffer, sizeof( MEMORYSTATUSEX ) );
		lpBuffer.dwLength = sizeof( MEMORYSTATUSEX );

		if ( GlobalMemoryStatusEx( &lpBuffer ) )
		{
			if ( lpBuffer.ullTotalPhys > 0xFFFFFFFFUL )
			{
	...
	else
	{
		...
		ZeroMemory( &lpBuffer, sizeof( MEMORYSTATUS ) );
	...
	if ( host_parms.memsize < WARNING_MEMORY )
	{
		Sys_Error( "Available memory less than %iMB!!! %i\n", ( WARNING_MEMORY / 1024 / 1024 ), host_parms.memsize );
	}

	// take one quarter the physical memory
	host_parms.memsize >>= 2;

	// Apply cap for 512MB systems
	// this allows us to use more memory on 1GB+ systems
	if ( host_parms.memsize <= ( MAXIMUM_WIN_MEMORY * 2 ) ) // If host memory is less than the maximum x2 (512)
	{
		if ( host_parms.memsize > ( MAXIMUM_WIN_MEMORY / 2 ) )
		{
			host_parms.memsize = ( MAXIMUM_WIN_MEMORY / 2 );
		}
	}
...
void Sys_LoadPhysicsDLL( void )
{
	...
	g_PhysicsDLL = FileSystem_LoadModule( "vphysics" DLL_EXT_STRING );
	physicsFactory = Sys_GetFactory( g_PhysicsDLL );
	if ( !physicsFactory )
	{
		Sys_Error( "Sys_LoadPhysicsDLL:  Failed to load VPHYSICS dynamic library\n" );
	}
}
...
void DLL_SetModKey( modinfo_t *pinfo, char *pkey, char *pvalue )
{
	...
	{
		Q_strncpy( pinfo->szHLVersion, pvalue, sizeof( pinfo->szHLVersion ) );
	}
}
...
void LoadEntityDLLs( const char *szBaseDir )
{
	...
	//	com_ignorecolons = true;
		com_breakset = COM_BREAKSET_DEFAULT;
	...
	//	com_ignorecolons = false;
		com_breakset = COM_BREAKSET_INCLUDING_COLONS;
	...
	Q_snprintf( dirs[1], MAX_PATH, com_defaultgamedir );
	...
	for ( int i = 0; i < (sizeof( dirs ) / sizeof( dirs[0] )); i++ )
	{
		...
		// Load the game .dll
#ifdef _WIN32
		Q_snprintf( szDllFilename, sizeof( szDllFilename ), "%s\\%s\\bin\\server" DLL_EXT_STRING, szBaseDir, testdirectory );
#elif _LINUX
		Q_snprintf( szDllFilename, sizeof( szDllFilename ), "%s/bin/server" DLL_EXT_STRING, testdirectory );
#else
...
#define SOURCE_ENGINE_APP_CLASS "LeakNet.Source"
...
void Sys_CreateFileAssociations( int count, FileAssociationInfo *list )
{
	...
	// HKEY_CLASSES_ROOT/LeakNet.Source/shell/open/command == "u:\tf2\hl2.exe" "%1" quoted
	...
	for ( int i = 0; i < count ; i++ )
	{
		...
		// HKEY_CLASSES_ROOT/.dem == "LeakNet.Source"
