...
// Determine if this is a valid game configuration
static bool Host_ValidGame( void )
{
	// No multi-client singleplayer games
	if ( svs.maxclients > 1 )
	{
		if ( deathmatch.GetInt() || coop.GetBool())
			return true;
	}
	// No single-client multiplayer games
	else if (svs.maxclients == 1)
	{
		if (!deathmatch.GetInt() && !coop.GetInt())
			return true;
	}

	Con_DPrintf("Unable to launch the game\n");
	return false;
}
