...
{
	// Check if using Quake-style console
	if ( Con_IsUsingWonConsole() )
	{
		if ( g_pQuakeConsole )
		{
			g_pQuakeConsole->Print(fmt);
		}
		return;
	}

	// Use standard VGUI console
...
void VGui_ColorPrintf( Color& clr, const char *fmt, ...)
{
	...
	if ( Con_IsUsingWonConsole() )
	{
		if ( g_pQuakeConsole )
		{
			g_pQuakeConsole->ColorPrint( clr, fmt );
		}
		return;
	}
...
int VGui_IsConsoleVisible()
{
	...
	if ( Con_IsUsingWonConsole() )
	{
		if ( g_pQuakeConsole )
		{
			return g_pQuakeConsole->IsVisible();
		}
		return false;
	}
...
bool VGui_IsInLevel()
{
	return staticUIFuncs && staticUIFuncs->IsInLevel();
//	return staticUIFuncs ? staticUIFuncs->IsInLevel() : false; // VXP: Or we can do it that way too
}

bool VGui_IsInBackgroundLevel()
{
	return staticUIFuncs && staticUIFuncs->IsInBackgroundLevel();
}