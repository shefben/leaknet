...
void InitStudioRender( void )
{
	...
	g_pStudioRenderModule = FileSystem_LoadModule( "studiorender" DLL_EXT_STRING );
	if( !g_pStudioRenderModule )
	{
		Sys_Error( "Can't load studiorender\n" );
	}
	CreateInterfaceFn studioRenderFactory = Sys_GetFactory( g_pStudioRenderModule );
	if (!studioRenderFactory )
	{
		Sys_Error( "Can't get factory for studiorender\n" );
	}
