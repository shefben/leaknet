...
#if defined(_WIN32)
	Q_snprintf(szSubKey, sizeof( szSubKey ), "Software\\LeakNet\\%s\\Settings", gpszAppName );
...
void Host_Init( void )
{
	...
	{
		const char *pDataTraceFilename = 0;
...
bool Host_NewGame( char *mapName, bool loadGame, bool bBackgroundLevel )
{
	...
	ContinueLoadingProgressBar( "Server", 7, 0.f );
	...
	serverGameDLL->LevelInit( mapName, CM_EntityString(), NULL, NULL, loadGame, bBackgroundLevel );
