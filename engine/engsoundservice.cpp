...
public:
	...
	virtual bool QuerySetting( const char *pszCvar, bool bDefault )
	{
		...
		while ( 1 )
		{
			...
			{
				if ( !Q_stricmp( com_token, pszCvar ) )
				{
					// Find the value
					pstart = COM_Parse( pstart );

					bResult = Q_atoi( com_token ) ? true : false;
					break;
				}
				else
				{
