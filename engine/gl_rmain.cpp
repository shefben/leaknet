...
public:
	CRender()
	{
		// VXP: Force the default screen aspect ratio to 4:3
		m_screenAspectRatio = ( 4.0f / 3.0f );
	}
	...
	// utility functions
	float GetProjectionMatrixScreenAspect();
	void SetProjectionMatrixScreenAspect( float screenaspect );
...
//-----------------------------------------------------------------------------
// Purpose: Gets a game screen aspect
// Output : Aspect ratio, saved previously
//-----------------------------------------------------------------------------
float CRender::GetProjectionMatrixScreenAspect()
{
	return m_screenAspectRatio;
}

//-----------------------------------------------------------------------------
// Purpose: Sets a game screen aspect
// Input  : Aspect ratio, better being not RT ones
//-----------------------------------------------------------------------------
void CRender::SetProjectionMatrixScreenAspect( float screenaspect )
{
	if ( m_screenAspectRatio != screenaspect )
		m_screenAspectRatio = screenaspect;
}
...
void CRender::SetProjectionMatrix( float fov, float zNear, float zFar, bool forceAspectRatio1To1 )
{
	...
	//	screenaspect = GetScreenAspect();
		...
		// VXP: Looks like m_view.width/m_view.height and width/height are always the same,
		// since the SCR_CalcRefdef is passing just the same output from GetRenderTargetDimensions
		// So I can get rid of GetRenderTargetDimensions call here
	//	int width, height;
	//	materialSystemInterface->GetRenderTargetDimensions( width, height );

		// VXP: This is not a RT or something like that, so let's just save it in cache as a game screen aspect ratio
		if ( m_view.width != m_view.height )
		{
			float aspect = ( ( float )m_view.width / ( float )m_view.height );
			SetProjectionMatrixScreenAspect( aspect );
		}

		// VXP: Set the same screen aspect ratio for every call, if forceAspectRatio1To1 is not set
		screenaspect = GetProjectionMatrixScreenAspect();
	}
