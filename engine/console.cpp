...
void Con_Init (void)
{
	...
	if ( con_usewonconsole )
	{
		if ( !g_pQuakeConsole )
		{
			g_pQuakeConsole = new CQuakeConsole();
		}
		g_pQuakeConsole->Initialize();
	}
#endif
...
{
#ifndef SWDS
	// Shutdown Quake console if it exists
	if ( g_pQuakeConsole )
	{
		g_pQuakeConsole->Shutdown();
		delete g_pQuakeConsole;
		g_pQuakeConsole = NULL;
	}
#endif
...
bool GameUI_IsVisible()
{
	return (VGui_IsGameUIVisible());
}

bool Con_IsUsingWonConsole()
{
	return con_usewonconsole;
}
...
void CConPanel::PaintBackground()
{
	...
	int wide = GetWide();
	char ver[ 100 ];
#if defined( GIT_COMMIT_HASH ) && defined( GIT_CURRENT_BRANCH )
	Q_snprintf(ver, sizeof( ver ), "Source Engine %i/%s (LeakNet 2.0 build %d (%s @ %s) - days after 9/30/03)", PROTOCOL_VERSION, gpszVersionString, build_number(), GIT_CURRENT_BRANCH, GIT_COMMIT_HASH );
#else
	Q_snprintf(ver, sizeof( ver ), "Source Engine %i/%s (LeakNet 2.0 build %d - days after 9/30/03)", PROTOCOL_VERSION, gpszVersionString, build_number() );
#endif
