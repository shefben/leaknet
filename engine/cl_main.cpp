...
{
	ContinueLoadingProgressBar( "ClientConnect", 2, 0.f );
...
void CL_ConnectionlessPacket (void)
{
	...
	if ( !Q_stricmp( cmd, "infostringresponse" ) )
	{
		CL_ServiceInfoStringResponse();
		return;
	}
...
=======================
*/
const char *CL_GetCDKeyHash( void )
{
...
void CL_SendConnectPacket (void)
{
	...
	ContinueLoadingProgressBar( "ClientConnect", 1, 0.f );
	...
	{
	default:
		Con_Printf("Bogus auth protocol, falling back to HASHEDCDKEY\n");
		s_connection.authprotocol = PROTOCOL_HASHEDCDKEY;
	...
	case 3:
		{
			...
			int i = Q_strlen( mapname );
			if ( i > 4 && !Q_strcasecmp( mapname + i - 4, ".bsp" ) )
			{
				mapname[i-4] = 0;
			}
