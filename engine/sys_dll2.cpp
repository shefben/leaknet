...
bool RunListenServer( void *instance, const char *basedir, CreateInterfaceFn launcherFactory )
{
...
{
public:
	bool Run( void *instance, const char *basedir, CreateInterfaceFn launcherFactory );
};
...
//			launcherFactory - 
//-----------------------------------------------------------------------------
bool CEngineAPI::Run( void *instance, const char *basedir, CreateInterfaceFn launcherFactory )
{
...
int BuildMapCycleListHints(char **hints)
{
	...
	*hints = (char*)malloc( hintsSize );
	...
	if ( length )
	{
		...
		if (   pFileList 
			&& ( 1 == g_pFileSystem->Read((void*)pFileList, length, pFile) )
		   )
		{
			while ( 1 )
			{
