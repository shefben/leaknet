...
void Cbuf_AdjustCmdText( char *text, int i )
{
	...
	// VXP: FIXME: cmd_text contains all the previous data, corrupted or not.
	// All the things are controlled by cmd_text.cursize, but anyways.
	// This could clean all the gunk out, but... Let's leave it as is for now
...
void Cbuf_Execute (void)
{
	...
	// find a \n or ; line break
		...
		{
			char tmpbuf[5020];
			memcpy( tmpbuf, text, i );
			tmpbuf[i] = 0;
			Warning( "Cbuf_Execute: cmd exceeded maximum string length (%i >= %i)! Tried to execute \"%s\"\n", i, sizeof(line), tmpbuf );
			
			// VXP: Re-adjust cmd_text anyway
			Cbuf_AdjustCmdText( text, i );

		//	return;
			continue; // VXP: Proceed all the remains here, so all the next cmds would execute in time
		}
...
// skip whitespace up to a /n
		...
		{
			j = Q_strlen( com_token );
			cmd_argv[cmd_argc] = (char *)new char[ j+1 ];
			Q_strncpy( cmd_argv[cmd_argc], com_token, j+1 );
			cmd_argc++;
		}
	}
