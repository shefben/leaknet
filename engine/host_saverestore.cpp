...
bool CSaveRestore::LoadGame( const char *pName )
{
	...
	if ( !validload )
	{
	//	Host_Disconnect(); // VXP: Not needed, because disconnect routine needs HS_RUN state, not HS_LOADGAME
		Con_Printf( "Save file %s is not valid\n", name );
		return false;
	}
...
//	header.version = ENGINE_VERSION;
	...
	for ( i = 0; i < pHeader->lightStyleCount; i++ )
	{
		...
		if ( updateGlobals )
		{
			...
			Q_strncpy( sv.lightstyles[light.index], light.style, sizeof( sv.lightstyles[light.index] ) );
		}
	}
}
...
void CSaveRestore::LoadAdjacentEnts( const char *pOldLevel, const char *pLandmarkName )
{
	...
	for ( i = 0; i < currentLevelData.levelInfo.connectionCount; i++ )
	{
		...
		if ( !Q_stricmp( currentLevelData.levelInfo.levelList[i].mapName, pOldLevel ) )
		{
			foundprevious = true;
		}
