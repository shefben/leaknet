...
public:
	...
	*/
	...
	virtual int PrecacheModel( const char *s, bool preload /*= false*/ )
	{
		...
		// GMod/sandbox compatibility: Downgrade to warning instead of fatal error
		// This allows runtime model spawning to fail gracefully when the table is full
		Warning( "CVEngineServer::PrecacheModel: '%s' overflow, too many models (max %d)\n", s, MAX_MODELS );
		return 0;
	}
	...
	*/
	...
	virtual const char		*Cmd_Args( void )
	{
		return ::Cmd_Args();
	}

	virtual const char		*Cmd_Argv( int argc )
	{
	...
	==============
	*/
	virtual const char* GetInfoKeyBuffer( edict_t *e )
	{
		int		e1;
		const char	*value;
	...
	virtual const char* InfoKeyValue( const char *infobuffer, const char *key )
	{
		return Info_ValueForKey( infobuffer, key );
	}


	virtual void SetClientKeyValue( int clientIndex, const char *infobuffer, const char *key, const char *value )
	{
		client_t	*pClient;
		...
		Info_SetValueForKey( (char*)infobuffer, key, value, MAX_INFO_STRING );
	...
	virtual const char *COM_ParseFile(const char *data, char *token)
	{
