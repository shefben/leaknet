...
public:
	...
	virtual bool	Init(int &rawFrameSize, int &encodedFrameSize);
...
bool FrameEncoder_Miles::Init(int &rawFrameSize, int &encodedFrameSize)
{
	Shutdown();
...
void FrameEncoder_Miles::EncodeFrame(const char *pUncompressedBytes, char *pCompressed)
{
	...
	Assert(len == (U32)m_nEncodedBytes);	
}
...
S32 FrameEncoder_Miles::EncodeStreamCB(
	U32 user,				// User value passed to ASI_open_stream()
	void FAR *dest,			// Location to which stream data should be copied by app
	S32 bytes_requested,	// # of bytes requested by ASI codec
	S32 offset				// If not -1, application should seek to this point in stream
...
S32 FrameEncoder_Miles::DecodeStreamCB(
	U32 user,				// User value passed to ASI_open_stream()
	void FAR *dest,			// Location to which stream data should be copied by app
	S32 bytes_requested,	// # of bytes requested by ASI codec
	S32 offset				// If not -1, application should seek to this point in stream
