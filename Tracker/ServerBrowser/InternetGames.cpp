...
CInternetGames::CInternetGames(vgui::Panel *parent, const char *panelName) : CBaseGamesPage(parent, panelName)
{
	...
	netadr_t adr;
	net->StringToAdr("half-life.west.won.net:27010", &adr);
	...
	if (kv->LoadFromFile((IBaseFileSystem*)filesystem(), "servers/MasterServers.vdf" ))
	{
		...
		{
			if (!net->StringToAdr(srv->GetString("addr"), &adr))
				continue;

			m_MasterAddresses.AddToTail(adr);
		}
	}
	else
	{
...
void CInternetGames::RequestServers(int Start, const char *filterString)
{
	...
	// UAVXP: Send A2M_GET_SERVERS_BATCH2 to all of masterservers
	for (int serverIndex = 0; serverIndex < m_MasterAddresses.Count(); serverIndex++)
	{
		m_pMaster->SendMessage(&m_MasterAddresses[serverIndex]);
	}
}
