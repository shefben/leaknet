...
CServerList::CServerList(IServerRefreshResponse *target) : m_Queries(0, MAX_QUERY_SOCKETS, QueryLessFunc)
{
	...
	char speedBuf[32];
	int internetSpeed;
	if (!vgui::system()->GetRegistryString("HKEY_LOCAL_MACHINE\\Software\\LeakNet\\Steam\\Rate", speedBuf, sizeof(speedBuf)-1))
	{
...
unsigned int CServerList::AddNewServer(serveritem_t &server)
{
	...
	// UAVXP: In case multiple masterservers has the same server - filter it
