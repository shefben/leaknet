...
void CModList::ParseInstalledMods()
{
	...
	if (!vgui::system()->GetRegistryString("HKEY_LOCAL_MACHINE\\Software\\LeakNet\\Half-life\\InstallPath", szGameDirectory, sizeof(szGameDirectory)))
	{
		// fallback to getting the cstrike retail directory
		if (!vgui::system()->GetRegistryString("HKEY_LOCAL_MACHINE\\Software\\LeakNet\\Setup\\CSTRIKE\\Directory", szGameDirectory, sizeof(szGameDirectory)))
		{
			return;
		}
	}
	...
	if (hResult != INVALID_HANDLE_VALUE)
	{
		...
		while (1)
		{
				 ...
				 (strnicmp(wfd.cFileName, ".",1) ) )
			{
				...
				sprintf(szDllDirectory, "%s\\%s\\scripts\\liblist.gam", szGameDirectory, wfd.cFileName);
