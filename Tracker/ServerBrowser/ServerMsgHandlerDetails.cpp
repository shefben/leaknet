...
// Purpose: Constructor
//-----------------------------------------------------------------------------
CServerMsgHandlerDetails::CServerMsgHandlerDetails( CServerList *baseobject, HANDLERTYPE type, void *typeinfo /*= NULL*/ ) 
	: CMsgHandler( type, typeinfo )
{ 
	m_pServerList = baseobject;
}

//-----------------------------------------------------------------------------
// Purpose: Process cracked message
//-----------------------------------------------------------------------------
bool CServerMsgHandlerDetails::Process( netadr_t *from, CMsgBuffer *msg )
{
	...
	m_pServerList->UpdateServer(from,	// index of server
					(serverType == 'p'),
					name,			
					map,	
					gamedir,
