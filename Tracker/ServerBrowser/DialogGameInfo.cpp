...
CDialogGameInfo::CDialogGameInfo(IGameList *gameList, unsigned int serverID, int serverIP, int serverPort) : Frame(NULL, "DialogGameInfo"), m_Servers(this)
{
	...
	LoadControlSettings("Servers\\DialogGameInfo.res", "PLATFORM");
}
...
/*	wchar_t unicode[512], unicodeName[64];
	if (titleName)
	{
		localize()->ConvertANSIToUnicode(titleName, unicodeName, sizeof( unicodeName ));
		localize()->ConstructString(unicode, sizeof( unicode ), localize()->Find("#ServerBrowser_GameInfoWithNameTitle"), 1, unicodeName);
	}
	else
	{
		localize()->ConstructString(unicode, sizeof( unicode ), localize()->Find("#ServerBrowser_GameInfoTitle"), 0);
	}
