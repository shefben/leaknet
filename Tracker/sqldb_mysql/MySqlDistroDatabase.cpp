...
bool CMySqlDistroDatabase::GetUserDistribution(CUtlVector<trackerserver_t> &serverList)
{
	...
	if (!res)
	{
		g_pConsole->Print(4, "** GetUserDistribution Error: %s", query.error());
		return false;
	}
	...
	virtual int RunCommand(Connection &connection)
	{
		...
		try
		{
			...
			if (query.execute())
			{
				...
				if (!query.execute())
				{
					g_pConsole->Print(4, "** UpdateUserDistribution Error: %s", query.error());
				}
			}
			else
			{
	...
	virtual int RunCommand(Connection &connection)
	{
		...
		try
		{
			...
			if (res)
			{
				...
				if (!query.execute())
				{
					g_pConsole->Print(5, "** ReserveUserID Error: %s", query.error());
				}
			}
		}
		catch (...)
