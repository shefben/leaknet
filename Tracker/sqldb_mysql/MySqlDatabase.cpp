...
bool CMySqlDatabase::Initialize(const char *serverName, const char *catalogName)
{
	...
	m_Connection.connect(catalogName, serverName, "trackeruser", "trackerpass");

	// check the connection
	if (!m_Connection.connected())
	{
		// try again, using tcp/ip to connect instead
		m_Connection.connect(catalogName, serverName, "trackeruser", "trackerpass", 3306);
	...
	g_pConsole->Print(5,	"Server info: %s, %s\n%s\n",
							m_Connection.server_version().c_str(),
							m_Connection.ipc_info().c_str(),
							m_Connection.server_status().c_str());
