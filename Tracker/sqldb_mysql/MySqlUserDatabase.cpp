...
public:
	...
	virtual int RunCommand(Connection &connection)
	{
		...
		DateTime currentTime, deleteTime;
		deleteTime = row[7];
		currentTime = row[8];
...
public:
	...
	virtual int RunCommand(Connection &connection)
	{
		...
		StoreQueryResult user = query.store();
		...
		StoreQueryResult userauth = query.store();
		if (!userauth)
		{
		...
		StoreQueryResult userblocked = query.store();
		if (!userblocked)
		{
		...
		StoreQueryResult message = query.store();
		if (!message)
		{
