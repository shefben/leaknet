...
{
	// load vgui
	CSysModule *vguiModule = Sys_LoadModule("vgui2" DLL_EXT_STRING);
	CSysModule *fileSystemModule = Sys_LoadModule("filesystem_stdio" DLL_EXT_STRING);
	CSysModule *netModule = Sys_LoadModule("../Friends/TrackerNET" DLL_EXT_STRING);
	...
	if (!netFactory)
	{
		printf("Fatal error: Could not load TrackerNET%s\n", DLL_EXT_STRING);
		return 3;
	}
