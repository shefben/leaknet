...
void CUser::PerformRandomAct()
{
	...
	float chance = (float)rand() / (float)VALVE_RAND_MAX;
...
{
	int newStatus = (int)((((float)rand() / (float)VALVE_RAND_MAX) * 4) + 1);
...
{
	// choose search type
	m_iSearchType = ((rand() * 4) / VALVE_RAND_MAX) + 1;

	// generate an user ID to search for
	int index = (rand() * gApp.GetNumUsers()) / VALVE_RAND_MAX;
	char buf[64];
