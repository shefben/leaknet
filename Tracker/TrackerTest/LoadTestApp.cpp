...
CLoadTestApp::CLoadTestApp()
{
	...
	srand(time(NULL));
	m_szID[0] = ((rand() * 26) / VALVE_RAND_MAX) + 'a';
	m_szID[1] = ((rand() * 26) / VALVE_RAND_MAX) + 'a';
	m_szID[2] = ((rand() * 26) / VALVE_RAND_MAX) + 'a';
	m_szID[3] = ((rand() * 26) / VALVE_RAND_MAX) + 'a';
	m_szID[4] = 0;
...
{
	// load interfaces
	m_hTrackerDBG = Sys_LoadModule("TrackerDBG" DLL_EXT_STRING);
	m_hTrackerNET = Sys_LoadModule("TrackerNET" DLL_EXT_STRING);
...
//		return;
	...
	index = (rand() * m_iMaxTest) / VALVE_RAND_MAX;
