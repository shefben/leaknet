...
void CBuddyButton::PerformLayout(void)
{
	...
	_statusImage = scheme()->GetImage(buf, true);
	
	if (!_statusImage)
	{
		_statusImage = scheme()->GetImage("resource/icon_blank", true);
	}
...
void CBuddyButton::OnMouseReleased(vgui::MouseCode code)
{
	if (m_bDragging)
	{
		...
		if (imouseOver)
		{
			Panel *mouseOver = ipanel()->GetPanel(imouseOver, GetModuleName()); // VXP: Returns vgui::GetControlsModuleName() from Panel
			KeyValues *keyVal = new KeyValues("DragDrop", "type", "TrackerFriend");
			keyVal->SetInt("friendID", m_iBuddyID);
