...
CServerSession::CServerSession() 
{
	...
	bool bSocketBind = true;
	if (Tracker_StandaloneMode())
	{
		bSocketBind = m_pNet->Initialize(27000, 27100);
	}
	else
	{
		// use a different port in the game
		bSocketBind = m_pNet->Initialize(27001, 27100);
	}

	if (!bSocketBind)
	{
		ivgui()->DPrintf2("ERROR: Cannot bind socket!\n");
		WriteToLog("ERROR: Cannot bind socket!\n");
	}
