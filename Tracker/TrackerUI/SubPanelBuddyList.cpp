...
void CSubPanelBuddyList::InvalidateAll()
{
	...
	for (KeyValues* dat = buddyData->GetFirstSubKey(); dat != NULL; dat = dat->GetNextKey())
	{
		...
		{
			// create the button
			// VXP: FIXME: Creating VGUI controls at PerformLayout is bad. Valve, please fix
	...
	//	m_BuddyButtonDar[i]->InvalidateLayout();
		m_BuddyButtonDar[i]->RefreshBuddyStatus();
		m_BuddyButtonDar[i]->InvalidateLayout(true, false);
		m_BuddyButtonDar[i]->Repaint();
	}
