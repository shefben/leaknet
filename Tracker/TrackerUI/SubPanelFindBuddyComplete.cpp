...
CSubPanelFindBuddyComplete::CSubPanelFindBuddyComplete(vgui::Panel *parent, const char *panelName) : WizardSubPanel(parent, panelName)
{
	...
	LoadControlSettings("Friends/SubPanelFindBuddyComplete.res", "PLATFORM");
}
...
void CSubPanelFindBuddyComplete::OnDisplayAsNext()
{
	...
	for (KeyValues *user = doc->FindKey("Users", true)->GetFirstSubKey(); user != NULL; user = user->GetNextKey())
	{
		...
		CBuddy *oldBuddy = GetDoc()->GetBuddy(targetID);
		if (!oldBuddy)
		{
		...
		{
			// VXP: User removed the buddy, but re-added them again
			oldBuddy->Data()->SetInt("Status", COnlineStatus::AWAITINGAUTHORIZATION);
			oldBuddy->Data()->SetInt("Removed", 0);
		}
