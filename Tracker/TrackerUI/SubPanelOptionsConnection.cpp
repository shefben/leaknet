	...
	: PropertyPage(NULL, "SubPanelOptionsConnection")
{
	...
	LoadControlSettings("Friends/SubPanelOptionsConnection.res", "PLATFORM");
}
...
void CSubPanelOptionsConnection::OnResetData()
{
	...
	char speedText[32];
	if (!system()->GetRegistryString("HKEY_CURRENT_USER\\Software\\LeakNet\\Tracker\\Rate", speedText, sizeof(speedText)-1))
	{
...
void CSubPanelOptionsConnection::OnApplyChanges()
{
	...
	system()->SetRegistryString("HKEY_CURRENT_USER\\Software\\LeakNet\\Tracker\\Rate", speedText);
}
