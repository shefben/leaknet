...
public:
	...
	virtual void ApplySchemeSettings(vgui::IScheme *pScheme)
	{
		...
		m_pDropdownText->SetFont(pScheme->GetFont("Marlett", IsProportional()));
...
CDialogChat::CDialogChat(int userID) : Frame(NULL, "ChatDialog")
{
	...
	LoadControlSettings("Friends/DialogChat.res", "PLATFORM");
...
// Input  : *msg - 
//-----------------------------------------------------------------------------
void CDialogChat::OnChatAddUser(KeyValues *msg)
{
	if (!m_bMultiUserChat)
	{
...
void CDialogChat::OnTextChanged(Panel *panel)
{
	if (panel == m_pTextEntry)
	{
		...
		char text[256];
		m_pTextEntry->GetText(text, sizeof(text)-1);
		if (strlen(text) > 0)
		{
...
void CDialogChat::OnSendMessage()
{
	...
	else if (!m_bMultiUserChat && GetDoc()->GetBuddy(m_iFriendID)->Data()->GetInt("Status") <= COnlineStatus::OFFLINE)
	{
		...
		m_pChatHistory->InsertString("\n");
		m_pChatHistory->InsertIndentChange(2);
		m_pChatHistory->InsertColorChange(GetSchemeColor("BrightBaseText", Color(255, 255, 255, 0), scheme()->GetIScheme(scheme()->GetDefaultScheme())));
...
void CDialogChat::ApplySchemeSettings(vgui::IScheme *pScheme)
{
	...
	m_pChatHistory->SetBgColor(GetSchemeColor("ChatBgColor", GetSchemeColor("WindowDisabledBgColor", pScheme), pScheme));

	m_pMessageState->SetFont(pScheme->GetFont("DefaultSmall", IsProportional()));

	SetFgColor(GetSchemeColor("WindowFgColor", pScheme));
	m_ChatTextColor = GetSchemeColor("Chat/TextColor", pScheme);
	m_ChatSelfTextColor = GetSchemeColor("Chat/SelfTextColor", pScheme);
	m_ChatSeperatorTextColor = GetSchemeColor("Chat/SeperatorTextColor", pScheme);
	m_URLTextColor = GetSchemeColor("URLTextColor", pScheme);
}
...
// Input  : *data - 
//-----------------------------------------------------------------------------
void CDialogChat::OnDragDrop(KeyValues *data)
{
	...
	else if (!_stricmp(data->GetString("type"), "Files"))
	{
		...
		m_pTextEntry->InsertString(data->GetString("list/0"));
	}
	else if (!_stricmp(data->GetString("type"), "Text"))
	{
		m_pTextEntry->GotoTextEnd();
		m_pTextEntry->InsertString(data->GetString("text"));
	}
}
