...
// Purpose: Constructor
//-----------------------------------------------------------------------------
CSQLiteDatabase::CSQLiteDatabase() : m_Connection(false)
{
//	m_Connection = sqlite::database("tracker.db");
}
...
{
	((CSQLiteDatabase *)param)->RunThread();
	return 0;
}
...
//-----------------------------------------------------------------------------
bool CSQLiteDatabase::Initialize(const char *serverName, const char *catalogName)
{
	// check the connection
	try
	{
		// attempt to connect to the database via named pipes
		m_Connection = sqlite::database(catalogName);
	}
	catch (std::exception& e)
	{
		g_pConsole->Print(5, "Failed to connect to SQLiteDB %s : %s\n", serverName, e.what());
		return false;
	}
...
//-----------------------------------------------------------------------------
void CSQLiteDatabase::RunThread()
{
	::EnterCriticalSection(&m_csThread);
	while (m_bRunThread)
	{
...
//-----------------------------------------------------------------------------
void CSQLiteDatabase::AddCommandToQueue(ISQLDBCommand *cmd, ISQLDBReplyTarget *replyTarget, int returnState)
{
	::EnterCriticalSection(&m_csInQueue);
...
//-----------------------------------------------------------------------------
int CSQLiteDatabase::QueriesInOutQueue()
{
...
//-----------------------------------------------------------------------------
int CSQLiteDatabase::QueriesInFinishedQueue()
{
