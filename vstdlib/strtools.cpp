...
int Q_snwprintf( wchar_t *pDest, int maxLen, const wchar_t *pFormat, ... )
{
	Assert( maxLen > 0 );
	AssertValidWritePtr( pDest, maxLen );
...
int Q_vsnwprintf( wchar_t *pDest, int maxLen, const wchar_t *pFormat, va_list params )
{
	Assert( maxLen > 0 );
...
char *Q_strncat(char *pDest, const char *pSrc, size_t maxLen, int max_chars_to_copy)
{
	int charstocopy = 0;
	...
	if ( destlen + charstocopy >= maxLen )
	{
		charstocopy = (maxLen - 1) - destlen;
	}
