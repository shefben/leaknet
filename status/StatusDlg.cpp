...
#define MODCOUNT 20
	...
	for ( j = 0; j < num_fakes; j++ )
	{
		...
		for ( int modcount = 0; modcount < nummods; modcount++ )
		{
			...
			float frand = (float)rand()/(float)VALVE_RAND_MAX;
			frand = 0.6f + 0.4f * frand;
			float frand2 = (float)rand()/(float)VALVE_RAND_MAX;
		...
		//	np += (int)( 2000.0 * (float)rand()/(float)VALVE_RAND_MAX);
		//	ns += (int)( 500.0 * (float)rand()/(float)VALVE_RAND_MAX);
