...
inline void ConvertPositionToIVP( const Vector &in, IVP_U_Float_Point &out )
{
	...
	out.k[0] = HL2IVP(in[0]);
	out.k[1] = (in[2] != 0.0f) ? -HL2IVP(in[2]) : 0.0f; // VXP: Should prevent from unnecessary negative zeroes
	out.k[2] = HL2IVP(tmpZ);
}
...
inline void ConvertPositionToIVP( float &x, float &y, float &z )
{
	...
	tmpZ = y;
	y = (z != 0.0f) ? -HL2IVP(z) : 0.0f; // VXP: Should prevent from unnecessary negative zeroes
	z = HL2IVP(tmpZ);
	x = HL2IVP(x);
}
...
inline void ConvertDirectionToIVP( const Vector &in, IVP_U_Float_Point &out )
{
	...
	out.k[0] = in[0];
	out.k[1] = (in[2] != 0.0f) ? -in[2] : 0.0f; // VXP: Should prevent from unnecessary negative zeroes
	out.k[2] = tmpZ;
}
...
inline void ConvertAngularImpulseToIVP( const AngularImpulse &in, IVP_U_Float_Point &out )
{
	...
	out.k[1] = (in[2] != 0.0f) ? -DEG2RAD(in[2]) : 0.0f; // VXP: Should prevent from unnecessary negative zeroes
