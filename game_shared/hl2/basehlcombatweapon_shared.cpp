...
#define HL2_BOB_OLD				0.01f

float	g_lateralBob;
float	g_verticalBob;

ConVar	cl_oldvmbob( "cl_oldvmbob", "0" );
...
float CBaseHLCombatWeapon::CalcViewmodelBob( void )
{
	...
	if ( cl_oldvmbob.GetBool() )
	{
		...
		g_lateralBob = sqrt( vel[0] * vel[0] + vel[1] * vel[1] )*HL2_BOB_OLD;
	...
	speed = clamp( speed, -320, 320 ); // himdeez: clamped the speed to prevent weird movements while having high velocity
