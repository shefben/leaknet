...
public:
	...
	int		ComputeRequiredHeight( void );

	// If bOnlyCalculateHeight is set to true, then the labels aren't actually repositioned,
	// it just calculates how much vertical space it needs to fit everything and returns that.
	int		RepositionLabels( bool bOnlyCalculateHeight=false );
...
private:
	...
	BitmapImage			*m_pScoreboardNeverSpoken;
	BitmapImage			*m_pScoreboardNotSpeaking;
	BitmapImage			*m_pScoreboardSpeaking;
	BitmapImage			*m_pScoreboardSpeaking2;
	BitmapImage			*m_pScoreboardSquelch;
	BitmapImage			*m_pScoreboardBanned;
	...
	BitmapImage			*m_pSpeakerLabelIcon;	// Icon next to speaker labels.

	// Lower-right icons telling when the local player is talking..
	BitmapImage			*m_pLocalBitmap;		// Represents the local client talking.
	BitmapImage			*m_pAckBitmap;			// Represents the server ack'ing the client talking.
	CVoicePanel		*m_pLocalLabel;			// Represents the local client talking.
