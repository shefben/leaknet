...
// ------------------------------------------------------------------------------------------------ //
	...
	static const char *g_pFlameThrowerSound = "fire/burning1.wav";
...
BEGIN_PREDICTION_DATA( CWeaponFlameThrower )
	...
	DEFINE_PRED_FIELD( CWeaponFlameThrower, m_bFiring, FIELD_BOOLEAN, FTYPEDESC_INSENDTABLE ),
...
void CWeaponFlameThrower::PrimaryAttack()
{
	...
	void CWeaponFlameThrower::StartSound()
	{
		if ( !m_bSoundOn )
		{
			...
			EmitSound( filter, entindex(), CHAN_WEAPON, g_pFlameThrowerSound, 1, ATTN_NORM );
	...
	void CWeaponFlameThrower::StopFlameSound()
	{
		if ( m_bSoundOn )
		{
			StopSound( entindex(), CHAN_WEAPON, g_pFlameThrowerSound );
			m_bSoundOn = false;
		}
	}
