...
LINK_ENTITY_TO_CLASS( team_fortress_gamerules, CTeamFortress );

IMPLEMENT_NETWORKCLASS_ALIASED( TeamFortress, DT_TeamFortress )

BEGIN_NETWORK_TABLE( CTeamFortress, DT_TeamFortress )
END_NETWORK_TABLE()
	...
	//-----------------------------------------------------------------------------
	// Wrapper to hide command arguments
	//-----------------------------------------------------------------------------
	class CObjectCommandArgumentsImp : public ICommandArguments
	{
	public:
		virtual int Argc()
		{
			return engine->Cmd_Argc() - 2;
		}

		virtual const char *Argv( int nIndex )
		{
			return engine->Cmd_Argv( nIndex + 2 );
		}
	};

	static CObjectCommandArgumentsImp g_ObjCommandArgs;
	...
	// Output :
	//-----------------------------------------------------------------------------
	bool CTeamFortress::ClientCommand(const char *pcmd, CBaseEntity *pEdict )
	{
		...
		if ( FStrEq( pcmd, "objcmd" ) )
		{
			...
			int entindex = atoi( engine->Cmd_Argv(1) );
	...
	//				if (flDistSq <= (MAX_OBJECT_COMMAND_DISTANCE * MAX_OBJECT_COMMAND_DISTANCE))
					{
						pObject->ClientCommand( pPlayer, engine->Cmd_Argv(2), &g_ObjCommandArgs );
					}
				}
			}
		...
		if ( FStrEq( pcmd, "buildvehicle" ) )
		{
			...
			int entindex = atoi( engine->Cmd_Argv(1) );
			int ivehicle = atoi( engine->Cmd_Argv(2) );
			edict_t *pEdict = INDEXENT(entindex);
			if (pEdict)
			{
				CBaseEntity *pBaseEntity = GetContainingEntity(pEdict);
		...
		{
			bool bTactical = engine->Cmd_Argv(1)[0] == '!' ? !pPlayer->GetLocalData()->m_nInTacticalView : (atoi( engine->Cmd_Argv(1) ) ? true : false);
		...
		else if ( FStrEq( pcmd, "addres" ) || FStrEq( pcmd, "ar" ) )
		{
			if ( engine->Cmd_Argc() == 3 )
			{
				...
				float flResourceAmount = atof( engine->Cmd_Argv(2) );
		...
		else if ( FStrEq( pcmd, "preftech" ) )
		{
			...
			if ( engine->Cmd_Argc() == 2 )
			{
				int iPrefTechIndex = atoi( engine->Cmd_Argv(1) );
		...
		else if( FStrEq( pcmd, "terrainmod" ) )
		{
			...
			if( engine->Cmd_Argc() >= 2 )
			{
				int type = atoi( engine->Cmd_Argv(1) );
				...
				case TMod_Sphere:
					{
						...
						if( engine->Cmd_Argc() >= 3 )
						{
							params.m_flRadius = atof( engine->Cmd_Argv(2) );
						}
		...
		{
			return pPlayer->ClientCommand(pcmd);
		}
	...
	void CTeamFortress::InitDefaultAIRelationships( void )
	{
		...
		CBaseCombatCharacter::SetDefaultRelationship(CLASS_EARTH_FAUNA,			CLASS_BULLSQUID,		D_FR, 2);
		...
		CBaseCombatCharacter::SetDefaultRelationship(CLASS_EARTH_FAUNA,			CLASS_CREMATOR,			D_FR, 1);
		...
		CBaseCombatCharacter::SetDefaultRelationship(CLASS_EARTH_FAUNA,			CLASS_HOUNDEYE,			D_FR, 2);
		...
		CBaseCombatCharacter::SetDefaultRelationship(CLASS_EARTH_FAUNA,			CLASS_MORTAR_SYNTH,		D_FR, 4);
		...
		CBaseCombatCharacter::SetDefaultRelationship(CLASS_EARTH_FAUNA,			CLASS_WASTE_SCANNER,	D_FR, 4);
	...
	void CTeamFortress::LevelInit( void )
	{ 
		BaseClass::LevelInit();
...
CAmmoDef *GetAmmoDef()
{
	...
	if ( !bInitted )
	{
		...
		def.AddAmmoType("Bullets",			DMG_BULLET,	TRACER_LINE, 0,	0, INFINITE_AMMO, 0);
		def.AddAmmoType("Rockets",			DMG_BLAST,	TRACER_LINE, 0,	0, 6, 0);
		def.AddAmmoType("Grenades",			DMG_BLAST,	TRACER_LINE, 0,	0, 3, 0);
		def.AddAmmoType("ShieldGrenades",		DMG_ENERGYBEAM,	TRACER_LINE, 0,	0, 5, 0);
		def.AddAmmoType("ShotgunEnergy",		DMG_ENERGYBEAM,	TRACER_LINE, 0,	0, INFINITE_AMMO, 0);
		def.AddAmmoType("PlasmaGrenade",		DMG_ENERGYBEAM|DMG_BLAST, TRACER_LINE, 0, 0, 30, 0);
		def.AddAmmoType("ResourceChunks",		DMG_GENERIC,TRACER_LINE, 0,	0, 4, 0);		// Resource chunks
		def.AddAmmoType("Limpets",			DMG_BLAST,	TRACER_LINE, 0,	0, 40, 0);
		def.AddAmmoType("Gasoline",			DMG_BURN,	TRACER_LINE, 0,	0, 80, 0);

		// Combat Objects
		def.AddAmmoType("RallyFlags",			DMG_GENERIC, TRACER_NONE, 0, 0,	1, 0);
		def.AddAmmoType("EMPGenerators",		DMG_GENERIC, TRACER_NONE, 0, 0,	1, 0);
	}
