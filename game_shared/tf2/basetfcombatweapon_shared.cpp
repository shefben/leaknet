...
#if !defined( CLIENT_DLL )
	// Connect to my CVars
	m_pDamageCVar = cvar->FindVar( UTIL_VarArgs( "%s_damage", STRING(pev->classname) ) );
	m_pRangeCVar = cvar->FindVar( UTIL_VarArgs( "%s_range", STRING(pev->classname) ) );
#endif
}
...
// Input  : sequence - 
//-----------------------------------------------------------------------------
void CBaseTFCombatWeapon::SendWeaponAnim( int iActivity )
{
	CBaseTFPlayer *pPlayer = ToBaseTFPlayer( GetOwner() );
	if ( !pPlayer )
		return;
...
BEGIN_PREDICTION_DATA( CBaseTFCombatWeapon )
	...
	DEFINE_PRED_FIELD( CBaseTFCombatWeapon , m_bReflectViewModelAnimations, FIELD_BOOLEAN, FTYPEDESC_INSENDTABLE ),
	DEFINE_FIELD( CBaseTFCombatWeapon , m_iLastReflectedActivity, FIELD_INTEGER )
