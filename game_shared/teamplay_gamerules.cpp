...
const char *CTeamplayRules::SetDefaultPlayerTeam( CBasePlayer *pPlayer )
{
	...
	const char *mdls = engine->InfoKeyValue( engine->GetInfoKeyBuffer( pPlayer->edict() ), "model" );
...
void CTeamplayRules::ClientUserInfoChanged( CBasePlayer *pPlayer, char *infobuffer )
{
	...
	const char *mdls = engine->InfoKeyValue( infobuffer, "model" );

	if ( !Q_stricmp( mdls, pPlayer->TeamName() ) )
		return;
