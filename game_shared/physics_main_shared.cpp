...
inline touchlink_t *AllocTouchLink( void )
{
	...
	{
		DevMsg( "AllocTouchLink:  Progamming error, failed to allocate touchlink_t.  Touchlinks not being freed correctly!!!\n" );
	}
...
void CBaseEntity::PhysicsNotifyOtherOfUntouch( CBaseEntity *ent, CBaseEntity *other )
{
	...
	touchlink_t *link = other->m_EntitiesTouched.nextLink;
	while ( link != &other->m_EntitiesTouched )
	{
...
void CBaseEntity::PhysicsRemoveTouchedList( CBaseEntity *ent )
{
	...
	link = ent->m_EntitiesTouched.nextLink;
	while ( link && link != &ent->m_EntitiesTouched )
	{
		nextLink = link->nextLink;
	...
	ent->m_EntitiesTouched.nextLink = ent->m_EntitiesTouched.prevLink = &ent->m_EntitiesTouched;
	ent->touchStamp = 0;
}
...
touchlink_t *CBaseEntity::PhysicsMarkEntityAsTouched( CBaseEntity *other )
{
	...
	for ( link = m_EntitiesTouched.nextLink; link != &m_EntitiesTouched; link = link->nextLink )
	{
	...
	link->flags = 0;
	// add it to the list
	link->nextLink = m_EntitiesTouched.nextLink;
	link->prevLink = &m_EntitiesTouched;
...
trace_t CBaseEntity::GetTouchTrace( void )
{
	return g_TouchTrace;
}
...
void CBaseEntity::UpdateWaterState()
{
	...
	Vector	point;
	point[0] = (GetAbsMins()[0] + GetAbsMaxs()[0]) * 0.5;
	point[1] = (GetAbsMins()[1] + GetAbsMaxs()[1]) * 0.5;
	point[2] = GetAbsMins()[2] + 1;
...
void CBaseEntity::PhysicsRigidChild( void )
{
	...
	if (GetLocalVelocity() != vec3_origin || GetLocalAngularVelocity() != vec3_angle )
	{
		// Angles move regardless; we don't have a good way of sweeping them.
		Vector vecOldAbsOrigin = GetAbsOrigin();
		QAngle angOldRotation =	GetLocalAngles();
		SimulateAngles( gpGlobals->frametime );

		// Figure out how far to sweep through local space
		Vector	vecMove;
		VectorMultiply( GetLocalVelocity(), gpGlobals->frametime, vecMove );
		
		// Try to push the entity though *world space* at see what it hits
		// based on the result, move only as far as possible
		trace_t	trace;
		Vector vecAbsMove;
		ComputeAbsDirection( vecMove, &vecAbsMove );
		PhysicsCheckSweep( vecOldAbsOrigin, vecAbsMove, &trace );

		// Move only as far as the sweep told us we could...
		if ((trace.startsolid) || (trace.allsolid))
		{
			// This could potentially happen in OBB case where we were free initially
			// but rotated into something due to local angular velocity
			SetLocalAngles( angOldRotation );
		}
		else
		{
			vecMove *= trace.fraction;
			Vector vecNewOrigin;
			VectorAdd( GetLocalOrigin(), vecMove, vecNewOrigin );
			SetLocalOrigin( vecNewOrigin );
			...
			engine->RelinkEntity( pev, true, &vecOldAbsOrigin );
#else
			Relink();
#endif
		}
