...
void CPredictionCopy::DescribeInt( difftype_t dt, int *outvalue, const int *invalue, int count )
{
	...
	if ( m_pCurrentField->flags & FTYPEDESC_MODELINDEX )
	{
		...
		if ( m )
		{
			...
			char shortfile[ 512 ];
			shortfile[ 0 ] = 0;
			engine->COM_FileBase( modelinfo->GetModelName( m ), shortfile );
		...
		case FIELD_CHARACTER:
			{
				...
				DescribeData( difftype, fieldSize, ((char *)pOutputData), (const char *)pInputData );
...
static typedescription_t *FindFieldByName_R( const char *fieldname, datamap_t *dmap )
{
	...
	for ( int i = 0; i < c; i++ )
	{
		...
		if ( !_stricmp( td->fieldName, fieldname ) )
		{
			return td;
		}
	}
...
static ConVar pwatchent( "pwatchent", "-1", 0, "Entity to watch for prediction system changes." );
static ConVar pwatchvar( "pwatchvar", "", 0, "Entity variable to watch in prediction system for changes." );
...
{
	Describe( "EHandle (%p)", invalue[ 0 ].Get() );
}
...
public:
	...
	void MakeDifferent( void )
	{
		...
		m_IntValue = (int)400;
		m_FloatValue = 4.0f;
		strcpy( m_szValue, "secondarydata" );
