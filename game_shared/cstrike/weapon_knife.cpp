...
//	UTIL_ImpactTrace( &m_trHit, DMG_BULLET );
	...
	// VXP: This below essentially is just like UTIL_ImpactTrace, but using the m_pTraceHitEnt
	m_trHit.m_pEnt = m_pTraceHitEnt;
	CBaseEntity *pEntity = m_trHit.m_pEnt;

	// Is the entity valid, is the surface sky?
	if ( !pEntity || (m_trHit.surface.flags & SURF_SKY) )
		return;

	if (m_trHit.fraction == 1.0)
		return;

	pEntity->ImpactTrace( &m_trHit, DMG_BULLET, NULL );
