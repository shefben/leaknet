...
#ifndef CLIENT_DLL // Apply damage. Server only.	
			float flDistanceMultiplier = 1.0f;

			if ( (tr.m_pEnt->GetSolid() == SOLID_BSP) && ( iPenetration != 0 ) )
			{
				flDistanceMultiplier = 0.5f;
			}
			else
			{
				iPenetrationPower = 42;
				flDistanceMultiplier = 0.75f;
				flDamageModifier = 0.75f;
			} 

			vecSrc = tr.endpos + (vecDir * iPenetrationPower);
			flDistance = (flDistance - flCurrentDistance) * flDistanceMultiplier;
			vecEnd = vecSrc + (vecDir * flDistance);
