...
static bool HammerSettingsFound(void)
{
	...
	{
		HKEY hkeyLeakNet;
		if (RegOpenKeyEx(hkeySoftware, "LeakNet", 0, KEY_READ | KEY_WRITE, &hkeyLeakNet) == ERROR_SUCCESS)
		{
			HKEY hkeyWorldcraft;
			if (RegOpenKeyEx(hkeyLeakNet, "Hammer", 0, KEY_READ | KEY_WRITE, &hkeyWorldcraft) == ERROR_SUCCESS)
			{
				HKEY hkeyConfigured;
...
static bool ValveHammerEditorSettingsFound(void)
{
	...
	if (RegOpenKeyEx(HKEY_CURRENT_USER, "Software", 0, KEY_READ | KEY_WRITE, &hkeySoftware) == ERROR_SUCCESS)
	{
		...
		{
			HKEY hkeyWorldcraft;
			if (RegOpenKeyEx(hkeyLeakNet, "Valve Hammer Editor", 0, KEY_READ | KEY_WRITE, &hkeyWorldcraft) == ERROR_SUCCESS)
			{
				HKEY hkeyConfigured;
				if (RegOpenKeyEx(hkeyLeakNet, "Configured", 0, KEY_READ | KEY_WRITE, &hkeyConfigured) == ERROR_SUCCESS)
				{
					bFound = true;
					RegCloseKey(hkeyConfigured);
				}
...
static bool WorldcraftSettingsFound(void)
{
	...
	if (RegOpenKeyEx(HKEY_CURRENT_USER, "Software", 0, KEY_READ | KEY_WRITE, &hkeySoftware) == ERROR_SUCCESS)
	{
		...
		{
			HKEY hkeyWorldcraft;
			if (RegOpenKeyEx(hkeyLeakNet, "Worldcraft", 0, KEY_READ | KEY_WRITE, &hkeyWorldcraft) == ERROR_SUCCESS)
			{
				bFound = true;
				RegCloseKey(hkeyWorldcraft);
			}
