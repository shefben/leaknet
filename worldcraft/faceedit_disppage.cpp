...
BEGIN_MESSAGE_MAP( CFaceEditDispPage, CPropertyPage )
	...
	ON_BN_CLICKED( IDC_DISPSTATE_RESUBDIV, OnStateResubdivide )
...
//-----------------------------------------------------------------------------
// Purpose: Toggle the resubdivide state.
//-----------------------------------------------------------------------------
void CFaceEditDispPage::OnStateResubdivide( void )
{
	// Get the displacement tool.
	CToolDisplace *pDispTool = g_pToolManager->GetDisplacementTool();
	if ( pDispTool )
	{
		pDispTool->ToggleResubdivideState();
	}
}
...
{
	g_pToolManager->SetTool( TOOL_FACEEDIT_DISP );
