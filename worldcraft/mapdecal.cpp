...
CMapDecal::~CMapDecal(void)
{
	...
	//
	POSITION pos = m_Faces.GetHeadPosition();
	while (pos != NULL)
	{
		DecalFace_t *pDecalFace = m_Faces.GetNext(pos);
...
{
	if (m_Solids.Find(pSolid) == NULL)
	{
		UpdateDependency(NULL, pSolid);
		m_Solids.AddTail(pSolid);
	}
}
...
CMapClass *CMapDecal::CopyFrom(CMapClass *pObject, bool bUpdateDependencies)
{
	...
	//
	m_Solids.RemoveAll();
	m_Solids.AddTail(&pFrom->m_Solids);
	...
	//
	m_Faces.RemoveAll();
	POSITION pos = pFrom->m_Faces.GetHeadPosition();
	while (pos != NULL)
	{
		...
		if (pDecalFace != NULL)
		{
			...
			{
				DecalFace_t *pFromDecalFace = pFrom->m_Faces.GetNext(pos);
...
//-----------------------------------------------------------------------------
void CMapDecal::OnParentKeyChanged(LPCSTR szKey, LPCSTR szValue)
{
	//
	...
	if (!_stricmp(szKey, "texture"))
	{
		...
		if (pTexNew != NULL)
		{
			...
			//
			POSITION pos = m_Solids.GetHeadPosition();
			while (pos != NULL)
			{
				CMapSolid *pSolid = (CMapSolid *)m_Solids.GetNext(pos);
				if (pSolid != NULL)
				{
...
void CMapDecal::OnNotifyDependent(CMapClass *pObject, Notify_Dependent_t eNotifyType)
{
	...
	if (pSolid != NULL)
	{
		...
		//
		pos = m_Solids.Find(pSolid);
		if (pos != NULL)
		{
			m_Solids.RemoveAt(pos);
...
//-----------------------------------------------------------------------------
void CMapDecal::OnPaste(CMapClass *pCopy, CMapWorld *pSourceWorld, CMapWorld *pDestWorld, CMapObjectList &OriginalList, CMapObjectList &NewList)
{
...
// Input  : pTransBox - 
//-----------------------------------------------------------------------------
void CMapDecal::DoTransform(Box3D *pTransBox)
{
...
// Input  : RefPoint - 
//-----------------------------------------------------------------------------
void CMapDecal::DoTransFlip(const Vector &RefPoint)
{
...
// Input  : Delta - 
//-----------------------------------------------------------------------------
void CMapDecal::DoTransMove(const Vector &Delta)
{
...
//			Angles - 
//-----------------------------------------------------------------------------
void CMapDecal::DoTransRotate(const Vector *pRefPoint, const QAngle &Angles)
{
...
//			Scale - 
//-----------------------------------------------------------------------------
void CMapDecal::DoTransScale(const Vector &RefPoint, const Vector &Scale)
{
