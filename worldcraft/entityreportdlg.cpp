...
{
	OnMark();
	m_pDoc->OnCmdMsg(ID_VIEW_CENTERONSELECTION, CN_COMMAND, NULL, NULL);
}

void CEntityReportDlg::OnMark() 
{
	// mark objects 
...
BOOL AddEntityToList(CMapEntity *pEntity, CEntityReportDlg *pDlg)
{
	...
	*/
	...
	LPCSTR pszClassName = pEntity->GetClassName();
	...
	MDkeyvalue tmpkv;
	for (int i = 0; i < pEntity->GetKeyValueCount(); i++)
	{
		...
		LPCSTR pszName = pEntity->GetKey(i);
...
BOOL CEntityReportDlg::OnInitDialog() 
{
