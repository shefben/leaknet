...
void CTextureConverter::ConvertWorldTextures( CMapWorld * pWorld )
{
	...
	// Bring the WC message window to the front, to display conversion info
	GetMainWnd()->pMsgWnd->ShowWindow( SW_SHOW );
	GetMainWnd()->pMsgWnd->SetWindowPos( &( CWnd::wndTopMost ), 0, 0, 0, 0, SWP_NOMOVE | SWP_NOSIZE | SWP_SHOWWINDOW );
	GetMainWnd()->pMsgWnd->BringWindowToTop();
	GetMainWnd()->pMsgWnd->SetFocus();
	...
	AfxMessageBox( "Conversion complete.  Check the Worldcraft \"Messages\" window for complete details." );
}
...
{
	ITextureList	tlMatches;
	IEditorTexture *		pNewTexture;

	GetNewTextureMatches( pFace->GetTexture()->GetName(), &tlMatches );

	switch( tlMatches.GetCount() )
	{
	case 0:
	...
	default:
		...
		MsgConvertFace( pFace, "WARNING: Multiple matches found.  Using first match (%s).", pNewTexture->GetName() );
		m_nWarnings++;
...
//-----------------------------------------------------------------------------
void CTextureConverter::GetNewTextureMatches( const char * pszOldName, ITextureList * pMatchList )
{
...
{
	IEditorTexture *	pOldTexture;
	bool		bCanRescale;

	pOldTexture = FindWAD3Texture( pFace->GetTexture()->GetName() );

	if ( pOldTexture == NULL )
	{
		MsgConvertFace( pFace, "WARNING: Couldn't find old texture.  Texture converted but not re-scaled." );
		m_nWarnings++;

		bCanRescale = FALSE;
	}
	else if ( !pOldTexture->Load() )	// make sure old texture is loaded
	{
		MsgConvertFace( pFace, "WARNING: Couldn't load old texture.  Texture converted but not re-scaled." );
		m_nWarnings++;
	...
	if ( bCanRescale )
	{
		RescaleFaceTexture( pFace, pOldTexture, pNewTexture );
	}
...
//-----------------------------------------------------------------------------
void CTextureConverter::RescaleFaceTexture( CMapFace * pFace, IEditorTexture * pOldTexture, IEditorTexture * pNewTexture )
{
	int nOldWidth;
	int nOldHeight;
	int nNewWidth;
	...
	nOldWidth	= pOldTexture->GetWidth();
	nOldHeight	= pOldTexture->GetHeight();
