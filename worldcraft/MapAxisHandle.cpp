...
//-----------------------------------------------------------------------------
CBaseTool *CMapAxisHandle::GetToolObject(int nHitData)
{
	...
	CToolAxisHandle *pTool = (CToolAxisHandle *)g_pToolManager->GetToolForID(TOOL_AXIS_HANDLE);
...
//-----------------------------------------------------------------------------
CMapClass *CMapAxisHandle::HitTest2D(CMapView2D *pView, const Vector2D &point, int &nHitData)
{
	for (int i = 0; i < 2; i++)
	{
		CMapClass *pHit = m_Point[i].HitTest2D(pView, point, nHitData);
		if (pHit != NULL)
		{
			nHitData = i;
			return this;
...
void CMapAxisHandle::Render3D(CRender3D *pRender)
{
	if (GetSelectionState() != SELECT_NONE)
	{
		...
		pRender->RenderLine(vec1, vec2, 255, 255, 255);
...
//-----------------------------------------------------------------------------
void CMapAxisHandle::DoTransform(Box3D *t)
{
	BaseClass::DoTransform(t);

	m_Point[0].Transform(t);
	m_Point[1].Transform(t);
...
// Purpose: Overridden to transform our endpoints.
//-----------------------------------------------------------------------------
void CMapAxisHandle::DoTransMove(const Vector &Delta)
{
	BaseClass::DoTransMove(Delta);

	m_Point[0].TransMove(Delta);
	m_Point[1].TransMove(Delta);
...
// Purpose: Overridden to transform our endpoints.
//-----------------------------------------------------------------------------
void CMapAxisHandle::DoTransRotate(const Vector *pRefPoint, const QAngle &Angles)
{
	BaseClass::DoTransRotate(pRefPoint, Angles);

	m_Point[0].TransRotate(pRefPoint, Angles);
	m_Point[1].TransRotate(pRefPoint, Angles);
...
// Purpose: Overridden to transform our endpoints.
//-----------------------------------------------------------------------------
void CMapAxisHandle::DoTransScale(const Vector &RefPoint, const Vector &Scale)
{
	BaseClass::DoTransScale(RefPoint, Scale);

	m_Point[0].TransScale(RefPoint, Scale);
	m_Point[1].TransScale(RefPoint, Scale);
...
// Purpose: Overridden to transform our endpoints.
//-----------------------------------------------------------------------------
void CMapAxisHandle::DoTransFlip(const Vector &RefPoint)
{
	BaseClass::DoTransFlip(RefPoint);

	m_Point[0].TransFlip(RefPoint);
	m_Point[1].TransFlip(RefPoint);
