...
{ 
	...
	if ( !s_bOnce )
	{
		...
		for ( int i = 0; i < Options.configs.nConfigs; i++ )
		{
			...
			Q_snprintf( buf, MAX_PATH, "%s\\hl2", pConfig->m_szGameExeDir );
			g_pFileSystem->AddSearchPath( buf, "GAME", PATH_ADD_TO_HEAD );

			if ( pConfig->m_szModDir && *pConfig->m_szModDir != '\0' )
			{
				g_pFileSystem->AddSearchPath( pConfig->m_szModDir, "GAME", PATH_ADD_TO_HEAD );
			}
		}
	}
