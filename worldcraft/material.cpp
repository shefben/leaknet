...
#include "ImageLoader.h" // hack : don't want to include this just for ImageFormat
...
static HINSTANCE materialSystemDLLHInst = NULL;
IMaterialSystem *g_pMaterialSystemInterface = NULL;
...
static bool g_bMaterialSystemLoaded = false;
...
// Purpose: 
//-----------------------------------------------------------------------------
static void LoadMaterialSystemInterface( void )
{
	char dllName[1024];

	materialSystemDLLHInst = LoadLibrary( "materialsystem" DLL_EXT_STRING );
	if ( !materialSystemDLLHInst )
	{
		Msg( mwError,"Can't load materialsystem%s", DLL_EXT_STRING );
		return;
	}

	GetModuleFileName(materialSystemDLLHInst, dllName, sizeof(dllName));

	g_MaterialSystemFactory = Sys_GetFactory( "materialsystem" DLL_EXT_STRING );
	if ( g_MaterialSystemFactory )
	{
		g_pMaterialSystemInterface = (IMaterialSystem *)g_MaterialSystemFactory( MATERIAL_SYSTEM_INTERFACE_VERSION, NULL );
		if ( !g_pMaterialSystemInterface )
		{
			Msg( mwError, "Could not get the material system interface from %s", dllName );
			return;
		}
	}
	else
	{
		Msg( mwError, "Could not find factory interface in library %s", dllName );
		return;
	}

	g_bMaterialSystemLoaded = true;
}
						...
						IMaterialEnumerator *pEnum, int nContext, int nFlags )
{
	...
	const char *pFileName = g_pFileSystem->FindFirst( pWildCard, &findHandle );
	while( pFileName )
	{
...
// that we can load up at a later time 
//-----------------------------------------------------------------------------
void CMaterial::EnumerateMaterials( IMaterialEnumerator *pEnum, int nContext, int nFlags )
{
	InitDirectoryRecursive( "materials", pEnum, nContext, nFlags );
}
...
//-----------------------------------------------------------------------------
IMaterial* CMaterial::GetMaterial()
{
	LoadMaterial();
	return m_pMaterial;
}
...
//			dwFlags - 
//-----------------------------------------------------------------------------
void CMaterial::Draw(CDC *pDC, RECT& rect, int iFontHeight, int iIconHeight, DWORD dwFlags)
{
...
bool CMaterial::LoadMaterialHeader( IMaterial *pMat )
{
	...
	int width, height;
	retVal = pMat->GetPreviewImageProperties(&width, &height, &eImageFormat, &translucentBaseTexture);
...
{
	StudioModel::ReleaseStudioModels();
}

static void RestoreMaterialSystemObjects()
{
	StudioModel::RestoreStudioModels();
}


//-----------------------------------------------------------------------------
// Purpose: 
// Input  : *pszMaterialsDir - 
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CMaterial::Initialize( char *pszMaterialsDir, HWND hwnd )
{
	memset( &g_materialSystemConfig, 0, sizeof(g_materialSystemConfig) );

	// NOTE: This gets set to true later upon creating a 3d view.
	g_materialSystemConfig.texGamma = 2.2f;
	g_materialSystemConfig.screenGamma = 2.2f;
	g_materialSystemConfig.overbright = 1.0f;
	g_materialSystemConfig.bAllowCheats = false;
	g_materialSystemConfig.bLinearFrameBuffer = false;
	g_materialSystemConfig.bFilterTextures = true;
	g_materialSystemConfig.bCompressedTextures = true;
	g_materialSystemConfig.bBumpmap = true;
	g_materialSystemConfig.bShowSpecular = true;
	g_materialSystemConfig.bShowDiffuse = true;
	g_materialSystemConfig.maxFrameLatency = 1;
	g_materialSystemConfig.bFilterLightmaps = true;
	g_materialSystemConfig.bMipMapTextures = true;
	g_materialSystemConfig.bBufferPrimitives = true;
	g_materialSystemConfig.bSoftwareLighting = true;
	g_materialSystemConfig.bEditMode = true;	// Yes, we're in WorldCraft.
	g_materialSystemConfig.dxSupportLevel = 0;
	...
	LoadMaterialSystemInterface();
    if (!g_bMaterialSystemLoaded)
    {
        return(false);
    }

	if (pszMaterialsDir == NULL)
	{
		return(false);
	}

	char* pShader = "shaderapidx9.dll";
	...
	g_pMaterialSystemInterface->AddRestoreFunc( RestoreMaterialSystemObjects );
	...
	if (!MaterialSystemInterface()->SetMode( hwnd, mode, 
		MATERIAL_VIDEO_MODE_WINDOWED | MATERIAL_VIDEO_MODE_RESIZING ))
	{
		return false;
	}
	
	ITexture *pCubemapTexture = MaterialSystemInterface()->FindTexture( "editor/cubemap", NULL, true );
	pCubemapTexture->IncrementReferenceCount();
	MaterialSystemInterface()->BindLocalCubemap( pCubemapTexture );
...
{
	if (g_pMaterialSystemInterface != NULL)
	{
		g_pMaterialSystemInterface->Shutdown();
	}

	FreeLibrary(materialSystemDLLHInst);
