...
//	void InitData( int power, int minTess, float smoothingAngle, Vector **dispVectorField, Vector **dispVectorOffset, float *dispDistances );
	...
	void RotateFieldData( QAngle const &vAngles );

	friend void UpdateDispTrans( Selection3D *pSelectionList );
	friend void CreateDispSelectionList( Selection3D *pSelectionList, IWorldEditDispMgr *pDispMgr );
	...
	bool Split( CMapFace *pFace, EditDispHandle_t hBuilderDisp );
	...
	int CollideWithDispTri( const Vector &rayStart, const Vector &rayEnd );
	...
	void Paint_UpdateSelfAndNeighbors( void );
	...
	void DispUVToSurf( Vector2D const &dispUV, Vector &surfPt )					{ m_CoreDispInfo.DispUVToSurf( dispUV, surfPt ); }
	void DispUVToSurfWithAttribs( Vector2D const &dispUV, Vector &surfPt, 
		                          Vector &normal, float &alpha )				{ m_CoreDispInfo.DispUVToSurfWithAttribs( dispUV, surfPt, normal, alpha ); }
	bool SurfToBaseFacePlane( Vector const &surfPt, Vector &planePt )			{ return m_CoreDispInfo.SurfToBaseFacePlane( surfPt, planePt ); }

	inline void ClampTo( float &value, float lower, float upper );
...
private:
	...
	void CheckAndUpdateOverlays( void );
	...
	void CalcColor( CRender3D *pRender, bool bIsSelected, SelectionState_t faceSelectionState, unsigned int r, unsigned int g, unsigned int b, unsigned int alpha, unsigned char *pColor );
