...
class CMapFace : public CMapAtom
{
	public:
		...
		void SetTexture(const char *pszNewTex);
		...
		void CreateFace(Vector *pPoints, int nPoints);
		...
		inline BOOL IsIgnored(void) const { return(m_bIgnored); }
		inline void SetIgnored(BOOL bIgnored) { m_bIgnored = bIgnored; }
		...
		inline void SetDisp( EditDispHandle_t handle );
...
//		void ComputeColor( CRender3D* pRender, bool bRenderAsSelected,
//							bool bIgnoreLighting, unsigned char* pColor );
		void DrawFace( Vector& viewPoint, unsigned char* pColor, RenderMode_t mode );
		void RenderGridIfCloseEnough( CRender3D* pRender, Vector& viewPoint );
		void RenderTextureAxes( CRender3D* pRender );

		void RenderFace( CRender3D* pRender, Vector& viewPoint, 
...
//-----------------------------------------------------------------------------
inline void CMapFace::SetDisp( EditDispHandle_t handle )
{
	m_DispHandle = handle;
}
