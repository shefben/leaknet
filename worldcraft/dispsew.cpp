...
#define DISPSEW_EDGE_COUNT		    4
#define DISPSEW_AT_EDGE_COUNT		2
#define DISPSEW_CORNER_COUNT        4
#define DISPSEW_AT_CORNER_COUNT     16 
...
void FaceListSewEdges( void )
{
	...
	// destroy all corners, t-junctions, edges
...
{
	assert( pCornerData->faceCount >= 0 );
	assert( pCornerData->faceCount < DISPSEW_FACES_AT_CORNER );
...
{
	assert( pTJData->faceCount >= 0 );
	assert( pTJData->faceCount < DISPSEW_FACES_AT_TJUNC );
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void SewEdge_Add( SewEdgeData_t *pEdgeData, CMapFace *pFace, int ndxEdge, int type )
{
	assert( pEdgeData->faceCount >= 0 );
	assert( pEdgeData->faceCount < DISPSEW_FACES_AT_EDGE );
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void SewEdge_AddToListMerge( SewEdgeData_t *pEdgeData, SewEdgeData_t *pCmpData )
{
	for( int i = 0; i < pEdgeData->faceCount; i++ )
	{
		...
		{
			SewEdge_Add( pCmpData, pEdgeData->pFaces[i], pEdgeData->ndxEdges[i], pEdgeData->type[i] );
		}
	}
}
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void SewEdge_AddToListTJunc( SewEdgeData_t *pEdgeData )
{
	...
	for( int i = 0; i < edgeCount; i++ )
	{
		...
		{
			SewEdge_AddToListMerge( pEdgeData, pCmpData );
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void SewEdge_AddToList( SewEdgeData_t *pEdgeData )
{
	...
	SewEdge_AddToListTJunc( pEdgeData );
}
		...
		for( int ndxPt = 0; ndxPt < ptCount; ndxPt++ )
		{
			...
			for( int ndxFace2 = 0; ndxFace2 < faceCount; ndxFace2++ )
			{
				...
				for( int ndxPt2 = 0; ndxPt2 < ptCount2; ndxPt2++ )
				{
					...
					{
						SewEdge_Add( pEdgeData, pFace2, ndxPt2, type2 );
						type1_keep = type1;
					}
				}
			}
			...
			{
				SewEdge_Add( pEdgeData, pFace, ndxPt, type1_keep );
				SewEdge_AddToList( pEdgeData );
			}
			// no neighbors -- de-allocate
			else
			{
