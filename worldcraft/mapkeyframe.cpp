...
#include "MapEntity.h"
#include "MapWorld.h"
#include "../common/KeyFrame/KeyFrame.h"
...
// Input  : *pClone - 
//-----------------------------------------------------------------------------
void CMapKeyFrame::OnClone( CMapClass *pClone, CMapWorld *pWorld, CMapObjectList &OriginalList, CMapObjectList &NewList )
{
...
float CMapKeyFrame::GetRemainingTime( CMapObjectList *pVisited )
{
	...
	//
	if ( pVisited->Find( this ) )
	{
		return 0.0f;
	}

	pVisited->AddTail( this );
...
//			value - 
//-----------------------------------------------------------------------------
void CMapKeyFrame::OnParentKeyChanged( LPCSTR key, LPCSTR value )
{
...
{
	return dynamic_cast<CMapEntity*>( Parent );
}
...
bool CMapKeyFrame::IsAnyKeyInSequenceSelected( void )
{
	...
	{
		if ( find->Parent && find->Parent->IsSelected() )
		{
			return true;
		}
	}
...
IPositionInterpolator* CMapKeyFrame::SetupPositionInterpolator( int iInterpolator )
{
	if( iInterpolator != m_iPositionInterpolator )
	{
		...
		if( pEnt )
		{
			for( int i=0; i < pEnt->GetKeyValueCount(); i++ )
			{
