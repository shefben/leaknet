...
CMapPathNode &CMapPathNode::operator=(const CMapPathNode &src)
{
	...
	kv.RemoveAll();
	int nKeyValues = src.kv.GetCount();
...
DWORD CMapPath::AddNode(DWORD dwAfterID, const Vector &vecPos)
{
	...
	{
		// add at tail
		m_Nodes.SetAtGrow(iPos, node);
	}
	else
	{
		m_Nodes.InsertAt(iPos, node);
	}
...
ResolveNamesAgain:
	...
	if(bFirstPass && m_iDirection == dirPingpong && m_nNodes > 2)
	{
		// redo loop
		bFirstPass = FALSE;
		iDirec = -1;
...
SaveAgain:
	...
	if(iDirec == 1 && m_iDirection == dirPingpong && m_nNodes > 2)
	{
		// redo loop
		iDirec = -1;
		iCurNode = m_nNodes-2;
...
CMapEntity *CMapPath::CreateEntityForNode(DWORD dwNodeID)
{
	...
	int nKeyValues = pNode->kv.GetCount();
	for (int i = 0; i < nKeyValues; i++)
	{
		pEntity->SetKeyValue(pNode->kv.GetKey(i), pNode->kv.GetValue(i));
	}
...
CChunkFileResult_t CMapPath::LoadKeyCallback(const char *szKey, const char *szValue, CMapPath *pPath)
{
	...
	else if (!_stricmp(szKey, "classname"))
	{
		pPath->SetClass(szValue);
	}
	else if (!_stricmp(szKey, "direction"))
	{
