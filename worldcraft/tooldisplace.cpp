...
//-----------------------------------------------------------------------------
void CToolDisplace::OnActivate(ToolID_t eOldTool)
{
	//
	// initialize masks
	//
...
//-----------------------------------------------------------------------------
void CToolDisplace::OnDeactivate(ToolID_t eNewTool)
{
	//
	// reset masks
	//
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CToolDisplace::OnLMouseDown3D( CMapView3D *pView, UINT nFlags, CPoint point )
{
	// Set down flags
	m_bLMBDown = true;
	...
	if( ( m_uiTool == DISPTOOL_PAINT ) && ( GetAsyncKeyState( VK_SHIFT ) & 0x8000 ) )
	{
		...
		{
			EditDispHandle_t handle = GetHitPos( pView, point );
	...
	if( m_uiTool == DISPTOOL_PAINT )
	{
		...
		if ( m_bSpatial )
		{
			...
			ApplySpatialPaintTool( nFlags, point, pDisp );
		}
		else
		{
			// setup for undo - modifying the displacement (painting)
			...
			ApplyPaintTool( nFlags, point, pDisp );    
		}
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CToolDisplace::OnLMouseUp3D( CMapView3D *pView, UINT nFlags, CPoint point )
{
	// left button up
	m_bLMBDown = false;
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CToolDisplace::OnRMouseDown3D( CMapView3D *pView, UINT nFlags, CPoint point )
{
	// left button down
    m_bRMBDown = true;
	...
	{
		// Do tagging.
		HandleTaggingReset( pView, point );
		return true;
	}
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CToolDisplace::OnRMouseUp3D( CMapView3D *pView, UINT nFlags, CPoint point )
{
	// left button up
	m_bRMBDown = false;
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CToolDisplace::OnMouseMove3D( CMapView3D *pView, UINT nFlags, CPoint point )
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CToolDisplace::LiftFaceNormal( CMapView3D *pView, CPoint point )
{
	//
	...
	if( ( ( pObject = pView->NearestObjectAt( point, ulFace ) ) != NULL ) )
	{
		if( pObject->IsMapClass( MAPCLASS_TYPE( CMapSolid ) ) )
		{
			...
			pView->BuildRay( point, vRayStart, vRayEnd );
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CToolDisplace::ApplyPaintTool( UINT nFlags, CPoint point, CMapDisp *pDisp )
{
	switch( m_uiEffect )
	{ 
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CToolDisplace::ApplySpatialPaintTool( UINT nFlags, CPoint point, CMapDisp *pDisp )
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CToolDisplace::HandleSelection( CMapView3D *pView, CPoint point )
{
	//
	// check for closest solid object
	//
...
//-----------------------------------------------------------------------------
void CToolDisplace::HandleTagging( CMapView3D *pView, CPoint point )
{
	...
	if( ( ( pObject = pView->NearestObjectAt( point, ulFace ) ) != NULL ) )
	{
		if( pObject->IsMapClass( MAPCLASS_TYPE( CMapSolid ) ) )
		{
			...
			if ( pFace && pFace->HasDisp() )
			{
				...
				GetRay( pView, point, vecStart, vecEnd );

				int iTri = pDisp->CollideWithDispTri( vecStart, vecEnd );
				if ( iTri != -1 )
				{
...
// Purpose: Handle the overriding of displacement triangle tag.
//-----------------------------------------------------------------------------
void CToolDisplace::HandleTaggingReset( CMapView3D *pView, CPoint point )
{
	// Get the displacement face (if any) at the 2d point.
	ULONG ulFace;
	CMapClass *pObject = NULL;
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
EditDispHandle_t CToolDisplace::GetHitPos( CMapView3D *pView, CPoint point )
{
	//
	// get ray info
	//
	Vector rayStart, rayEnd;
	GetRay( pView, point, rayStart, rayEnd );
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CToolDisplace::GetRay( CMapView3D *pView, CPoint point, Vector& rayStart, Vector& rayEnd )
{
	//
	// get screen coordinates from client point
	//
	Vector2D clientPt;
	clientPt[0] = point.x;
	clientPt[1] = point.y;
	...
	Vector2D screenPt;
	pView->ClientToScreen( screenPt, clientPt );

	//
	// get world coordinate from screen point
	//
	Vector clickPt;
	pView->ScreenToWorld( clickPt, screenPt );

	//
	// build a ray from view point through "click" point (on near clipping plane)
	//
	const CCamera *pCamera = pView->GetCamera();
	pCamera->GetViewPoint( rayStart );

	//
	// set ray start and end
	//
	Vector ray;
	VectorSubtract( clickPt, rayStart, ray );

	VectorNormalize( ray );
	VectorScale( ray, 8192.0f, ray );
	VectorAdd( rayStart, ray, rayEnd );
}
