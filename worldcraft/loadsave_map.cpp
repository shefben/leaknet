...
static int SaveSolidChildrenOf(CMapClass *pObject, std::fstream& file, BoundBox *pIntersecting = NULL)
{
	...
	POSITION pos;
	CMapClass *pChild = pObject->GetFirstChild(pos);
	while (pChild != NULL)
	{
		int iRvl = -1;
		...
		pChild = pObject->GetNextChild(pos);
	}
...
{
	ASSERT((mf == mfHalfLife) || (mf == mfHalfLife2) || (mf == mfTeamFortress2));
	MapFormat = mf;
}
...
int CMapWorld::SerializeMAP(std::fstream &file, BOOL fIsStoring, BoundBox *pIntersecting)
{
	...
	if (fIsStoring)	
	{
		...
		//
		if (!CMapDoc::GetActiveMapDoc()->m_bHidePaths)
		{
			POSITION p = m_Paths.GetHeadPosition();
			while (p)
			{
