...
bool StudioModel::Initialize()
{
	...
	studioRenderDLL = Sys_LoadModule( "studiorender" DLL_EXT_STRING );
	if( !studioRenderDLL )
	{
		Msg( mwWarning, "Can't load studiorender%s", DLL_EXT_STRING );
		return false;
	}

	CreateInterfaceFn studioRenderFactory = Sys_GetFactory( "studiorender" DLL_EXT_STRING );
	...
	if (!m_pStudioRender)
	{
		Msg( mwWarning, "Can't get version %s of studiorender%s", STUDIO_RENDER_INTERFACE_VERSION, DLL_EXT_STRING );
		return false;
	}

	if( !m_pStudioRender->Init( g_MaterialSystemFactory, g_MaterialSystemClientFactory ) )
	{
		Msg( mwWarning, "Can't initialize studiorender%s", DLL_EXT_STRING );
		m_pStudioRender = NULL;
	}
	UpdateStudioRenderConfig( false, false );

	// VXP: Shared anim group routine
	CSysModule *filesystemDLL = Sys_LoadModule( "filesystem_stdio" DLL_EXT_STRING );
	if( !filesystemDLL )
	{
		Msg( mwWarning, "Can't load filesystem_stdio%s", DLL_EXT_STRING );
		return false;
	}
	CSysModule *engineDLL = Sys_LoadModule( "engine" DLL_EXT_STRING );
	if( !engineDLL )
	{
		Msg( mwWarning, "Can't load engine%s", DLL_EXT_STRING );
		return false;
	}

	CreateInterfaceFn filesystemFactory = Sys_GetFactory( "filesystem_stdio" DLL_EXT_STRING );
	if ( !filesystemFactory )
	{
		Msg( mwWarning, "Can't get filesystem factory" );
		return false;
	}
	CreateInterfaceFn engineFactory = Sys_GetFactory( "engine" DLL_EXT_STRING );
	if ( !engineFactory )
	{
	...
	if ( !filesystem )
	{
		Msg( mwWarning, "Can't get version %s of filesystem_stdio%s", BASEFILESYSTEM_INTERFACE_VERSION, DLL_EXT_STRING );
		return false;
	}
