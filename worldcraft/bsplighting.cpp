...
CBSPLighting::CMaterialBuf::~CMaterialBuf()
{
	if( m_pMesh )
	{
		MaterialSystemInterface()->DestroyStaticMesh( m_pMesh );
	}
...
bool CBSPLighting::Load( char const *pFilename )
{
	...
	g_pFileSystem->AddSearchPath( pFilename, "GAME" );
	...
	g_pFileSystem->RemoveSearchPath( pFilename, "GAME" );
	...
	pMatSys->FlushLightmaps();
	pMatSys->SetForceBindWhiteLightmaps( true );
...
#define DRAWLIGHTMAPPAGE
	...
	{
		IMaterial *g_materialDebugLightmap = materialSystemInterface->FindMaterial( "debug/debuglightmap", 0 );
...
void CBSPLighting::Draw()
{
	...
	pMatSys->Flush();
	pMatSys->SetForceBindWhiteLightmaps( false );

#if defined( DRAWLIGHTMAPPAGE )
	static bool bDrawIt = false;
	if( bDrawIt )
	{
		pMatSys->MatrixMode( MATERIAL_VIEW );
		pMatSys->PushMatrix();
		pMatSys->LoadIdentity();

		pMatSys->MatrixMode( MATERIAL_MODEL );
		pMatSys->PushMatrix();
		pMatSys->LoadIdentity();
		
		pMatSys->MatrixMode( MATERIAL_PROJECTION );
		pMatSys->PushMatrix();
		pMatSys->LoadIdentity();
		pMatSys->Ortho( 0, 0, 300, 300, -99999, 99999 );
	...
	FOR_EACH_LL( m_FaceMaterials, iMat )
	{
		...
		FOR_EACH_LL( pMat->m_MaterialBufs, iBuf )
		{
			...
			for( int iCmd=0; iCmd < pBuf->m_DrawCommands.Count(); iCmd++ )
			{
				...
				pMatSys->BindLightmapPage( pCmd->m_LightmapPageID );
	...
	)
{
	...
	FOR_EACH_LL( m_FaceMaterials, iMat )
	{
		...
		FOR_EACH_LL( pMat->m_MaterialBufs, iBuf )
		{
			...
			pBuf->m_pMesh = pMatSys->CreateStaticMesh( pMat->m_pMaterial );
			if( !pBuf->m_pMesh )
				continue;
...
CBSPLighting::CFaceMaterial* CBSPLighting::FindOrAddMaterial( CBSPInfo &file, int stringTableID )
{
	...
	else
	{
		...
		if( pMaterialName )
			pMaterial = MaterialSystemInterface()->FindMaterial( pMaterialName, 0 );
