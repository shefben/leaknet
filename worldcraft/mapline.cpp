...
CMapClass *CMapLine::Create(CHelperInfo *pHelperInfo, CMapEntity *pParent)
{
	...
	const char *pszStartKey = pHelperInfo->GetNextParameter(pos);
	const char *pszStartValueKey = pHelperInfo->GetNextParameter(pos);

	const char *pszEndKey = pHelperInfo->GetNextParameter(pos);
	const char *pszEndValueKey = pHelperInfo->GetNextParameter(pos);
...
//			value - 
//-----------------------------------------------------------------------------
void CMapLine::OnParentKeyChanged( LPCSTR key, LPCSTR value )
{
	CMapWorld *pWorld = (CMapWorld *)GetWorldObject(this);
	if (pWorld != NULL)
	{
...
void CMapLine::Render2D(CRender2D *pRender)
{
	if ((m_pStartEntity != NULL) && (m_pEndEntity != NULL))
	{
		...
		if (IsSelected())
		{
			pRender->SetLineType(CRender2D::LINE_SOLID, CRender2D::LINE_THIN, SELECT_FACE_RED, SELECT_FACE_GREEN, SELECT_FACE_BLUE);
		}
		else
		{
...
void CMapLine::Render3D(CRender3D *pRender)
{
	...
	if ((m_pStartEntity != NULL) && (m_pEndEntity != NULL))
	{
		...
		// FIXME: Can't do this...! glLineWidth(1);
	}
...
// Input  : pTransBox - 
//-----------------------------------------------------------------------------
void CMapLine::DoTransform(Box3D *pTransBox)
{
...
// Input  : RefPoint - 
//-----------------------------------------------------------------------------
void CMapLine::DoTransFlip(const Vector &RefPoint)
{
...
// Input  : Delta - 
//-----------------------------------------------------------------------------
void CMapLine::DoTransMove(const Vector &Delta)
{
...
//			Angles - 
//-----------------------------------------------------------------------------
void CMapLine::DoTransRotate(const Vector *pRefPoint, const QAngle &Angles)
{
...
//			Scale - 
//-----------------------------------------------------------------------------
void CMapLine::DoTransScale(const Vector &RefPoint, const Vector &Scale)
{
