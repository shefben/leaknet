...
//-----------------------------------------------------------------------------
// Purpose: Updates server config with current gameplay settings
//-----------------------------------------------------------------------------
void CCreateMultiplayerGameGameplayPage::UpdateServerConfig(KeyValues *pServerConfig)
{
	if (!pServerConfig || !m_pDescription)
		return;
	...
	while (pObj && pList)
	{
		if (pObj->cvarname && strlen(pObj->cvarname) > 0)
		{
			switch (pList->type)
			{
			case O_BOOL:
				{
					CheckButton *pBox = (CheckButton *)pList->pControl;
					if (pBox)
					{
						pServerConfig->SetInt(pObj->cvarname, pBox->IsSelected() ? 1 : 0);
					}
				}
				break;
			case O_STRING:
				{
					TextEntry *pEdit = (TextEntry *)pList->pControl;
					if (pEdit)
					{
						char szValue[256];
						pEdit->GetText(szValue, sizeof(szValue));
						pServerConfig->SetString(pObj->cvarname, szValue);
					}
				}
				break;
			case O_NUMBER:
				{
					...
					if (pEdit)
					{
						char szValue[256];
						pEdit->GetText(szValue, sizeof(szValue));
						pServerConfig->SetFloat(pObj->cvarname, atof(szValue));
					}
				}
				break;
			case O_LIST:
				{
					...
					if (pCombo)
					{
						pServerConfig->SetInt(pObj->cvarname, pCombo->GetActiveItem());
					}
				}
				break;
			}
