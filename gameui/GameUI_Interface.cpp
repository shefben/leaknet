...
//	g_pSystemWrapper = system;
	...
	if (FindPlatformDirectory(m_szPlatformDir, sizeof(m_szPlatformDir)))
	{
		...
		char language[128];
		if (vgui::system()->GetRegistryString("HKEY_LOCAL_MACHINE\\Software\\LeakNet\\Steam\\Language", language, sizeof(language)))
		{
		...
		*/
		...
		//!! can be removed
		{
			...
			if (ERROR_SUCCESS == g_pVCR->Hook_RegOpenKeyEx(HKEY_CURRENT_USER, "Software\\LeakNet\\Tracker", 0, KEY_READ, &key))
			{
...
/*	// this function exported by cdkey.obj (we don't have the source code)
	...
	char cdkey[255];
	if (!force && (!engine->CheckParm("-forcevalve", NULL) && vgui::system()->GetRegistryString("HKEY_CURRENT_USER\\Software\\LeakNet\\Half-Life\\Settings\\Key", cdkey, sizeof(cdkey)) 
			&& strlen(cdkey) > 0) )
	{
	...
	else if (!force && vgui::system()->GetRegistryString("HKEY_CURRENT_USER\\Software\\LeakNet\\Half-Life\\Settings\\ValveKey", cdkey, sizeof(cdkey)) 
			&& strlen(cdkey) > 0)
	{
	...
	else if (!force && vgui::system()->GetRegistryString("HKEY_CURRENT_USER\\Software\\LeakNet\\Half-Life\\Settings\\yeK1", cdkey, sizeof(cdkey)))
	{
		// TODO: Add weak encrypted key validation check here!
		if (strlen(cdkey) == 254)
		{
			if(vgui::system()->GetRegistryString("HKEY_CURRENT_USER\\Software\\LeakNet\\Half-Life\\Settings\\yeK2", cdkey, sizeof(cdkey)))
			{
				if(strlen(cdkey) == 108)
				{
					// success
...
//	TRACE_FUNCTION("CGameUI::HideGameUI");
	...
	bool bIsInLevel = ( levelName && levelName[0] );
	bool bIsLevelBackground = engine->IsLevelMainMenuBackground();

	if ( !bIsInLevel )
		return;

	if ( !bIsLevelBackground )
	{
