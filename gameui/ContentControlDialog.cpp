...
void CContentControlDialog::ResetPassword()
{
	// Set initial value
	HKEY key;
	if ( ERROR_SUCCESS == g_pVCR->Hook_RegOpenKeyEx(HKEY_CURRENT_USER, "Software\\LeakNet\\Half-Life\\Settings", 0, KEY_READ, &key))
	{
		DWORD type;
...
void CContentControlDialog::WriteToken( const char *str )
{
	// Set initial value
	HKEY key;
	if ( ERROR_SUCCESS == g_pVCR->Hook_RegOpenKeyEx(HKEY_CURRENT_USER, "Software\\LeakNet\\Half-Life\\Settings", 0, KEY_WRITE, &key))
	{
		DWORD type = REG_SZ;
